...
TL;DR: 提出AtmoFuseNet框架，融合多视角天空相机图像、毫米波云雷达和云高仪数据，生成云微物理场和风场的4D估计，在干旱半干旱站点实现液态水含量MAE 0.026 g/m³和风速MAE 1.18 m/s。


<details>
  <summary>Details</summary>
Motivation: 地面稀疏仪器对云微物理场的稠密体积重建仍是一个开放问题，因为可用测量在模态和空间覆盖上异构。

Method: 三阶段框架：跨模态分层聚合模块（图像特征金字塔与仪器垂直剖面通过层间交叉注意力融合）、条件变分细化模块（在可微雷达和图像前向模型下映射为物理一致场）、基于相关的运动估计器（从连续体积重建恢复每体素3D风矢量）。

Result: 液态水含量MAE 0.026 g/m³，风速MAE 1.18 m/s，优于现有检索基线。消融实验验证了各模块贡献。

Conclusion: AtmoFuseNet有效融合多模态异构观测数据，实现了云微物理场和风场的稠密4D重建。
...
TL;DR: 本文提出流式高斯编码器（Streaming Gaussian Encoder），通过维护持久化的体积场景表示（潜空间高斯查询）进行4D全景占据跟踪，在保持与现有掩码管线兼容的同时，显著提升了时序一致性，并在nuScenes和Waymo数据集上达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有基于掩码的4D全景占据跟踪方法在每个时间步重新计算体积表示，导致几何时序一致性受限，尤其在遮挡和静态场景元素下表现不佳。

Method: 1) 将场景建模为固定大小的潜空间高斯查询集；2) 通过自运动补偿传播高斯查询，并在置信度引导的预算约束下刷新；3) 利用深度监督塑造高斯不透明度作为可见性代理，使置信度随时间累积；4) 采用基于热身的多帧训练策略。

Result: 在Occ3D扩展的nuScenes和Waymo数据集上达到新的最先进水平，在近乎无额外计算开销的情况下提升了跟踪一致性。

Conclusion: 所提方法通过持久化的体积场景表示实现了超越仅解码器跟踪的表示级时序一致性，且完全兼容现有掩码管线。
...
TL;DR: 本文提出了一个学习数据集潜在表征并通过分布度量量化其相似性的框架，在24个主要运动预测数据集上的研究表明，由此得到的可迁移性分数与跨数据集模型性能高度相关，为数据集选择、预训练和构建大规模基础模型提供了实用指导。


<details>
  <summary>Details</summary>
Motivation: 现有运动预测模型在一个数据集上训练后往往无法推广到其他领域，因为场景布局、智能体行为和传感条件存在差异，缺乏系统的方法来量化数据集之间的相似性和可迁移性。

Method: 提出一个学习数据集潜在表征的框架，利用分布度量来量化数据集之间的相似性，并在24个主流运动预测数据集上进行大规模实证研究，计算可迁移性分数并验证其与跨数据集模型性能的相关性。

Result: 可迁移性分数与跨数据集模型性能之间存在强相关性，证实了该框架的有效性。

Conclusion: 该研究为运动预测领域的数据集选择、预训练策略和大规模基础模型构建提供了实用指导，有助于开发更具泛化性和鲁棒性的预测系统。
...
TL;DR: 本文提出随机目标监督混合（RTSM），一种将少量标注目标域样本通过监督检测损失融入源无关域自适应目标检测（SFDA-OD）的简单方法。实验表明，RTSM在多种设置下稳定提升性能（AP50提升1.7-18.3），而更复杂的稀疏标签反馈策略效果有限，印证了“简单监督难以被击败”的经验教训。


<details>
  <summary>Details</summary>
Motivation: 在源无关域自适应目标检测中，当只有少量目标域图像被标注时，如何有效利用这些稀疏标签来提升自适应性能。

Method: 提出随机目标监督混合（RTSM），在原有无标注自适应分支不变的基础上，对少量标注样本添加监督检测损失，同时评估了十种稀疏标签反馈插件（包括伪标签选择、目标补全、优化控制等）的效果。

Result: RTSM在四种SFDA-OD方法、两种检测器、多种自适应任务及1%-10%标签预算下，相比纯SFDA方法提升1.7-18.3 AP50；而十种更复杂的稀疏标签反馈插件带来的增益有限且依赖具体方法。

Conclusion: 简单监督策略（RTSM）在稀疏标签场景下效果显著且鲁棒，复杂的反馈机制难以带来额外收益，为稀疏标签SFDA-OD提供了一个简单而有效的基准方案。
...
TL;DR: GaussLite是一种任务驱动的3D高斯泼溅映射系统，根据自然语言任务描述动态分配表示密度，在有限计算资源下提升感兴趣区域的重建质量，并支持多代理地图融合。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS系统均匀分配表示容量，忽略了机器人任务只关注场景部分几何结构的事实，导致计算资源浪费在无关区域。

Method: 使用一次性LLM解析器提取目标和锚定物体，通过开放词汇检测器逐帧分割生成像素级相关性掩码，根据任务相关性分配种子密度、梯度流和缩放。

Result: 在Replica数据集上ROI PSNR平均提升+2.72 dB，在真实硬件室内外场景提升+2.23 dB；多代理融合相比直接拼接提升+3.42 dB，仅共享平均7.08%的地图。

Conclusion: GaussLite通过任务驱动的资源分配策略，在有限高斯预算下显著提升任务相关区域的重建质量，并支持高效的多代理地图融合。
...
TL;DR: 本文提出AVTok，一种统一的音频-视频分词器，通过共享编码器-解码器和模态特定的可学习查询，将音频-视频对编码为紧凑的一维潜在表示，并采用分层训练策略解决异质信息不平衡问题，在重建和下游生成任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有音频-视频生成方法采用双分支设计，模态间存在表征鸿沟且计算资源需求大，缺乏统一的联合分词方案。

Method: 提出AVTok统一分词器，包含双流Transformer架构、共享编码器-解码器、模态特定可学习查询、统一码本，以及分层训练策略逐步实现各模态的重建能力。

Result: 在音频-视频重建以及音频到视频、视频到音频、类别条件联合生成等下游任务中均表现出色。

Conclusion: AVTok为联合音频-视频分词铺平了道路，为构建统一的音频-视频生成大规模多模态模型提供了潜在方向。
...
TL;DR: SyncCache是一种针对音频驱动人像动画中DiT模型的无训练缓存加速方法，通过空间非对称探测和模态解耦缓存，实现高达4.12倍加速且保持近无损质量。


<details>
  <summary>Details</summary>
Motivation: 现有无训练缓存加速方法主要针对文本条件生成，忽略了音频驱动人像动画中存在的空间和模态不平衡问题（高频动态集中在人脸区域，低频背景变化缓慢），导致缓存效率低下。

Method: 1) 空间非对称探测：优先关注动态人脸区域的误差敏感性；2) 模态解耦缓存：重用稳定的块间残差，同时持续计算轻量音频块以保持唇形同步；3) 记忆自适应缓存选择：将缓存分配建模为离线动态规划问题。

Result: 在HunyuanVideo-Avatar上实现4.12倍加速，在Wan-S2V上实现3.75倍加速，视觉保真度近无损且音频对齐精确。

Conclusion: SyncCache在速度与质量之间取得了优越的平衡，为音频驱动人像动画提供了一种高效的训练无关加速方案。
...
TL;DR: 提出标签模仿游戏(LIG)框架，通过图灵测试网络(TTN)以对抗性交互方式筛选伪标签，无需监督即可提升视觉-语言模型标签质量，并在零样本任务迁移中实现显著效果。


<details>
  <summary>Details</summary>
Motivation: 基础模型零样本伪标签存在幻觉错误，传统阈值过滤方法无法有效消除这些错误。

Method: 提出标签模仿游戏(LIG)，将伪标签筛选形式化为对抗性交互过程；训练图灵测试网络(TTN)作为任务无关的"裁判"，在数据集全局上下文中评估候选伪标签质量。

Result: 在四个数据集上，TTN持续提升三个SOTA视觉-语言模型的标签准确性；将图像分类数据集训练的TTN迁移到目标检测任务中，F1分数提升28%（最差基线）和44%（任务微调后），并观察到"类别复兴"现象。

Conclusion: 学习到的语义上下文逻辑可作为空间几何验证的鲁棒替代方案，实现独特的零样本任务迁移能力。
...
TL;DR: 提出了一种从单目图像估计物体实际尺寸的方法，通过将物体分解为多个结构元素并整合设计规则、几何关系和常规尺寸等外部知识，生成接近真实比例的3D资产，用于提升自动驾驶车载摄像头在虚拟环境中的验证准确性。


<details>
  <summary>Details</summary>
Motivation: 虚拟环境与真实环境中被验证物体的尺寸差异会导致摄像头识别性能下降，而高海拔安装的交通标志难以用LiDAR或立体相机测量距离，因此需要从单目图像进行尺寸估计。

Method: 将物体分解为多个结构元素，从单目图像检测每个组件，结合结构关系与周围元素的尺寸一致性，利用设计规则、几何关系和常规尺寸等外部知识估计各组件尺寸，并重建生成3D资产。

Result: 能够生成与真实环境比例相近的3D资产，并将其放置在数字孪生空间中。

Conclusion: 该方法有望提升自动驾驶在虚拟环境中车载摄像头的验证精度。
...
TL;DR: GRAPE是一个基于原型的医学图像分类架构，通过图注意力机制、概念不匹配安全检查和开放词汇原型锚定，分别解决了原型方法中独立处理发现、放大不安全反馈和需完整重训练三个临床局限性，性能显著提升且延迟极低（仅+1ms）。


<details>
  <summary>Details</summary>
Motivation: 现有的基于原型的医学图像分类器存在三个临床局限：将医学发现视为独立不相关、会无声放大不安全的医生反馈、以及每增加一个新发现就需要完整重训练。这些局限阻碍了原型方法在临床中的实际应用。

Method: 三个核心组件：1）图注意力任务头（Graph Attention Task Head）用于建模解剖概念共现关系；2）概念不匹配安全检查（Concept-Mismatch Safety Check）检测医生标注区域内的主导发现与标签是否冲突；3）开放词汇原型锚定（Open-Vocabulary Prototype Anchoring）将视觉原型与临床文本对齐，支持从单张标注图像添加新发现。

Result: 在TBX11K上宏F1提升13.8个百分点；概念不匹配检查捕获85%的错误标注（MC-Dropout仅51%）；单张胸腔积液示例即恢复全监督定位精度；TBX11K上原型图病灶定位效果比端到端基线好2.6倍；三个组件总共仅增加约1ms延迟。

Conclusion: GRAPE通过统一架构同时解决了原型医学图像分类器的三个关键临床局限性，性能大幅提升且计算开销极微，为原型方法在临床部署提供了可行方案。
...
TL;DR: 提出了一种几何约束的测试时提示微调框架，通过感知可靠性估计、空间门控和时间平滑原型对齐，实现LiDAR语义分割在部署环境变化下的稳定自适应，无需额外标注。


<details>
  <summary>Details</summary>
Motivation: LiDAR语义分割在实际部署中因传感条件变化而性能下降，重新收集标注成本太高；现有测试时自适应方法直接应用于LiDAR数据时，由于伪标签可靠性在空间上存在异方差性（随距离变化的稀疏性和遮挡），统一更新全局共享参数会引入不稳定梯度，导致自适应不稳定。

Method: 1) 从深度一致的波束终止和邻域支持估计每个位置的感知可靠性，用于重新加权空间监督；2) 自适应仅限于插入冻结主干的轻量提示适配器，并通过空间门控防止不可靠区域扰动全局共享表示；3) 引入时间平滑的原型对齐策略，随时间累积可靠语义证据以稳定在线更新。

Result: 在标准LiDAR基准上的实验表明，所提方法在部署环境变化下实现了更好的自适应稳定性和分割性能，且无需额外标注。

Conclusion: 通过感知可靠性估计和轻量级提示适配器的设计，有效解决了LiDAR语义分割中测试时自适应不稳定的问题，为实际部署提供了一种实用且高效的解决方案。
...
TL;DR: 本文提出Prost-RL，将微超声前列腺癌检测转化为强化学习驱动的空间注意策略问题，在6607个活检核心上实现了79.0 AUROC，比最强基线高出2.1个点。


<details>
  <summary>Details</summary>
Motivation: 微超声前列腺癌诊断依赖临床经验，存在观察者间变异性大；监督信号稀疏（仅有核心级病理标签）且类别不平衡，训练可靠深度学习模型困难。

Method: 将强化学习策略集成到基础模型编码器-解码器中生成空间注意力图作为软提示，提出自适应策略优化稳定混合训练，以及对称交叉熵+负熵正则化来缓解弱标签噪声。

Result: 在693名患者6607个活检核心数据上，核心级检测AUROC 79.0±3.5，80%特异性下灵敏度64.6±6.3%；临床显著性癌症分类AUROC 79.3±5.8。

Conclusion: Prost-RL通过强化学习策略提供了可解释的空间定位证据和量化风险预测，代码已开源。
...
TL;DR: PhotoQuilt是一种无需训练的框架，通过引导式分块降噪过程，能够生成任意分辨率的光镶嵌图像，既保持全局布局又保证局部细节真实感。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型难以同时满足高分辨率和分块生成的需求：直接高分辨率生成成本高且倾向于平滑图像而非马赛克效果；而分块平铺虽保留局部细节但丢失全局结构。因此需要一种方法能兼顾全局一致性和局部真实性。

Method: PhotoQuilt采用引导式分块降噪过程：首先在低分辨率下生成全局构图以固定布局，然后在潜在空间上采样并重新注入噪声以恢复生成能力。降噪过程在固定分块内进行，使每个分块形成独立图像，同时共享的全局结构将它们保持在统一布局中。

Result: 实验表明，PhotoQuilt在全局结构和局部真实感方面均优于当前基线方法，并且由于分块生成独立处理，能够扩展到大型画布而无需二次注意力成本。

Conclusion: PhotoQuilt有效解决了光镶嵌图像生成中全局与局部尺度难以兼顾的问题，提供了一种可扩展、无需训练的高分辨率生成方案。
...
TL;DR: 本文揭露了现有AI生成视频检测基准中的评估漏洞，提出审计协议VidAudit，证明简单分类器因混杂因素可虚高超高AUC，而经审计后性能大幅下降；推荐使用多维度评估指标而非单一数值，并发布包含14个检测器的统一工具包。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成视频检测基准（如GenVidBench, AIGVDBench）的评估协议存在未控制的混杂变量，导致报告的泛化性能被虚高，实际检测能力被高估。

Method: ① 设计三特征片段长度分类器作为存在性证明；② 调查20篇论文发现无一采用全部六项标准控制；③ 提出审计协议，对6种特征来源（3个已发表检测器+3个信号源）进行跨数据集审计；④ 提出TemporalSpec（基于编码运动向量）和跨基底特征融合（XSFF）作为白盒阳性对照。

Result: 未审计下三特征分类器AUC达0.998，审计后降至0.529（近随机）；CLIP基线被发现携带数据集身份；WaveRep经审计后LOGO AUC仍达0.996；在0.1% FPR下多个高AUC方法召回率降至个位数，排行榜顺序改变。

Conclusion: 推荐使用审计后的多维指标（AUC、高于底线的余量、操作点召回率、校准度）而非单一数值；发布VidAudit工具包，推动评估从排行榜排名转向结果是否真正测度其所声称的目标。
...
TL;DR: 本文提出一种利用SAM 3视觉基础模型实现稀疏动作感知地标定位的轻量级框架，无需像素级掩码标注，通过零样本点提示结构掩码融合视觉与热图特征，在7,867个手术视频片段上实现尖端72.4%和锚点58.0%的F1分数。


<details>
  <summary>Details</summary>
Motivation: 手术视觉中，现有基础模型（如SAM 3）能提供可迁移的物体级结构信息，但其分割输出无法显式编码定义功能性手术地标的动作条件语义。估计器械范围和几何形状与定位与夹持、抓取或切割相关的尖端或锚点不同，因此需要一种不依赖密集像素标注的稀疏、动作感知地标定位方法。

Method: 提出轻量级细化框架，以SAM 3为结构先验。粗粒度多帧网络预测尖端和锚点提示，生成非Oracle掩码，与视觉和热图特征融合以细化功能性地标预测。对比研究了三种结构信息融入方式：直接掩码增强监督、预测派生掩码先验细化、辅助掩码监督。

Result: 在来自YouTube、Cholec80、HeiChole、SurgVU、CRCD等60个手术视频的7,867个片段上评估，无需像素级掩码标注，模型在尖端和锚点定位上分别达到72.4%和58.0%的F1分数。直接对热图目标施加掩码会使学习偏向工具区域，而预测派生先验和辅助监督能提供有效的中间结构指导。

Conclusion: 视觉基础模型派生的结构信息应通过预测派生先验或辅助监督的方式融入高精度定位系统，而非直接强加于热图目标，这样能更有效地引导动作依赖的地标预测。
...
TL;DR: 本文提出DSAFormer，一种用于多光谱目标检测的双稀疏聚合Transformer，通过空间和通道维度的稀疏注意力机制减少冗余信息交互，并在四个公开数据集上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 已有的基于Transformer的多光谱检测方法在计算相似度时使用了所有可用的token，导致来自无关区域的信息交互产生冗余，从而降低了检测性能。

Method: 提出DSAFormer，包含两个核心模块：(1) 双稀疏Transformer(DSFormer)，由空间稀疏多头交叉注意力(SSMHCA)、通道稀疏多头交叉注意力(CSMHCA)和多尺度特征精化层(MSFRL)组成，用于增强跨模态互补信息；(2) 可学习加法融合模块(LAFB)，通过特征重加权融合模态内和模态间特征。

Result: 在MFAD、FLIR、M^3FD和LLVIP四个公开数据集上，DSAFormer均取得了优于现有最先进方法的检测性能。

Conclusion: 通过稀疏化注意力机制有效消除了跨模态信息交互中的冗余，提升了多光谱目标检测的精度。
...
TL;DR: 提出了一种基于显著性引导的扭曲-还原框架（WarpI2I），通过在编码前将空间表示重新分配到显著区域，在不增加潜空间分辨率的情况下提升图像到图像翻译中的结构细节保持能力，并配套了基于外推的合成数据生成方法。


<details>
  <summary>Details</summary>
Motivation: 现有紧凑型潜扩散模型因编码器将高分辨率输入压缩到空间下采样的潜空间，难以保留细粒度结构信息。

Method: 提出显著性引导的扭曲-还原框架：先对图像进行显著性引导的空间扭曲，将表示资源集中于重要区域，送入原始扩散模型处理，再通过逆扭曲还原回原始空间。同时设计了一种基于外推的合成数据生成流程用于图像重光照。

Result: 在人体重光照、驾驶场景重光照及翻译任务上均实现了更好的结构保持、光照一致性和图像质量，并自然地扩展到视频应用，具有良好的时间稳定性。

Conclusion: 该方法模型无关、无需修改架构、计算开销可忽略，为解决潜扩散模型在I2I任务中结构细节丢失问题提供了简洁有效的方案。
...
TL;DR: TerraDiT-Ω是一个统一的卫星图像生成框架，能够直接使用任意地理空间基元（如多边形、折线、边界框、点）进行可控图像生成，并通过几何感知局部注意力机制提升生成质量，在多个下游任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有卫星图像生成方法要么依赖密集栅格（高标注成本），要么依赖稀疏提示（保真度有限），且与GIS中常用的矢量基元不兼容，难以平衡标注成本与生成精度。

Method: 提出TerraDiT-Ω框架，支持多边形、折线、边界框、点等多种原生地理空间基元作为生成条件；并提出几何感知局部注意力机制，将显式几何线索注入注意力空间，增强对空间结构的控制。

Result: 在所有条件格式下，该方法在生成质量上均优于密集控制和稀疏控制基线；作为数据增强工具，能提升土地覆盖分割、目标检测、道路图提取和场景分类等下游任务的性能。

Conclusion: TerraDiT-Ω实现了从任意地理空间矢量基元直接生成卫星图像，兼顾标注成本与生成保真度，兼容端到端GeoAI工作流，在城市规划等应用场景具有广阔前景。
...
TL;DR: 提出预测性可微渲染（PDR）范式，通过2D高斯表示和可微渲染器解决视频预测中的模糊和细节缺失问题，在多个基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有确定性视频预测模型在离散像素空间操作，使用MSE损失导致预测结果过度平滑、缺乏精细视觉细节，无法准确预测高保真的未来世界。

Method: 提出PDR范式，包含两个核心组件：1) PredGS——基于2D高斯表示的轻量即插即用适配器，可无缝集成到现有像素空间预测器中；2) predgsplat——支持任意通道的CUDA加速可微2D高斯渲染器，每个高斯由5+C个可学习参数定义，渲染速度比基线快10倍。优化使用L1和SSIM损失联合训练。

Result: 在TaxiBJ、WeatherBench、KTH、Human3.6M等多个真实世界基准上，PDR在细节保留、视觉逼真度和预测精度方面一致超越现有方法。

Conclusion: PDR通过连续表示和可微渲染弥合了离散与连续空间之间的鸿沟，有效克服了MSE损失带来的模糊倾向，显著提升了视频预测的细节保真度。
...
TL;DR: 本文提出ADAPT框架，通过直接干预多模态大模型中的文本-图像交叉注意力动态来减少幻觉，包含视觉锚点细化、注意力监督推理和偏好对齐三个组件，在多个基准上将幻觉率降低40%-60%。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型存在严重的幻觉问题，即生成与图像不一致的内容。现有缓解策略多基于结果驱动，未能针对幻觉的内部注意力动态退化这一根本原因进行干预。

Method: 提出ADAPT框架，包含三个关键组件：(1) 交叉注意力视觉锚点细化，提供稳定的空间定位；(2) 注意力监督推理机制，在线检测和纠正注意力漂移；(3) 视觉注意力引导DPO，偏好对齐至视觉可信的响应。

Result: 各组件均有贡献，完整框架在多个幻觉基准上达到新最优结果，在不同主流骨干网络上将幻觉率降低40%-60%，且保持通用多模态能力。

Conclusion: 从注意力内部动态视角出发，直接干预文本-图像交叉注意力可以有效缓解幻觉，为多模态大模型的幻觉问题提供了新的解决思路。
...
TL;DR: 本文提出一种将物理逆向渲染与扩散模型相结合的方法，利用扩散模型预测作为相似性核进行正则化约束，实现高质量几何、材质和光照的联合重建，在多个数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 物理逆向渲染虽然提供了精确的图像形成模型，但严重欠约束，容易将光照信息烘焙到材质中，导致泛化性差；而数据驱动的扩散模型虽能预测视觉合理的材质，却不满足渲染方程，无法直接用于物理渲染。因此需要弥合这两种范式的鸿沟。

Method: 将预训练扩散模型的预测值视为相似性核而非目标材质值，构建正则化损失：在扩散预测近似恒定的表面区域，惩罚优化材质的偏离；在其他区域则自由优化以匹配输入图像。基于该正则化器，构建端到端管线联合重建几何、材质和光照。

Result: 在Synthetic4Relight、Stanford-ORB和DTC-Synthetic数据集上，该方法在重建精度和重光照质量方面均显著优于当前最先进的基线方法。

Conclusion: 该方法生成的3D资产可直接用于标准渲染管线并实现逼真的重光照，有效融合了物理精确性与数据驱动先验的优势。
...
TL;DR: 提出了一种混合UNet-Transformer架构，可从复合材料微观结构图像直接预测应力应变场，作为FEM的高效替代方案，在多种构型下达到高精度（R²最高0.9991），并通过Grad-CAM分析验证了模型关注力学关键区域。


<details>
  <summary>Details</summary>
Motivation: 传统有限元方法计算微观结构力学场耗时巨大（分钟至天），亟需高效代理模型以实现物理信息驱动的材料设计。

Method: 提出混合UNet-Transformer架构，输入为复合材料微观结构几何图像，输出十种应力/应变场，涵盖正方形、六边形、三角形等多种二维两相复合构型及不同边界条件。

Result: 在周期性拼贴几何上达到R²=0.9991、SSIM=0.9936、MAE=0.0050；六个子数据集MAE<0.05（归一化[0,1]尺度）；在非规则稀疏软相构型上性能下降（S11应力R²=0.7735）。

Conclusion: 模型具备物理有意义的内部表征能力，能聚焦相界、韧带连接点等力学关键区域；但在处理应力不连续性问题时存在平滑损失模型的固有局限性。
...
TL;DR: 提出了一种基于在线分层3D场景图（HSG）的零样本语义导航框架，通过层次化信念规划在长距离导航中显著提升成功率与效率。


<details>
  <summary>Details</summary>
Motivation: 现有语义导航方法缺乏结构化全局表示，导致决策依赖局部观察和贪婪策略，在长距离场景中探索效率低、行为短视。

Method: 在线增量构建分层3D场景图（HSG），形成对象-区域-地带的多粒度语义拓扑；基于HSG引入层次信念规划，融合语义先验与探索证据，在HSG模拟器上执行有限视界 rollout 以显式估计宏观动作的长期期望回报。

Result: 在高保真仿真环境的多任务多数据集实验中，长距离场景下成功率（SR）平均提升9.4%，路径效率（SPL）平均提升5.0%，超越现有最先进方法。

Conclusion: HSG结合层次信念规划能有效实现全局一致的语义导航决策，减少冗余回溯，尤其适用于长距离导航场景。
...
TL;DR: AnyMatch是一种利用单视图图像低成本生成大规模多模态训练数据的框架，通过单目深度估计、3D重投影、扩散修复和跨模态翻译合成具有3D几何一致性的多视角多模态图像对，构建的Any-syn数据集显著提升了多模态匹配网络的性能。


<details>
  <summary>Details</summary>
Motivation: 现有真实多模态数据集成本高、场景多样性有限，且SfM-MVS管线存在误差累积；合成方法难以同时保证3D几何一致性和逼真外观。因此需要一种低成本、高几何精度且可扩展的多模态训练数据生成方法。

Method: AnyMatch框架整合了单目深度估计、3D重投影、基于扩散的图像修复和跨模态图像翻译等技术，从单视图图像出发，合成具有严格3D几何一致性的多视角多模态图像对，并通过可调节的输入和相机参数控制场景多样性与标注难度。

Result: 构建了大规模数据集Any-syn，将LoFTR、EDM、RoMa等匹配网络在其上微调后，在多模态基准上取得了显著的性能提升，泛化能力和鲁棒性优于现有数据训练的模型。

Conclusion: AnyMatch有效解决了多模态匹配训练数据稀缺的问题，通过显式3D重投影避免了SfM-MVS误差累积，具有良好的可扩展性和实用性。
...
TL;DR: 本文提出从静态泛化向动态适应范式转变，通过Fleet框架（持续少样本进化+约束路由修正）和Treasure基准（64模型/36万图像），将AIGI检测性能从20.4%提升至73.1%（10-shot适应）。


<details>
  <summary>Details</summary>
Motivation: 现有AIGI检测方法基于静态特征空间假设，依赖历史数据中的不变伪影实现零样本泛化。然而面对快速演变的生成模型（如SD3、Nano Banana Pro），静态方法性能严重下降，亟需新范式。

Method: 提出Fleet框架：1）持续少样本进化范式，快速对齐新兴生成威胁；2）用约束路由修正替代无约束特征更新，通过解耦子空间中的回避路由将AI样本从非AI主导路由中重定向。

Result: 构建Treasure基准（64模型/360k图像，含20个闭源商业引擎）。静态SOTA在新型生成器上彻底失败，而Fleet在豆包Seedream 4.0上以10-shot适应将性能从20.4%恢复至73.1%。

Conclusion: AIGI检测需从静态泛化转向动态适应范式，Fleet框架验证了持续少样本进化+约束路由修正的有效性，为开放世界对抗防御提供了新方向。
...
TL;DR: 本文提出CasaMaestro，一种前馈模型，仅需20-50张稀疏多视角室内全景图即可直接预测度量深度和相机位姿，实现全屋快速点云重建，解决了传统针孔相机管道在大规模室内重建中效率低、易漂移的问题。


<details>
  <summary>Details</summary>
Motivation: 家庭部署的具身AI系统需要快速、度量的3D重建来支持导航和任务执行，但传统针孔相机3D重建管道视场角有限，覆盖多房间需数千张图像且易产生累积漂移。

Method: 提出前馈模型CasaMaestro，以20-50张稀疏多视角室内全景图为输入，直接预测度量深度和相机位姿，实现快速点云重建。

Result: 在真实和合成场景中均能鲁棒地提供高质量重建结果，支持全屋尺度的3D室内资产获取。

Conclusion: CasaMaestro是首个支持多视角全景图进行房屋级重建的模型，可为闭环仿真中的3D室内资产获取提供坚实基础。
...
TL;DR: InterTalk是一个面向对话式说话人脸生成的高效框架，支持多轮多人实时对话，通过运动架构、迭代生成策略和面部组件解耦，实现了30FPS的实时生成与高交互质量。


<details>
  <summary>Details</summary>
Motivation: 现有对话式说话人脸生成方法在灵活性（多人多轮对话）、自然性（连贯运动与非言语反馈）和效率（实时生成与低计算开销）三者之间难以平衡。

Method: 1) 基于运动架构的实时对话合成；2) 显式建模多轮对话动态，不限制参与者数量；3) 融合多人运动反馈的迭代生成策略；4) 将运动解耦为多个面部组件（如唇部同步、眨眼）进行针对性优化；5) 构建多人物对话数据集并利用3D人脸数据增强。

Result: InterTalk在保持30FPS实时性能的同时，在交互质量上优于现有方法。

Conclusion: InterTalk通过运动解耦与迭代生成策略，成功实现了交互自然性、多人灵活性与实时效率的统一，为对话式人脸生成提供了可行的解决方案。
...
TL;DR: 提出ART两阶段框架，利用真实锚定循环优化实现妆容迁移，并构建首个2K分辨率化妆人脸数据集MF2K。


<details>
  <summary>Details</summary>
Motivation: 现有妆容迁移方法受限于缺乏真实配对训练数据，依赖弱先验或合成伪目标，导致细节退化、伪影和身份漂移。

Method: 两阶段框架：Stage I用伪目标初始化建立语义对齐和全局妆容布局；Stage II从真实参考图像出发，通过可微循环从素颜重建参考，惩罚遗漏细节并覆盖伪影。

Result: 在妆容 fidelity、背景稳定性和身份保持方面优于现有方法，尤其擅长复杂妆容风格。

Conclusion: 真实锚定循环能有效克服伪目标监督的局限性，实现更高质量的妆容迁移。
...
TL;DR: 提出结构保持平均流（SPMF）框架，将血管图像翻译建模为拓扑不变的传输过程，通过正交约束和原型引导结构细化模块，在保留血管拓扑结构的同时实现跨模态翻译，在两个数据集上取得领先结果。


<details>
  <summary>Details</summary>
Motivation: 现有跨模态翻译方法主要关注像素级保真度或视觉真实性，将结构保持视为最终输出的属性而非生成过程的不变量，导致结构不连续和伪影，影响解剖连贯性和临床可靠性。

Method: 基于结构不变性原理推导流速场的正交约束，将其作为时间加权替代目标嵌入布朗桥扩散模型以在每个扩散步骤保持拓扑；提出原型引导结构细化（PGSR）模块，对齐退化的推理时结构与可靠的训练时结构。

Result: 在NIRII-to-2PF和眼底数据集上均优于现有方法，峰值PSNR分别达到24.96 dB和24.83 dB。

Conclusion: 通过将拓扑保持作为生成过程的不变量而非最终输出属性，有效解决了血管图像翻译中的结构不连续和伪影问题，提升了临床可靠性。
...
TL;DR: Horizon3D 是一种将高斯原语与稀疏 BEV 特征相结合的长距离雷达-相机融合框架，通过关键点引导初始化、对象中心稀疏融合和双路径时序融合实现先进的 3D 检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有雷达-相机融合方法在长距离 3D 目标检测中表现有限：BEV 方法计算量大且丢失细节，查询方法缺乏场景上下文，时序融合难以同时处理稀疏远距离观测和快速运动物体。

Method: 1) Keypoint-Guided Gaussian Initialization 在雷达和相机估计的关键点初始化高斯原语；2) Object-Centric Sparse Fusion 精化原语；3) 将原语溅射到 BEV 平面融合对象细节与稀疏雷达 BEV 上下文；4) Dual-Path Temporal Fusion 通过 BEV 路径进行场景级累积、高斯路径进行对象级运动传播。

Result: 在 TruckScenes 验证集上，Horizon3D 相比之前最佳方法 NDS 提升 +3.0，mAP 提升 +1.6，同时保持有竞争力的推理速度，达到 SOTA 性能。

Conclusion: Horizon3D 通过高斯原语与稀疏 BEV 特征的创新结合，有效解决了长距离检测中场景上下文与对象细节的平衡问题，以及时序融合中的累积与运动建模需求。
...
TL;DR: PiLoT v2是一种轻量级无人机地理定位系统，通过将像素直接注册到2.5D正交地图（TDOM+DSM）而非3D网格，显著降低了存储和计算成本，同时保持甚至超越前代精度。


<details>
  <summary>Details</summary>
Motivation: 解决PiLoT v1依赖重型3D网格导致的高存储开销、复杂地图获取和巨大计算渲染成本，阻碍其在嵌入式平台部署的瓶颈问题。

Method: 1）使用TDOM和DSM替代3D网格，将GPU密集型3D渲染替换为CPU友好的地图裁剪操作；2）训练跨视角特征配准网络，桥接2.5D正交裁剪与自由视角倾斜无人机图像之间的几何差异；3）将重力方向和单点激光测距等机载传感器先验直接集成到位姿优化流形中。

Result: PiLoT v2实现了与v1（Pixel-to-3D）相当甚至更优的性能，同时存储和计算成本大幅降低。

Conclusion: 通过从像素到3D注册转向像素到正交地图注册，PiLoT v2在保持高精度定位能力的同时，显著提升了系统轻量性和实用性，更适合在GNSS受限环境下部署于嵌入式平台。
...
TL;DR: 提出View-PNDF框架，通过神经元级别的视角特异性检测与微调，实现多视图X光报告的视角一致性生成，并在两个基准上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理多视图X光图像时采用直接特征融合，忽视了不同视图间潜在的临床不一致性和不准确性，影响模型性能和临床可靠性。

Method: 包含三模块：视角特异性神经元检测模块（识别响应特定视图的神经元）、验证模块（量化神经元存在性）、选择性微调策略（增强检测到的神经元，保留视角无关表示）。仅更新视角特异性神经元，用LLM整合视角报告为完整报告。

Result: 在两个医学RRG基准上，View-PNDF显著提升了视角特异性胸部X光报告生成质量，同时保持通用视角性能稳健。

Conclusion: 从神经元层面解决多视图报告生成的不一致性问题，参数高效且计算成本低，具有良好的临床可靠性和实用性。
...
TL;DR: TaxoMIL是一种基于临床分类学约束的框架，将全切片图像诊断重构为多粒度文本生成任务，利用双头Transformer解码器和分类学导向目标，在三个WSI数据集上优于现有MIL和VLM方法。


<details>
  <summary>Details</summary>
Motivation: 传统MIL将WSI诊断视为平面分类任务，忽略了临床推理中固有的层次化、从粗到细的特性；现有层次分类器和VLM方法未能捕捉相关诊断间的语义连续性，或产生不受约束的文本生成导致分类学幻觉和标签层级违反。

Method: 提出TaxoMIL框架：1) 将WSI诊断重新定义为多粒度文本生成任务；2) 使用双头Transformer解码器生成粗粒度和细粒度诊断文本；3) 引入分类学导向目标，显式结构化标签嵌入空间，并严格将切片级视觉表示锚定在临床分类学中。

Result: 在三个不同WSI数据集上的大量实验表明，TaxoMIL一致优于最先进的MIL分类器和VLM生成方法，产生准确且具有层次感知的诊断预测。

Conclusion: TaxoMIL通过分类学约束有效解决了传统MIL和VLM在层次化病理诊断中的不足，证明了将临床分类学知识显式融入模型设计能够提升诊断准确性和层次一致性。
...
TL;DR: InfiniVerse提出了一种从单帧图像生成长时间、可控、时空一致的动态城市场景的统一流水线，通过3D占用重建、自回归扩展和视频扩散模型实现2D-3D对齐与跨模态增强，在Waymo和nuScenes数据集上达到SOTA性能（FID 6.4, FVD 67.97）。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶领域缺乏能够生成逼真、可控且时序连贯的动态城市场景的有效方法，现有方法在长序列生成和时空一致性上存在不足。

Method: 1) 从输入的多视角单帧重建3D占用表示；2) 沿任意轨迹自回归扩展场景；3) 视频扩散模型将粗糙占用网格转化为逼真的时序一致视频序列；4) 分层草图-优化范式，通过视频反投影作为图像条件反馈来增强3D占用表示，实现视觉与空间域的跨模态对齐与相互增强。

Result: 在Waymo Open Dataset和nuScenes上达到SOTA，FID为6.4，FVD为67.97，在生成时长和稳定性上显著优于现有基准。

Conclusion: InfiniVerse有效解决了长距离动态城市场景生成的挑战，实现了2D-3D对齐的可控合成，为自动驾驶仿真提供了新思路。
...
TL;DR: 本文提出基于神经雅可比场（NJF）的单目人体动态建模方法，通过自监督学习预测雅可比矩阵解决泊松方程，并引入约束泊松求解器、符号距离正则化和残差光流损失，有效处理自遮挡和不可见区域，生成时间稳定且几何一致的逼真人体化身。


<details>
  <summary>Details</summary>
Motivation: 单目设置下，生成包含复杂运动（如衣物动态）的真实人体化身，需同时建模全局和局部变形，但现有方法在应对自遮挡和不可见表面时存在困难。

Method: 1) 利用神经雅可比场（NJFs）表示半刚性变形；2) 自监督学习预测雅可比矩阵，通过求解泊松方程得到姿态依赖变形；3) 提出三个关键组件：约束泊松求解器、基于符号距离的雅可比正则化、变形引导的残差光流损失，分别抑制边界伪影、恢复频繁遮挡区域（如腋窝、大腿）、增强时间一致性。

Result: 在基准测试和真实视频上的实验表明，该方法生成的时间稳定且几何一致的人体化身优于现有最先进方法。

Conclusion: 所提方法有效解决了单目人体动态建模中的自遮挡和边界伪影问题，为逼真人体化身生成提供了新的思路。
...
TL;DR: WildProp是一种无需训练的框架，通过从大规模非约束图像库中检索和匹配，自动估计野生动物身体比例分布，适用于多种物种且无需逐物种训练。


<details>
  <summary>Details</summary>
Motivation: 传统形态测量依赖受控成像或物理样本处理，难以大规模扩展。作者希望利用海量网络图像实现群体水平的形态测量，突破传统方法的规模限制。

Method: 基于检索驱动的对应问题：用户提供一张标注图像，WildProp利用基础模型特征进行姿态感知检索，通过密集补丁级匹配迁移部位端点，结合几何一致性过滤预测，并聚合多张检索图像的测量结果来估计群体比例分布。

Result: 在三个大型形态测量数据集（鸟类和两栖动物）上，中位相对误差为10-20%；在鸟类、青蛙、昆虫和花卉等多个类群的案例研究中展示了广泛适用性。

Conclusion: 精心整理的2D对应关系可从网络规模图像中提供可扩展的形态测量代理，支持跨 taxa、地理和季节性的比较与子群分析。
...
TL;DR: 本文提出SkillSpotter，一种用于增强现实眼镜或固定摄像头场景中实时个性化指导的姿势感知多视角架构，能够同时检测和分级技能动作，在Ego-Exo4D基准上实现mAP从12.40提升至21.82（+76%），平衡准确率从55.99%提升至60.40%。


<details>
  <summary>Details</summary>
Motivation: 现有系统需要理解人物执行活动的质量而不仅仅是动作内容，但现有时间动作检测方法在技能分级任务上表现近乎随机，无法同时实现检测与分级。

Method: 提出SkillSpotter，包含三个任务特定模块：(1)自适应时间抑制模块处理不同密度技能动作；(2)门控3D人体姿态融合模块利用身体运动学作为视觉特征的补充信号；(3)双向跨视角注意力模块有效结合第一人称和第三人称视角。

Result: SkillSpotter在Ego-Exo4D基准上将类特定mAP从12.40提升至21.82（+76%），平衡准确率从55.99%提升至60.40%，且各模块可迁移至其他时间动作检测模型，在HoloAssist数据集上也表现良好。

Conclusion: SkillSpotter通过多视角感知和姿态融合，有效解决了技能动作的检测与分级联合任务，为AR眼镜等场景中的实时个性化指导提供了可行的技术方案。
...
TL;DR: 提出LINet，一种通过线性积分卷积算子在每层实现连续跨模态融合的多流RGB-D场景分类网络，解决了传统融合策略的结构性折中问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态架构将特征融合视为离散的、特设的事件，早期融合过早纠缠表征，晚期融合隔离至最后一层，混合或注意力方法需要架构猜测来放置中间融合块，缺乏系统性的跨模态学习方法。

Method: 维护三条并行流（RGB、深度、整合），提出LIConv2d算子在非线性激活前连续整合跨模态信号；采用1/N常数初始化解决梯度崩溃问题；使用渐进式模态丢弃课程学习防止通路坍塌。

Result: 在SUN RGB-D 19类场景分类中，从头训练达到45.2%平均类准确率（ResNet18规模），使用ScanNet域内预训练提升至49.6%，超越现有从头训练方法。

Conclusion: 连续跨模态融合策略有效，解决了离散融合的结构性问题；初始化策略和渐进式模态丢弃对训练稳定性至关重要。
...
TL;DR: FROST是一种无训练的小样本分割方法，利用冻结的DINOv3特征和基于密度的非参数方法，在遥感图像上超越现有方法，且模型体积小。


<details>
  <summary>Details</summary>
Motivation: 现有小样本分割方法要么绑定在训练分布上，要么将类别压缩为有损摘要（单原型、聚类原型等），无法保留多模态类别的内部结构。作者认为类别更适合用分布而非点来描述，且冻结的自监督特征已足够直接估计该分布。

Method: 将参考前景和背景视为DINOv3特征单位球面上的两个点云，通过非参数密度比对查询token进行分类，贝叶斯规则在等先验下将阈值固定在零。所有参数（核带宽、空间门控等）均从支持集中读取而非调参。

Result: 在17个遥感基准上，FROST超越无训练和学习型方法，单样本标注领先5.6 mIoU，随支持集增多优势扩大，且模型尺寸最小之一。

Conclusion: 基于密度的无训练方法能更有效捕捉遥感图像中多实例分散的类别结构，且随参考样本增多决策更加锐利。
...
TL;DR: WaterGen是一种通过解耦场景内容和水体效果生成大规模、真实水下图像的方法，在潜在扩散框架中分别控制场景生成和物理准确的水体退化，有效提升了水下恢复和语义分割的下游性能。


<details>
  <summary>Details</summary>
Motivation: 现有水下视觉任务受限于训练数据匮乏，已有方法要么可控性高但真实性和多样性不足，要么能生成真实场景却无法独立准确地模拟水体介质效应。

Method: 将水下图像合成解耦为两阶段：1）微调潜在扩散U-Net生成无退化的水下场景内容潜在嵌入；2）对潜在嵌入进行物理准确的水体退化合成作为条件解码过程。

Result: 生成的大规模合成数据集在水下恢复和语义分割任务中持续提升了下游性能。

Conclusion: 场景生成与介质建模可在潜在扩散框架中解耦，实现多样化场景生成与精确可控的水下外观的兼顾。
...
<details>
  <summary>Details</summary>
Motivation: 现有3DVG方法在整个场景上进行推理，导致预测模糊且计算成本高。观察到指代表达通常依赖局部空间上下文，仅对应受限空间区域而非整个场景。

Method: 1)语言引导空间剪枝(LGSP)：利用冻结VLM识别语言相关区域；2)多视角条件描述重构(MCDR)：将复杂表达分解为目标-锚点关系，通过多视角推理补充空间线索；3)LLM-Grounder：将检测预训练的空间LLM改造为语言条件定位模型。

Result: 在ScanRefer三个设置上均达到SOTA，在Nr3D/Sr3D的10个设置中9个达到SOTA。

Conclusion: PruneGround作为即插即用框架，通过局部空间约束和语言引导有效提升了3DVG的准确性和效率。
...
TL;DR: 提出FAT框架，将基础模型与专用模型通过任务分解而非模型替代的方式进行协作，专用模型生成结构化预测假设，基础模型通过代理任务（如选择或验证）进行优化，在多项任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 基础模型在具身智能的结构化预测任务（如几何、数值估计）中不如专用模型，存在能力错配问题，需要探索两者如何更好地协作而非替代。

Method: 提出FAT框架，将结构化预测分解为三个阶段：专用模型预测、信息空间重建、基础模型代理推理。具体实例化为ProxySelect方法，使用VLM对专用模型生成的多模态候选进行选择或验证。

Result: 在2D/3D目标检测、轨迹预测、语义分割四项任务中，ProxySelect一致提升了专用基线模型性能，并显著优于直接使用基础模型进行回归，同时计算成本更低。

Conclusion: 专用模型保留任务特定结构，基础模型通过上下文代理推理优化假设，这种协作模式可作为通用原则指导基础模型与专用模型的融合。
...
TL;DR: 本文提出了一种双推理器系统（常规推理器+审慎推理器），通过FlatRoPE位置编码和Action-aware RL（AARL）后训练，将自动驾驶VLA规划器的推理步骤延迟降低约4倍。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中的VLA规划器使用链式因果推理（CoC）产生轨迹，但推理过程是自回归的，主导了推理延迟，而延迟是自动驾驶的操作性约束，因此加速推理步骤是核心问题。

Method: 1) 将推理拆分为两个专门路径：常规推理器（处理常规延续，关注轨迹历史）和审慎推理器（处理新情况，关注当前视觉证据），采用投机解码框架协作。2) 提出FlatRoPE（1D旋转位置嵌入）打破目标模型的3D M-RoPE旋转对称性，将注意力从视觉令牌重定向到轨迹历史令牌。3) 提出Action-aware RL（AARL）后训练阶段，使用动作质量奖励和静态参考KL锚。

Result: 双推理器系统相比原始Alpamayo规划器，推理步骤运行时间减少约4倍。

Conclusion: 通过两个专门化的推理路径分工协作，在保持推理质量的同时大幅加速了自动驾驶VLA规划器的推理过程，验证了非对称投机解码在自动驾驶领域的有效性。
...
TL;DR: 本文证明场景坐标回归（SCR）方法尽管被声称具有隐私保护特性，但实际上可以通过基于查询的攻击从训练模型中重建出高精度的3D几何结构和近似颜色外观，从而泄露训练环境的隐私信息。


<details>
  <summary>Details</summary>
Motivation: SCR方法被广泛用于视觉定位，且因其不显式存储图像或地图而常被认为是隐私保护的。本文旨在验证这一假设是否成立，揭露潜在的安全风险。

Method: 通过向SCR模型发送与目标场景无关的代理图像批次，获取密集的逐像素3D坐标；利用小扰动下的稳定性筛选可靠点，并在白盒环境下进一步优化；跨批次累积稳定点以恢复场景几何，同时反演网络特征从任意视角合成图像以获取外观信息。

Result: 在室内外数据集上，成功从SCR模型中重建出训练环境的大部分区域，几何保真度高；此外还恢复了近似颜色外观，暴露了可识别的场景布局和潜在敏感元素。

Conclusion: 直接反驳了SCR表示具有设计上隐私保护的声称，揭示了在私密或安全关键场景中部署此类系统存在真实隐私泄露风险。
...
TL;DR: 本文提出S-OBI基准，通过合成清晰的句子级甲骨文实例，评估多模态大语言模型在句子级甲骨文理解上的能力。实验表明，当前模型在句子级理解上表现不佳，仍过度依赖字符级识别。


<details>
  <summary>Details</summary>
Motivation: 现有甲骨文研究多停留在字符级别，忽略了完整占卜辞中的长程文本连贯性和上下文依赖关系。

Method: 通过字形替换与合成，将95片专家验证过的原始拓片中的字符替换为清晰的字形样本，生成标准化句子级甲骨文实例，并设计语义匹配、语义槽提取和上下文推理三种任务，共695个问答对。

Result: 当前多模态大语言模型在句子级甲骨文理解上表现不佳，视觉感知误差会通过推理链传播，导致对掩码字符的预测错误，表明模型仍强依赖于字符级识别。

Conclusion: S-OBI提供了一个诊断基准，用于评估MLLMs是否能够超越孤立字符识别，实现结构化的句子级甲骨文理解。
...
TL;DR: 本文提出HSDF-Lane方法，通过隐式建模道路表面的高度对齐符号距离场（HSDF）和车道感知语义位置编码（LSPE），在单目3D车道检测和高度图估计任务上取得了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有单目3D车道检测方法存在两个主要问题：一是基于平坦地面假设的BEV投影会导致真实道路上的几何畸变；二是现有方法虽然开始预测高度图以捕捉非平面表面，但仍依赖稀疏的锚点回归，并且仅将恢复的几何信息用于空间变换而非语义理解。

Method: HSDF-Lane方法包含两个核心组件：1) 高度对齐符号距离场（HSDF），在密集采样的3D特征体积上隐式建模道路表面，通过可微渲染同时生成精确的高度图和表面对齐特征；2) 车道感知语义位置编码（LSPE），将源自表面对齐特征的车道存在先验注入Transformer查询中，实现几何结构与语义引导的耦合。

Result: 在OpenLane基准上的大量实验表明，HSDF-Lane在3D车道检测和高度图估计两项任务中均达到最先进性能。

Conclusion: 通过隐式神经场建模道路表面几何，并将几何信息与语义信息深度融合，可以有效克服传统方法的几何畸变和语义-几何分离问题，提升单目3D车道检测的准确性。
...
TL;DR: 本文提出GaussianMap，一种在线高精地图构建框架，通过学习自适应高斯场景表示（由一组高斯基元组成），替代传统固定分辨率密集网格的BEV表示，在nuScenes和Argoverse 2数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统BEV表示使用固定分辨率密集网格编码周围空间，但地图元素在空间上是稀疏的且需要精细的几何定位，导致均匀分配的BEV表示冗余且低效。

Method: 提出GaussianMap框架：1) 学习一组在BEV平面上的高斯基元作为场景表示，每个基元编码灵活局部区域的几何属性和特征向量；2) 设计前馈高斯编码器，通过高斯交互建模和多传感器特征聚合逐步优化基元；3) 将优化后的高斯表示通过splatting转换为BEV特征图，再解码为矢量化地图预测。

Result: 在nuScenes和Argoverse 2数据集上，GaussianMap在纯相机和相机-激光雷达融合设置下均达到SOTA性能。

Conclusion: 自适应高斯表示能更有效地分配模型容量到地图相关区域，解决了均匀密集网格表示的冗余问题，提升了在线高精地图构建的效果。
...
TL;DR: 本文提出UCF101-AD基准，用于评估多模态大模型识别“动作否定”（即动作未发生）的能力。实验发现20个SOTA模型在动作肯定任务上准确率超85%，但在否定任务上低于50%，暴露出模型缺乏因果推理能力。作者提出CausalAct因果图框架，通过自然语言提示注入因果线索，有效减少了假阳性。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在标准视频理解基准上表现优异，但缺乏对“动作否定”这一反事实推理能力的评估，即当上下文强烈暗示某个动作正在发生时，模型能否正确判断该动作并未实际发生。作者希望揭示并解决这一盲点。

Method: 1) 构建UCF101-AD基准，包含正样本（动作发生）与负样本（动作未发生）的配对视频，负样本保留相同的人物、物体、场景等上下文线索，仅缺失核心动作；2) 评估20个SOTA MLLMs；3) 提出CausalAct因果图，用自然语言提示显式表达场景结构中的上下文、交互与动作之间的因果关系。

Result: 20个SOTA模型在动作肯定任务上准确率超过85%，但在动作否定任务上准确率低于50%，表明模型倾向于“确认”而非“验证”。引入CausalAct因果线索后，假阳性显著降低。

Conclusion: 现代视频理解模型存在关键的因果推理盲点，无法判断动作是否真实发生。“否定”是一种可学习的推理技能，因果建模是缓解该问题的有效方向。UCF101-AD为诊断和改善多模态模型的因果推理提供了新视角。
...
TL;DR: 提出了一种用于经直肠超声前列腺实时分割的时间一致学习框架，通过知识蒸馏将时间一致性注入2D网络，并引入置信加权时间一致性目标和双尺度原型对齐模块，在保持单帧推理效率的同时实现了最先进的准确性和时间一致性。


<details>
  <summary>Details</summary>
Motivation: 解决TRUS视频前列腺分割中2D方法忽略时间上下文导致帧间不一致，而3D方法延迟过高无法实时应用的问题。

Method: 1）置信加权时间一致性目标（基于光流残差抑制不稳定区域梯度）；2）双尺度原型对齐模块（局部边界+全局语义的对比学习）；3）几何等变伪标签+知识蒸馏，避免密集帧标注。

Result: 在SUN-SEG和自建TRUS-V基准（2679帧）上实现了最先进的准确性和时间一致性，且达到实时速度。

Conclusion: 利用前列腺几何稳定性的临床观察，通过选择性时序约束和语义对齐，有效平衡了分割精度、时间一致性与推理效率之间的矛盾。
...
TL;DR: ExPLoRe利用Soft MoE的调度权重作为逐补丁损失系数，通过损失耦合机制实现多目标掩码图像建模中不同补丁的自适应损失加权，在ImageNet-1K和下游任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有MIM方法使用全局标量加权多个学习目标，忽略了不同图像块之间的空间异质性，无法实现内容自适应的损失分配。

Method: 提出ExPLoRe框架，将Soft MoE的调度权重重新用作逐补丁的损失系数；核心机制是损失耦合（loss-coupling），允许损失梯度通过调度权重反向传播到路由器，实现内容依赖的专化；同时开发了下游迁移适配策略（冻结路由、专家丢弃、冻结注意力）。

Result: 在ImageNet-1K上，ViT-Base实现Token+CLS组合：k-NN +0.5%，线性探测 +4.4%；Token+Pixel组合：k-NN +2.2%；线性探测达80.6%，微调达85.3%。消融实验证实损失耦合是关键机制，梯度阻断后性能下降1.6%。下游迁移中，MoE微调提升+1.5%，在ADE20K上缩小2.5-2.9 mIoU分割差距。

Conclusion: ExPLoRe通过逐补丁的自适应损失加权有效解决了MIM中空间异质性问题，损失耦合机制是核心创新，提出的迁移适配策略使得MoE模型在下游任务中能够匹配或超越非MoE基线。
...
TL;DR: 本文提出AC3S框架，通过自适应条件控制解决扩散模型在3D结构约束下生成图像时过度条件化导致的失真问题，结合多智能体视觉语言模型生成3D感知提示，实现了高质量、结构一致的合成数据生成。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在生成3D结构一致图像时，依赖边缘图等视觉提示进行条件控制，但存在过度条件化问题，导致图像真实感下降，限制了合成数据集的生成质量。

Method: 提出AC3S框架，包含两个核心组件：(1) 自监督视觉提示调制器，动态调整ControlNet条件控制强度，防止过度条件化；(2) 多智能体视觉语言模型框架，生成与几何结构对齐的3D感知提示，增强多样性和语义一致性。

Result: 大量实验表明，该方法在图像质量和下游任务实用性上均取得显著提升。

Conclusion: AC3S框架通过自适应条件机制有效平衡了3D结构约束与图像真实感，为大规模高质量合成数据集生成提供了可行方案。
...
TL;DR: 本文提出了AA，一个多视图多模态的屏幕式 gaze 估计数据集，包含8个屏幕固定摄像头和2个侧视摄像头，支持多视角下的 gaze 估计研究。


<details>
  <summary>Details</summary>
Motivation: 现有 gaze 数据集多为单视角，难以处理视角变化和遮挡问题，缺乏多视角覆盖的数据支持。

Method: 采集屏幕固定摄像头（8个）和侧视摄像头（2个）的同步面部观测数据，并配准精确的屏幕空间 gaze 目标，同时提供结构化面部区域裁剪以支持全局和局部多模态学习。

Result: 构建了一个包含多视角面部观测和结构化面部区域裁剪的数据集，提供 subject-independent 评估划分和标准化数据处理流程。

Conclusion: AA 数据集通过多视角多模态设计，能够提升 gaze 估计模型在视角变化和遮挡情况下的鲁棒性，并支持可重复研究。
...
TL;DR: CooperScene是一个高保真协作自主数据集，包含真实C-V2X通信特征，涵盖3辆CAV和1个RSU，59K帧中标注344K对象，支持多智能体扩展和实际部署性能评估。


<details>
  <summary>Details</summary>
Motivation: 现有协作自主数据集忽视了实际部署中的复杂因素，如有限通信带宽及其动态性、异构感知模态以及超越单一协作伙伴的可扩展性。

Method: 构建包含交叉口、高速匝道和停车场等场景的数据集，涉及3辆CAV和1个RSU，配备多模态传感器和商用C-V2X通信无线电，实现严格同步、厘米级定位、精确标定及3GPP标准兼容的C-V2X通信。

Result: 数据集包含59K帧，344K个标注对象，以10Hz频率提供全局一致的3D标签。

Conclusion: CooperScene为评估多智能体扩展和实际可部署环境中的性能提供了严格的基准。
...
TL;DR: ForgeDrive提出了一种“先行动后想象”的自动驾驶范式，通过统一自回归扩散框架和视觉-动作交叉条件化，避免了传统方法中视觉生成错误向动作规划级联的问题，在NAVSIM上优于现有规划器。


<details>
  <summary>Details</summary>
Motivation: 传统自动驾驶的“先想象后行动”范式存在严重缺陷：视觉生成阶段的错误会级联传导至动作规划阶段，导致整体性能受限。作者希望打破这一耦合，让动作生成不再依赖于完美的未来帧预测。

Method: 提出ForgeDrive框架：(1)将未来分解为逐时间步的帧-动作对序列；(2)训练时解耦两模态的扩散时间步，采用UniDiffuser风格噪声调度器实现跨模态推理；(3)推理时采用“先行动后想象”范式，利用生成的动作提升未来帧质量；(4)引入未来自车状态预测增强规划能力。

Result: 在NAVSIM数据集上的大量实验表明，ForgeDrive不仅将驾驶模拟、规划和视觉里程计统一到单一模型中，而且无需任何后训练策略即超越现有强规划器。

Conclusion: 通过“先行动后想象”的范式逆转，成功解决了视觉-动作级联错误问题，证明了动作生成可以作为内在能力被模型内化，无需依赖未来帧作为先决条件，反而能提升未来帧预测精度。
...
TL;DR: 本文提出了首个超高清多焦点融合数据集UHD-MFF，并设计了尺度专用的查找表框架UMF-LUT，实现了4K实时融合，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有MFF方法局限于低分辨率，在超高清场景面临数据、模型适配和部署三大障碍，亟需突破。

Method: 提出UHD-MFF数据集和UMF-LUT框架，包含粗区域LUT（低分辨率梯度+语义联合查询）和细节边缘LUT（高分辨率拉普拉斯边缘决策）。

Result: 在视觉保真度和量化指标上均超越SOTA方法，实现实时4K融合，并具备手机端部署潜力。

Conclusion: 该工作有效推动了多焦点融合向超高清成像场景发展，代码已开源。
...
TL;DR: 提出了一种双曲空间的手术视频-语言预训练框架，通过保留层次结构（如动作→步骤→阶段）来改进手术相位识别，在零样本和少样本场景下取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 现有手术视觉-语言基础模型将多层次异构信号（细粒度动作、中粒度步骤、全局上下文）压缩到同一扁平嵌入空间，忽略了跨层次的语义包含关系（如动作属于步骤、步骤组成阶段），导致长程依赖建模能力减弱。

Method: 提出双曲手术视频-语言预训练框架：1) 在双曲空间中显式保持层级结构；2) 缓解结构性的假负样本问题；3) 强制父阶段与子步骤之间的语义一致性。

Result: 在多个手术基准数据集上，零样本和少样本相位识别任务均取得一致性提升，跨术式和跨机构泛化能力增强。

Conclusion: 在双曲空间中建模手术知识的层次结构能有效提升视频理解性能，尤其在标注稀缺场景下优势明显。
...
### [57] [Learning from Failure: Inference-Time Self-Improvement for Computer-Use Agents](https://arxiv.org/abs/2606.31270)
*Xueqiao Sun,Xiaohan Wang,Ludwig Schmidt,Serena Yeung-Levy,Yuhui Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种“失败驱动”的自我改进范式，利用LLM分析失败轨迹、生成代码补丁来提升计算机使用代理的性能，在OSWorld基准上将SOTA模型成功率从42.3%提升至48.9%，无需额外训练成本。


<details>
  <summary>Details</summary>
Motivation: 现有计算机使用代理的自我改进范式仅利用成功轨迹进行微调，丢弃了大量包含丰富信息的失败轨迹，导致数据利用不充分，模型改进效率受限。

Method: 提出失败驱动的自我改进循环：使用LLM诊断失败模式、提出推理时解决方案并生成代码补丁，经人工轻量验证后将补丁升级到代理系统中。

Result: 在OSWorld基准上，将OpenCUA-72B模型的成功率从42.3%提升至48.9%，提升6.6个百分点，且无需额外训练成本，仅需少量推理开销。

Conclusion: 失败驱动的自我改进是一种可行的、与基于成功的管道互补的代理改进范式，能够更高效地利用数据提升模型性能。
...
TL;DR: 本文提出了一个紧凑的多模态时序建模框架，在EI-MIGA-IJCAI挑战赛Track 3中获得第一名（测试准确率0.76923），通过交叉注意力机制、多实例学习自适应池化等方法解决长视频中微弱稀疏隐式情绪证据的识别问题，并揭示了视觉基础模型在微动态任务中的表征坍缩现象。


<details>
  <summary>Details</summary>
Motivation: 长视频中隐式情绪证据微弱且稀疏，现有方法难以有效捕捉动态微表情和时序线索。

Method: 1) 多源特征融合（2D/3D骨架、面部Blendshape、DINOv2/v3、X-CLIP、Gemini语义先验）；2) 交叉注意力机制（静态姿态Base作为Query，动态微动差分Offset作为Key/Value）；3) 基于多实例学习的自适应池化提取瞬时情绪并抑制背景噪声。

Result: 在Track 3中取得测试准确率0.76923，获得第一名。

Conclusion: 视觉基础模型在微动态任务中存在表征坍缩现象，网络因捷径学习和死记硬背陷入排行榜驱动的伪泛化。
...
### [59] [Visual Semantic Entropy: Do Vision Language Models Recognize Visual Ambiguity?](https://arxiv.org/abs/2606.31407)
*Ta Duc Huy,Trang Nguyen,Townim Chowdhury,Ankit Yadav,Minh-Son To,Zhibin Liao,Johan W. Verjans,Vu Minh Hieu Phan*

Main category: cs.CV

TL;DR: 现有视觉语言模型（VLM）在视觉模糊输入下会产生过度自信的偏见预测。本文提出视觉语义熵（VSE），通过仅扰动图像、固定文本查询，并基于语义原型聚类与质量加权离散度计算不确定性。在5个现代VLM和5个VQA基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有基于熵的不确定性估计方法（如语义熵SE）在视觉模糊输入时，因过度自信的视觉嵌入抑制了输出多样性，导致低估不确定性；而基于输入扰动（文本改写或图文联合扰动）的方法产生的变异性多由文本变化主导，无法反映真正的视觉模糊性。

Method: 提出视觉语义熵（VSE）：固定文本查询不变，仅对输入图像进行扰动以探索附近的视觉变体；对生成的答案进行语义原型聚类，并计算各原型之间的质量加权离散度作为不确定性度量。

Result: 在5个现代VLM和5个VQA基准上的广泛评估表明，VSE有效捕获了视觉模糊性，达到了VLM不确定性估计的新SOTA。

Conclusion: 仅扰动图像而非文本或联合扰动，能更准确地反映视觉层面的不确定性，避免文本敏感性干扰。
...
TL;DR: 线性探针和引导恢复会高估VLM对图像的 grounding，通过空白图像控制实验发现空间推理存在三种机制：grounded（视觉依赖）、prior（视觉独立）和 inverted（符号反转），其中 depth 轴在多个模型中呈现反转现象。


<details>
  <summary>Details</summary>
Motivation: 现有方法（线性探针+引导恢复）会系统性地高估VLM对图像的 grounding，标准评估无法区分模型是真的感知了视觉信息还是依赖先验或反转表征。

Method: 使用"空白图像"作为仲裁控制（causal control），对比正常图像与空白图像下的解码表现；结合训练-free 的投影旋转和训练的低秩编辑来纠正反转问题。

Result: 在14个模型（2B-27B，6个LM家族）中，horizontal轴是grounded，vertical轴是prior，depth轴是inverted；反转现象在7/8个模型的5个家族中复现；训练-free的旋转在干净模型上可匹配训练编辑，分布式反转需要低秩编辑。

Conclusion: 空白图像仲裁是一种廉价、自校准的控制方法，能清晰区分grounded perception、inverted perception和prior substitution，应成为VLM中潜在知识和引导研究的默认控制手段。
...
### [61] [SpikeLogBERT: Energy-Efficient Log Parsing Using Spiking Transformer Networks](https://arxiv.org/abs/2606.31781)
*Thuan Bui,Duong Do,Tung Vu,Duc-Tho Mai,Cong-Kha Pham*

Main category: cs.CV

TL;DR: 本文提出 SpikeLogBERT，一种基于脉冲神经网络（SNN）的日志解析框架，通过脉冲变压器架构和知识蒸馏技术，在 HDFS 数据集上以 0.99997 的解析精度超越了传统 ANN 模型，同时理论能耗降低高达 62.6%。


<details>
  <summary>Details</summary>
Motivation: 现有神经网络日志解析方法依赖密集矩阵乘法，导致计算成本高、能耗大。为了在保持语义表示能力的同时实现节能的日志解析，作者提出了 SpikeLogBERT。

Method: SpikeLogBERT 采用脉冲变压器架构，结合从 BERT 教师模型进行知识蒸馏，利用脉冲神经网络的稀疏脉冲激活和事件驱动特性，减少推理过程中的活跃操作数。

Result: 在 HDFS 数据集上，SpikeLogBERT 的解析准确率达到 0.99997，优于基于 ANN 的神经日志解析模型，并在标准 45nm CMOS 假设下理论能耗降低高达 62.6%。

Conclusion: SpikeLogBERT 是一种高效节能的日志解析方案，证明了脉冲神经网络在日志分析领域具备与 ANN 相当的解析能力，同时大幅降低能耗，适合资源受限场景下的部署。
...
TL;DR: WarpHammer是一种无需训练的框架，通过引入显式3D对象重建来增强投影条件的新视图合成，解决了大视角变化下的伪影和内容丢失问题，并首次支持融合外部辅助视图（无需相机位姿）。


<details>
  <summary>Details</summary>
Motivation: 现有投影条件NVS方法在小视角变化下效果良好，但在大轨道运动下，由于遮挡表面增多和镜像伪影，扭曲变得稀疏，导致生成器丢失像素内容和隐式相机线索。

Method: 提出WarpHammer：1）利用原生3D生成先验（如SAM3D）对物体进行显式3D重建，补充缺失的前景表面并遮挡不应再可见的背景点；2）联合参考图像和辅助视图，通过预训练的多视图几何基础模型预测统一点云，融合到3D对象重建中，无需用户提供辅助视图的相机位姿。

Result: 在五个基准上，WarpHammer在视角偏差大的情况下生成稳定的新视图，而强基线方法在此条件下崩溃。它是首个能自然融合外部、未知位姿辅助视图的场景级NVS方法。

Conclusion: WarpHammer通过显式3D对象重建和辅助视图融合，有效解决了大视角下NVS的退化问题，扩展了场景级新视图合成的能力边界。
...
TL;DR: CLIMB是一种结合层次化质心记忆与知识蒸馏的在线持续自监督学习方法，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有OCSSL方法要么依赖利用潜在空间结构的重放缓冲区，要么仅依赖正则化，缺乏两者结合的方案。

Method: 提出层次化质心记忆库（CLIMB），对相似图像分组为质心以有限存储覆盖分布多样性，结合知识蒸馏防止表征漂移。

Result: 在Split CIFAR-100和Split ImageNet-100上超越现有OCSSL方法，在不规则任务分布的新协议下同样表现优异。

Conclusion: 同时结合重放与正则化是提升OCSSL性能的有效策略。
...
TL;DR: MICE是一种无需训练的多实例图像编辑策略，通过修改联合注意力机制中的加性偏置来调控实例间交互，实现了单次前向传播中多个元素的解耦编辑，在最多8.5个并发编辑场景下优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理多个编辑指令时，由于语义干扰导致属性泄露和编辑解耦效果差，尤其随着编辑数量增加问题愈发严重。

Method: MICE通过修改多模态扩散Transformer中联合注意力的加性偏置，允许实例内部注意力，惩罚相邻区域token之间的交互，抑制无关的跨实例注意力，从而实现属性绑定并保持全局视觉一致性。

Result: 在LoMOE-Bench和新提出的MICE-Bench（平均每图8.5个并发编辑）上，MICE在视觉质量保持和编辑指令忠实度方面均优于强基线和近期竞品方法。

Conclusion: MICE是一种有效的训练免费策略，能够在大规模多实例图像编辑中实现良好的解耦效果和视觉一致性。
...
TL;DR: 本文提出 ToothForge，一种基于谱域的深度生成框架，通过同步谱嵌入对牙冠几何形状进行建模，能在小样本数据集上实现高效、紧凑且几何可解释的3D牙齿形状生成与重建。


<details>
  <summary>Details</summary>
Motivation: 牙冠形态的精确建模对诊断、正畸规划和计算机辅助修复设计至关重要，但适合训练此类模型的数据集通常规模有限，亟需一种能在小样本条件下有效工作的建模方法。

Method: 提出 ToothForge 框架，在谱域中通过同步谱嵌入学习3D牙齿形状的潜在流形，利用谱同步缓解Laplace-Beltrami特征基的不稳定性，在低维空间中实现高效学习，确保不同样本间的一致建模。

Result: 经过鲁棒性分析、消融实验以及与PCA统计形状模型和基于点的生成框架对比，同步谱建模在重建和生成性能上达到或超过空间方法，同时保持紧凑性和几何可解释性。

Conclusion: 紧凑的同步系数和低维学习空间使该框架特别适用于数据有限（如牙科和医学领域）的场景，且能应用于临床间形状连接不一致的实际场景。
...
TL;DR: 本文提出了一种基于小波优化的伪3D加速扩散模型（CL-DM），用于解决计算层析成像（CL）中因视野限制导致的投影数据截断问题，通过扩展重建目标至数据不完全区域，结合2D扩散模型与3D模型迭代重建，显著提升成像范围与重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法通常依赖2D架构且缺乏严格的数据一致性约束，同时将伪影去除限制在视野内，丢弃了视野外可能恢复的信息。本文旨在克服这些局限，扩展有效成像范围并提高扫描效率。

Method: 首先对CL视野进行综合分析，将空间划分为数据完全、数据不完全和数据缺失区域；提出小波优化的伪3D加速扩散模型（CL-DM），采用标准2D扩散模型进行切片聚合，结合3D模型迭代重建保证数据一致性，引入z方向小波正则化、平移不变机制和低频保持策略，并设计3D快速采样架构加速推理。

Result: 大量仿真和真实实验表明，CL-DM能有效消除截断伪影，恢复高保真、连续的3D结构。

Conclusion: 该方法通过扩展重建目标和创新模型设计，显著提升了CL在视野受限情况下的成像性能。
...
TL;DR: 提出ALM方法，无需训练即可在扩散模型反向采样过程中直接优化未观测区域，生成全局一致的内容，并引入加速策略提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在部分输入生成任务中存在局限：基于训练的方法计算成本高、泛化差；无训练方法未显式优化未观测变量，导致全局不一致。

Method: 提出ALM（Accelerated Likelihood Maximization），一种集成到反向扩散过程中的无训练采样策略，直接优化未观测区域，并引入加速策略提升计算效率。

Result: ALM在多个数据域和任务上持续优于现有最先进方法。

Conclusion: ALM建立了一种强大的通用内容生成范式，兼顾全局一致性与计算效率。
...
TL;DR: Vega是一个统一视频理解与生成的框架，通过共享词汇表联合建模文本和视觉表示，采用自回归预测关键帧语义Token与扩散模型渲染高分辨率视频的混合架构。


<details>
  <summary>Details</summary>
Motivation: 视频领域统一理解与生成的研究尚不充分，核心挑战在于视频理解需要紧凑的语义表示，而视频生成需要密集的视觉细节和时序连贯性。视频天然包含空间语义和时间动态，比静态图像更适合统一多模态建模。

Method: 提出Vega框架，使用共享词汇表联合建模文本和视觉表示，采用混合架构：自回归模型预测关键帧的语义视觉Token提供结构化表示，扩散模块基于该表示渲染密集的高分辨率视频帧。

Result: 在VBench（视频生成）和VideoMME（视频理解）等基准测试上取得了强性能表现。

Conclusion: Vega证明了统一视频理解与生成的可行性，混合AR+扩散架构能有效平衡语义紧凑性和视觉细节保真度。
...
TL;DR: 本研究在MURA肘部X光片上，评估了多种预处理流程（含DnCNN学习模块）对骨科异常检测的影响。结果表明，与原始输入的DenseNet121基线相比，预处理带来的判别性能提升有限且依赖配置，仅在校准指标上有所改善，未有一致的判别优势。


<details>
  <summary>Details</summary>
Motivation: 探究在面向分诊的骨科异常检测任务中，学习型预处理流程是否能够有效提升模型性能，尤其是在患者级别、泄漏感知的评估协议下。

Method: 使用MURA肘部X光数据集，采用患者级别的泄漏感知评估协议，对比多种预处理流程（含/不含轻量级DnCNN模块），使用DenseNet121作为骨干网络，通过AUROC、PR-AUC、ECE、Brier评分及高特异性操作点分析进行评价。

Result: 不同预处理策略之间的差异较小且依赖配置，无一致判别优势超过原始输入基线。原始或多样化输入结合DnCNN可降低ECE和Brier评分，但CLAHE结合DnCNN未改善校准性能。

Conclusion: 在患者级别评估下，预处理带来的增益有限且依赖具体配置；原始输入的DenseNet121基线在各项指标上仍保持竞争力，未发现某种预处理策略能带来一致的判别优势。
...
TL;DR: 本文提出RCL-Mamba，一种基于状态空间模型的双域图像恢复网络，用于解决旋转扫描计算层析成像中的旋转模糊和稀疏伪影问题，在保证重建质量的前提下将扫描视角从512减少到64，提升检测效率约8倍。


<details>
  <summary>Details</summary>
Motivation: 现有旋转扫描计算层析成像（RCL）在稀疏视角快速扫描中面临跨域退化问题：投影域存在旋转模糊，重建图像域存在稀疏伪影，缺乏有效的联合处理方法。

Method: 提出RCL-Mamba框架，采用级联联合处理策略：先在投影域校正旋转模糊，再在图像域抑制稀疏伪影；并设计Mamba-CNN双分支模块，自适应平衡大尺度模糊校正与局部细节恢复。

Result: 在模拟数据和真实PCB扫描数据上，RCL-Mamba在模糊去除、伪影抑制和结构保真方面超越现有基线；将扫描视角从512降至64，检测效率提升约8倍且重建质量不降。

Conclusion: RCL-Mamba为高吞吐RCL检测提供了一种鲁棒的面向测量的恢复方案，显著提升了结构测量保真度。
...
TL;DR: 提出LA-SR框架，利用视觉语言模型将无配对超分问题映射到语言空间解决，通过从真实图像中提取不同深度区域的LR补丁，实现无需合成数据的单图像超分辨率重建。


<details>
  <summary>Details</summary>
Motivation: 现有SR方法依赖人工合成降质（手工核函数或ISP调整）来生成LR-HR配对数据，但合成降质无法捕捉真实LR图像的复杂性，导致模型在实际应用中泛化能力差。真实场景中获取配对HR-LR数据极为困难。

Method: 利用单张高质量图像中不同深度区域自然存在分辨率差异（远处为LR，近处为HR）的特性，提取真实降质LR补丁；提出LA-SR框架，通过视觉语言模型将无配对SR问题重新定义到语言空间，设计语言内容损失和语言质量损失两种语言引导损失函数，实现语义保真度和感知真实性的双重对齐。

Result: LA-SR能够有效对真实LR输入进行超分辨率重建，生成逼真的输出结果，克服了基于合成数据训练方法的局限性。

Conclusion: 将无配对超分问题映射到语言空间是解决真实场景SR的有效途径，视觉语言模型可以桥接LR-HR之间的差距，无需依赖人工合成降质数据。
...
TL;DR: 本文提出双流双层循环优化框架，将Cycle Self-Training（CST）从图像分类扩展到目标检测领域，通过双流结构、均值教师和额外归一化解决伪标签不可靠、训练不稳定和回归损失爆炸三大挑战，在四个跨域场景中取得显著效果。


<details>
  <summary>Details</summary>
Motivation: CST在图像分类中成功，但直接扩展到目标检测面临三大挑战：三类不可靠伪标签（分类错误、定位错误及其组合）限制性能上界；多目标检测导致训练不稳定；回归坐标数值范围宽导致损失爆炸。

Method: 提出Dual-Stream Bilevel-Cycle Optimization框架，将CST同时应用于分类和回归分支；基于Mean Teacher构建CST防止训练不稳定；使用额外归一化将回归边界框映射到标准化空间解决损失爆炸问题；并提供了回归边界的理论推导。

Result: 在四个跨域标准场景上的大量实验表明，所提框架取得了显著的结果。

Conclusion: 通过双流设计和理论保障，成功将CST从图像分类推广到目标检测，有效解决了伪标签不可靠、训练不稳定和回归损失爆炸等关键问题。
...
TL;DR: 提出了一种名为MAPE的深度学习防御方法，通过多源对抗扰动消除机制有效防御多种可迁移对抗攻击，在CIFAR-10和Mini-ImageNet上分别达到95.1%和71.5%以上的平均防御率。


<details>
  <summary>Details</summary>
Motivation: 对抗样本可导致神经网络高置信度误分类，其中可迁移对抗攻击因其隐蔽性强、检测难度大而成为防御重点，现有方法难以有效应对多种源模型生成的对抗扰动。

Method: 提出MAPE框架，包含两个核心组件：1）SAPE机制，采用通道注意力U-Net作为防御模型，利用单个预训练模型生成的对抗样本进行训练以消除已知扰动；2）PPSA算法，通过模型差异量化和负动量策略调度多个预训练模型，最大化训练过程中对抗样本的差异性，增强模型鲁棒性。

Result: 在黑盒攻击场景下，以ResNet-34为目标模型，MAPE在CIFAR-10上平均防御率超过95.1%，在Mini-ImageNet上超过71.5%，达到最先进水平。

Conclusion: MAPE能有效消除多种对抗样本中的对抗扰动，为来自不同替代模型的攻击提供鲁棒防御。
...
TL;DR: 提出MS-Resampler，通过多范围视觉重采样框架，在固定token预算下同时捕获局部细节和全局上下文，提升多模态大模型的视觉理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM的重采样器使用单一固定聚合范围的全局交叉注意力，导致细粒度局部证据模糊，难以在固定token预算下同时捕获局部细节和全局上下文。

Method: MS-Resampler实例化多个范围特定的重采样器，通过注入显式的空间范围先验到重采样注意力中，使每个分支从局部到全局以特定粒度聚合视觉信息，并自适应融合各分支输出。

Result: 在十个公共多模态基准上，MS-Resampler一致优于传统单范围重采样器，且仅引入极小的计算开销。

Conclusion: 多范围重采样策略能有效提升MLLM的视觉理解和多模态推理能力，是一种轻量且高效的改进方案。
...
TL;DR: OVOW是一个无需训练的4D网格重建系统，能从单目视频生成实例级、可直接用于物理仿真的水密网格场景，在多个基准测试中取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有4D重建方法虽然渲染质量高，但输出格式（隐式场、高斯原语、点云）缺乏水密拓扑、实例分离和标准化物理接口，无法直接用于物理仿真和具身AI。

Method: 四阶段pipeline：1）视觉语言模型发现、标注并分类运动实例；2）类别感知重建生成刚体网格和变形体拓扑一致网格序列；3）迭代渲染-匹配-优化恢复度量尺度和6-DoF位姿轨迹；4）物理接地组装保证地面接触和物体间支撑关系。

Result: 在静态和4D合成基准上取得最优布局和几何精度，最低光度学和语义误差；速度比基线快一到两个数量级；下游物理仿真验证了物理稳定性。

Conclusion: OVOW首次实现了从单目视频到结构化、仿真就绪的4D网格场景的无需训练重建，并为结构化视频到4D的评估建立了新基准。
...
TL;DR: 提出TemporalLens诊断框架和YOLO-3D时空检测器，揭示单阶段视频目标检测器是否真正利用时序信息，而非仅依赖单帧。


<details>
  <summary>Details</summary>
Motivation: 标准指标仅关注预测正确性，无法区分模型是真正利用时序上下文还是仅依赖单帧信息，这一差距亟待解决。

Method: (1) 提出TemporalLens诊断框架，通过受控扰动、结构化遮挡、时序打乱、冗余注入和分辨率退化等手段探测时序依赖性；(2) 提出YOLO-3D模块化实时时空检测器，保留骨干网络中的时序深度。

Result: 堆叠2D模型在移除目标帧时崩溃，而时空模型能从早期帧恢复预测；YOLO-3D在32帧平均下mAP@50提升3.7个百分点。

Conclusion: TemporalLens和YOLO-3D将“检测器是否进行时序推理”转化为可测量、可操作的问题。
...
TL;DR: 本文提出一种无提示的扩散图像隐写框架，通过级联仿射耦合模块（CACM）建立秘密图像与潜表示的确定性双射映射，并融合风格语义先验控制隐写图像生成，结合预测-校正机制优化扩散轨迹，在安全性、重建精度和可控性上达到先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本提示的隐写方法受限于文本的僵化性，难以生成高质量隐写图像并准确恢复秘密图像。

Method: 1）CACM模块建立秘密图像与潜表示的双射映射；2）融合风格语义先验控制扩散过程；3）预测-校正机制迭代优化生成轨迹。

Result: 在安全性、秘密图像重建精度和可控性方面优于现有方法。

Conclusion: 提出的无提示扩散隐写框架有效解决了文本提示的僵化问题，提升了隐写图像质量和秘密信息恢复准确率。
...
TL;DR: 本研究比较了不同时间训练集设计（最小两帧、生理子集、全27帧）对动态4DCT中左心房及左心耳分割性能的影响，发现全帧训练在低对比早期表现最佳，但生理子集在充盈期后性能相当，前景归一化可改善低对比帧性能但无法完全弥补差距。


<details>
  <summary>Details</summary>
Motivation: 动态增强心脏CT中准确分割左心房和左心耳对于房颤患者血液淤滞评估至关重要，但由于时间对比度剧烈变化且每序列仅有一个标注，训练集设计面临鲁棒性与标签噪声之间的权衡。

Method: 使用nnUNet框架，比较三种训练集设计（最小两帧反映临床实践、生理选择子集、全27帧序列），并评估前景归一化的影响。

Result: 全帧训练在早期低对比阶段表现最佳；生理选择子集从充盈期开始性能与全帧相当；基于全数据集归一化参数可提升低对比帧性能，但未完全消除差距。

Conclusion: 训练数据的时间多样性对动态CT稳健分割至关重要，但精心选择的帧子集可在性能与效率之间取得有效平衡。
...
TL;DR: 本文针对格里高利音符识别中四套数据集编码不统一的问题，设计了基于S-GABC的通用编码，将所有数据集统一后训练了一个端到端基础模型，在四个数据集上均达到了最新最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有的四套格里高利音符数据集编码各不相同，阻碍了模型的泛化能力，亟需统一编码并训练更通用的识别模型。

Method: 基于S-GABC提案设计通用编码，将四个不同编码的数据集统一转换，训练了一个共享的端到端基础模型。

Result: 在所有四个数据集上均取得了领先的性能，建立了新的最优结果。

Conclusion: 统一编码联合训练的策略是有效的，能够提升格里高利音符识别的泛化能力并达到新水平。
...
TL;DR: SatAgent是一种无人机-卫星协同空间推理模型，通过双路径视觉机制结合几何感知3D重建编码器、多视角拓扑-语义对齐模块和一致性损失，在复杂城市环境中显著优于现有模型（比通用基础模型高25.91%，比专业空间推理模型高11.69%）。


<details>
  <summary>Details</summary>
Motivation: 现有无人机空间推理方法存在单视角感知易受遮挡和视角畸变影响、缺乏显式几何建模导致推理不一致等问题，亟需更鲁棒、准确的空间推理方案。

Method: 受人类视觉双通路机制启发，提出无人机-卫星协同模型SatAgent，包括：1) 几何感知3D重建编码器将2D特征提升为显式3D表示；2) 多视角拓扑-语义对齐模块在统一BEV坐标系中融合跨视角特征；3) 多视角一致性损失促进视角不变表示；4) 构建首个大规模无人机-卫星协同多视角空间推理数据集SatAgent-SR130K。

Result: 在多种任务上，SatAgent比通用基础模型高出25.91%，比专业空间推理模型高出11.69%，在复杂几何关系推理中尤其精准。

Conclusion: 无人机与卫星协同的多视角空间推理范式能有效克服单视角局限，为航空航天具身智能的空间理解提供了强有力的解决方案。
...
TL;DR: 提出了一种异步架构的开放词汇3D场景图构建方法，将轻量级在线建图与重量级语义精化并行运行，使场景图在探索过程中即可查询并随时间不断丰富语义信息。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D场景图方法采用"先重建、后用视觉语言模型丰富"的两阶段顺序流程，导致探索期间场景图无法查询。作者认为这种顺序耦合是不必要的。

Method: 提出异步架构：1）基于概率体素的轻量级在线建图骨干网络，增量式维持稳定的物体身份；2）后台VLM智能体逐步丰富场景图；3）通过语义闭环检测解决重复物体轨迹；4）多目标帧调度器选择信息量最大的少量帧来分摊VLM计算成本。

Result: 在语义分割任务上与现有方法持平或更优（ScanNet, Replica）；在三个视觉定位基准（Sr3D+, Nr3D, ScanRefer）上以A@0.25指标超越先前最优方法15.3到18.8个百分点。

Conclusion: 该异步框架摆脱了传统两阶段顺序耦合的限制，实现了探索即查询的能力，且语义丰富度随时间增长，性能显著优于现有方法。
...
TL;DR: DrivingDepth提出通过残差像素尺度校正，利用稀疏LiDAR作为几何提示校准冻结的深度基础模型，在保留密集视觉几何结构的同时实现高精度度量深度估计，在nuScenes上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中的密集深度估计面临几何-尺度冲突：深度基础模型能提供像素对齐的密集视觉几何，但缺乏可靠的度量尺度；而投影LiDAR提供度量锚点，却稀疏、含噪且与图像结构不对齐。现有方法从头生成深度，覆盖了基础模型的连贯几何结构，导致视觉连续表面上出现结构伪影。

Method: 提出DrivingDepth，将稀疏LiDAR视为几何提示，通过残差像素尺度校正局部校准冻结的基础先验，从而保留密集视觉几何结构。核心思想是基础模型已捕获几何连贯的相对深度，只需为每个像素学习一个尺度因子将其映射到度量坐标。

Result: 在nuScenes数据集上，使用4帧环视输入，DrivingDepth的AbsRel达到11.19，EdgeCR达到5.741，在度量精度和几何一致性上均超越MapAnything（11.99/1.914），达到SOTA水平。

Conclusion: DrivingDepth通过将稀疏LiDAR作为几何提示而非重头生成深度，有效解决了几何-尺度冲突，验证了冻结基础模型加残差尺度校正范式的有效性。
...
TL;DR: 本文提出受人类视觉过程启发的HVPNet，通过视网膜集成模块（RIM）和皮层解码器（CD）实现简单高效的跨模态显著/伪装目标检测，在22个数据集上的7项任务中取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态显著目标检测（SOD）和伪装目标检测（COD）方法依赖复杂跨模态融合与解码结构，导致模型参数过大且性能不理想。受人类视觉过程高效性的启发，探索能否借鉴生物视觉机制实现更简单且准确的检测模型。

Method: 提出HVPNet，包含两个核心模块：1）视网膜集成模块（RIM），模拟视网膜多层信息整合，采用层级特异性多阶段融合策略；2）皮层解码器（CD），将解码过程分解为低级和高级视觉阶段，模拟人类视觉皮层的层级处理机制。

Result: HVPNet可扩展到4种模态的7项任务，在22个数据集上建立了优异的精度-效率平衡，无需复杂工程技巧即达到优秀性能。

Conclusion: 借鉴人类视觉过程的生物启发式建模可以有效简化多模态目标检测结构，在保持高准确性的同时实现高效计算，为SOD和COD提供新的研究思路。
...
TL;DR: 本文提出了一种基于深度学习的全自动框架，用于在OCT图像中对地理萎缩（GA）的关键生物标志物（RPE丢失、EZ丢失、EZ变薄）进行像素级高精度分割，在多个数据集上验证表现出高准确性和可重复性（Dice>0.87, ICC>0.98）。


<details>
  <summary>Details</summary>
Motivation: 年龄相关性黄斑变性（AMD）继发的地理萎缩（GA）需要精确监测相关结构性生物标志物，以评估疾病阶段、进展和治疗反应，因此亟需一种高精度、全自动的分割方法。

Method: 使用三个专用的语义分割模型分别分割RPE丢失、EZ边界（含中断处）和Bruch膜；在298个SD-OCT体积（涵盖AMD全表型谱）上开发，并在独立外部数据集（n=43）上验证，还评估了重复性、读片者间可靠性、B-scan密度影响及亚组性能。

Result: RPE丢失Dice=0.88，EZ丢失Dice=0.87，Pearson's r>0.99；总EZ厚度测量亚像素平均偏差2.15μm；重复性ICC>0.98。

Conclusion: 该全自动框架能准确一致地量化光感受器外层退化和RPE丢失，为临床试验和常规眼科护理中的GA评估提供高度可靠的工具。
...
TL;DR: SimpleSearch-VL是一个高效、可靠且实用的多模态智能搜索框架，通过改进智能体自身的搜索与验证过程（而非扩展数据、工具或辅助模型组件），在仅使用5K监督数据和2K强化学习数据的情况下，使Qwen3-VL智能体基线提升15.8-16.0平均分，30B-A3B版本性能与Gemini-3-Pro智能体相当。


<details>
  <summary>Details</summary>
Motivation: 现有多模态智能搜索方法通常依赖扩展数据规模、工具或辅助模型组件，导致效率低下且部署复杂。作者希望探索一种更高效、可靠且实用的方案，通过改进智能体自身的搜索与验证过程来提升性能，而非简单堆叠资源。

Method: 1) 因子化自适应展开(FAR)：通过形成更具信息量的训练组提升采样效率，同时利用冗余样本缓解长尾延迟并暴露难样本；2) 证据验证推理：使用链式思维验证评估检索到的视觉和文本线索与原始上下文的相关性；3) 轻量级工具接口与网页自摘要：智能体内部完成网页摘要，无需外部依赖。

Result: 仅使用5K监督工具交错轨迹和2K强化学习数据，SimpleSearch-VL使Qwen3-VL 8B和30B-A3B变体的智能体基线分别提升15.8和16.0平均分。30B-A3B版本性能与Gemini-3-Pro智能体相当。

Conclusion: SimpleSearch-VL证明通过优化智能体自身的搜索与验证过程，可以在极少量数据下实现显著的性能提升，验证了"改进过程而非扩展资源"这一思路在多模态智能搜索中的有效性和实用性。
...
TL;DR: PRISM利用预训练VAE潜空间的低相干性特性，将单幅图像反射去除重新定义为潜空间线性分离问题，结合流匹配速度场和对比学习策略，在六个基准上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有SIRR方法在像素空间操作，sRGB非线性模型导致透射层与反射层纠缠严重，泛化能力受限。作者观察到预训练VAE潜空间中图像层间相干性远低于像素空间，为分解提供了更有利的空间。

Method: 提出PRISM模型：(1) 将SIRR重新解释为潜空间线性分离问题；(2) 在预训练FLUX骨干上学习流匹配速度场，单次前向恢复两层；(3) 潜空间组成一致性(LCC)策略，通过交换反射潜变量和循环损失实现解纠缠；(4) 层级对比分离(LCS)损失，通过块级对比学习促进语义分离。

Result: 在六个基准数据集上，PRISM一致且显著优于现有最先进方法，并展现出对真实场景图像的强泛化能力。

Conclusion: 潜空间线性分解范式比像素空间方法更有效，预训练VAE潜空间的低相干性特性为反射去除这一病态问题提供了新的解决思路。
...
TL;DR: 本文提出MV-GEL框架，通过多视角选择与VLMs结合，实现从自然语言查询定位3D网格上的精细几何实体（边、面、曲面），相比基线方法在面级IoU提升1.7倍、边级F1提升4.5倍以上。


<details>
  <summary>Details</summary>
Motivation: 现有VLMs在图像域的指代分割已很成熟，但扩展到3D结构化几何实体定位面临视角敏感性问题（遮挡、透视缩短等），需要一种能自动选择最优观察视角的方法来实现可靠的3D几何实体定位。

Method: 提出GELviews排行榜模块，根据语言提示的几何实体可观察性对视角排序；选择最优视角后使用VLM推理分割骨干生成掩码，再通过几何感知光线投射将预测掩码提升到对应网格上。框架无需CAD先验，仅依赖3D网格。

Result: 面级别IoU提升1.7倍，边级别F1提升4.5倍以上，显著优于CLIP基线和随机视角采样方法，尤其对薄结构和视角敏感结构效果突出。

Conclusion: 通过视角选择与VLM分割的结合，MV-GEL有效解决了3D几何实体定位的视角敏感性问题，实现了无需CAD先验的细粒度几何实体定位，为CAD、机器人操作和科学模拟提供了有力工具。
...
TL;DR: 提出DataEvolver，一个自进化的多智能体框架，通过利用被拒绝样本的反馈信号，迭代改进文本丰富图像的训练数据构建，显著提升了OCR相关指标。


<details>
  <summary>Details</summary>
Motivation: 现有文本丰富图像数据构建采用静态的爬取-过滤-冻结范式，被拒绝的样本被直接丢弃，但这些样本包含有用的失败信号（如OCR错误、语义不匹配），导致后续构建轮次重复相同的失败模式。

Method: DataEvolver是一个自进化的多智能体框架，包含四个模块：Retriever（收集候选样本）、Verifier（分配质量分数和拒绝原因）、Critic（将轮次反馈总结为语义反馈）、Generator（通过定向合成补充未覆盖区域）。反馈记忆不断更新，指导下一轮构建。

Result: 在0.75M数据量下，PixArt-alpha上DataEvolver相比最强基线：TextScenesHQ上OCR-F1提升85.3%，LongTextBench上提升35.3%。改进在不同基准和下游生成器（Show-o2）上均保持一致。

Conclusion: 被拒绝的样本能为改进文本丰富图像数据构建提供可操作的反馈信号，DataEvolver的自我进化范式优于固定数据集方法。
...
TL;DR: AugSplat通过利用辐射场的视图增强技术，在稀疏视角场景下显著提升了3D高斯泼溅的重建质量，同时保持实时渲染能力。


<details>
  <summary>Details</summary>
Motivation: 稀疏视角下，高斯泼溅方法因初始几何质量高度敏感而表现不佳，而神经辐射场虽重建稳健但计算成本高、推理慢。本文旨在结合两者优势，实现稀疏视图下的高质量实时渲染。

Method: 先训练一个辐射场，用其合成附近新颖视角的辅助图像，增加有效视图覆盖；再将这些合成视图作为额外监督，用于优化高斯泼溅。提出两种变体：Staged AugSplat（先用合成视图优化，后切换真实图像）和Dual AugSplat（联合训练真实与合成视图，并衰减合成损失权重）。

Result: 在稀疏视图的mip-NeRF 360场景上，AugSplat相比标准高斯泼溅显著提升重建质量。Staged AugSplat平均性能最佳，Dual AugSplat接近且全程保留真实图像监督，两种变体均保持实时推理。

Conclusion: AugSplat有效融合了辐射场的稳健重建与高斯泼溅的实时渲染优势，为稀疏视角下的高质量新视角合成提供了简洁而有效的解决方案。
...
TL;DR: 本文提出MPL-MAE框架，通过抑制3D掩码自编码中的位置信息过度依赖，平衡空间先验与语义特征，提升点云表征质量。


<details>
  <summary>Details</summary>
Motivation: 现有3D MAE框架的解码器过度依赖位置信息，弱化了语义表示学习，导致特征质量次优。

Method: 提出MPL-MAE框架，包含两个关键模块：1）重校准位置嵌入模块，抑制度量主导的坐标信号同时保留几何拓扑；2）门控位置接口模块，在重建过程中动态调节位置信息注入。

Result: 在下游任务的大量实验表明，MPL-MAE consistently achieves competitive performance（一致取得竞争性性能）。

Conclusion: 通过促进空间先验与语义特征的更平衡交互，该方法能生成鲁棒且信息丰富的表征，验证了其有效性。代码已开源。
...
TL;DR: 提出了一种物理感知神经算子Transformer（PNOT），用于实时重建偏滤器温度场，通过图注意力捕捉空间物理依赖、物理感知神经算子建模热扩散，以及Sobolev正则化损失保证物理一致性，在提高预测精度的同时保持物理一致性。


<details>
  <summary>Details</summary>
Motivation: 传统数值方法（如有限元法）计算成本高，无法用于实时应用，因此需要一种快速且可泛化的方法来实现偏滤器温度场的实时重建与控制。

Method: 提出PNOT模型，将边界热通量关系建模为结构化图并使用图注意力捕捉空间物理依赖；设计物理感知神经算子模块，通过切片聚合具有相似物理条件的查询点并建模热扩散；采用梯度约束的Sobolev正则化损失来强制函数值与其导数的一致性。

Result: 实验结果表明，这些物理约束在保持物理一致性的同时提高了预测精度。

Conclusion: PNOT方法能够快速、准确地实现偏滤器温度场的时空演化建模，适用于实时控制场景，代码将开源。
...
TL;DR: 本文研究了局部化共形预测在视觉-语言模型图像分类中的应用，发现直接使用余弦相似度无法优于非局部基线，提出了一种非线性变换方法，在保持边缘覆盖保证的同时显著减少了平均集合大小。


<details>
  <summary>Details</summary>
Motivation: 共形预测具有强边缘覆盖保证，但完全条件保证不可实现。现有方法通过调整测试样本的共形集来近似条件行为，在回归任务中表现良好，但在图像分类领域的应用尚未充分探索。

Method: 基于开源实现的局部化共形预测算法，在自然图像分类任务上对视觉-语言模型进行广泛基准测试。提出对余弦相似度进行简单非线性变换，以替代直觉上合理但效果不佳的直接余弦相似度使用。

Result: 直接使用余弦相似度无法改善非局部基线方法的表现；所提出的非线性变换方法在保持边缘覆盖保证的前提下，实现了统计上显著的平均集合大小缩减。

Conclusion: 对于视觉-语言模型的图像分类任务，简单的余弦相似度局部化策略效果有限，而非线性变换能够有效提升局部化共形预测的性能。
...
TL;DR: 本文发现基于相机参数的位置编码在3D新视角合成任务中，由于将旋转和平移信息存储在同一维度导致训练后期性能停滞。为此提出DPPE（解耦姿态位置编码），通过显式分离旋转和平移信息，实现了稳定的大规模训练和更好的泛化能力。
...
Method: 提出Decoupled Pose Positional Encoding (DPPE)，将旋转和平移信息在位置编码中显式解耦，分别编码到不同的维度空间中。

Result: DPPE在NVS任务上实现稳定的长期训练，在大规模训练设置下表现优异，并在外推场景（如增加视角数量、缩放场景）中展现出优越的泛化性能。

Conclusion: 通过解耦旋转和平移的位置编码可以有效解决训练后期性能停滞问题，提升模型的可扩展性和泛化能力。
...
TL;DR: 提出ViToS双流强化学习框架，通过主动剪枝视觉令牌（VTP）实现医学多模态推理加速，在7个医学基准上将视觉令牌减少至原长度的77%，相对性能提升至108.27%。


<details>
  <summary>Details</summary>
Motivation: 医学图像通常具有极其稀疏的视觉证据，现有VLM+RL方法在医学多模态推理中表现不佳，缺乏统一的强化学习框架来实现主动视觉令牌剪枝与医学推理。

Method: 提出双流RL框架ViToS，包含两个任务分支：一个专注于定位（grounding），另一个在VTP后执行令牌稀疏推理。引入交叉反馈顺序优化解决耦合策略学习问题，避免梯度冲突并促进共享策略模型收敛。

Result: 在7个医学基准上，视觉令牌减少至原序列长度的77%，Lingshu-7B相对性能达108.27%，HuatuoGPT-Vision-7B相对性能达104.16%。
...
TL;DR: 提出了一种不确定性引导的合成上下文增强策略，通过预测熵识别模型不确定区域，仅对互补视觉上下文进行修复，在不改变原始标签的前提下提升语义分割模型在稀疏或罕见区域上的性能。


<details>
  <summary>Details</summary>
Motivation: 语义分割模型在数据稀疏、罕见或视觉多样性高的区域（如密集区域、小物体）表现不佳。直接合成增强数据可能导致标签与生成像素错位，现有方法依赖外部模型或采用粗粒度启发式策略，浪费容量在无信息像素上。

Method: 利用基线分割器的预测熵识别不确定语义区域，仅对互补视觉上下文进行修复（inpainting）。微调时仅计算原始像素的损失，排除修复区域，使模型专注于未修改的不确定区域并呈现于新上下文中。

Result: 在Cityscapes、UAVID和BDD100K数据集上取得显著的mIoU提升，尤其在罕见和困难类别（如公交车、火车、航空视角下的汽车）上增益最大。

Conclusion: 不确定性引导的上下文增强是一种高效的语义分割改进手段，无需外部模型即可严格保持标签有效性并最大化像素信息量。
...
TL;DR: 本文提出了一种统一的高阶结构对齐框架用于多视角雷达分割，通过可学习超图捕获高阶依赖关系，利用非平衡最优传输实现无对应特征对齐，并采用自适应注意力机制融合多视图互补信息，在CARRADA和RADIal基准上分别达到63.8%和83.4% mIoU，比之前最佳方法提升+1.7和+2.3 mIoU。


<details>
  <summary>Details</summary>
Motivation: 现有雷达分割方法依赖基于网格的编码和成对交互，难以捕获同一物理对象多个雷达回波形成的高阶关系结构。

Method: 提出统一高阶结构对齐框架：利用可学习超图建模高阶依赖；采用非平衡最优传输实现异质视图间无对应特征对齐；设计自适应注意力机制融合多视角特征；结合监督分割与跨视图一致性正则化训练。

Result: 在CARRADA上达到63.8% mIoU，RADIal上达到83.4% mIoU，分别比之前最佳方法提升+1.7和+2.3 mIoU。

Conclusion: 高阶关系建模对提升雷达感知鲁棒性至关重要，验证了所提框架在稀疏噪声雷达数据上的有效性。
...
TL;DR: 提出主动任务驱动记忆（ATMem）方法，将GUI智能体内存从被动存储转变为主动维护的执行状态，并引入STR-GRPO在线强化学习方法，使智能体能根据记忆对任务完成的贡献选择性使用记忆，在长序列移动任务中避免重复或遗漏操作。


<details>
  <summary>Details</summary>
Motivation: 现有记忆方法将记忆视为被动存储，智能体需要从积累的记录中推断某个值当前的角色（是否已使用、是否应现在使用或等待后续依赖），在长序列任务中这种隐式重建不可靠，容易导致重复或遗漏操作。

Method: 提出了ATMem（主动任务驱动记忆），将任务相关信息维护为持续更新的执行状态，将每个值与其角色及当前状态链接；同时引入STR-GRPO，一种在线强化学习方法，通过对比有记忆和无记忆的轨迹来估计何时使用记忆能改善执行，并使用记忆成本感知奖励来抑制无益的记忆使用。

Result: 构建了一个具有挑战性的移动基准测试，要求智能体从大量相似条目中根据指令执行所有符合条件的目标，同时拒绝违反约束的条目，以评估智能体在长序列执行中完成所有范围内任务并避免范围外操作的能力。

Conclusion: 通过将记忆从被动存储转变为主动维护的执行状态，并结合选择性记忆使用的强化学习，可以显著提升GUI智能体在长序列任务中的可靠性和执行效率。
...
TL;DR: 提出了一种基于双DRL智能体的无人机-无人船协同降落框架，在仿真中实现了100%的降落成功率，并在恶劣海况下将降落平台稳定在水平面1度以内。


<details>
  <summary>Details</summary>
Motivation: 海上基础设施的自主空中巡检常受随机海况影响，存在高动能冲击、着陆后倾覆和传感器遮挡等风险，亟需更可靠的无人机降落方案。

Method: 采用解耦的多飞行器降落框架，同步配备3-RPU稳定平台的USV与UAV；使用两个独立DRL智能体：一个SAC智能体提供高频波浪运动补偿，另一个多模态RL智能体负责UAV的最终进近。

Result: 在高保真海事仿真中，系统在15次试验中实现了100%的降落成功率；平均稳定效能为87.8%，在恶劣条件下96%的任务时间内将着陆面保持在水平面1度以内。

Conclusion: 该解耦双智能体框架有效提升了海况下的无人机降落安全性与稳定性。
...
TL;DR: PrISM-IQA将智能手机图像质量评估重构为多问题有序诊断任务，预测每个ISP相关问题的严重等级，而非单一分数，以更好地支持实际的ISP调优。


<details>
  <summary>Details</summary>
Motivation: 传统IQA方法将感知质量简化为单一分数，与ISP工程师需要识别具体质量问题、评估严重程度并决定是否需要干预的实际需求严重脱节。

Method: 提出PrISM-IQA模型，结合累积顺序编码与结构化推理，在预测每个ISP相关问题的有序严重等级（无、轻微、严重、致命）时，同时捕获问题内的单调性以及问题间的包含与排斥关系。

Result: 在重构的SPAQ基准（含53个ISP质量问题）和小规模专家标注真实数据集上，PrISM-IQA在问题级诊断任务中表现出色，线性探测实验揭示了可迁移的感知质量表征。

Conclusion: PrISM-IQA能够为ISP调优提供可操作且有意义的指导，实现了从单一分数回归到结构化多问题诊断的范式转变。
...
TL;DR: LiteMatch是一个轻量级立体匹配框架，通过成本体积稳定化技术实现强零样本泛化，无需昂贵的3D卷积。它采用双编码器（CVCE和HFE）和CVC-Loss损失函数，在多个数据集上取得了竞争性表现。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的立体匹配方法虽然精度高，但依赖重型骨干网络和计算密集的3D成本体积处理，导致大量内存和运行时开销，并且跨域泛化能力差，限制了实际部署。

Method: 提出LiteMatch框架，包含：1) 两个互补编码器——Cross-View Correspondence Encoder (CVCE)捕获全局跨视图交互，High-Frequency Encoder (HFE)通过FFT增强细粒度结构细节；2) Cost Volume Consistency Loss (CVC-Loss)作为体素级二元交叉熵目标，应用于softmax归一化的成本分布，促进锐利单峰视差概率；3) 轻量级细化模块实现低迭代全分辨率视差输出。

Result: 模型参数规模从3.36M到9.58M可调，在Scene Flow、KITTI、Middlebury、ETH3D和DrivingStereo等多个数据集上实现了具有竞争力的EPE和D1性能，展现了优异的零样本泛化能力。

Conclusion: 轻量级架构确实可以在不牺牲精度的情况下实现跨域泛化，打破了高性能立体匹配需要重型模型的传统认知。
...
TL;DR: RoboSpatialBrain 是基于 RoboBrain2.5-8B-NV 构建的视觉-语言模型，通过推理时提示策略和参考框架重定向管道提升空间推理能力，在 RoboSpatial 挑战赛中获得第一名，整体成功率达 80.9%。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型在通用感知任务上表现良好，但在具身任务所需的空间推理方面存在不足。

Method: 1）强制 <think> 前缀激活策略 + 任务特定后提示，引导模型在上下文和兼容性任务上进行审慎推理；2）显式参考框架重定向管道，解决相机中心与物体中心之间的歧义；3）探索了在兼容性数据上微调 RoboBrain2.5 并与提示交互分析。

Result: 在 RoboSpatial Challenge 中获得第一名，在 RoboSpatial-Home 数据集上整体成功率达 80.9%。

Conclusion: 通过训练免费推理时机制有效提升空间推理能力，代码已开源。
...
TL;DR: 本文提出Q-Margin，一种基于α-散度的损失函数，通过在参考测度中编码边界惩罚来替代传统几何边界，在面部和说话人验证任务中实现竞争性或更优性能，尤其在低误接受率下表现突出，并支持稀疏、内存高效的大规模训练。


<details>
  <summary>Details</summary>
Motivation: 传统基于几何边界的损失函数（如CosFace、ArcFace）无法自然扩展到α-散度框架，而α-散度（α>1时）具有稀疏性的优势。现有方法缺乏一种在概率框架下引入边界的原则性方式。

Method: 提出Q-Margin损失函数，将边界惩罚直接编码到参考测度（先验概率）中，而非像传统方法那样对logits施加几何惩罚。这样既保留了α-散度的稀疏性，又能鼓励判别性嵌入。

Result: 在IJB-B、IJB-C人脸验证基准和VoxCeleb说话人验证上达到竞争性或更优性能；在低误接受率（FAR）下持续优于ArcFace和CosFace基线；后验的极端稀疏性支持精确且内存高效的训练。

Conclusion: Q-Margin提供了一种在α-散度框架中引入概率边界的有效方法，兼具高性能、高安全性下的低FAR优势和大规模数据集的可扩展性。
...
TL;DR: 本文提出SAMBA，一种基于散射引导双向Mamba的自监督预训练基础模型，用于SAR目标识别。该模型通过线性复杂度的Mamba编码器、物理先验引导的三级层次化掩码策略（SG-MAE）以及轻量级特征交互模块，解决了Transformer高计算复杂度和传统掩码策略忽略SAR电磁散射特性两大问题。在多个下游数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: SAR自动目标识别的实际部署受限于标注数据稀缺。现有自监督预训练方法中，Transformer架构计算复杂度呈二次增长，且通用掩码策略忽略了SAR图像特有的电磁散射物理机制，导致预训练任务与SAR成像机理不匹配。

Method: (i) 线性复杂度的Mamba编码器配合序列中间类别令牌；(ii) 基于SAR物理先验的三级层次化散射引导掩码自编码器（SG-MAE）策略；(iii) 轻量级SpatialMix特征交互模块用于跨区域特征融合；(iv) 两阶段跨领域预训练流程。

Result: SAMBA在所有预训练配置下均表现优异，参数量远少于CNN和Transformer基线。SG-MAE策略相比标准MAE掩码策略显著提升少样本迁移能力。在7个下游数据集的分类和检测任务上，大多数指标达到SOTA。

Conclusion: SAMBA验证了将SAR物理先验融入自监督预训练的有效性，为SAR目标解译提供了一种高效、轻量且泛化能力强的预训练基础模型框架。
...
TL;DR: WorldRoamBench是一个面向开放世界的长跨度稳定性基准，从动作、视觉、物理和记忆四个维度评估交互式世界模型，涵盖600+测试案例，实验表明现有模型均无法同时满足所有维度的要求。


<details>
  <summary>Details</summary>
Motivation: 现有交互式世界模型（IWMs）的评估基准仅在轨迹层面衡量动作跟随能力，忽略了记忆和交互物理特性，缺乏细粒度、多维度的系统性评估方法。

Method: 构建WorldRoamBench基准，包含四个创新维度：(i) 动作：逐帧动作度量避免跨模型语义尺度差异；(ii) 视觉：基于片段的漂移度量捕捉非单调的中序列崩溃；(iii) 物理：可控性门控评估，覆盖力学、光学和3D一致性；(iv) 记忆：动作解耦协议，通过3D点云重建和追踪加VLM推理评估场景与主体记忆。包含600+测试案例，覆盖自然、城市、室内场景及第一/三人称视角，支持WASD 10-60秒连续交互。

Result: 评估了10余种开源/闭源模型，发现没有模型能可靠地满足所有维度要求，最佳模型仅获得中等得分。

Conclusion: WorldRoamBench上的进步是迈向稳定、物理可信、记忆忠实且可部署于实际应用的交互式世界模型的关键步骤。
...
TL;DR: REDI提出了一种结合TF-IDF类特定语料统计与注意力图的令牌缩减方法，在DINOv3 ViT-B/16上实现46.8%的序列缩减，ImageNet-1K Top-1准确率达84.706%，优于密集基线（83.514%）。


<details>
  <summary>Details</summary>
Motivation: 现有令牌缩减方法通过剪枝、合并或池化寻求精度与效率的权衡，但缺乏受控的监督参考来指导如何在图像各块间分配固定令牌预算。

Method: 将DINOv3末层补丁表示量化为视觉词汇，构建基于TF-IDF的类条件语料分数；对验证图像，用真实类别选择TF-IDF行，经四种变换视图生成与参考中心裁剪对齐的TF-IDF图；通过密集前向获得注意力图，两者经归一化后逐元素乘积作为REDI分数；根据分数排名分配补丁角色，分数幅度加权合并与压缩。

Result: REDI将序列长度从201降至107令牌（缩减46.8%），Top-1准确率84.706%，优于密集基线（83.514%）、仅用注意力（82.634%）和仅用TF-IDF（81.796%）。

Conclusion: 类特定语料统计与图像特定注意力在补丁排序中提供互补信号，结合两者优于各自单独使用。
...
TL;DR: ShellMaker是一个语言引导的建筑外观生成框架，能在保持内部结构约束的前提下，为建筑生成风格一致的外墙、屋顶和PBR材质，兼容多种输入格式。


<details>
  <summary>Details</summary>
Motivation: 现有室内场景生成方法可以生成平面图和墙体布局，但缺少风格一致的外墙和屋顶，而直接使用无约束生成模型会破坏已有的结构约束（如足迹、墙体几何和开口语义）。

Method: ShellMaker结合参数化屋顶生成、基于大语言模型的部件级提示优化、联合墙-屋顶材质检索和几何感知装配，在给定建筑骨架和文本风格提示下生成完整的外部网格及PBR材质。

Result: ShellMaker兼容室内生成器、CityGML和CAD输入，在保持结构一致性的同时，相比检索方式和无约束生成基线在建筑连贯性上取得更好效果。

Conclusion: ShellMaker有效解决了在结构约束下生成风格一致建筑外观的难题，且方法具有格式无关性，可泛化到多种输入来源。
...
TL;DR: 本文提出直方图约束图像生成（HIG）方法，通过在生成过程中精确施加用户指定的分布约束（如颜色直方图或潜变量分布），实现介于全局文本引导与局部密集条件之间的中等粒度控制，并利用最优传输理论进行采样引导。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型控制机制在控制粒度上存在空白：文本提示提供全局语义但缺乏局部精确性，ControlNet等方法提供精确局部结构但灵活性不足，需要一种介于两者之间的中等粒度控制方案。

Method: 将分布约束建模为最优传输问题，在扩散采样过程中施加显式引导变换，使生成轨迹与目标直方图对齐。支持颜色直方图和潜变量标记分布两种约束形式。

Result: HIG在颜色/潜变量直方图约束生成以及直方图级信息嵌入等任务中展现了良好效果，且与现有控制机制完全兼容。

Conclusion: 分布控制作为一种灵活且可解释的控制方案，填补了控制粒度的中间地带，可丰富可控图像生成的混合策略。
...
TL;DR: 提出双范围-体素表示（DRVR），利用单次扫描点云的range-view上下文和voxel-view几何信息进行3D语义占用预测，避免多次扫描带来的效率与鲁棒性问题，在nuScenes等数据集上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖多扫描点云堆叠来弥补单次点云的稀疏性和不完整性，但带来了额外的计算负担和位姿变换噪声问题，阻碍了实际应用。

Method: 1) 使用range-view编码器提取场景紧凑上下文；2) 设计几何感知的voxel-view编码器，分别提取多尺度体素特征并融合；3) 提出range-voxel融合模块，通过体素到范围和范围到体素的双向融合协同两类特征。

Result: 在nuScenes-Occupancy上，单次扫描DRVR相比多次扫描方法mIoU提升5.4%，推理速度加速2.1倍。

Conclusion: 单次扫描结合range-view与voxel-view的双重表示能有效替代多扫描策略，兼顾精度与效率。
...
TL;DR: 本文提出IS-SNN架构，通过拓扑感知权重标准化和改进残差连接消除批归一化层，在保持SNN精度的同时显著降低硬件资源消耗（FPGA查找表减少96.4%），在ImageNet上达到68.05%的准确率。


<details>
  <summary>Details</summary>
Motivation: 深度SNN依赖批归一化（BN）实现高性能，但现有动态BN变体引入大量运行时乘法运算，削弱了SNN的硬件高效性优势。本文旨在解决这一矛盾，探索无归一化的高效SNN架构。

Method: 1) 识别出无归一化SNN性能严重下降的主因是灾难性发放率衰减；2) 提出IS-SNN架构，通过拓扑感知权重标准化和改进残差连接实现信号稳态；3) 将标准化操作离线折叠为静态权重，消除运行时统计追踪和乘法运算。

Result: 在VGG、ResNet和Transformer模型上取得与/优于动态BN方法的性能；ImageNet上68.05%准确率；FPGA查找表资源消耗减少96.4%；克服了先前无BN方法深度受限的问题。

Conclusion: IS-SNN为构建精确且硬件友好的深度神经形态系统提供了实用框架，恢复了以累加为导向的推理数据路径。
...
TL;DR: 本文系统评估了稀疏自编码器（SAE）在扩散模型中用于概念擦除和引导的效果，发现SAE虽然能可靠检测语义概念，但直接干预其潜在空间会导致分布外激活和视觉伪影；作者提出将SAE仅用作语义检测器，通过基于检测的补丁替换实现更干净的擦除效果，揭示了概念检测与概念干预之间的根本差距。


<details>
  <summary>Details</summary>
Motivation: SAE被提出作为可解释的概念级操作工具，但其假设（孤立特征可作为可控干预点）尚未在扩散模型中得到系统验证。本文旨在评估这一假设，探究SAE能否真正用于扩散模型的对象擦除和引导任务。

Method: 将SAE激活仅作为语义检测器，识别包含目标对象的图像区域，然后用不包含该对象的补丁嵌入替换这些区域的嵌入，从而保留扩散模型的激活统计特性，避免直接潜在空间干预带来的伪影。

Result: SAE能可靠检测和定位扩散模型中的语义概念，但直接干预其潜在空间频繁导致分布外激活和严重视觉伪影。基于检测的补丁替换方法比潜在引导产生更干净的擦除效果。

Conclusion: 单语义或稀疏特征本质不适合作为扩散模型的控制旋钮；概念检测与概念干预之间存在根本性差距；SAE作为生成模型可解释性分析工具功能强大，但直接用于操作（如遗忘学习）存在重要局限性。
...
TL;DR: 本文提出Phantom框架，通过联合约束潜在空间和空间域的对抗扰动，有效防御换脸深度伪造，在多个先进模型上显著提升保护成功率并保持视觉质量。


<details>
  <summary>Details</summary>
Motivation: 换脸深度伪造对个人隐私构成严重威胁，现有对抗方法主要针对人脸识别模型，缺乏对换脸场景的适配；固定或随机目标导致潜在引导模糊，缺乏空间约束使扰动扩散到无关区域。

Method: 提出Phantom统一防御框架，自适应合成身份偏移但属性保持的目标用于潜在空间优化，同时在语义相关面部区域施加掩码扰动，实现潜在域和空间域的联合约束。

Result: 在躲避场景下，对UniFace、INSwapper、SimSwap的保护成功率分别提升27.8%、25.6%、16.6%；在冒充场景下，保护率提升达10.2%，同时改善视觉质量。

Conclusion: 联合利用潜在和空间约束能有效实现鲁棒且连贯的面部隐私保护，验证了该框架的通用性和有效性。
...
TL;DR: 本文提出了WIDER-FAIR数据集，在WIDER-FACE基础上为16,256张图像标注了种族（亚洲、黑人、印度、白人）和性别信息。实验表明，人脸检测模型对黑人个体的检测性能显著较低，且移除该群体数据会加剧公平性差异。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的人脸检测模型在不同人口统计群体间存在性能差异，但缺乏带有敏感属性标注的人脸检测数据集，阻碍了偏差的研究与缓解。

Method: 基于WIDER-FACE构建WIDER-FAIR数据集，人工标注种族（4类）和性别（2类）；使用人脸嵌入、K近邻分类器和t-SNE可视化验证标注质量；基于YOLOv5进行消融实验。

Result: 检测性能在黑人个体上显著较低；排除黑人群体训练会比其他种族排除导致更大的公平性差异。

Conclusion: 人口统计注释数据集对于理解和评估人脸检测模型中的偏差具有重要价值。
...
TL;DR: UniCoder提出统一RL框架，通过符号属性对齐（细粒度奖励）和参考引导代码优化（突破探索停滞），解决视觉到代码生成中像素级对齐难题，8B模型超越所有开源基线，性能比肩闭源模型。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs通过SFT无法实现像素级精确对齐，而RL面临奖励粗糙（语义指标无法惩罚细粒度偏差）和探索停滞（稀疏异构代码空间导致策略无法启动有效轨迹）两大障碍。

Method: 提出UniCoder统一RL框架，包含：(1)符号属性对齐，利用轻量辅助LLM将生成代码解析为离散视觉属性实现密集元素级奖励；(2)参考引导代码优化，向低性能rollout组动态注入真实轨迹，将盲探索转化为引导策略改进。

Result: 在ChartMimic、UniSVG、Design2Code和ScreenBench基准上，8B模型超越所有开源基线，性能达到与闭源模型相当的最先进水平。

Conclusion: UniCoder通过精细奖励设计与引导式探索策略，有效解决了视觉到代码生成中的像素级对齐问题，为通用视觉到代码合成建立了新范式。
...
TL;DR: 本文提出MemLearner，一种基于学习的自适应上下文查询方法，通过查询令牌连接上下文和预测令牌，解决了视频世界模型在长时间生成中缺乏记忆导致的场景不一致问题，在遮挡和动态场景下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 视频世界模型在长时间生成中缺乏记忆机制，导致生成场景不一致。现有基于规则的方法在场景遮挡和动态物体情况下泛化能力差。

Method: 提出MemLearner，一种学习型自适应上下文查询方法，利用查询令牌桥接上下文令牌和预测令牌；利用预训练视频生成模型本身的视觉先验进行上下文查询，无需额外从头训练模块；引入高效训练推理策略；构建带遮挡和动态物体的长视频数据集及相机位姿标注；提出多数据集训练策略。

Result: 在场景一致性和记忆能力上显著优于先前视频世界模型，尤其在具有挑战性的遮挡和动态场景下表现突出。

Conclusion: 基于学习的自适应上下文查询机制能有效增强视频世界模型的长期记忆能力，提升生成场景的一致性。
...
TL;DR: 提出RhythmJEPA框架，通过联合嵌入预测学习、循环节律状态规划器、双序Mamba编码器和空间脉冲混合器，有效从面部视频中提取远程光电容积描记（rPPG）信号，在多个数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有自监督rPPG方法主要重建掩码像素或低级视觉表征，导致模型偏向面部外观而非潜在生理动态；同时Mamba-based方法仅按时间顺序扫描，未能充分利用脉搏信号的循环结构。

Method: 提出RhythmJEPA框架，包含三个核心模块：1) Cyclic Rhythm-State Planner (CRSP) 通过动态规划估计帧级生理状态并解码最优循环状态路径；2) Dual Order Mamba Encoder (DOM) 结合常规时间扫描和状态顺序扫描；3) Spatial Pulse Mixer (SPM) 提取紧凑的脉冲敏感面部令牌。

Result: 在PURE、UBFC-rPPG和MMPD三个数据集上取得了优于代表性rPPG方法的性能。

Conclusion: RhythmJEPA通过预测潜在教师表征而非重建RGB帧，以及在嵌入空间中显式建模脉搏的循环时序结构，有效提升了rPPG信号估计的准确性。
...
TL;DR: 本文提出了JL1-CC&QA多任务基准数据集，在JL1-CD遥感变化检测数据集基础上，新增了变化描述（CC）和变化问答（QA）两种标注，统一了变化分割、描述和问答三大任务。


<details>
  <summary>Details</summary>
Motivation: 传统遥感变化检测仅关注像素级的二值分割，只能定位变化的位置，却无法描述变化的内容和原因。为了弥合这一语义鸿沟，作者构建了同时包含变化描述和变化问答标注的多任务基准。

Method: 基于5,000对吉林一号卫星（0.5-0.75m分辨率）的双时相影像，构建了JL1-CC（17,021条质量验证的描述）和JL1-QA（20,060个问答对，涵盖8种问题类型）。标注采用三阶段流程：多模态大模型生成、视觉驱动的LLM评判、人类专家验证。

Result: 成功构建了统一变化掩码、变化描述和变化问答三大任务的高质量遥感变化理解基准数据集，并通过三阶段标注流程保证了数据质量。

Conclusion: JL1-CC&QA作为首个在同一遥感影像集上统一变化分割、描述和问答的基准，将为推动遥感多任务变化理解研究提供宝贵资源。
...
TL;DR: 本文利用卷帘快门相机的畸变效应，从单帧图像中估计高速运动球体的三维平移和角速度，提出了一种无需对应点的球形图案和两阶段优化方法。


<details>
  <summary>Details</summary>
Motivation: 传统方法将卷帘快门畸变视为需要校正的伪影，而本文逆向思维，将其作为宝贵的时间信息源，用于从单帧图像中恢复高速运动物体的三维速度。

Method: 设计了一种鲁棒且易于检测的球形图案，提出无需对应点的公式化方法，通过反投影框架中的几何一致性约束解耦平移和旋转运动，采用两阶段优化估计速度参数。

Result: 在合成数据和真实数据上的大量实验表明，该方法在挑战性的高速运动条件下能够准确且鲁棒地估计运动参数。

Conclusion: 卷帘快门扭曲可作为时间信息源有效用于高速物体运动估计，所提方法对无纹理物体同样有效。
...
TL;DR: 本文提出Barycentric Dominance Field (BDF)，一种将网格拓扑连接编码为连续表面信号的表示方法，使扩散模型能够生成高质量、可扩展且支持纹理的原生3D网格，克服了自回归模型在面数、顶点分辨率和纹理支持上的限制。


<details>
  <summary>Details</summary>
Motivation: 自回归模型在原生3D网格生成中虽取得进展，但受限于其固有范式，导致生成网格面数有限、顶点分辨率受限、难以支持纹理。为解决这些瓶颈，需要一种能桥接离散网格拓扑与连续扩散生成建模的表示方法。

Method: 提出Barycentric Dominance Field (BDF)，一种在三角网格表面上定义的连续表示，通过将顶点拓扑连接转换为连续表面信号，优雅地编码网格拓扑连接性。BDF作为网格的内在属性，与纹理图高度相似，可无缝集成到现有3D扩散管道中，无需修改网络架构。

Result: 大量实验表明，BDF使扩散模型能够生成原生网格，相较于最先进的自回归方法，在质量、可扩展性和鲁棒性上均显著提升。

Conclusion: BDF有效弥合了离散网格拓扑与连续扩散模型之间的鸿沟，为扩散模型生成高质量原生3D网格提供了可行方案，在面数、分辨率和纹理支持上均优于自回归方法。
...
TL;DR: 本文提出了一种自监督时间正则化后处理方法，用于增强基于图的心脏超声分割模型的时间一致性，同时保留解剖对应关系，并自动映射到AHA 17节段临床标准。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的心脏分割模型在独立帧上训练，导致时序不连续，影响临床测量的可靠性。

Method: 提出自监督时间正则化后训练精炼阶段，通过惩罚连续帧间的速度和加速度不连续性，强制时序一致性，并利用解剖对应关系自动将标志点映射到AHA 17节段标准。

Result: 在CAMUS数据集上验证了该方法的时间一致性和自动区域映射的临床实用性。

Conclusion: 自监督时间正则化可有效提升心脏超声分割的时序连贯性，结合自动区域映射有助于标准化临床评估和异常运动检测。
...
TL;DR: MuSViT是首个乐谱基础视觉模型，基于ViT架构和掩码自编码器在970万页IMSLP数据上预训练，采用两阶段课程学习，在四个下游任务上表现优异，其嵌入空间能直接编码符号音乐结构。


<details>
  <summary>Details</summary>
Motivation: 视觉和语言领域已有强大的基础模型，但乐谱作为一种视觉化的音乐语言编码，缺乏专门的领域基础模型。作者旨在填补这一空白。

Method: 采用ViT编码器，通过掩码自编码器(MAE)在970万页IMSLP乐谱上预训练，使用两阶段课程：先在合成排版乐谱上热身，再在大规模真实IMSLP语料上训练。

Result: 在线性探测（冻结编码器）下，MuSViT持续优于通用视觉编码器；在微调下，通常优于任务特定SOTA方法。嵌入-转录一致性分析表明MuSViT的表示空间直接编码符号音乐结构。

Conclusion: MuSViT可作为乐谱理解的基石模型，通用视觉表示在符号音乐结构上存在系统性不足，而领域专用预训练至关重要。
...
TL;DR: 提出TopoGPT，一种基于自回归序列建模的生成式车道拓扑推理框架，通过预训练学习车道图结构先验，解决现有方法中几何不一致和因遮挡导致的车道图不完整问题。


<details>
  <summary>Details</summary>
Motivation: 现有车道拓扑推理方法采用检测+关联范式，独立处理每条车道实例，导致连接端点处几何不一致，且视觉遮挡会造成车道图不完整。

Method: 提出TopoGPT生成式框架，通过自回归序列建模学习车道图结构先验。具体包括：1）构建330万场景的大规模地图数据集；2）车道分词器将车道图序列化为离散词元，场景上下文编码器提取全局特征作为场景词元；3）通过场景条件化的下一词元预测预训练自回归车道序列Transformer；4）设计感知适配器将BEV特征与预训练场景条件对齐，将学习到的几何先验迁移到基于传感器的车道图预测中。

Result: 在OpenLane-V2基准上，TopoGPT在车道级指标上平均提升+6.4，点级指标上平均提升+11.6。

Conclusion: TopoGPT在OpenLane-V2基准上，车道级指标平均提升+6.4，点级指标平均提升+11.6，能够生成几何一致且结构完整的车道图。
...
TL;DR: 提出了一种吸收引导的距离解耦估计与优化（ADER）框架，用于长波红外高光谱被动测距，通过B样条平滑先验、臭氧吸收分类和贪心波段选择，在保持测距精度的同时实现约两个数量级的加速。


<details>
  <summary>Details</summary>
Motivation: 长波红外高光谱观测中包含与距离相关的大气吸收特征，为远距离被动测距提供了物理基础。但自然场景中，这些特征与目标温度、材料发射率和路径辐射非线性耦合，使得从观测辐射反演距离成为病态问题。现有方法计算成本高，且未有效利用尖锐的大气吸收结构。

Method: 1) 使用B样条控制点表示发射率，施加平滑先验抑制对大气吸收结构的过拟合；2) 利用臭氧吸收特征将像素分为发射主导和反射主导两类；3) 对发射主导像素，补偿路径辐射和透过率，通过一维吸收残差最小化估计距离；4) 对反射主导像素，基于完整辐射模型进行下行辐射补偿优化初始估计；5) 引入基于多场景有效Fisher信息的贪心波段选择策略减少光谱冗余。

Result: 在真实场景实验中，ADER在全波段和20波段设置下均恢复出与LiDAR一致的空间距离结构，提高了测距精度，相比公开的全波段高光谱测距方法实现了约两个数量级的加速。

Conclusion: ADER框架有效解决了长波红外高光谱被动测距的病态问题，通过距离-发射率解耦、像素分类优化和波段压缩，在显著降低计算代价的同时保持了高精度测距能力，具有实际应用潜力。
...
TL;DR: 提出MRPO，一种通过逐步过程奖励来优化临床图像推理中早期错误级联的强化学习算法，显著减少早期推理失败并提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有后训练管线以结果为中心，依赖最终答案正确性或序列级偏好，导致稀疏的信用分配，无法优化临床应用中至关重要的推理过程。早期推理失败的级联错误是医学VQA中错误预测的主要原因。

Method: 提出Medical Reasoning-aware Policy Optimization (MRPO)，一种结合逐步过程奖励的强化学习算法。当最终答案错误时，MRPO对早期无效推理步骤的token分配指数级更大的惩罚，以打破失败级联而不影响成功路径。

Result: 在三个多模态LLM骨干上，MRPO一致优于标准GRPO和最近的RL基线；在Qwen3-VL-8B-Instruct上超越更大的医疗MLLM（如HuatuoGPT-Vision-34B）2.79分；早期推理失败率从64.0%降至13.0%。

Conclusion: 针对性地缓解级联失败能同时提高推理质量和最终答案准确性，MRPO有效实现了这一目标。
...
TL;DR: 本文提出地理空间视觉先验和双记忆架构的记忆增强模块，为端到端自动驾驶提供视觉空间预判能力，在NAVSIM-v2基准测试上持续提升性能，并对传感器损坏具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶方法仅依赖瞬时传感器观测，缺乏人类驾驶员通过先前经验获得的预期性预判能力，导致行为反应性强而前瞻性不足。

Method: 提出地理空间视觉先验（street-level visual context anchored to intended route）和记忆增强模块，该模块包含双记忆架构（上下文记忆+持久后备记忆）和自适应记忆门，可轻松集成到现有端到端方法中。

Result: 在NAVSIM-v2基准测试上，该方法在多个端到端基线模型上持续提升性能，并对传感器损坏具有固有的鲁棒性。

Conclusion: 地理空间视觉先验能有效增强自动驾驶的前瞻性，双记忆设计在检索先验不可靠时提供安全回退，提升了系统的鲁棒性和安全性。
...
TL;DR: 本文提出RT-SFOD方法，基于YOLOv10双头检测器，通过DHF伪标签融合策略和MARD多尺度特征多样化损失，在源无关目标检测中实现更优的速度-精度-模型大小权衡，相比现有方法mAP提升1.4-3.5%，吞吐量提升1.3倍，参数量减少约2倍。


<details>
  <summary>Details</summary>
Motivation: 现有源无关目标检测方法依赖重型架构，仅追求精度而忽视延迟和内存约束，无法满足自动驾驶、监控和机器人等实际应用中的严格实时性要求。

Method: 1) 基于YOLOv10无NMS双头检测器构建框架；2) 提出DHF（双头伪标签融合）策略，选择性融合O2O和O2M头的预测结果；3) 提出MARD（多尺度自适应表示多样化）损失，通过对多尺度特征图施加检测感知的方差和协方差约束来缓解域偏移导致的多尺度特征判别性崩溃。

Result: 在域偏移基准上，mAP提升1.4-3.5%，吞吐量提高1.3倍，参数量减少约2倍；在YOLO和DETR系列双头检测器上均验证了泛化性。

Conclusion: 速度、精度和模型大小之间的权衡并非必要，通过合理设计可在保持高效的同时达到最优的域适应检测性能。
...
TL;DR: 本文提出Consispace，一种语义感知重采样框架，通过ODE约束保持层间解剖连续性和预训练模型特征保持层内语义一致性，作为预处理步骤能显著提升医学图像分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割研究主要关注网络架构创新，但忽视了物理体素间距（voxel spacing）的影响。图像重采样作为分割前的常规预处理步骤，其与分割任务之间的交互关系尚未被系统性地研究和利用。

Method: 1) 提出Consispace语义感知重采样框架，在轴向方向上实现一致的体素间距并保持解剖与语义一致性；2) 引入基于ODE的解剖约束，通过连续插值器建模层间动态变化，超越传统离散插值；3) 利用预训练视觉模型提取密集特征构建层内语义相关性图，通过特征重加权注入类别级语义一致性；4) 将层内和层间约束整合到隐式神经网络中，支持任意尺度的重采样。

Result: 在多个数据集上的实验表明，Consispace在重建质量和感知保真度上优于现有方法，能生成更平滑的层间解剖结构，作为预处理步骤可有效提升下游分割性能。

Conclusion: Consispace通过联合建模重采样过程中的解剖连续性和语义一致性，为医学图像分割提供了一种有效的预处理手段，填补了重采样与分割任务交互研究的空白。
...
TL;DR: 本文提出了SENSE-VAD，首个面向自动驾驶中社交复杂异常（如儿童逃离监护人、追逐等）的合成视频异常检测基准。该基准明确区分了社交异常与基于运动或外观的异常，并证明现有SOTA方法尚无法解决这一挑战。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶异常检测方法主要关注基于运动统计的异常（如突然变道、物体坠落），但忽略了社交关系层面的危险场景（如追逐、被抱持、儿童跑离监护人等）。这些情况在运动信号上并不显著，却在社交语境下构成异常。

Method: 基于CARLA模拟器和Unreal Engine生成合成视频异常检测基准SENSE-VAD，包含多类社交复杂异常场景（个体行为、群体行为、人-物交互、自行车交互、车辆与智能体），每个场景提供逐帧二元标签。设计原则上将社交异常与运动/外观异常分离。同时提供真实世界视频作为模拟到真实的迁移探测。

Result: 评估了多个SOTA视频异常检测基线方法，结果表明社交复杂异常构成一个独特的、当前尚未被解决的挑战，现有方法无法有效检测此类异常。

Conclusion: 社交复杂异常是自动驾驶安全中一个独立且重要的研究方向，现有异常检测方法在此类场景中表现不足，SENSE-VAD为该方向提供了标准化评估基准。数据集、标注和生成代码均已公开。
...
TL;DR: RESOLVE是一个大规模真实世界路边多模态感知基准数据集，包含10万+图像和2.6万点云帧，用于系统评估不同LiDAR分辨率下（单模态vs融合架构）的3D检测与跟踪性能，揭示多模态融合如何补偿LiDAR点稀疏性问题。


<details>
  <summary>Details</summary>
Motivation: LiDAR已集成到交通摄像头中用于路边协同感知，但单模态和相机-LiDAR融合架构在不同LiDAR点稀疏度（由传感器配置和场景条件引起）下的行为尚未被充分探索。

Method: 构建了RESOLVE数据集，包含多分辨率路边LiDAR和同步相机-LiDAR感知数据，在真实城市交叉口采集，涵盖不同光照和天气条件、10类交通参与者、22万+人工标注框；支持三种LiDAR分辨率级别的受控评估，同时保持其他传感和环境因素固定。

Result: 大规模基准实验揭示了多模态融合如何补偿LiDAR点稀疏性，展示了分辨率变化、感知距离以及训练-推理分辨率不匹配下的点云分布偏移对架构性能的影响。

Conclusion: RESOLVE为设计成本高效的路边多模态感知提供了线索，数据集和基准代码已开源。
...
TL;DR: 本文提出了一种操作符级别的视觉令牌跳过框架，在保留完整视觉令牌序列的同时，选择性跳过冗余的Attention和FFN算子，从而在降低计算量的同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM加速方法通常粗暴地移除视觉令牌或跳过整层更新，这可能会丢弃细粒度证据或同时抑制有用和冗余的算子。本文从答案可观察的角度发现视觉令牌更新存在"答案沉默冗余"现象，即后期视觉令牌更新对答案令牌表示影响很小，因此需要更精细的加速策略。

Method: 将每个Transformer层分解为Attention和FFN两个算子，从答案可观察角度分析发现视觉计算的有用性是算子主导且层相关的。据此提出操作符级别的视觉令牌跳过框架，在保留完整视觉令牌序列的前提下，根据冗余程度选择性跳过Attention、FFN或两者。

Result: 在三种MLLM架构和10个VQA基准上，该方法实现强效率-准确率权衡，在Qwen3-VL上减少33.7% TFLOPs，同时保留99.5%的原始模型性能。

Conclusion: 操作符级别的细粒度跳过策略优于传统的粗粒度视觉令牌移除或整层跳过方法，能在显著降低计算量的同时保持模型性能。
...
TL;DR: DriveWeaver提出了一种基于视频修复的框架，通过点云条件控制实现前景车辆的可控插入，解决了现有方法中光照不一致、依赖有限3D资产的问题，支持大规模自动驾驶场景增强。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶仿真中插入前景车辆的方法依赖预重建的3D资产，导致前景与背景光照不一致，且受限于少量人工筛选的3D资产，难以大规模部署。

Method: 1) 基于视频修复的方法，以车辆点云为条件生成高质量、时序一致的前景车辆；2) 全局到局部的分层修复策略，保证长序列生成中车辆身份和外观的一致性；3) 通过城市重建流水线提取显式3D高斯表示，实现实时渲染。

Result: 在多个数据集上的实验表明，DriveWeaver在视觉真实感和几何一致性上均优于现有基线方法，能有效支持可扩展的自动驾驶场景增强。

Conclusion: DriveWeaver是一种鲁棒的自动驾驶场景增强工具，无需预重建3D资产即可实现光照一致、时序连贯的前景车辆插入，为大规模自动驾驶仿真提供了实用方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有可控图像生成方法（如ControlNet）在多实例场景中容易出现实例属性混淆问题，而最近的方法需要人工实例标注，成本高昂。

Method: 利用视觉语言模型（VLM）自动解析文本提示中的实例描述，并基于视觉条件预测实例掩码；同时引入自适应掩码细化策略在生成过程中动态优化实例掩码。

Result: 在多个实验上优于现有最先进方法，实现了更高的生成保真度和精确的实例级控制。

Conclusion: InstanceControl无需实例标注即可有效解决多实例场景中的属性混淆问题，实现了更优的生成效果。
...
TL;DR: 提出了VidPair-Halluc基准，通过背景相似但前景语义不同的视频对来评估大视频模型中的幻觉问题，配套PairFlow管道和1K视频对+11K问答对。


<details>
  <summary>Details</summary>
Motivation: 现有视频幻觉基准大多依赖文本扰动或对抗性问题，忽略了视觉背景一致性，无法准确归因模型错误是否为真实幻觉。

Method: 提出PairFlow管道，利用文本到图像和视频生成技术系统性地生成对抗性视频对，覆盖10个语义维度的空间和时间推理。

Result: 构建了1K高质量对抗性视频对和11K时空问答对，主流大视频模型在对抗性设置下表现持续困难。

Conclusion: VidPair-Halluc通过严格控制背景和前景变化，能更精准地评估视频模型的幻觉问题，揭示了现有模型在细粒度视频理解上的持续不足。
...
TL;DR: 本文提出世界叙事模型(WNM)，通过将物理叙事（4D场景表示）与像素生成解耦，利用协作智能体将多模态输入转化为可编辑的4D世界表征，再驱动视频基础模型进行渲染，实现了对几何、运动、相机参数和光照的确定性定量控制，从而解决了工业级视频生成中缺乏可控性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型将视频视为像素分布采样问题，缺乏对几何、运动、相机参数和光照的显式定量控制，导致内容创作陷入"抽卡"循环，专业视频制作效率极低且成本高昂。

Method: WNM将"渲染什么"（结构化物理叙事）与"如何渲染"（像素生成过程）解耦。协作智能体将稀疏多模态输入（文本、参考视频、草图）转化为可编辑的4D世界表征，包含场景几何、物体布局、骨骼运动、轨迹、相机运动和光照等定量物理参数，作为确定性蓝图驱动视频基础模型（冻结或轻量适配）进行渲染。

Result: 实验表明，WNM大幅减少了概率性"抽卡"调用次数，生成的视频在布局、运动和摄影方面紧密遵循创作者意图。

Conclusion: WNM提供了一种开放模块化的新范式，极大地提升了视频生成的可控性，各组件可独立改进，为专业影视制作流程提供了可行方案。
...
TL;DR: AnyBokeh提出了一种基于物理引导的任意到任意景深编辑框架，通过估计源图像的符号弥散圆图和视差图，实现了无需先恢复全聚焦图像即可进行后景深编辑的方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常假设输入为全聚焦图像，或先恢复全聚焦图像再渲染新景深，这会丢弃源图像中的模糊线索并传播重建伪影。

Method: AnyBokeh通过估计符号弥散圆图和视差图，建模弥散圆与视差差异的线性关系，提取源图像光学指纹，并利用条件生成编辑器进行相对模糊合成，实现空间自适应的去模糊、保持和散焦渲染。

Result: 在真实世界基准测试中，AnyBokeh在任意到任意景深编辑、全聚焦到散焦渲染和散焦去模糊任务上均实现了忠实且可控的编辑效果。

Conclusion: AnyBokeh避免了现有方法所需的中间全聚焦图像重建和测试时景深级别校准，实现了高效的任意到任意景深编辑。
...
TL;DR: 提出利用平面场景中的单应性分解来估计相机位姿的统一框架，通过图方法和谱嵌入滤除不可靠边，在平面场景和复杂3D场景中均表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统SfM方法在平面场景中因对极几何退化而失效，作者希望将平面表面从"局限性"转变为"几何约束来源"，以解决高度平面场景（如体育馆）的位姿估计难题。

Method: 1) 利用多视图可见的平面表面通过单应性分解估计相对相机位姿；2) 构建位姿图，采用谱嵌入方法将单应性估计映射到实线上，基于几何和视觉一致性过滤不可靠边；3) 提取最大一致性生成树用于位姿恢复。

Result: 在篮球场等高度平面场景中优于现有方法，在IMC Phototourism非约束户外场景基准上达到或超越最先进水平。

Conclusion: 提出了一种统一框架，能同时处理平面场景和一般3D环境，将平面表面从SfM的痛点转化为优势。
...
TL;DR: LUNA是一种无需LBS的通用神经动画模型，可直接将多种2D控制信号（图像、关键点、草图等）映射为3D高斯变形，实现高保真、可驱动的3D人体化身生成，并支持零样本跨身份泛化。


<details>
  <summary>Details</summary>
Motivation: 现有基于LBS和参数化人体模型的方法受限于表达能力和拟合伪影，且依赖显式身体拟合，无法灵活支持多种2D驱动方式。

Method: 提出无LBS的通用神经动画模型LUNA，核心包括：1) 基于Transformer的运动回归器，解耦全局刚体运动与局部非刚性动态；2) 混合监督策略，从LBS教师模型蒸馏软结构先验，并结合有限拟合数据和大规模无标签视频进行训练。

Result: 与LBS方法相比，LUNA在视觉保真度上具有竞争力，能生成逼真的人体运动，并实现零样本跨身份泛化，支持多种驱动模态。

Conclusion: LUNA是首个支持隐式2D驱动的端到端3D可动画化模型，突破了传统LBS框架的局限性。
...
TL;DR: 提出ERA框架，通过熵引导的视觉标记剪枝和注意力对数校正，在不重新训练的情况下减少多模态大语言模型的推理成本，解决注意力对数坍缩问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型因长视觉标记序列导致推理成本高昂，现有训练-free的视觉标记缩减方法会扭曲注意力分布，引发注意力对数坍缩问题。

Method: ERA包含三个组件：双视角熵剪枝（DEP）通过联合建模视觉多样性和头级显著性识别代表性锚点标记；偏置感知标记回收（BTR）将剪枝标记回收至对应锚点并估计聚类级对数偏置；对数保持注意力校正（LAR）将估计偏置注入注意力对数以校正坍缩。

Result: 在单图像、多图像和视频设置下，ERA在多种MLLMs上实现鲁棒性能，即使在激进压缩下也能保留视觉证据，并带来实际加速效果。

Conclusion: ERA建立了对数保持视觉标记剪枝作为高效MLLMs的框架，统一了理论基础、算法设计和实际部署。
...
TL;DR: CoLT是一种新颖的多模态大模型推理框架，通过链式潜在思维表示（仅需3步）替代冗长的文本链式思维推理，在保持高性能的同时，将推理时间缩短10.1倍，文本解码时间缩短22.6倍。


<details>
  <summary>Details</summary>
Motivation: 传统的基于文本的链式思维推理速度慢（需生成数千token），且受限于自然语言的表达能力。作者希望探索一种更高效、更紧凑的潜在思维推理范式。

Method: 提出CoLT框架，用链式潜在思维表示替代文本推理步骤。引入轻量级外部解码器提供前向（将潜在思维解码为下一文本推理）和后向（对齐解码器隐藏状态与潜在思维）两步监督，并加入内部监督确保潜在步骤的连贯过渡。推理时移除解码器和内部监督以保持高效。

Result: 在8个基准测试上，CoLT优于现有潜在推理方法（如CODI、SIM-CoT）和依赖辅助图像的潜在视觉推理方法。推理时间减少10.1倍，文本解码时间减少22.6倍。

Conclusion: CoLT证明了通过潜在思维表示进行高效推理的可行性，在速度与性能之间取得了良好平衡，为多模态大模型的推理加速提供了新方向。
...
TL;DR: AutoBackSwap通过自动交换图像的前景和背景来减少深度神经网络对虚假背景特征的依赖，只需少量样本标注即可训练辅助网络，在多个图像分类任务中优于先前方法。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络分类器容易依赖训练数据中的虚假相关性（如图像背景）进行预测，导致泛化能力差，而现有方法往往需要训练数据中存在打破虚假相关性的样本。

Method: 使用辅助网络分离前景和背景，通过填充合成完整背景，然后将不同前景与修复后的背景组合来增强训练数据。仅需几百个样本的块级标注即可训练辅助网络。

Result: 即使训练数据中没有打破虚假相关性的样本，AutoBackSwap依然有效，在多个具有虚假背景的图像分类任务中一致优于先前方法。

Conclusion: AutoBackSwap是一种有效减少分类器对虚假背景依赖的数据增强方法，且对标注需求低，适用范围广。
...
TL;DR: 本文提出Bridge，一种轻量级潜空间接口，用于解决跨空间蒸馏（Cross-Space Distillation）问题——即教师和学生模型因潜空间分辨率与VAE参数化不同而无法直接进行知识蒸馏。Bridge通过冻结的学生VAE解码器与可学习的投影器，将学生潜变量映射到教师空间，实现高效蒸馏，例如将SD 1.5的HPSv3从5.4提升至9.4。


<details>
  <summary>Details</summary>
Motivation: 现有单步扩散模型的蒸馏方法假设教师与学生模型必须处于相同的潜空间（Shared-Space constraint），这限制了将高容量教师模型（如SD 3.5、Flux）的知识迁移到紧凑、可部署的学生模型（如SD 1.5）中，因为它们在潜空间分辨率和VAE参数化上存在差异。

Method: 提出Bridge，一种轻量级潜空间接口，包含冻结的学生VAE解码器作为空间先验，以及一个紧凑的可学习投影器。通过潜空间重建和注意力保真度损失进行训练，实现学生潜变量到教师空间的稳定对齐。

Result: Bridge在多种现代教师模型上均取得了显著提升，例如将SD 1.5的HPSv3从5.4提升至9.4，同时保持单步推理、低延迟和广泛的生态兼容性。

Conclusion: 通过轻量级潜空间接口，异构的大容量教师模型可以有效地蒸馏到高效、可部署的学生骨干网络中。
...
TL;DR: 提出FLORA深度学习框架，利用异构LiDAR点云和辅助数据预测六种森林属性，在法国32,052个NFI样地训练，实现了跨季节鲁棒预测，主导高rRMSE约12.3%，总体积约39%。


<details>
  <summary>Details</summary>
Motivation: 国家LiDAR计划在欧洲推广，传感器、飞行参数、季节和扫描角度的差异导致现有模型在异构条件下鲁棒性不足，需要一种能处理异构LiDAR数据的大规模森林属性估算方法。

Method: FLORA框架：基于八叉树的骨干网络 + 生态与时空辅助变量，通过后融合门控机制结合，在法国LiDAR HD数据和32,052个NFI样地上训练。

Result: 单模型跨季节训练优于季节专用模型；辅助变量对物种特异性体积预测贡献更大；主导高rRMSE=12.3%（R²=0.88），总体积rRMSE=39%（R²=0.74）。

Conclusion: FLORA为异构国家LiDAR项目的大规模森林属性估算提供了稳健基线，能够有效处理跨季节和异构采集条件的LiDAR数据。
...
TL;DR: 提出SpheRoPE框架，通过将球形先验直接注入预训练扩散Transformer，实现零样本、无需训练和优化的360度全景图像与视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有全景生成方法要么依赖稀缺全景数据的昂贵微调导致泛化受限，要么采用多步优化带来高推理延迟。作者发现预训练模型虽天然具备一定全景先验，但不足以满足等距柱状投影的严格拓扑约束。

Method: 提出Spherical RoPE：低频通道重参数化为3D笛卡尔坐标以编码球形流形，高频通道谐波量化强制精确周期性；配合语义失真分类器无引导（CFG）显式引导几何结构，无需重新训练。

Result: 在多个骨干网络（Flux.1、Flux.2、LTX-Video）上实现文本到全景生成，性能与基准方法相当，且完全无需训练。

Conclusion: 零样本、无需训练的SpheRoPE框架有效解决了全景生成的拓扑约束问题，具有良好的跨骨干网络和生成模态的泛化能力。
...
TL;DR: PointSplat提出了一种直接从点云预测3D高斯原语的人体重建方法，通过3D空间预测消除多视角冗余，在保持高保真度的同时显著压缩表示，适用于实时流媒体系统。


<details>
  <summary>Details</summary>
Motivation: 现有前馈式重建方法在多个视角下重复编码相同的主体内容，导致视角间存在大量冗余，在计算资源和传输带宽有限的实时流媒体系统中效率低下。

Method: 首先估计粗略几何代理并执行光线投射以修剪冗余点并建立显式2D-3D对应关系，然后使用点-图像Transformer融合外观和几何特征，在单次前向传播中预测高斯属性，将预测限制在前景区域。

Result: 在多个数据集上，PointSplat在效率和渲染质量上都优于现有方法，并且对视角数量和图像分辨率的变化表现出强鲁棒性。

Conclusion: 直接在3D空间进行预测可以有效消除多视角冗余，实现紧凑且高质量的人体3D表示，为实时沉浸式直播系统提供了可行的解决方案。
...
TL;DR: GEAR提出了一种端到端联合训练VQ分词器和自回归生成器的方法，通过表示对齐引导，解决了非可微VQ索引导致的分词器无法接收梯度的问题，相比基线方法在ImageNet gFID收敛速度上提升高达10倍。


<details>
  <summary>Details</summary>
Motivation: 传统视觉生成模型采用两阶段训练（先训练分词器再训练生成器），导致分词器不了解生成器容易建模的内容，两者解耦限制了生成性能。

Method: 采用双读出的码本分配方式：硬one-hot分支训练AR进行下一词预测，可微软分支携带表示对齐损失反向传播以引导分词器。AR模型引导分词器朝向更易预测的索引分布。

Result: ImageNet gFID收敛速度相比LlamaGen-REPA基线提升高达10倍，学习到更好的patch级和空间一致特征，可泛化到多种量化器（VQVAE、LFQ、IBQ）和文生图任务。

Conclusion: GEAR通过端到端联合训练有效对齐了分词器和生成器，将对齐负担从分词器转移到AR模型，取得了显著的性能提升和泛化能力。
...
TL;DR: FaceMoE是首个将混合专家（MoE）Transformer架构应用于低分辨率人脸识别的方法，通过引入多个FFN专家和top-k路由器实现分辨率感知的特征提取，在11个数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 低分辨率人脸识别面临特征提取困难、HR与LR图像域差距大、单一编码器难以泛化以及灾难性遗忘等问题，亟需一种能同时处理多分辨率且保持预训练知识的方法。

Method: 提出FaceMoE，在Transformer中集成多个FFN专家和top-k路由器，动态分配token到合适的专家，实现分辨率感知的特征提取；采用稀疏专家激活机制，结合人脸识别损失、router z-loss和负载均衡损失进行训练。

Result: 在11个数据集（包括HR、混合质量和LR基准）上的大量实验表明，FaceMoE显著优于当前最先进的方法。

Conclusion: FaceMoE首次将MoE成功应用于低分辨率人脸识别，有效解决了域差距和灾难性遗忘问题，兼顾了模型容量与计算效率。
...
TL;DR: 提出了一种自动化描述优化管线，将企业级AI Agent的技能描述调优人工成本降低32倍（从120分钟降至3.8分钟），F1分数达到79.2%，与人工调优的79.4%几乎无差异。


<details>
  <summary>Details</summary>
Motivation: 当Agent扩展至数十个技能时，技能描述重叠会导致路由LLM误判（技能碰撞），手动调优描述成为工程瓶颈。

Method: 部署自动化描述优化管线，利用假阳性和假阴性案例进行单次LLM重写，在生产群聊Agent（9技能，372回归案例）上验证。

Result: F1达79.2%（人工79.4%），单技能差异-0.20%（低于噪声底限0.78%）；工程耗时从120分钟降至3.8分钟（32倍加速）。消融实验表明单次LLM重写即捕获大部分改进，其他设计选择影响<0.5%。

Conclusion: 描述优化可解决重叠描述导致的技能碰撞，但无法解决技能本身范围重叠；提出训练-验证F1差距指标用于识别后者。
...
TL;DR: 提出Indi-RomCoM基准，评估大语言模型在印度语罗马化代码混合指令上的表现，发现模型随混合密度增加性能下降，推理任务退化少于检测任务。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在单语和原生文字基准上表现优异，但在罗氏化代码混合（RCM）内容上的指令跟随和推理能力尚未被系统探索。

Method: 构建Indi-RomCoM基准，涵盖7项指令遵循任务、4种印度语言、3种控制混合强度，在零样本和少样本设置下评估多种LLM。

Result: LLM在RCM指令上表现不佳，性能随代码混合密度增加而下降；推理任务比检测任务（如毒性检测）退化更少，因为生成解释提供了必要上下文。

Conclusion: Indi-RomCoM有助于社区开发更具包容性的多语言系统，揭示了当前LLM在罗马化代码混合场景下的局限性。
...
TL;DR: 本文提出使用生成式AI代理作为行为引擎，对个性化推荐算法进行黑盒审计。通过在X平台部署1120个代理（14种人物角色、3种反事实条件），收集超20万条内容曝光数据，发现算法推荐流相比时间线流更放大毒性、极化、政治化和右倾内容，且放大效应因用户意识形态而异；反事实分析表明人口统计信号对内容推送的影响取决于用户角色。


<details>
  <summary>Details</summary>
Motivation: 现有审计方法存在两难：真人实验成本高且难以控制，傀儡账号审计虽可扩展但缺乏真实性。同时，两种方法都难以分离用户属性与行为对个性化的因果影响。

Method: 基于调查数据构建具有固定人物角色的生成式AI代理，通过推理平台内容并选择行动来模拟真实用户行为。实验保持行为固定，仅扰动平台可见信号（年龄、性别、位置），实现反事实因果审计。

Result: X算法推荐流相比时间线流更放大毒性、极化、政治化和右倾内容；这种放大效应随用户意识形态变化显著；人口统计信号的因果效应依赖于具体人物角色，聚合效应基本为零，子组效应方向与大小各异。

Conclusion: 生成式AI代理可作为算法审计的有效新工具，能够克服传统方法的局限，实现可扩展、真实且具备因果推断能力的黑盒审计。
...
TL;DR: 本文提出ACE框架，通过控制模型准确率来实现更公平的跨模型校准比较，发现原始全局校准指标在跨模型比较中不可靠，排名反转现象常见。


<details>
  <summary>Details</summary>
Motivation: 现有研究使用全局校准指标（如ECE、Brier Score）比较不同大语言模型的校准性能，但这些比较因模型准确率差异而产生混淆，无法公平评估校准能力。

Method: 提出ACE（Accuracy-Controlled Evaluation）框架，包含三种互补视角：实例对齐校准、分布对齐校准和候选对齐校准，在控制准确率条件下评估模型校准。

Result: 原始全局校准指标下的优势在控制准确率后显著削弱；排名反转频繁，原始指标偏好的模型在准确率控制后不再受青睐。
...
TL;DR: 本文发现Transformer语言模型在"不可能语言"上表现出生成能力的显著缺陷，而非语法敏感性缺失，这为解释人类语言中为何不存在此类结构提供了新视角。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要基于样本效率和困惑度差异来论证语言模型对"不可能语言"的偏好，缺乏对语言习得能力的直接评估。本文旨在提出更合理的理论假设，解释为什么某些语言结构在人类语言中不存在。

Method: 使用GPT-2风格模型，在英语的"不可能"变体上进行训练，通过BLiMP最小对任务评估语法敏感性，同时评估模型的生成能力（句子质量和长度）。

Result: 模型在语法敏感性上仅表现出渐进式退化（受信息局部性调节），但在生成任务中表现出显著缺陷，尤其是在长句生成时高质量句子数量大幅减少。

Conclusion: 生成缺陷和传递失败是连接语言模型行为与"不可能语言"非存在性的合理假设。
...
TL;DR: 本文提出GradeSQL框架，通过训练结果奖励模型(ORM)作为语义评分函数，在Text-to-SQL任务中替代传统的启发式测试时选择策略（如Best-of-N和多数投票），显著提升了推理可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在结构化推理任务（如Text-to-SQL）中的推理时可靠性是一个核心挑战。现有的测试时策略（如Best-of-N采样和多数投票）依赖执行成功或输出频率等启发式信号，这些信号在候选输出之间提供的语义区分能力有限，导致选择效果不佳。

Method: 提出GradeSQL框架：(1) 通过自动候选生成和执行标签构建，无需人工标注即可训练任务特定的ORM；(2) 将ORM集成到验证驱动的Best-of-N流水线中；(3) 在BIRD和Spider基准上评估多种开源LLM家族。

Result: ORM-based选择在BIRD上提升高达+4.33%，在Spider上提升高达+2.10%；ORM在更大候选集上表现更好，且对复杂查询的提升更显著。

Conclusion: ORM-based验证为Text-to-SQL提供了一种简单、有效且可扩展的替代方案，优于传统启发式测试时选择策略。
...
TL;DR: 该论文描述了SemEval-2026任务9的多语言、多文化、多事件在线极化检测系统，使用RoBERTa-base（英语）和AfroXLMR-base（斯瓦希里语）模型，结合类别加权损失函数和每标签阈值调优，在三个子任务上取得了有竞争力的F1分数，并指出模型在非人化检测和共情缺失方面存在困难。


<details>
  <summary>Details</summary>
Motivation: 针对多语言、多文化、多事件的在线极化检测任务，解决严重的标签不平衡问题，提升多标签分类性能。

Method: 采用基于Transformer的模型（英语用RoBERTa-base，斯瓦希里语用AfroXLMR-base），使用类别加权损失函数处理标签不平衡，并通过每标签阈值调优优化多标签分类。

Result: 子任务1：F1宏平均0.7901（英语）和0.7910（斯瓦希里语）；子任务2：0.4615（英语）和0.4808（斯瓦希里语）；子任务3：0.4791（英语）和0.5830（斯瓦希里语）。

Conclusion: 所提方法在处理不平衡多标签极化检测任务上效果显著，在排行榜上表现有竞争力；误差分析显示模型在非人化检测和共情缺失方面仍有不足。
...
TL;DR: 提出TheraAlign框架，通过可操作的多维评估信号（TheraJudge）指导心理治疗响应生成（TheraAgent），将治疗质量提升0.43分（5分量表），低质量响应改善2.45分，恢复率94%。


<details>
  <summary>Details</summary>
Motivation: 现有LLM用于心理支持时，评估仅作为被动指标，未能有效指导模型改进；需要将评估转化为可操作的控制信号来提升治疗质量。

Method: 两阶段框架：Stage I训练开源治疗评估器TheraJudge（基于偏好优化与人类标注，覆盖7个心理维度）；Stage II构建TheraAgent，通过Critic（批评）、Coach（教练）、Therapist（治疗师）三个角色协同，将评估信号转化为针对性响应修正。

Result: TheraJudge与临床医生评分高度一致（ICC=0.87-0.95），超越监督基线及强闭源评估器；TheraAgent将治疗质量提升0.43分，盲评下96%临床医生间信度，低质量响应改善2.45分，恢复率94%。

Conclusion: 心理健康的LLM对齐应基于可操作的人类对齐评估信号，而非单纯依赖更强的生成模型。
...
TL;DR: 本文针对孟加拉语事件检测系统，构建了包含9,979条标注句子的基准数据集，涵盖清洁文本、ASR转录和拼写错误文本，系统评估了编码器模型与解码器大语言模型在不同噪声条件下的鲁棒性表现。


<details>
  <summary>Details</summary>
Motivation: 现有事件检测系统大多在清洁、精心整理的文本上评估，缺乏对真实世界噪声（尤其是低资源语言如孟加拉语）鲁棒性的系统研究。

Method: 构建了包含40个事件子类型的孟加拉语新闻事件本体和9,979条标注句子基准，涵盖清洁新闻文本、真实ASR转录和拼写错误文本；系统评估了编码器模型（BanglaBERT、XLM-R）和指令微调解码器大语言模型（Llama 3、Gemma 3）。

Result: 编码器模型在清洁文本上表现更好但噪声下性能显著下降；解码器LLM鲁棒性更强，尤其在事件触发词被破坏时；嵌入标注指南进行指令微调可提高噪声文本基线但效果不一致；模型规模扩展持续提升解码器LLM鲁棒性；联合清洁与噪声数据训练作为有效正则化策略，显著缩小了鲁棒性差距。

Conclusion: 揭示了编码器与解码器架构在噪声鲁棒性上的根本性权衡，为低资源语言事件检测系统在实际部署中的架构选择提供了重要指导。
...
TL;DR: 本文构建了阿拉伯语-俄语科学翻译基准，包含约27,000句对的混合平行语料库，并微调了三个多语言模型（mT5-base、NLLB-200-distilled-1.3B、Qwen2.5-7B-Instruct）。Qwen2.5-7B模型在QLoRA（rank 8）下取得最佳效果（BLEU 23.15, COMET 0.758），显著优于零样本基线，而少样本提示未能提升性能。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语和俄语是科学交流的重要语言，但语言障碍阻碍了这两个研究社区之间的成果交换，影响了国际合作与可持续发展相关研究的进展。

Method: 构建约27,000句对的混合平行语料库（科学摘要与通用领域文本），使用LoRA（rank 8,16,32,64）微调三个多语言模型：mT5-base（580M）、NLLB-200-distilled-1.3B、Qwen2.5-7B-Instruct，并对比零样本基线与少样本提示效果。

Result: Qwen2.5-7B模型在QLoRA（rank 8）下取得BLEU 23.15、chrF 43.89、BERTScore 0.906、COMET 0.758，比零样本基线提升+4.36 BLEU和+0.051 COMET。少样本提示（3个示例）未能提升性能。

Conclusion: 领域特定微调对于提升翻译质量至关重要；该工作降低了阿拉伯语与俄语研究者之间的语言障碍，促进了知识交换，有助于可持续发展目标（SDG 9和SDG 17）。
...
TL;DR: 本文通过分析社交媒体文本，研究语言距离（linguistic distancing）这一情绪调节策略如何随年龄变化，发现年龄越大，语言距离的使用比例越高，这与心理学中老年人情绪幸福感提升的结论一致。


<details>
  <summary>Details</summary>
Motivation: 情绪调节对心理健康至关重要，而心理距离是情绪调节的一种方式，会在语言中留下标记（语言距离）。然而，语言距离如何随年龄变化尚不清楚，探索这一问题有助于理解情绪调节的年龄差异。

Method: 基于已有的语言距离度量方法，系统分析大量社交媒体文本（人们表达情绪的场所），按年龄组划分，考察语言距离在不同年龄段的变化模式。

Result: 语言距离随年龄增长而增加，即年龄越大的人在表达情绪时越倾向于使用心理距离的语言标记。

Conclusion: 研究结果进一步证实了语言距离随年龄增加的假设，与心理学中老年人情绪幸福感提升的发现一致。语言距离可作为幸福感的标记，为健康干预提供参考，并为在文本数据中探索情绪调节奠定基础。
...
TL;DR: 本文将Fricative Policy Optimization（FPO）从命题不对称扩展至感知不对称场景，发现摩擦信号在有限信息视角下才有效，单一方视角优于全知视角，并提出两种标注改进方法。


<details>
  <summary>Details</summary>
Motivation: FPO将对话中的摩擦视为共建共同知识的有用信号，但现有实现假设参与者共享感知语境（命题不对称）。现实对话中常存在感知不对称，即同一指称表达在不同参与者的信息状态下产生不同指称对象。需要验证FPO在感知不对称下的有效性。

Method: 跨语料库分析与LLM探针实验，以HCRC MapTask为主要任务，对比命题不对称与感知不对称场景下的摩擦函数表现，评估不同信息视角（单方视角vs全知视角）下的对齐效果。

Result: FPO的摩擦函数仅在参与者各自的信息视野内评估时有效；不同地标配置产生不同性质的接地失败模式，少数模糊配置引发大部分误解；具备"正确视角"的单一方胜过拥有双方语境的全知视角。

Conclusion: 提出两种标注改进：待定接地状态的子类型分解与容纳感知差异的对齐分类，以更好地建模感知不对称下的对话接地过程。
...
TL;DR: 本文提出了一种基于大语言模型（LLM）的推理模式评分方法（EQMs），用于评估预测竞赛中专家书面解释的质量。通过对超过55,000条预测-理由对的分析，发现EQMs在预测和预测者层面均能有效预测准确性，优于传统文本分析方法，且能识别表现不佳者，但难以区分顶尖预测者。


<details>
  <summary>Details</summary>
Motivation: 决策者依赖专家判断及其书面解释，但大规模评估解释质量困难。预测竞赛提供了一个天然测试环境，因此需要一种可扩展、可解释的方法来从书面解释中提取与判断相关的信息。

Method: 定义了60个理论引导的推理模式（EQMs），利用大语言模型（LLM）对超过55,000条预测-理由对进行评分，并分析其与预测准确性的相关性。同时与人类评分和传统文本分析方法进行对比。

Result: EQMs在预测和预测者层面均能预测准确性，优于传统方法；超过90%的显著相关性符合假设方向；信号不对称，能可靠识别表现不佳者但难以区分顶尖预测者；EQMs是预测层面最强的预测因子，在预测者层面具有竞争力；人类评分与准确性的相关性较弱且过度依赖解释长度。结果在独立研究中得到验证。

Conclusion: EQMs提供了一种可扩展、可解释的方法，能从书面解释中提取与判断相关的信息，为大规模评估专家解释质量提供了有效工具。
...
TL;DR: 本文发现大语言模型中存在“演绎刻板印象”这一失效模式，并提出Fair-GCG框架通过推理时注入触发词来提升模型公平性，实验证明该方法在多个基准上有效。


<details>
  <summary>Details</summary>
Motivation: 尽管推理能力提升通常能改善LLM的公平性，但一些失败案例仍然存在。作者希望识别并解决模型在推理过程中产生的社会偏见问题。

Method: 提出推理时注入（reasoning-time injection）框架，并设计Fair-GCG方法系统性地发现有效的注入短语，引导模型进行公平感知推理。

Result: Fair-GCG发现的注入短语在多个公平性基准上提升性能，可从小模型泛化到大模型，改善推理层面公平性，减少开放生成中的偏见，并迁移到真实世界的公平敏感任务。

Conclusion: 推理时注入是一种有效的公平性干预手段，Fair-GCG能够自动发现有效的注入短语，为提升LLM社会公平性提供新思路。
...
TL;DR: CORTEX是一种针对检索增强生成（RAG）的令牌级幻觉检测方法，通过比较有无检索文档时LLM内部表示的差异，并结合前序令牌的信息传播与后处理平滑，实现对细粒度幻觉的准确定位。


<details>
  <summary>Details</summary>
Motivation: 在长文本RAG输出中，幻觉往往出现在局部片段而非整段内容，现有方法缺乏对幻觉的细粒度定位能力，因此需要令牌级的检测方法。

Method: CORTEX通过对比LLM在有/无检索文档条件下的内部表示来捕捉文档对令牌的影响；利用前序令牌中已吸收的文档信息减少误报；并采用后处理平滑步骤，利用幻觉标签在连续跨度上的持续性降低局部噪声，确保跨度一致预测。

Result: 在两个RAG基准和三个LLM上的实验表明，CORTEX显著提升了令牌级幻觉检测性能，且每个组件均对性能提升有稳定贡献。

Conclusion: CORTEX通过细粒度、多组件的设计有效提升了RAG中令牌级幻觉检测的准确性和鲁棒性。
...
TL;DR: 本文提出了LoFa基准测试，用于评估大语言模型在面对逻辑谬误攻击时的鲁棒性，并引入LFR@k指标量化模型对谬误的抵抗能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LLM是否能识别或分类逻辑谬误，但对模型在谬误性说服下的鲁棒性缺乏系统研究。

Method: 通过多智能体流水线构建LoFa基准，将事实性问题与谬误性论证配对；提出多轮辩论框架评估模型在持续对抗性说服下的韧性；提出LFR@k指标来区分谬误鲁棒性与模型固有知识局限。

Result: 不同LLM在不同类型的逻辑谬误下表现出不同程度的鲁棒性，揭示了各模型独特的脆弱性分布。

Conclusion: LLM在逻辑谬误攻击下存在显著的脆弱性差异，需要针对性提升模型的推理鲁棒性。
...
TL;DR: 本文提出了一种基于语义层的NL2SQL代理系统，通过中间表示SMQ将语义意图与物理SQL执行解耦，在Spider2-snow基准上达到94.15%的执行准确率，大幅优于直接基于schema的方法。


<details>
  <summary>Details</summary>
Motivation: 企业级数据库的NL2SQL任务远比学术基准困难，因为企业模式包含数百张物理表、晦涩的列名、异构SQL方言以及复杂的分析工作负载（嵌套聚合、时序推理、多表连接）。现有方法直接基于原始schema生成SQL，效果不佳。

Method: 提出语义层介导的NL2SQL代理：1）构建语义层和紧凑中间表示SMQ；2）代理基于SMQ推理而非原始schema；3）确定性编译器将SMQ转换为方言特定SQL；4）采用受限的思考-执行循环（think-act loop）；5）支持SQLite、BigQuery、Snowflake三种后端。

Result: 在Spider2-snow基准的547个任务上，使用Gemini 3 Pro达到94.15%的执行准确率，在官方排行榜上位列第三，大幅超越仅基于schema的方法。

Conclusion: 语义层方法能有效提升企业级NL2SQL的性能，但需要进一步讨论语义层质量与过拟合之间的权衡。
...
TL;DR: 提出了一种基于百分位匹配的语音-语音AI对话韵律评估协议，通过构建匹配参考体系替代传统池化统计，使韵律指标偏差可解释。


<details>
  <summary>Details</summary>
Motivation: 现有S2S评估缺乏可解释的语音原生韵律度量，池化的人类统计数据无法准确校准特定输出结果。

Method: 利用4000+小时对话数据构建匹配参考体系，定义百分位评估协议，将S2S输出与最接近的人类参考层比较。

Result: 匹配参考相比池化参考能降低误报率（接近10%），并使偏差方向可解释。

Conclusion: 该方法可作为感知和用户评估的补充，提供行为合理性检查。
...
TL;DR: 本研究以自然语言处理领域为例，通过细粒度知识实体（方法、数据集、工具、指标）的组合来测量论文新颖性，发现产学研混合团队比纯工业团队更易产生新颖论文，且不同团队类型关注的新颖性类型存在差异。


<details>
  <summary>Details</summary>
Motivation: 现有研究对机构组成影响论文新颖性的关注有限，且新颖性测量多停留在宏观层面，难以解释新颖性的具体来源和类型。

Method: 将作者团队分为学术机构、工业机构、混合学术与工业机构三类；从全文中提取方法、数据集、工具、指标四类细粒度知识实体；基于实体组合测量新颖性，并分析不同实体类型的两两组合对新颖论文的贡献。
...
TL;DR: 本研究构建了包含文本、图像、音频三种模态的学术论文关键词数据集（1000样本），实验发现不同模态文本具有不同特征，多模态信息融合能有效提升关键词提取性能。


<details>
  <summary>Details</summary>
Motivation: 现有关键词提取任务仅依赖文本数据，忽略了图像和音频模态中的视觉与音频特征，导致信息丰富度不足、潜在关联被忽视，限制了模型表示学习能力和预测准确性；同时多模态关键词提取数据集极其匮乏，阻碍了相关研究进展。

Method: 构建了包含1000个样本的学术论文多模态数据集，每个样本包含论文文本、图像、音频和关键词；基于无监督和有监督的关键词提取方法，分别使用论文文本、图像提取文本、音频提取文本进行实验，并探索多模态信息融合的效果。

Result: 不同模态的文本在模型中表现出不同的特征；将论文文本、图像文本和音频文本进行拼接融合，能够有效提升学术论文关键词提取的性能。

Conclusion: 多模态信息融合能够弥补单一文本模态的不足，提升关键词提取任务的效果，为后续多模态关键词提取研究提供了数据基础和方法参考。
...
TL;DR: 提出Triospect检测框架，通过增加内容（核心思想）和表达（风格元素）两个视角，显著提升AI生成文本检测器对各类攻击的鲁棒性，在基准测试中AUROC提升最高22.3%。
...
Method: 提出Triospect检测框架，在原有文本特征基础上引入内容（核心思想）和表达（风格元素）两个额外视角，采用统计方法进行多维度检测。

Result: 在Humanize-16K攻击子集上AUROC提升22.3%、TPR01提升13%；在adversarial RAID上AUROC提升9.1%、TPR01提升22%。覆盖17种攻击、12个领域、17个源模型。

Conclusion: Triospect框架是统计方法中首个提升检测对抗攻击可靠性的开创性工作，代码和数据已开源。
...
TL;DR: 本文提出Training-Free Gated Reranking方法，通过模型不确定性判断是否需要对少样本示例进行重排序，发现重排序并非总是有益，反而可能降低性能。在多个LLM和数据集上，该方法可降低15%-80%计算成本并提升最高2%的平均性能。


<details>
  <summary>Details</summary>
Motivation: 挑战"重排序检索到的示例总能提升性能"的传统观点，识别出重排序步骤可能反而降低性能，并希望提出一种更高效的方法。

Method: 提出Training-Free Gated Reranking，基于模型的不确定性来决定是否对少样本示例进行重排序。
...
TL;DR: 本文研究了ASR系统在非典型语音（特别是口吃语音）评估中存在的双重转录标准问题（逐字转录与意图转录），发现这两种参考标准会导致模型性能排名显著差异，强调了根据使用场景选择合适的转录参考的重要性。


<details>
  <summary>Details</summary>
Motivation: ASR系统在非典型语音上表现不佳，而评估时往往混淆了两种有效的转录参考标准（逐字转录和意图转录），这导致模型评估和选择存在偏差。

Method: 对11种ASR模型（包括编码器-解码器、CTC和换能器系列）进行基准测试，分别使用逐字参考和意图参考两种转录标准，以口吃语音作为非典型语音案例进行分析。

Result: 使用两种不同的转录参考标准时，模型性能和排名存在显著差异。

Conclusion: 在选择ASR模型时，必须根据具体应用场景选择合适的转录参考标准，特别是在非典型语音识别中。
...
TL;DR: SeKV是一种分辨率自适应的语义KV缓存方法，通过将上下文组织为熵引导的语义跨度，并将其存储在GPU-CPU混合内存层次中，在不丢弃信息的前提下实现高效的长上下文推理，相比最强基线平均提升5.9%，GPU内存减少53.3%。


<details>
  <summary>Details</summary>
Motivation: 长上下文场景下KV缓存成为主要内存瓶颈，现有压缩方法（token驱逐、语义分组）在效率与忠实保留上下文之间难以取得平衡，且无法在生成过程中从压缩的跨度中恢复token级别的细节。

Method: 将上下文组织为熵引导的语义跨度，GPU保留轻量摘要向量用于粗粒度路由，CPU存储低秩SVD基用于按需token级重建；训练一个"缩放"机制在解码时选择性展开与查询相关的跨度。

Result: 在四个基准测试上，相比最强语义压缩基线平均提升5.9%，与全量KV缓存相比减少53.3%的GPU内存（128K上下文）；仅增加少于0.05%的可训练参数，基础LLM完全冻结。

Conclusion: SeKV通过跨内存层次的分辨率自适应设计，在不丢弃信息的前提下实现了高效、可重建的KV缓存压缩，平衡了内存效率与上下文保真度。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将文本和图拓扑分开处理，GNN仅用浅层文本特征，LLM混合方法也将语言模型仅作为文本编码器，缺乏真正的联合推理。

Method: 基于掩码扩散语言模型（双向注意力+生成式解码），将局部邻域线性化为token序列，通过拓扑注意力掩码注入图结构以执行消息传递，通过改变提示适应不同任务。

Result: 在三个TAG基准测试的两个任务上，超越GNN、图Transformer和LLM基线，比最强基线提升最高3.9个点。

Conclusion: 所提方法能够统一文本推理与图消息传递，无需特定目标微调即可泛化到节点分类、链接预测和跨数据集迁移等任务。
...
TL;DR: 提出了一种多视图门控图注意力网络，通过语义、依存和共现图从“内容-结构-流”框架分析自发语音，用于阿尔茨海默病检测，准确率达90%。
...
Method: 使用ASR转写音频，构建语义图、依存图和共现图（基于PMI），通过自适应门控融合机制动态整合多视图特征。

Result: 在ADReSSo数据集上达到90.00%的准确率。

Conclusion: PMI共现图和异质性感知门控机制对跨临床人群的鲁棒分类至关重要。
...
TL;DR: 本文构建了包含307个道德困境的MoralAltDataset，发现当引入折中方案时，LLM和人类的道德判断会发生显著改变，且LLM生成的替代方案在质量和伦理标准上往往优于人类编写的方案。


<details>
  <summary>Details</summary>
Motivation: 现有研究忽视了人类道德认知中一个核心能力——在给定选项之外想象替代方案，因此需要系统研究LLM在面对道德困境时是否能像人类一样考虑折中与重新框架的替代选项。

Method: 构建MoralAltDataset（307个道德困境，含叙事型Advisor困境和AI型Agent困境），每个困境配有折中方案和重新框架方案；通过15个LLM进行实验，比较人类与LLM在引入替代方案后的判断变化；使用成对偏好和专家标准评估LLM生成替代方案的质量。

Result: 折中替代方案往往被优先选择，显著改变道德选择；LLM生成的替代方案在偏好和细粒度结构及伦理标准上优于人类编写的方案，但在结构质量与实践可行性之间存在权衡。

Conclusion: LLM在道德困境中具备生成高质量替代方案的能力，但需注意结构合理性与实际可行性之间的平衡。
...
TL;DR: PSALM是一个基于法律标准的LLM版权合规评估框架，通过十项评估维度检测模型在风格、内容和法定例外等方面的侵权风险，发现现有仅针对逐字记忆的防护措施不足以应对欧盟版权法下的更广泛侵权标准。


<details>
  <summary>Details</summary>
Motivation: 现有技术防护仅聚焦于逐字记忆检测，而欧盟版权法采用"实质性相似"标准，涵盖风格选择、叙事结构和创造性改编，两者之间存在重大合规鸿沟。

Method: 提出PSALM框架，将LLM作为裁判，通过十个评估器分别评估计算重叠、风格维度（写作风格、叙事声音）、内容维度（角色、情节、场景、世界观构建）以及法定例外（戏仿、模仿、引用、场景必现）。应用于基于荷兰历史文学作品微调的Llama 3.2模型。

Result: 1）指令微调模型在接触语料前已存在非平凡的基线风格相似性；2）微调系统性地导致跨所有侵权维度的风格挪用，超越逐字记忆延伸至抽象叙事模式；3）负偏好优化去学习可降低相似性，但残留可检测的风格模式。

Conclusion: 仅针对逐字复制防护不足，PSALM为可审计、合法合规的评估提供了基础设施，但自动相似度评分与侵权判定之间的关系仍需法律专家验证。
...
TL;DR: 本文研究了任务型对话系统中，当数据库调用失败时，大语言模型容易产生不安全响应（幻觉）的问题。提出了一种轻量级的提示式恢复策略（Guided-Retry），无需重新训练或额外模型调用。在6个开源模型家族和4种数据库故障条件下测试，发现Guided-Retry可将幻觉率降低42%-50%，但残余幻觉仍然存在（6-37%）。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在任务型对话中，当后端数据库调用失败、返回空结果或信息不匹配时，常常会编造不存在的场地、确认信息或预订详情，产生不安全响应。现有方法需要重训练或额外模型调用，成本高。因此需要一种轻量级、无需重训练的鲁棒性改进方法。

Method: 提出三种响应策略进行对比，核心是Guided-Retry——一种基于结构化数据库状态条件引导的恢复提示。在6个开源模型家族（DeepSeek-R1、Gemma-2、Llama-3、Mistral、Phi-3、Qwen-2.5）和4种数据库条件（空结果、错误域检索、API错误、干净检索）上测试。使用基于MultiWOZ 2.2（5个域）和SGD（20个域）构建的故障注入基准。

Result: 朴素代理在MultiWOZ上30.5%的故障轮次产生幻觉，在SGD上为20.9%。Guided-Retry策略将MultiWOZ上的幻觉率从30.5%降至15.3%（降低50%），SGD上从20.9%降至12.2%（降低42%）。但残余幻觉仍显著（6-37%），其中错误域检索是最难处理的情况。结果在两种数据集和所有6个模型家族上一致。人工标注与自动评估指标具有较高一致性。

Conclusion: Guided-Retry是一种有效的轻量级方法，能显著降低任务型对话中数据库故障引起的幻觉，但无法完全消除。错误域检索是最具挑战性的故障类型。该策略跨模型和数据集具有良好泛化性，且自动评估指标可靠性得到人工验证。
...
TL;DR: 本文提出了一种结合潜在序数原型对齐(LOPA)和语义锚定层路由(SALR)的方法，在不依赖大规模MLLM微调的情况下，实现与十亿参数系统相媲美的口语评估性能(RMSE=0.361)。


<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大语言模型(MLLM)的口语评估方法虽然有效，但忽视了语言习得的内在序数结构，且依赖大规模模型参数，计算成本高昂。

Method: 提出LOPA正则化器在潜在空间施加序数几何先验，结合SALR自适应地从冻结的Whisper编码器中提取多深度表示，无需LLM微调。

Result: RMSE达到0.361，性能媲美十亿参数级别的系统。

Conclusion: 该方法为口语评估提供了一种高效、可解释且序数感知的替代方案，挑战了当前依赖大规模模型的范式。
...
TL;DR: 本文提出BlockPilot，一种样本自适应策略，根据预填充阶段表示预测最优推理块大小，以提升扩散式投机解码效率，在Qwen3-4B上实现5.92接受长度和4.20倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有扩散式投机解码方法采用固定推理块大小，假设所有输入共享同一最优解码策略，但作者发现最优块大小因样本而异，且对性能影响显著，因此需要自适应方法。

Method: 将块大小选择建模为轻量级策略学习问题，基于预填充阶段表示预测最优块大小，预测仅在预填充后执行一次，实现即插即用。

Result: 在Qwen3-4B上，温度T=1时达到5.92的接受长度和4.20倍加速，且开销极小。

Conclusion: BlockPilot能自适应预测最优块大小，显著提升扩散式投机解码效率，且易于集成。
...
TL;DR: 本文提出一种基于教师-学生对抗学习的语言不变性欺骗检测框架，通过变分信息瓶颈抑制语言线索，在9个数据集上实现最高36.2%的等错误率降低。


<details>
  <summary>Details</summary>
Motivation: 当前语音欺骗检测器在域内表现良好但跨域泛化能力差，研究发现这是由于模型过度依赖训练数据中的语言线索（语言偏见），导致在跨数据集场景下鲁棒性不足。

Method: 提出语言不变性欺骗检测框架：(1) 语言感知教师模型在外部数据集上预训练，学习语言内容特征；(2) 学生检测器通过梯度反转（对抗学习）最小化语言信息；(3) 引入变分信息瓶颈（VIB），选择性抑制主要语言线索同时保留非语言线索。

Result: 在9个DF Arena数据集上，与基线相比，等错误率（EER）相对降低最高达36.2%。

Conclusion: 证明了语言偏见是导致语音欺骗检测跨域泛化差的关键因素，所提框架通过对抗学习去偏和VIB保留有效特征，显著提升了跨数据集检测鲁棒性。
...
TL;DR: BiRG-LoRA是一种针对医学多选题的单一适配器秩门控LoRA方法，通过双轴门控机制根据问题动态选择稀疏的秩原子子集，在多个医学QA基准上达到了最优性能，同时参数效率更高。


<details>
  <summary>Details</summary>
Motivation: 医学多选题涉及异构知识领域和推理操作，不同问题需要不同的低秩更新，而部分回忆型问题应保持基础模型表示。现有参数高效微调方法无法灵活适应这种多样性。

Method: 提出BiRG-LoRA，每层仅保留一个LoRA模块，但通过双轴门控机制（结合隐藏语义证据、专业/职业先验、临床操作先验及其交互）为每个问题动态选择稀疏的top-k秩原子子集，并引入标量注入系数控制适配器更新强度。

Result: 在Qwen3-8B和CMB源协议下，BiRG-LoRA在CMB、CMExam、MedQA和MedMCQA四个基准上平均准确率为69.31%，超越MoELoRA 0.89个百分点且参数少28.1%，95%置信区间[0.42, 1.37]；相比vanilla LoRA r16和r4提升0.83宏平均点。

Conclusion: 临床结构化的秩分配能提升跨基准医学问答性能，在匹配的单种子协议下效果显著，且对中等标签噪声不敏感；训练种子方差的影响留待未来工作。
...
TL;DR: 本文发现并修复了RVL-CDIP数据集中12%的标签错误和35%的测试-训练重叠问题，修复后分类准确率在错误移除时提升，但在重复项移除时下降；在OOD基准测试中，错误修正数据使监督模型平均提升8.1个百分点的准确率。


<details>
  <summary>Details</summary>
Motivation: RVL-CDIP是文档分类的流行基准数据集，但存在大量标签错误和测试-训练重叠问题，这些问题可能影响模型性能评估的准确性。

Method: (1)检测并修复标签错误；(2)检测并解决测试-训练重叠问题；生成多个修正版本的RVL-CDIP数据集，并在这些新版本上基准测试文档分类性能。

Result: RVL-CDIP包含12%的标签错误和约35%的测试-训练重复；修复错误后分类准确率提升，移除重复项后准确率下降；在RVL-CDIP-N OOD基准上，错误修正数据使监督模型平均提升8.1个百分点，最高达14个百分点。

Conclusion: 数据集质量对模型评估有显著影响，修正标签错误能有效提升模型性能尤其是OOD泛化能力，而测试-训练重叠则会导致性能虚高。
...
TL;DR: 提出基于LLM的流水线，用于对用户按时间顺序发布的帖子进行多层次心理健康分析（帖子级评估+用户级时间建模）。


<details>
  <summary>Details</summary>
Motivation: 全球心理健康问题日益严重，专业护理资源有限，亟需可扩展的计算方法辅助早期检测和持续监测。

Method: 构建基于LLM的统一流水线框架，同时支持帖子级别的独立评估和用户级别的时间序列建模。

Result: 作为CLPsych共享任务的一部分，该流水线能对按时间排序的用户帖子进行全面的心理健康分析。

Conclusion: 该流水线为心理健康分析提供了一种统一、可扩展的LLM方案，兼顾局部文本评估与全局行为模式建模。
...
TL;DR: 本文针对班巴拉语等非洲语言儿童语音识别研究不足的问题，构建了端到端开源系统，包括55小时儿童语音数据集、Soloni和QuartzNet模型对比、课堂验证等，最佳模型将词错误率从0.42降至0.22。


<details>
  <summary>Details</summary>
Motivation: 非洲语言（包括班巴拉语）的儿童语音识别研究匮乏，现有系统无法支持儿童读写能力的可重复评估，亟需开源解决方案。

Method: 通过移动应用采集60名儿童55小时语音数据，构建公开基准；对比Soloni（基于Fast-Conformer+TDT/CTC解码器）与QuartzNet（紧凑卷积架构）的微调效果，并分析重复朗读、SpecAugment数据增强等策略。

Result: Soloni最佳模型将WER从0.42降至0.22，CER从0.15降至0.08，显著优于QuartzNet；重复朗读对QuartzNet提升明显但对Soloni增益有限；10岁以下儿童是主要错误来源。

Conclusion: 所提系统能有效评估班巴拉语儿童阅读能力，未来需针对低龄儿童收集更多数据，课堂验证支持其实际应用价值。
...
TL;DR: 本文提出"指令显著性衰减"（MSD）现象，即LLM金融智能体在长期部署中会逐渐偏离初始行为指令。通过FinPersona-Bench基准测试发现，MSD随时间累积且模型相关，而周期性指令重置对保守型智能体有效，但对激进型智能体可能适得其反。


<details>
  <summary>Details</summary>
Motivation: LLM作为金融智能体时，初始赋予的行为指令（如"保本"）在长期市场环境中会逐渐失效，但缺乏对此现象的正式定义和客观衡量方法。

Method: 构建FinPersona-Bench模拟基准，将市场价格与隐藏基本面价值解耦，检测三种失败模式；评估18个前沿LLM，分配三种行为画像，并对比静态智能体与周期性指令重置智能体的表现。

Result: MSD随时间累积且模型相关；在崩盘场景中，行为差距从第一季度的基线到最后一个季度扩大4.4倍；指令重置对保守智能体有帮助，但对激进智能体在低信号市场中反而恶化行为。

Conclusion: 长期可靠部署需要根据智能体画像和市场状态进行选择性、指令感知的重置策略，而非简单的周期性重置。
...
TL;DR: AutoTrainess是一个LM智能体，通过将后训练操作标准化为智能体-计算机接口，在PostTrainBench上显著优于CLI基线，展现了跨模型和平台的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 语言模型的后训练过程高度依赖人工，现有智能体在自主进行规划、数据构建、训练、评估等长周期任务时面临挑战，亟需一种自动化的解决方案。

Method: 提出AutoTrainess框架，将后训练操作封装为规划、数据准备、训练、评估和日志等智能体-计算机接口，并外部化先验人类经验为显式工作流、规则和执行约束，以引导智能体高效可靠地执行训练任务。

Result: 在PostTrainBench上，AutoTrainess搭配GPT-5.4 (Codex) 获得26.94平均分，优于CLI基线的23.21；将DeepSeek-V4-Flash (OpenCode) 从12.13提升至19.58。

Conclusion: AutoTrainess验证了将人类后训练经验结构化外化给智能体的有效性，能够跨模型和框架提升自主训练性能，为自动化后训练提供了可行范式。
...
TL;DR: 提出双嵌入水印（DEW）方案，结合上下文和词级嵌入，通过信号处理方法增强大语言模型文本水印对抗改写和翻译的鲁棒性，实验表明检测效果显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有语义水印在遭遇文本改写（paraphrasing）和翻译攻击时鲁棒性不足，需要一种能在语义偏移下仍能保持检测能力的水印方案。

Method: 利用上下文嵌入和词级嵌入的双重嵌入机制，通过伪随机矩阵（由密钥生成）对嵌入向量进行投影来隐藏水印，采用信号处理中的代数向量空间操作实现水印信号的优雅降级。

Result: 在多个大语言模型上的实验表明，DEW在改写后检测能力提升，保持竞争性文本质量；翻译后仍可检测，而此前语义水印显著退化。

Conclusion: DEW是一种实用且鲁棒的LLM文本水印方案，有助于解决负责任AI部署中的关键问题。
...
TL;DR: CLExEval是一个人机协同框架，通过渐进式信息遮蔽评估LLM临床推理能力，发现三大失败模式：冗长偏差、隐藏知识悖论和推理-输出不匹配，并揭示纯自动化评估可能高估临床可靠性。


<details>
  <summary>Details</summary>
Motivation: LLM在医学基准上表现良好，但其临床推理难以可靠评估，存在"评估幻觉"风险——即使最终诊断错误，流畅的解释也可能看起来具有临床说服力。

Method: 提出CLExEval框架，结合5,600条专家标注和40个罕见病例的200条临床推理轨迹，采用渐进式信息遮蔽方法评估LLM临床推理。

Result: 发现三大失败模式：(1)冗长偏差：GPT-4o-mini在信息稀缺时准确率从95.0%降至32.5%；(2)隐藏知识悖论：专家模型无法在冗长上下文中可靠检索知识；(3)68.6%的推理-输出不匹配。GPT-4o-mini批准了47.9%的临床错误输出。

Conclusion: 纯自动化临床评估若无专家验证，可能大幅高估临床可靠性。
...
TL;DR: 针对6种南班图语，提出基于音调条件的课程学习框架，结合混合难度评分、门控适配器和分阶段训练，将W2V-BERT的零样本WER从>100%降至平均28.41%，发现单一模型无法适配所有语言。


<details>
  <summary>Details</summary>
Motivation: 现有ASR模型对南班图语的零样本WER超过100%，无法满足教育和公共服务等实际应用需求，亟需针对这些语言的低资源特性设计有效的语音识别方案。

Method: 提出音调条件课程学习框架，包括：1) 混合难度评分机制；2) 基于音调统计的门控适配器；3) 分阶段课程训练。在社区语料库上训练，并在NCHLT上测试迁移鲁棒性。

Result: W2V-BERT在恩古尼语上比Whisper低3-4个WER点，而Whisper在索托-茨瓦纳语上表现更优。W2V-BERT+音调条件在数据集上平均WER为28.41%，在Xitsonga迁移上达23.79%。

Conclusion: 没有单一模型能适配所有6种语言，实际部署需按语言配对模型选择，并结合跨语料库验证。
...
TL;DR: 研究发现当前公平性评估高估了LLM的道德安全性：当人口统计信息以显式标签呈现时模型表现公平，但需要从上下文中推断相同信息时公平性显著下降。作者将这种现象称为"表演性合规"，并提出了"线索可见性差距"这一评估指标来区分真实与表演性道德安全。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在医疗、法律、招聘等道德攸关场景中承担角色，需要检验其道德行为是真实还是表面的。现有公平性评估存在高估道德安全的问题。

Method: 提出线索变异方法论（cue-variation methodology）：保持道德困境和人口统计身份不变，仅改变身份信息的传达方式（显式标签 vs. 需推断）。提出Cue Visibility Gap指标来衡量模型对线索变化的鲁棒性。

Result: 隐藏显式标签后，有害决策增加+4.4个百分点，模型安全排名发生变化。即使模型正确推断出人口统计身份，这种偏移仍然存在，排除了归因错误的可能性。

Conclusion: 缺乏线索变异的公平性评估只能衡量表面合规性，而非道德鲁棒性，不应作为高风险场景中模型部署决策的依据。
...
TL;DR: 本文介绍了TalentCLEF挑战赛第二届（CLEF 2026），包含任务A（中英文简历-职位匹配排序）和任务B（英文职位-技能匹配与分类），吸引了113个注册团队和400余次提交，总结了数据集、评估设置及参赛结果。


<details>
  <summary>Details</summary>
Motivation: 推动自然语言处理在人力资本管理领域的研究，建立共享评估基准以满足研究社区对人才管理任务（如人岗匹配、技能匹配）标准化评估的需求。

Method: 组织TalentCLEF挑战赛，设置两个任务：任务A为上下文人岗匹配（中英文简历与职位空缺的排序），任务B为职位-技能匹配与技能类型分类（英文）。提供数据集和评估设置，接收团队提交的系统结果。

Result: 吸引了113个注册团队，收到400余次提交，展示了研究社区对该共享基准的高度兴趣。

Conclusion: TalentCLEF第二届成功推动了人力和技能匹配的NLP研究，反映了社区对人力资本管理评估基准的持续需求。
...
TL;DR: 本文研究了低资源语言罗马尼亚语的跨语言关系抽取（RE），通过LLM翻译SemEval-2010 Task 8数据集，评估Gemma 4 31B在不同配置下的表现，并与小型编码器模型对比。结果显示，QLoRA微调显著提升性能，但31B模型相比小模型优势微弱，部署性价比不高。


<details>
  <summary>Details</summary>
Motivation: 低资源语言缺乏标注语料，关系抽取任务受限。本文探究通过自动数据集翻译结合大语言模型推理，实现罗马尼亚语跨语言关系抽取的可行性。

Method: 使用LLM翻译管道将SemEval-2010 Task 8从英语翻译成罗马尼亚语；在零样本、少样本和QLoRA微调配置下评估Gemma 4 31B；与四种编码器基线（XLM-RoBERTa、Romanian BERT、RoBERT-large）对比；评估关系分类和端到端抽取两种任务形式。

Result: 罗马尼亚语相比英语在纯提示设置下下降3-5个百分点；少样本提示相比零样本提升有限；QLoRA微调使两个语言的宏F1提升超22个百分点，跨语言差距从3.3pp降至1.4pp；编码器基线在罗马尼亚语上与QLoRA Gemma差距仅1-4pp，但模型小50-250倍。

Conclusion: 在计算资源受限的部署场景下，使用31B大模型进行单任务关系抽取的必要性不强，小模型（如Romanian BERT）在性价比上更具优势。
...
TL;DR: 本文研究了视觉语言模型（VLM）在协作对话中能否区分“可能共享的信息”与“已经共享的信息”（即共同基础 grounding）。通过在HCRC MapTask对话数据集上的13077个标注参考表达上进行解释匹配任务，发现提供真实地图图像虽提升整体性能，但会导致模型过度预测对齐，且错误地将地图内容视为共同理解的证据而非追踪对话历史中共同基础的建立过程。


<details>
  <summary>Details</summary>
Motivation: 在协作对话中，共享感知并不保证共享解释，相互理解必须通过交互建立。现有视觉语言模型能否区分潜在共享与已共享信息尚不清楚，因此需要系统性地检验VLM在对话中建立共同基础（grounding）的能力。

Method: 构建解释匹配任务，使用HCRC MapTask对话数据集的13077个标注参考表达。通过系统控制对话上下文和地图信息访问方式（真实图像、文本描述、非信息性图像），评估多个VLM（重点分析Qwen3-VL-8B-Instruct及四个额外模型）的表现，并进行校准分析和参考链追踪。

Result: 真实地图图像提升整体性能但导致过度预测对齐；文本描述复制了这种偏差；非信息性图像完全抑制对齐预测。偏差由任务相关地图内容驱动而非视觉通道本身。非对齐案例的准确率下降。模型依赖静态指示范例而非追踪对话历史中的共同基础建立过程。

Conclusion: 当前VLM存在将地图内容（无论视觉还是文本形式）视为相互理解证据的偏差，混淆了潜在共同基础与已建立共同基础。这种偏差在多个模型中普遍存在，表明VLM在对话中建立真正的相互理解方面仍有根本性局限。
...
TL;DR: 本文通过个性化微调Whisper基础ASR模型，在92小时阅读语音和8.8小时用户纠正数据上，将构音障碍语音的词错误率降至9.7%，证明个性化适配可显著提升基础ASR模型对构音障碍语音的实用性。


<details>
  <summary>Details</summary>
Motivation: 自动语音识别系统在构音障碍语音上表现不佳，限制了患者在日常交流中的使用。

Method: 使用TEQST工具收集92小时阅读语音和8.8小时用户纠正数据，基于Whisper基础模型进行个性化微调适配，并对比了LoRA适配和Qwen3-ASR作为基础模型的效果。

Result: 1.4小时适配数据将词错误率降至15.8%，22.5小时降至10.7%，使用全部数据（含纠正数据）达到最佳9.7%。LoRA和Qwen3-ASR效果较差。

Conclusion: 个性化微调能使基础ASR模型对构音障碍语音显著更有效，适合实际部署。
...
TL;DR: 本文提出了STEB（风格文本嵌入基准），这是一个涵盖96个数据集、7种语言的开源基准，用于标准化风格嵌入的评估。研究发现语义嵌入在风格任务中表现不佳，且不存在在所有任务中普遍优越的风格嵌入。


<details>
  <summary>Details</summary>
Motivation: 现有风格嵌入评估方法零散，各自使用不同的任务和数据集，缺乏统一的标准化评估标准。

Method: 构建包含96个数据集、7种语言的STEB基准，涵盖作者验证、作者检索、AI文本检测、语言特征探测等多种任务，对现有语义嵌入和风格嵌入进行系统性评估。

Result: 语义嵌入在风格任务中持续失败；不存在在所有评估任务中普遍优越的风格嵌入。

Conclusion: STEB为风格嵌入提供了标准化评估框架，揭示了当前风格嵌入的局限性，并为未来研究指明了方向。
...
TL;DR: 本文提出三种互补技术用于训练计算高效的语言模型：(1)选择性监督（SGT），仅对15%的语义关键token进行监督，利用梯度耦合使其余85%无监督token也显著提升，实现4.5倍监督效率；(2)深度压缩与循环恢复，将48层1B参数量模型压缩至6层227M，通过循环展开恢复至接近566M模型性能；(3)压缩专家融合（MoEE），组合多个压缩模型实现更好性能。在CHERRY-1.8B韩语基座模型上验证。


<details>
  <summary>Details</summary>
Motivation: 提升语言模型训练的计算效率，减少对大量标注数据和计算资源的依赖，同时保持模型性能。

Method: (1) Selective Ground Truth Token Training (SGT)：仅对约15%承载语义负载的输出token进行监督训练，利用正梯度耦合系数（gamma-bar=0.72）使无监督token受益；(2) 深度压缩：将48层Transformer通过相邻层平均压缩至6层，再通过学习的循环展开恢复深度；(3) Mixture of Efficient Experts (MoEE)：融合多个压缩模型作为专家集合，结合多token预测。

Result: SGT在step-100评估时，用15%监督恢复了约67%的全序列损失降低；深度压缩实现2.5倍参数量缩减（227M vs 566M），损失仅差0.008；MoEE（2专家）损失2.789优于最佳单模型2.926。

Conclusion: 三种互补技术可显著提升语言模型训练的计算效率，且效果依赖于自然语言结构（在打乱文本上失效）。作者明确说明了证据范围的局限性（单一模型家族、韩语数据、基于损失的评估指标）。
...
TL;DR: 本文提出一种参数中性的NC-FFN替换方案，将Transformer前馈层每个隐藏单元显式建模为模糊集运算（交集和集差），在125M参数规模下达到与GELU基线相当的困惑度。引入序列量词块（软存在和软比例）解决了逻辑运算局部化于第0层和语法缺陷问题，使FFN具有可解释性：单元在语义层充当语法许可探测器，无需字典学习即可识别许可词并预测被许可词。


<details>
  <summary>Details</summary>
Motivation: Transformer的FFN子层虽然具体化了注意力收集的区分信息，但无法解释其计算了什么。现有门控/双线性单元缺乏否定能力，而否定是语言推理的关键操作。

Method: 1) 提出NC-FFN：每个隐藏单元为显式模糊集运算（交集A*B和集差A*(1-B)），实现有界正否定；2) 引入序列量词块：软存在和软比例量词，每个带有可学习的遗忘率（从粘性初始化）；3) 在N位奇偶校验任务和125M参数OpenWebText上进行评估。

Result: NC-FFN在N位奇偶校验中是最参数高效的推理基础；在125M规模下与GELU基线困惑度持平；序列量词块恢复语法缺陷（第一个epoch即减半第二个epoch的差距），在LAMBADA上略有领先；语义层单元可直接读取为语法许可探测器（比较级、被动分词、负极词等），中位半衰期约1.5个token，零锁存单元。

Conclusion: 提出了一种参数中性、达到语言模型质量的Transformer替代方案，其FFN具有可解释性——不仅解释了前馈层表示什么，还解释了它如何实现语法许可机制。这种可读性是局部的且仅在分区条件下自由（完全布尔FFN会训练发散），但提供了首个可解释的语言模型内部工作机制。
...
TL;DR: 本文提出了非对话式规划心智理论（NCP-ToM）评估框架，测试LLM代理能否通过物理动作（而非对话）诱导其他智能体产生特定信念状态。实验发现GPT-5在80%任务中成功且优于人类，但所有模型在诱导错误信念时表现较差。


<details>
  <summary>Details</summary>
Motivation: 现有ToM基准多基于被动问答，但LLM正被部署为更具自主性的代理形式，需要新的评估方式来衡量其通过行动而非对话操纵他人信念的能力。

Method: 提出NCP-ExploreToM框架，颠覆传统任务结构：给模型设定信念状态目标，要求其通过移动物体或引导角色进入房间来实现目标。评估了6个前沿模型和人类被试，共600个任务实例。

Result: GPT-5在代理设置中成功率约80%，是唯一超越人类表现的模型；所有模型在诱导真实信念任务上表现优于诱导错误信念任务。

Conclusion: LLM在非对话式任务中展现出新兴的社会推理能力，但鲁棒性仍不及人类；评估代理式社会认知能力对于理解自主社交智能体的安全性和对齐至关重要。
...
TL;DR: 本文提出了LuxEmo，一个21小时的卢森堡语情感语音数据集，并设计了一套半自动标注流程，同时评估了多种情感语音合成系统在该低资源语言上的表现。


<details>
  <summary>Details</summary>
Motivation: 低资源语言如卢森堡语在语音技术研究中严重缺乏情感语音数据资源，现有数据集多集中于广泛使用的语言。

Method: 从RTL青年广播中采集数据，采用半自动标注流程（包括VAD、去噪、语言识别、ASR分割、自动情感预测、词汇线索和人工审核），并构建了21小时4个情感类别的语料库。

Result: 成功构建了LuxEmo数据集，并对比了五种情感TTS系统在卢森堡语上的表现，使用客观指标和人工评估进行评价。

Conclusion: 该工作为卢森堡语情感语音研究提供了首个大规模公开数据集，并验证了半自动标注流程的有效性，为其他低资源语言的情感语音研究提供了参考。
...
TL;DR: 提出了DigitalCoach数据集，包含72场人机教学会话，评估发现当前AI模型在软件教学上缺乏解释、错误诊断和视觉 grounding，导致学习者被动跟随指令而缺乏深度参与。


<details>
  <summary>Details</summary>
Motivation: 现有AI Agent能自动化软件任务，但能否教会人类自己使用软件这一问题尚未被系统研究。作者旨在探索AI作为教学代理的可行性。

Method: 构建了DigitalCoach多模态数据集（72场教学会话、22,752轮对话、28.1小时屏幕及输入事件记录），通过自动评估和交互式评估两种方式比较人类教练与AI模型的差异。

Result: 自动评估：模型更多给出直接指令，但缺乏解释、错误诊断和知识检查问题；固定教学方法后，模型生成的语句虽有相似性但视觉 grounding 差。交互评估：模型教练导致学习者被动跟随指令，缺乏深度参与，视觉 grounding 不足。

Conclusion: DigitalCoach为协作式和主动式计算机使用教学代理奠定了基础，揭示了当前模型在教学场景中的关键不足。
...
TL;DR: 本文提出通过技能蒸馏（Skill Distillation）从人类浏览器交互轨迹中提取可复用的自然语言技能，并构建技能图谱（Skill Graph）来提升浏览器智能体的决策能力，从而利用互联网用户集体行为作为可扩展的技能来源。


<details>
  <summary>Details</summary>
Motivation: 当前浏览器智能体的瓶颈在于不完全信息下的决策制定，而非底层操作；而人类交互轨迹中已隐含着智能体所缺乏的先验知识，因此需要一种可扩展的方式从中提取和复用技能。

Method: 采用可扩展的行为克隆（Scalable Behavior Cloning）方法，通过技能蒸馏将用户交互轨迹转化为紧凑的自然语言技能，使智能体能够直接读取、检索、复用和组合这些技能，并进一步将技能组织成技能图谱，通过整合而非无限制积累的方式实现增长。

Result: 浏览器智能体的可扩展性可能更多来自互联网用户集体表达的技能，而非人工设计的任务。

Conclusion: 该研究展示了利用人类浏览行为作为可扩展技能来源的潜力，为浏览器智能体的发展提供了新方向。
...
TL;DR: 提出SkillComposer方法，将技能组合形式化为结构化技能组合问题，通过约束自回归解码器联合预测技能子集、数量和顺序，在编程任务上相比无技能基线提升23.1%和18.2%的通过率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在技能库增长时，选择合适技能组合成为瓶颈。已有方法要么暴露整个技能库给推理过程，要么通过嵌入检索或重排序，但都忽略了技能组合的结构化本质——子集、数量、顺序三者是联合决策、不可解耦的。

Method: 提出SkillComposer，将结构化技能组合建模为任务条件化的技能序列预测。使用约束自回归解码器在技能标识符上解码，使子集、数量和顺序在一次解码中联合生成，并自然捕获技能间依赖关系。通过真实人工标注的技能库构建训练数据。

Result: 在保留测试集上评估组合质量，并在SkillsBench上用两个生产级编程智能体评估下游任务成功率。在GPT-5.2-Codex和Gemini-3-Pro-Preview上，SkillComposer比无技能基线分别提升23.1和18.2个百分点，超越top-3检索方法，匹配黄金技能检索上限且提示token成本更低。

Conclusion: SkillComposer通过结构化技能组合方法有效解决了技能选择瓶颈，证明了联合预测子集、数量和顺序的有效性，在性能上达到甚至超越检索方法的上限，同时更高效。
...
TL;DR: 论文首次系统评估了大型语言模型在表格任务中的数据引用错误（DREs），发现所有模型（1.7B-20B）都存在该问题；通过基于批评者的过滤和拒绝采样，可将答案准确率提升最多12%；训练了一个4B参数的轻量级批评模型，在分布内和外检测中F1达78.2%，并能有效辅助更大模型推理。


<details>
  <summary>Details</summary>
Motivation: 现有研究对表格任务中数据引用错误（DREs）仅有有限的小规模分析，缺乏系统性评估，而DREs会直接影响推理步骤的正确性和可靠性。

Method: 首次对多个模型（1.7B-20B参数）在不同表格任务上进行系统性评估，并提出基于数据引用批评者的方法（包括批评者过滤和拒绝采样），同时训练了一个4B参数的轻量级批评模型。

Result: 所有测试模型均存在DREs；采用批评者方法后答案准确率提升至多12.0%；训练出的4B批评模型在分布内和分布外DRE检测中平均F1达78.2%。

Conclusion: 数据引用错误是表格任务中普遍存在的问题，引入数据引用批评者能有效提升模型回答的准确性和可靠性，轻量级批评模型可作为辅助工具帮助更大模型进行推理。
...
TL;DR: 本文提出强化学习与元认知反馈（RLMF）和元认知数据选择两种新机制，通过让LLM自我评判表现来提升其元认知能力，在忠实校准任务上取得SOTA结果，比标准RL提升高达63%。


<details>
  <summary>Details</summary>
Motivation: LLM在元认知方面存在系统性缺陷：高置信度幻觉、无法识别知识边界、错误表达内部不确定性，这些问题损害了可信度和可靠性。

Method: 提出两种机制：1) RLMF（元认知反馈强化学习）——在偏好优化中基于模型自我评判质量改善完成排序；2) 元认知数据选择——利用自我评判识别高价值训练样本。采用两阶段解耦方法，先校准置信度再映射到自然语言表达。

Result: RLMF在多种任务上实现可泛化的SOTA忠实校准，同时保持准确性；比标准RL提升高达63%，增强了模型评估和表达自身能力边界的能力。

Conclusion: RLMF是增强LLM元认知的有前景范式，可提升能力与对齐，元认知表现可作为有效的RL信号克服先前内在反馈方法的局限。
...
TL;DR: 训练语言模型生成解释时，使用固定反事实解释数据集（来自早期检查点或行为相似的不同模型）作为监督，反而能让模型产生比训练目标更忠于自身当前行为的"内省"解释。这种内省耦合发生在训练解释与当前行为保持足够相关时，并且能追踪行为变化，无需更新监督信号。


<details>
  <summary>Details</summary>
Motivation: 探究训练语言模型生成解释时，产生的解释是真正的内省（忠实于模型自身行为），还是表面模仿（忠实于训练目标）。

Method: 使用来自模型早期检查点或行为相似的不同家族模型产生的固定反事实解释作为训练监督，训练LM生成关于其输入特征如何影响其行为的解释，然后评估这些解释对模型当前行为的忠实程度。

Result: 1) 模型产生的解释经常比训练目标更忠实于自身当前行为（内省耦合）；2) 当训练解释与当前行为保持足够相关时，内省耦合发生；3) 内省耦合能追踪行为变化，无需更新监督信号；4) 该现象在多个任务（如谄媚和拒绝）中稳健出现，且对标签噪声鲁棒。

Conclusion: 即使使用固定的反事实解释数据集，也能为语言模型提供可扩展、可泛化的内省训练信号，说明训练解释与当前行为之间的相关性是内省耦合的关键。
