...
Motivation: 针对多光谱影像土地覆盖分割任务，需要有效融合不同光谱通道信息并处理不同光谱配置（4通道RGB+NIR和6通道包含NDVI/NDWI），同时考虑计算效率以适应资源受限环境。

Method: 提出多分支编码器-解码器架构：1) 双ConvNeXt编码器分别处理可见光和不可见光通道；2) 独立解码器重建空间信息；3) 专用融合解码器在多尺度上集成中间特征；4) 使用CBAM注意力增强特征融合；5) 采用ASAU激活函数实现稳定高效优化。
...
Conclusion: MeCSAFNet通过有效的多光谱特征融合和注意力机制，在多光谱土地覆盖分割任务中取得了显著性能提升，其紧凑变体为资源受限环境提供了实用解决方案。
...
TL;DR: 本文综述了多模态大语言模型在图表理解领域的研究现状，系统梳理了该领域的发展脉络、核心挑战、任务分类、方法演进和未来方向。


<details>
  <summary>Details</summary>
Motivation: 图表理解作为典型的信息融合任务，需要整合图形和文本数据来提取意义。虽然多模态大语言模型已经革新了这一领域，但相关研究仍然分散且缺乏系统性组织。本文旨在为该新兴领域提供全面的路线图。

Method: 通过结构化分析领域核心组件：首先分析图表中视觉与语言信息融合的基本挑战；然后对下游任务和数据集进行分类，引入规范和非规范基准的新分类法；接着全面梳理方法演进，从经典深度学习技术到最先进的多模态大语言模型范式；最后批判性分析当前模型的局限性。

Result: 建立了图表理解领域的系统性框架，提出了规范和非规范基准的分类法，梳理了从传统方法到多模态大语言模型的技术演进路径，识别了当前模型在感知和推理方面的缺陷。

Conclusion: 本文为研究人员和从业者提供了多模态大语言模型如何变革图表信息融合的结构化理解，旨在推动开发更鲁棒可靠的系统。未来方向包括先进的校准技术和用于认知增强的强化学习。
...
TL;DR: 本文提出了一种名为MPA的多模态原型增强小样本学习框架，通过大语言模型生成多样化的类别描述、多视图增强和自适应不确定类吸收器，显著提升了小样本学习的性能。


<details>
  <summary>Details</summary>
Motivation: 现有小样本学习方法大多只关注视觉模态，直接从原始支持图像计算原型，缺乏全面丰富的多模态信息。这限制了模型对少数样本的理解和泛化能力。

Method: MPA框架包含三个核心组件：1) LLM-based Multi-Variant Semantic Enhancement (LMSE)：利用大语言模型生成多样化的类别描述，丰富支持集的语义线索；2) Hierarchical Multi-View Augmentation (HMA)：利用自然和多视图增强（如视角距离、相机角度、光照条件变化）增强特征多样性；3) Adaptive Uncertain Class Absorber (AUCA)：通过插值和高斯采样引入不确定类，有效吸收不确定样本。

Result: 在4个单域和6个跨域小样本学习基准测试中，MPA在大多数设置下都优于现有最先进方法。特别是在5-way 1-shot设置中，MPA在单域和跨域设置下分别比第二名方法高出12.29%和24.56%。

Conclusion: MPA框架通过整合多模态信息、增强特征多样性和建模不确定性，有效解决了小样本学习中信息不足的问题，在单域和跨域设置中都取得了显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在长上下文和复杂推理任务中存在性能瓶颈，需要深入理解其内部机制以改进模型表现。

Method: 通过注意力分析识别出关键的视觉证据检索头，提出VERA框架，利用模型不确定性检测触发视觉证据的显式语言化。

Result: VERA显著提升了开源VLM的长上下文理解能力，在Qwen3-VL-8B-Instruct和GLM-4.1V-Thinking上分别实现了21.3%和20.1%的平均相对改进。

Conclusion: 视觉证据检索头是VLM长上下文处理的关键机制，基于此发现的VERA框架能有效提升模型性能，为VLM的长上下文理解提供了新思路。
...
TL;DR: 本文提出了一套用于3D高斯溅射表示（3DGS）的交互式工具集，专注于高斯溅射的选择和分割，支持用户引导的2D到3D选择传播，并开发了基于自定义视频扩散模型的用户引导局部编辑方法。


<details>
  <summary>Details</summary>
Motivation: 尽管3D高斯溅射表示在物理模拟和动画等应用中越来越受欢迎，但从野外捕获中提取可用对象仍然具有挑战性，且该表示的可控编辑技术有限。现有技术大多专注于自动解决方案或高级编辑，缺乏灵活的用户交互工具。

Method: 提出了一套交互式工具集，包括：1）快速AI驱动方法，将用户引导的2D选择掩码传播到3DGS选择；2）灵活的手动选择和分割工具；3）基于自定义视频扩散模型的用户引导局部编辑方法，允许用户控制AI修改的区域。

Result: 工具集在3D高斯溅射选择方面优于现有技术，能够实现非结构化3DGS场景的几乎任何二进制分割。编辑工具无需额外优化即可用于任何野外捕获，为用户提供了直接控制AI修改区域的能力。

Conclusion: 该交互式工具集填补了3D高斯溅射表示在用户可控编辑方面的空白，通过结合AI驱动方法和手动工具，为用户提供了灵活的选择和编辑能力，支持各种下游应用。
...
TL;DR: 该论文提出了首个视觉到视觉的越狱攻击方法VJA，通过纯视觉输入传达恶意指令，揭示了图像编辑模型的新安全漏洞，并建立了IESBench安全基准和训练免费防御方法。


<details>
  <summary>Details</summary>
Motivation: 随着图像编辑模型从文本驱动转向视觉提示编辑，用户意图直接从视觉输入推断，这虽然提高了可用性，但也引入了新的安全风险：攻击面本身变为视觉。目前对这种新兴威胁的研究不足，需要系统性的安全评估和防御方法。

Method: 提出了Vision-Centric Jailbreak Attack (VJA)，这是首个纯视觉输入的越狱攻击方法；建立了IESBench安全基准来系统评估图像编辑模型的安全性；提出了基于内省多模态推理的训练免费防御方法，无需辅助防护模型且计算开销可忽略。

Result: VJA攻击对最先进的商业模型有效，在Nano Banana Pro上攻击成功率达80.9%，在GPT-Image-1.5上达70.1%；提出的防御方法显著提高了未对齐模型的安全性，使其达到与商业系统相当的水平。
...
TL;DR: 提出了一个名为PMMA的辅助移动行人检测数据集，包含轮椅、拐杖、助行器等9类行人，评估了7种检测模型和3种跟踪算法，YOLOX、Deformable DETR和Faster R-CNN表现最佳。
...
Method: 在户外环境中收集志愿者使用轮椅、拐杖和助行器的数据，构建包含9类行人的PMMA数据集。使用MMDetection框架实现了7种目标检测模型（Faster R-CNN、CenterNet、YOLOX、DETR、Deformable DETR、DINO、RT-DETR）和3种跟踪算法（ByteTrack、BOT-SORT、OC-SORT）进行基准测试。

Result: 实验结果显示，YOLOX、Deformable DETR和Faster R-CNN在检测性能上表现最佳，而三种跟踪算法之间的差异相对较小。数据集和代码均已公开。

Conclusion: PMMA数据集填补了辅助移动行人检测领域的空白，为相关研究提供了基准测试平台，有助于推动该领域的技术发展。
...
<details>
  <summary>Details</summary>
Motivation: 从单图像生成3D内容是一个病态问题，因为遮挡区域缺乏几何和纹理信息。现有方法通过合成辅助视图提供额外监督，但这些视图包含几何不一致和纹理错位，会在3D重建过程中传播和放大伪影。

Method: ERGO框架基于超额风险分解，将3D高斯泼溅的优化损失分解为两个分量：量化当前参数与最优参数之间差距的超额风险，以及建模合成视图中固有不可约噪声的贝叶斯误差。该分解使ERGO能够动态估计视图特定的超额风险，并在优化过程中自适应调整损失权重。此外，引入了几何感知和纹理感知目标，与超额风险权重机制协同工作，建立了全局-局部优化范式。

Result: 在Google Scanned Objects数据集和OmniObject3D数据集上的广泛实验表明，ERGO在监督噪声下表现出鲁棒性，同时持续提升了重建3D内容的几何保真度和纹理质量，优于现有最先进方法。

Conclusion: ERGO通过超额风险分解和自适应权重调整，有效利用了不完美的监督信号，解决了单图像3D生成中合成视图噪声带来的挑战，显著提升了3D重建质量。
...
Result: 在面部和风景图像上进行了大量实验，LoRD方法在对抗样本和干净样本上微调的潜在扩散模型仍能生成高质量图像，相比基线方法显示出显著更好的防御性能。

Conclusion: LoRD是一种有效的防御策略，能够保护潜在扩散模型免受对抗攻击，确保模型在对抗环境下的鲁棒性和生成质量。
...
TL;DR: UA-Flow在流匹配基础上增加异方差不确定性预测，通过传播速度不确定性评估样本可靠性，并利用不确定性指导采样提升生成质量
...
Result: 图像生成实验表明，UA-Flow产生的不确定性信号与样本保真度的相关性比基线方法更高，并且不确定性引导采样进一步提高了生成质量

Conclusion: UA-Flow通过引入不确定性估计机制，不仅能够评估生成样本的可靠性，还能利用这些不确定性信息指导采样过程，从而提升生成模型的整体性能
...
<details>
  <summary>Details</summary>
Motivation: 相干成像中的散斑噪声建模为乘性噪声，最大似然估计(MLE)是散斑抑制的理论框架，但在有限孔径数字全息术等高分辨率应用中，高维矩阵求逆的计算成本过高，阻碍了物理精确孔径建模的MLE重建应用。

Method: 提出投影梯度下降与蒙特卡洛估计(PGD-MC)方法：利用随机线性代数技术，避免显式矩阵求逆；通过利用传感矩阵的结构特性，结合共轭梯度进行似然梯度评估；支持物理精确的孔径建模，无需简化假设。

Result: PGD-MC方法：(1)对多样且物理精确的孔径模型具有鲁棒性；(2)在重建质量和计算效率上取得显著提升；(3)能有效扩展到高分辨率数字全息术；在结合三种代表性去噪器作为正则化时，在准确性和速度上均优于先前的即插即用模型迭代重建方法。

Conclusion: PGD-MC为有限孔径数字全息术提供了一个灵活有效的MLE重建框架，通过随机线性代数技术解决了高维矩阵求逆的计算瓶颈，实现了物理精确孔径建模下的可扩展优化。
...
<details>
  <summary>Details</summary>
Motivation: 人工智能可以从已有的放射影像中自动提取成像生物标志物，这种机会性成像模式无需额外成像成本或患者辐射暴露。然而，许多开源图像分析解决方案缺乏严格验证，而商业解决方案缺乏透明度，导致部署时出现意外失败。

Method: 开发了两个完全开源、FDA-510(k)批准的深度学习管道：腹部主动脉量化（AAQ）和骨密度（BMD）估计，包含在Comp2Comp包中。AAQ分割腹部主动脉以评估动脉瘤大小；BMD分割椎体以估计骨小梁密度和骨质疏松风险。分别在258例和371例患者扫描上进行外部验证。

Result: AAQ的整体平均绝对误差为1.57毫米（95% CI 1.38-1.80毫米）。BMD的敏感性为81.0%（95% CI 74.0-86.8%），特异性为78.4%（95% CI 72.3-83.7%）。两种算法均显示出足够的临床使用准确性。

Conclusion: Comp2Comp AAQ和BMD具有足够的临床使用准确性。开源这些算法提高了FDA审批过程的透明度，允许医院在繁琐的临床试点前测试算法，并为研究人员提供了最佳方法。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态任务中表现出色，但容易受到内在语言偏置导致的幻觉问题影响。现有缓解方法往往忽视了语言偏置驱动的底层幻觉模式。

Method: 1. 设计新颖的管道来准确合成幻觉诱导图像；2. 揭示场景条件幻觉模式：模型倾向于提及场景中典型对象，即使视觉证据被移除；3. 建立掩码对象幻觉基准来量化模型对此模式的敏感性；4. 利用幻觉诱导图像构建高质量偏好数据集进行细粒度对齐。

Result: 实验结果表明，该方法能有效缓解幻觉同时保持模型的一般能力。在标准幻觉基准上，相比当前最先进方法实现了高达38%的改进。

Conclusion: 通过合成幻觉诱导图像揭示和量化语言偏置驱动的幻觉模式，并利用这些图像构建偏好数据集进行细粒度对齐，是缓解视觉语言模型幻觉问题的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统LiDAR传感器设计独立于下游任务（如点云配准），基于固定配置采集数据导致次优数据收集，需要大量计算开销进行采样、噪声过滤和参数调优

Method: 提出自适应LiDAR感知框架，将配准反馈集成到感知循环中，动态调整传感器参数，联合优化LiDAR采集和配准超参数，平衡点密度、噪声和稀疏性

Result: 在CARLA仿真环境中评估，该方法优于固定参数基线，同时保持泛化能力

Conclusion: 自适应LiDAR在自主感知和机器人应用中具有潜力，通过联合优化采集和配准过程可显著提高系统性能
...
TL;DR: 该研究通过分析多分辨率哈希编码（MHE）的点扩散函数，从物理系统角度理解其空间行为，发现模型有效分辨率由平均分辨率而非最高分辨率决定，并提出旋转MHE架构来减轻各向异性。


<details>
  <summary>Details</summary>
Motivation: 多分辨率哈希编码作为Instant Neural Graphics Primitives的基础技术，虽然功能强大，但其空间行为缺乏从物理系统角度的严格理解，导致超参数选择依赖启发式方法。需要建立基于物理原理的分析框架来理解和优化MHE。

Method: 引入基于点扩散函数（PSF）的分析方法，将PSF视为系统的格林函数。推导无碰撞PSF的闭式近似，揭示网格诱导的各向异性和对数空间轮廓。分析有限哈希容量对信噪比的影响。提出旋转MHE（R-MHE）架构，在不同分辨率级别对输入坐标应用不同旋转。

Result: 发现理想化空间带宽（FWHM）由平均分辨率Navg决定，而非最高分辨率Nmax。证明模型有效分辨率由经验FWHM（因此由Navg）决定，这种展宽效应源于优化动态。有限哈希容量会引入散斑噪声并降低信噪比。R-MHE能够减轻各向异性，同时保持原始MHE的效率和参数数量。

Conclusion: 本研究建立了基于物理原理的方法论，超越了启发式方法，能够表征和优化多分辨率哈希编码。通过点扩散函数分析揭示了MHE的空间行为特性，提出的R-MHE架构在保持效率的同时改善了各向异性问题，为神经场参数化提供了更坚实的理论基础。
...
Method: 使用稀疏自编码器在SegFormer和U-Net模型上训练，将分割模型激活分解为可解释的潜在特征，通过跨架构和跨数据集的潜在对齐分析不同人群（健康、成人、儿童、撒哈拉以南非洲胶质瘤队列）的数据。

Result: 识别出共享表示的稳定骨干，数据集偏移由对人群特定潜在特征的依赖差异驱动。这些潜在特征作为分割失败的因果瓶颈，针对性干预可以纠正错误并改善跨数据集适应，无需重新训练，在70%的失败案例中恢复性能，Dice分数从39.4%提升到74.2%。
...
<details>
  <summary>Details</summary>
Motivation: 视觉基础模型部署需要高效适配策略，传统全微调成本高、效率低，而delta-tuning在LLMs中的优势无法直接迁移到视觉模型的微调流程中。

Method: 提出CoLin适配器，采用复数线性投影优化，设计低秩复数适配器架构，理论上分析低秩复合矩阵的训练收敛问题，并通过定制化损失函数解决该挑战。

Result: 在目标检测、分割、图像分类和旋转目标检测（遥感场景）等任务上的大量实验表明，CoLin仅用1%参数首次超越全微调和经典delta-tuning方法。
...
Method: 提出3DXTalker框架：1）通过2D到3D数据整理管道和分离表示实现可扩展身份建模；2）引入帧级振幅和情感线索超越标准语音嵌入；3）使用基于流匹配的transformer统一这些线索；4）支持基于提示的条件控制实现风格化头部姿态运动生成。

Result: 实验表明，3DXTalker在统一的框架内集成了唇部同步、情感表达和头部姿态动态，在3D说话头像生成方面实现了优越性能。

Conclusion: 3DXTalker通过创新的数据整理、音频表示和可控性方法，解决了表达性3D说话头像生成的关键挑战，为虚拟通信和数字人应用提供了更自然、更具表现力的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前用于评估视觉语言模型在地图推理方面的数据集范围狭窄，局限于特定领域，且过度依赖人工生成内容，缺乏对真实地理空间推理的深度评估。现有的大型语言模型和视觉语言模型在地图知识推理方面仍存在困难。

Method: 构建MapVerse基准测试，基于真实世界地图，包含11,837个人工编写的问题-答案对，覆盖1,025张地图和10个地图类别。对10个最先进的模型进行评估，建立基线，并进行细粒度分类分析，评估模型在多个维度上的推理能力。

Result: 评估发现，当前视觉语言模型在分类式任务上表现有竞争力，但开源和闭源模型在需要复杂空间推理的高级任务上都表现不足。通过细粒度分析揭示了影响推理结果的视觉因素。

Conclusion: MapVerse填补了地图推理评估的空白，为评估地图阅读、解释和多模态推理提供了丰富环境。研究表明当前模型在复杂空间推理方面仍有明显不足，需要进一步改进。
...
TL;DR: 提出高质量大规模数据集（73万+图像）解决现有AI生成图像检测数据集泛化能力差、质量低、提示简单、多样性不足的问题，并基于图像噪声熵提出轻量级检测方法。


<details>
  <summary>Details</summary>
Motivation: 生成式AI快速发展导致高度逼真的假图像能以低成本生成，增加了社会风险。现有检测数据集存在泛化能力有限、图像质量低、提示过于简单、图像多样性不足等问题。

Method: 1) 构建包含73万+图像的大规模高质量数据集，涵盖真实和AI生成图像；2) 使用最先进方法生成图像：文本到图像生成（基于1万+精心设计的提示）、图像修复、图像精修、人脸替换；3) 提出基于图像噪声熵的轻量级检测方法，将原始图像转换为非局部均值噪声的熵张量再进行分类。

Result: 相比现有数据集，基于该数据集训练的检测模型展现出更优的泛化能力。提出的噪声熵检测方法具有竞争力，为未来研究建立了坚实基础。数据集和源代码已公开。

Conclusion: 该高质量大规模数据集不仅为检测方法评估提供了强大基准，还促进了AI生成图像检测技术的鲁棒性发展。提出的轻量级检测方法表现出色，为后续研究奠定了良好基础。
...
<details>
  <summary>Details</summary>
Motivation: 当前弱监督多模态视频异常检测中文本模态潜力未充分挖掘，文本能提供显式语义信息增强异常表征并减少误报，但存在两个挑战：1)通用语言模型难以捕捉异常特定细节；2)相关描述数据稀缺；3)多模态融合存在冗余和失衡问题

Method: 提出文本引导框架：1)基于上下文学习的多阶段文本增强机制，生成高质量异常文本样本来微调文本特征提取器；2)多尺度瓶颈Transformer融合模块，使用压缩瓶颈token逐步跨模态整合信息，缓解冗余和失衡

Result: 在UCF-Crime和XD-Violence数据集上实现了最先进的性能

Conclusion: 提出的文本引导框架有效解决了文本特征提取和多模态融合的挑战，在视频异常检测任务上取得了优异表现
...
TL;DR: C²RoPE：改进的RoPE位置编码，专门针对3D多模态模型中的视觉处理问题，通过建模局部空间连续性和空间因果关系来提升性能


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的3D大模型使用RoPE位置编码存在两个主要问题：1）一维时间位置索引破坏了视觉特征在列维度的连续性，导致空间局部性损失；2）RoPE假设时间上更接近的图像token因果相关性更强，导致注意力长程衰减，随着序列增长模型会逐渐忽略早期视觉token

Method: 提出C²RoPE改进方法：1）引入时空连续位置嵌入机制，将1D时间位置与基于笛卡尔坐标的空间坐标结合，构建三元组混合位置索引；2）采用频率分配策略在三索引分量上编码时空位置信息；3）引入切比雪夫因果掩码，通过计算图像token在2D空间中的切比雪夫距离来确定因果依赖关系
...
TL;DR: MetaphorStar是一个端到端的视觉强化学习框架，专门解决图像隐喻理解任务，在图像暗示基准测试中平均提升82.6%性能，超越20+主流MLLMs达到SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在基础视觉问答方面表现出色，但在理解图像中蕴含的文化、情感和上下文暗示等微妙含义方面存在困难。这种困难源于任务需要复杂的多跳推理、文化背景和心理理论能力，而现有模型缺乏这些能力。

Method: 提出了MetaphorStar框架，包含三个核心组件：细粒度数据集TFQ-Data、视觉强化学习方法TFQ-GRPO以及结构良好的基准测试TFQ-Bench。这是一个端到端的视觉强化学习框架，专门用于图像暗示任务。

Result: MetaphorStar-32B在多项选择和开放式问题上达到SOTA水平，在判断题上显著优于顶级闭源模型Gemini-3.0-pro。在图像暗示基准测试中平均性能提升82.6%。实验还显示，学习图像暗示任务能提升模型的通用理解能力，特别是复杂视觉推理能力。

Conclusion: MetaphorStar框架有效解决了图像隐喻理解这一关键挑战，通过视觉强化学习方法显著提升了模型性能。该方法具有广泛适用性，作者开源了所有模型权重、数据集和方法代码。
...
TL;DR: 基于YOLOv11n架构，提出了一种针对低质量监控视频的儿童检测增强方案，通过领域特定的数据增强和推理优化，在保持实时性能的同时显著提升了检测精度。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的儿童走失警报和日托监控系统通常依赖低质量的监控录像，这些录像存在遮挡、小目标、低分辨率、运动模糊和光照不足等挑战，需要一种轻量级且实用的解决方案来提升儿童检测性能。

Method: 1. 基于高效的YOLOv11n架构构建部署就绪的管道；2. 引入领域特定的数据增强策略，包括空间扰动（部分可见性、截断、重叠）和光度退化（光照变化和噪声）；3. 在推理时集成切片辅助超推理（SAHI）以提升小目标和部分遮挡实例的召回率；4. 在Roboflow日托数据集的儿童专用子集上进行训练和评估。

Result: 相比基线YOLOv11n，增强系统在mAP@0.5上达到0.967，在mAP@0.5:0.95上达到0.783，分别获得0.7%和2.3%的绝对提升。整个管道保持与低功耗边缘设备的兼容性，支持实时性能。

Conclusion: 该研究提出了一种实用且轻量级的儿童检测增强方案，通过领域特定的数据增强和推理优化，在保持实时性能的同时显著提升了在低质量监控视频中的检测精度，特别适合低成本或资源受限的工业监控部署。
...
TL;DR: 该论文使用YOLOX和Edge AI加速器硬件在火车站进行乘客检测，比较了Hailo-8和Jetson Orin Nano的性能，结果显示Hailo-8在准确性和延迟方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 火车站区域存在乘客不注意安全线导致的事故，需要开发更先进的安全技术来减少事故数量。图像处理和对象检测技术可以应用于交通领域提高安全性。

Method: 使用YOLOX目标检测算法结合Edge AI加速器硬件，在火车站场景进行乘客检测。比较了两种硬件平台：Hailo-8 AI硬件加速器和Jetson Orin Nano的性能。

Result: Hailo-8 AI硬件加速器相比Jetson Orin Nano有超过12%的准确率提升，并且延迟降低了20毫秒，在准确性和实时性方面都表现更优。
...
TL;DR: VRFT-Aug：针对医学影像领域的视觉强化微调框架，通过感知增强和推理优化策略，在多个医学数据集上超越传统监督微调和强化微调基线。


<details>
  <summary>Details</summary>
Motivation: 当前基于规则的强化微调方法在跨模态视觉领域应用不足，特别是在医学影像领域，需要同时具备强大的视觉感知和结构化推理能力，现有方法存在局限性。

Method: 提出VRFT-Aug框架，包含先验知识注入、感知驱动的策略优化、医学知识引导的奖励塑造和行为模仿等训练策略，旨在增强感知和推理能力并稳定强化微调过程。

Result: 在多个医学数据集上的实验表明，该方法一致优于标准监督微调和强化微调基线，并提供了可推广到其他医学图像任务的实用训练启发。
...
<details>
  <summary>Details</summary>
Motivation: 当前医学基础模型在受控基准测试中表现良好，但广泛部署受到任务特定微调的阻碍。研究旨在开发无需任务特定适应就能提供有效临床决策支持的皮肤病学基础模型。

Method: 通过掩码潜在建模和对比学习在超过400万个多模态数据点上训练DermFM-Zero模型，在20个基准测试中评估零样本诊断和多模态检索能力，并在三个多国读者研究中验证临床实用性。

Result: 在初级保健环境中，AI辅助使全科医生对98种皮肤病的鉴别诊断准确性几乎翻倍；在专科环境中，模型在多模态皮肤癌评估中显著优于委员会认证的皮肤科医生；在协作工作流程中，AI辅助使非专家超越未辅助的专家；模型的潜在表示具有可解释性。

Conclusion: DermFM-Zero证明基础模型能够提供有效、安全、透明的零样本临床决策支持，无需任务特定微调即可实现广泛部署。
...
Method: 1) 使用像素重排操作消除VAE，将潜在空间GenDR转为像素空间GenDR-Pix；2) 提出多阶段对抗蒸馏逐步移除编码器和解码器；3) 使用随机填充增强生成特征并避免判别器崩溃；4) 引入掩码傅里叶空间损失惩罚振幅异常值；5) 集成基于填充的自集成与无分类器引导改进推理缩放。

Result: GenDR-Pix相比GenDR实现2.8倍加速和60%内存节省，视觉质量下降可忽略。能在仅1秒和6GB内存内恢复4K图像，超越其他一步扩散超分辨率方法。

Conclusion: 通过消除VAE瓶颈并将模型转为像素空间，GenDR-Pix显著提升了扩散模型在超分辨率任务中的推理效率和内存使用，为实际应用提供了可行的解决方案。
...
TL;DR: VideoSTF是首个系统测量视频大语言模型输出重复问题的框架，通过三个n-gram指标和1万条多样化视频测试集，发现现有VideoLLMs普遍存在严重输出重复问题，且对时间扰动高度敏感，暴露了安全漏洞。


<details>
  <summary>Details</summary>
Motivation: 现有VideoLLMs在视频理解任务中表现良好，但存在一个未被充分探索的生成失败问题：严重的输出重复，即模型陷入自我强化的重复短语或句子循环。现有基准主要关注任务准确性和事实正确性，无法捕捉这种重复问题。

Method: 提出VideoSTF框架，使用三个互补的n-gram指标形式化重复问题，提供包含1万条多样化视频的标准化测试平台，以及受控时间变换库。通过普遍测试、时间压力测试和对抗性利用，在10个先进VideoLLMs上进行评估。
...
TL;DR: MP-HOI是一个用于文本驱动的3D人-物交互运动生成的新框架，通过多模态数据先验、增强的对象表示、多模态感知的专家混合模型和级联扩散机制，解决了现有方法在人体运动、物体运动和交互质量方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有文本驱动的3D人-物交互运动生成方法主要依赖直接的文本到HOI映射，由于显著的跨模态差距存在三个关键局限：次优的人体运动、不自然的物体运动以及人与物体之间的弱交互。需要解决这些跨模态差距带来的挑战。

Method: 提出MP-HOI框架，基于四个核心洞察：1) 利用大型多模态模型的多模态数据（文本、图像、姿态/物体）作为先验指导HOI生成；2) 通过几何关键点、接触特征和动态属性增强对象表示；3) 提出多模态感知的专家混合模型进行有效特征融合；4) 设计级联扩散框架，在专门监督下逐步细化人-物交互特征。

Result: 综合实验表明，MP-HOI在生成高保真和细粒度的人-物交互运动方面优于现有方法。

Conclusion: MP-HOI通过多模态数据先验、增强的对象表示、多模态感知的专家混合模型和级联扩散机制，有效解决了文本驱动的3D人-物交互运动生成中的关键挑战，能够生成更高质量的交互运动。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本引导扩散模型在修改原始文本提示以实现预期语义调整时，往往会导致意外的全局结构变化，破坏用户意图。现有方法依赖于经验性的特征图选择进行干预，其性能严重依赖于适当的选择，导致稳定性不佳。

Method: 从频率角度分析噪声潜在变量频谱对生成过程中结构框架和细粒度纹理分层出现的影响，提出使用具有动态衰减的频率相关加权函数的训练免费频率调制方法，直接操作噪声潜在变量，避免经验性选择内部特征图。

Result: 大量实验表明，所提出的方法显著优于当前最先进的方法，在保持结构和实现语义更新之间实现了有效平衡。

Conclusion: 通过频率调制方法，可以在不依赖经验特征图选择的情况下，有效保持图像生成的结构框架一致性，同时允许有针对性的语义修改，解决了现有方法稳定性不足的问题。
...
TL;DR: AMAP-APP是一款跨平台桌面应用，通过优化算法将足细胞形态分析速度提升147倍，同时保持与原始方法的高度相关性，降低了计算门槛。


<details>
  <summary>Details</summary>
Motivation: 原始AMAP方法存在计算需求高、缺乏用户界面和Linux依赖等障碍，限制了其在肾脏研究中的广泛应用。需要开发一个更易用、跨平台的解决方案来推广足细胞形态分析。

Method: 用经典图像处理替代计算密集的实例分割，保留原始语义分割模型；引入改进的ROI算法提高精度；使用365张小鼠和人类图像（STED和共聚焦）进行验证，通过Pearson相关性和TOST测试与原始AMAP进行性能对比。

Result: 在消费级硬件上处理速度提升147倍；形态计量输出（面积、周长、圆形度、裂隙隔膜密度）与原始方法高度相关（r>0.90）且统计等效（TOST P<0.05）；新ROI算法比原始方法更准确，与手动描绘的偏差更小。

Conclusion: AMAP-APP通过消除对高性能计算集群的需求，为Windows、macOS和Linux提供用户友好界面，使基于深度学习的足细胞形态计量分析更加普及，有望在肾脏病学研究和临床诊断中得到广泛应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉思维链方法主要局限于静态场景，难以捕捉任务（如指令、预测、相机运动）所需的时间动态性，这限制了在动态视觉推理中的应用。

Method: 1) 构建TwiFF-2.7M数据集：从270万个视频片段中提取的大规模时序视觉思维链数据集；2) 创建TwiFF-Bench评估基准：包含1078个样本，评估推理轨迹的合理性和最终答案的正确性；3) 提出TwiFF模型：结合预训练视频生成和图像理解能力，迭代生成未来动作帧和文本推理，产生时序一致的视觉推理线索。

Result: TwiFF模型在动态推理任务上显著优于现有视觉思维链方法和文本思维链基线，充分验证了其在动态场景中视觉问答的有效性。

Conclusion: 该研究填补了视觉思维链在时序动态性方面的空白，通过大规模数据集和统一模型架构，为动态视觉推理提供了有效的解决方案，推动了多模态推理在动态场景中的应用。
...
TL;DR: 提出OmniVL-Guard框架，通过平衡强化学习解决多模态伪造检测与定位中的难度偏差问题，实现检测与定位的联合优化。
...
Method: 提出OmniVL-Guard框架，包含两个核心设计：1) Self-Evolving CoT Generation（自演进思维链生成）合成高质量推理路径，有效克服冷启动挑战；2) Adaptive Reward Scaling Policy Optimization (ARSPO) 动态调节奖励尺度和任务权重，确保平衡的联合优化。

Result: 大量实验表明，OmniVL-Guard显著优于最先进的方法，并在跨域场景中表现出零样本鲁棒泛化能力。

Conclusion: OmniVL-Guard通过平衡强化学习框架成功解决了多模态伪造检测与定位中的难度偏差问题，实现了检测性能与定位精度的平衡优化，具有强大的泛化能力。
...
TL;DR: 提出了一种将深度估计集成到视觉-语言-动作模型中的新框架，通过VGGT深度估计提取3D几何特征，并引入动作助手模块约束学习到的3D表示，从而增强VLA模型在复杂3D环境中的空间理解和动作定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型主要依赖基于2D图像训练的视觉语言模型，这限制了它们在复杂3D环境中的空间理解和动作定位能力。为了解决这一局限性，需要将深度信息集成到VLA模型中，以丰富3D特征表示。

Method: 1. 使用VGGT深度估计基线从标准RGB输入中提取几何感知的3D线索；2. 引入动作助手模块，用动作先验约束学习到的3D表示，确保其与下游控制任务的一致性；3. 将增强的3D特征与传统的2D视觉标记融合。
...
TL;DR: (MGS)²是一个针对跨视角地理定位的几何基础框架，通过宏几何结构过滤和微几何尺度适应模块解决无人机视角与卫星参考图像之间的几何不对齐问题，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有跨视角地理定位方法主要在2D流形上操作，忽略了3D几何结构，其中视角相关的垂直立面（宏结构）和尺度变化（微尺度）严重破坏了特征对齐，导致在GNSS受限的无人机导航中表现脆弱。

Method: 提出(MGS)²框架：1) 宏几何结构过滤(MGSF)模块，利用扩张几何梯度物理过滤高频立面伪影，增强视角不变的水平平面；2) 微几何尺度适应(MGSA)模块，利用深度先验通过多分支特征融合动态校正尺度差异；3) 几何-外观对比蒸馏(GACD)损失，严格区分倾斜遮挡。

Result: 在University-1652数据集上Recall@1达到97.5%，在SUES-200数据集上达到97.02%，实现了最先进的性能，并展现出优异的跨数据集泛化能力，能有效对抗几何模糊性。

Conclusion: (MGS)²通过显式建模3D几何结构，有效解决了跨视角地理定位中的几何不对齐问题，为GNSS受限的无人机导航提供了鲁棒的解决方案，在多个基准测试中表现出卓越性能。
...
TL;DR: 本研究探索地理空间基础模型在生态应用中的效用，通过微调Prithvi-E0-2.0和TerraMind模型在三个用例中表现优于基线ResNet-101模型，TerraMind在加入额外模态时表现最佳。
...
Result: 在所有实验中，地理空间基础模型均优于基线ResNet模型；TerraMind总体上略优于Prithvi，但在加入额外模态时显著优于所有对比模型。
...
<details>
  <summary>Details</summary>
Motivation: 真实世界低分辨率图像的超分辨率处理会产生复杂、不规则的退化，这些退化高度不可预测且随场景变化，而现有方法主要针对合成LR图像设计，无法有效评估真实SR图像质量，特别是在数据稀缺领域。

Method: 提出S3 RIQA方法：1）采用自监督对比学习框架，将同一SR算法生成的图像作为正样本对，不同算法生成的图像作为负样本对；2）包含针对性预处理提取补充质量信息；3）引入辅助任务处理不同SR缩放因子相关的退化特征；4）构建新数据集SRMORSS支持无监督预训练。

Result: 在真实SR-IQA基准测试中，S3 RIQA持续优于大多数最先进的相关指标，证明了其在真实世界超分辨率图像质量评估中的有效性。

Conclusion: 该方法为高度病态的真实世界超分辨率设置提供了有效的无参考质量评估解决方案，特别适用于数据稀缺领域，通过利用SR算法特性而非图像内容来评估质量，填补了现有方法的空白。
...
<details>
  <summary>Details</summary>
Motivation: 矢量图形虽然计算需求高，但在设计和建筑领域具有可扩展性和质量保持的优势。现有矢量图形生成方法存在局限性，需要更高效的文本到矢量生成方案和更好的栅格到矢量转换算法。

Method: 1. 提出从文本描述生成矢量住宅平面图的新方法，利用对直角的内置处理和灵活设置；2. 提出将栅格平面图矢量化成结构化矢量图像的新算法。

Result: 1. 文本到矢量生成方法在基于CLIPScore的视觉质量上比现有方案提升约5%；2. 栅格到矢量算法生成的图像在CLIPScore上比其他方法提升约4%。

Conclusion: 提出的方法在矢量住宅平面图生成和栅格平面图矢量化方面都取得了显著改进，为设计和建筑领域的矢量图形应用提供了更高质量的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 扩散和流式生成模型的迭代采样速度慢是实际部署的主要瓶颈。一致性模型作为最先进的蒸馏方法，仍受限于训练不稳定和采样不灵活两大问题。现有方法通过架构调整或正则化目标来缓解，但忽略了轨迹选择的关键依赖。

Method: 提出双端一致性模型（DE-CM），通过选择关键子轨迹簇实现稳定有效训练。将PF-ODE轨迹分解并选择三个关键子轨迹作为优化目标，利用连续时间CM目标实现少步蒸馏，使用流匹配作为边界正则化器稳定训练。提出新颖的噪声到噪声映射（N2N），可将噪声映射到任意点，缓解第一步的误差累积。

Result: 在ImageNet 256×256数据集上，DE-CM实现了单步生成FID 1.70的SOTA分数，超越了现有的基于一致性模型的单步方法。

Conclusion: 通过轨迹选择策略和N2N映射，DE-CM有效解决了训练不稳定和采样不灵活问题，为高效生成模型提供了新的解决方案。
...
TL;DR: 该论文针对自行车骑行者安全需求，提出了首个自行车视角的视觉问答基准CyclingVQA，评估了31+个视觉语言模型在自行车感知和推理方面的能力，发现现有模型在自行车特定交通规则理解方面存在不足。


<details>
  <summary>Details</summary>
Motivation: 城市交通中自行车骑行者经常面临安全关键情况，需要辅助系统支持安全决策。现有视觉语言模型在自动驾驶基准上表现良好，但评估主要从车辆视角出发，缺乏对自行车骑行者视角的感知和推理能力评估。

Method: 提出CyclingVQA诊断基准，专门从自行车骑行者视角评估感知、时空理解和交通规则到车道推理能力。评估了31+个最新视觉语言模型，包括通用模型、空间增强模型和自动驾驶专用模型。

Result: 当前模型显示出有希望的能力，但在自行车特定感知和推理方面有明显改进空间，特别是在解释自行车特定交通线索和将标志与正确导航车道关联方面。值得注意的是，几个驾驶专用模型表现不如强大的通用视觉语言模型，表明从车辆中心训练到自行车辅助场景的迁移有限。

Conclusion: 通过系统错误分析识别了常见失败模式，为开发更有效的自行车辅助智能系统提供指导。需要专门针对自行车视角的模型改进，以更好地支持城市交通中的自行车骑行者安全。
...
TL;DR: 本文提出了RSHallu，一个针对遥感多模态大语言模型幻觉问题的系统性研究，包括定义遥感幻觉分类、构建评估基准和数据集、并提出训练友好和即插即用的缓解策略。


<details>
  <summary>Details</summary>
Motivation: 遥感多模态大语言模型在遥感视觉定位和问答等任务中表现出色，但幻觉问题（模型输出与输入遥感图像不一致）严重阻碍了其在应急管理和农业监测等高风险场景的部署，且该问题在遥感领域尚未得到充分探索。

Method: 1) 提出面向遥感的幻觉分类法，引入图像级幻觉概念；2) 构建RSHalluEval评估基准（2,023个问答对）支持双模式检查；3) 创建RSHalluShield数据集（30k问答对）用于训练友好缓解，并提出无需训练的即插即用策略，包括解码时对数校正和遥感感知提示。

Result: 在代表性遥感多模态大语言模型上，提出的缓解策略将无幻觉率提高了最多21.63个百分点，同时在遥感视觉问答和视觉定位等下游任务上保持了有竞争力的性能。

Conclusion: RSHallu为遥感多模态大语言模型的幻觉问题提供了系统性解决方案，包括定义、评估和缓解策略，显著提升了模型可靠性，有助于推动遥感多模态大语言模型在高风险场景的实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D点云异常检测方法大多依赖类别特定训练，在少样本场景下灵活性受限，需要一种无需训练即可跨类别检测的方法

Method: 将点云转换为固定数量的真实深度图像，利用冻结的CLIP视觉编码器提取多视角表示，通过加权特征相似度进行异常检测，无需微调或类别适配

Result: 在ShapeNetPart数据集上的实验表明，DMP-3DAD在少样本设置下达到最先进性能
...
TL;DR: DC-SFT通过基于样本难度筛选训练数据，解决了视觉语言模型微调中的泛化差距问题，比监督微调和强化学习都更有效。


<details>
  <summary>Details</summary>
Motivation: 研究发现视觉语言模型微调中存在泛化差距：强化学习微调的模型在分布外性能上优于监督微调模型。作者认为这可能源于强化学习隐含的数据筛选机制，优先处理中等难度样本。

Method: 提出难度筛选监督微调（DC-SFT）方法，通过显式地基于样本难度筛选训练数据集，优先选择中等难度样本进行训练。

Result: 实验表明DC-SFT不仅显著提升了分布外泛化性能，超越了标准监督微调，甚至优于强化学习训练，同时提供了更好的稳定性和计算效率。

Conclusion: 该工作从数据角度解释了视觉语言模型的分布外泛化差距，并建立了一条更高效实现鲁棒泛化的路径，为模型微调提供了新的数据筛选策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型通常依赖基于单次视觉编码的纯文本推理，导致细粒度视觉信息丢失。虽然"thinking with images"尝试通过外部工具或代码操作图像来缓解这一问题，但生成的视觉状态往往缺乏语言语义基础，影响跨模态对齐效果，特别是在需要跨远距离区域或多图像进行视觉语义或几何关系推理时。

Method: 提出"chatting with images"新框架，将视觉操作重构为语言引导的特征调制。在表达性语言提示的指导下，模型动态地对多个图像区域进行联合重新编码，实现语言推理与视觉状态更新的紧密耦合。具体实现为ViLaVT模型，配备专为交互式视觉推理设计的动态视觉编码器，采用两阶段课程训练（监督微调+强化学习）来促进有效推理行为。

Result: 在八个基准测试上的广泛实验表明，ViLaVT实现了强大且一致的改进，在复杂多图像和视频空间推理任务上取得了特别显著的增益。

Conclusion: 通过语言引导的特征调制框架"chatting with images"和相应的ViLaVT模型，能够有效解决现有LVLMs在视觉推理中的局限性，特别是在需要跨区域和多图像推理的复杂任务上表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 自回归模型虽然能生成超长视频，但顺序生成过程非常缓慢。现有的缓存策略假设所有帧在相同时间步具有均匀的去噪需求，这一假设在自回归模型中不成立，因为不同视频块在相同时间步表现出不同的相似性模式。

Method: 提出FlowCache框架，核心洞察是每个视频块应保持独立的缓存策略。采用分块缓存策略，动态适应每个块的独特去噪特征；结合联合重要性-冗余优化的KV缓存压缩机制，在固定内存边界内保持生成质量。

Result: 在MAGI-1上实现2.38倍加速，在SkyReels-V2上实现6.7倍加速，质量下降可忽略（VBench：分别增加0.87和减少0.79）。成功解锁自回归模型实时生成超长视频的潜力。

Conclusion: FlowCache为高效大规模视频合成建立了新基准，证明了专门针对自回归视频生成的缓存框架的有效性，实现了质量与速度的良好平衡。
...
TL;DR: 本文提出Chart Specification方法，通过结构化中间表示和强化学习奖励机制，显著提升图表图像到代码生成的结构保真度。
...
Result: 在三个公共基准测试中始终优于先前方法；仅用3K训练样本就实现了强大的数据效率，在复杂基准上超过领先基线达61.7%；扩展到4K样本时在所有评估指标上建立了新的最先进结果。

Conclusion: 精确的结构监督为高保真图表到代码生成提供了高效途径，结构化中间表示和强化学习奖励机制能显著提升生成代码的结构正确性。
...
TL;DR: 提出TR-World模型，专注于动态物体建模，通过计算场景表示的时间残差提取动态信息，结合FGTR模块实现轨迹优化，在nuScenes和NAVSIM数据集上达到SOTA规划性能。


<details>
  <summary>Details</summary>
Motivation: 当前世界模型在驾驶场景中存在两个主要问题：1）对静态区域的冗余建模；2）与轨迹缺乏深度交互。这限制了世界模型在端到端自动驾驶框架中的规划准确性。

Method: 提出TR-World模型，通过计算场景表示的时间残差提取动态物体信息，无需依赖检测和跟踪。同时提出FGTR模块，在先验轨迹和未来BEV特征之间进行交互，利用未来路况优化轨迹，并为未来BEV特征提供稀疏时空监督防止模型坍塌。

Result: 在nuScenes和NAVSIM数据集上的综合实验表明，ResWorld方法实现了最先进的规划性能。

Conclusion: TR-World通过专注于动态物体建模和深度轨迹交互，有效提升了自动驾驶规划性能，解决了现有世界模型的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有Unified Sequence Parallelism (USP)实现存在显著低效问题，包括过多的内核启动开销和次优的计算-通信调度，影响大规模扩散模型（如FLUX 12B、Stable Diffusion 3 8B）的推理效率

Method: 提出FastUSP多级优化框架：1）编译级优化（CUDA Graphs图编译和计算-通信重排序）；2）通信级优化（FP8量化集体通信）；3）算子级优化（双缓冲流水线Ring attention）

Result: 在FLUX 12B模型上，FastUSP相比基线USP实现1.12×-1.16×端到端加速，编译级优化贡献主要改进；在Qwen-Image模型上，2 GPU实现1.09×加速，4-8 GPU因PyTorch Inductor兼容性问题无法应用编译优化

Conclusion: FastUSP有效解决了USP实现中的性能瓶颈，分析表明在现代高带宽GPU互连上，内核启动开销（而非通信延迟）是分布式扩散推理的主要瓶颈
...
Method: 采用可泛化的神经辐射场方法，从机器人第一人称视角构建全局工作空间框架中的3D占用表示。模型集成灵活的源视图，无需场景特定微调即可泛化到新的物体布局。

Result: 在40个真实场景上训练后，模型实现了26mm的重建误差（包括被遮挡区域），验证了其推断完整3D占用情况的能力，超越了传统立体视觉方法。

Conclusion: 该方法成功构建了全局工作空间框架中的3D占用表示，可直接应用于机器人操作，展示了在未见物体布局上的良好泛化能力，为机器人感知提供了新的解决方案。
...
TL;DR: 该研究使用深度学习模型InceptionV3和ResNet50对番石榴疾病进行分类，通过数据增强和混合技术提高模型鲁棒性，InceptionV3达到98.15%准确率。


<details>
  <summary>Details</summary>
Motivation: 番石榴果实常受多种疾病影响，这会损害果实质量和产量。早期识别对于减少损害和确保果实健康非常重要。

Method: 研究使用从Mendeley Data收集的473张原始图像，经过调整大小至256x256像素并应用数据增强技术，生成3784张图像。使用InceptionV3和ResNet50两种深度学习模型进行分类，并应用CutMix和MixUp数据混合方法增强模型鲁棒性。使用混淆矩阵评估性能，SHAP分析提高模型可解释性。

Result: InceptionV3模型取得了98.15%的准确率，ResNet50模型取得了94.46%的准确率。两种模型都能有效分类番石榴疾病。

Conclusion: 该研究展示了先进深度学习模型在番石榴疾病分类中的有效性，特别是InceptionV3模型表现出色，为早期疾病检测提供了可靠工具。
...
TL;DR: 本文介绍了DFIC数据集，这是一个包含约58,000张标注图像和2,706个视频的全面人脸图像数据集，专门用于自动化ICAO合规性验证。该数据集覆盖了广泛的不合规条件，并提供了比现有公共数据集更平衡的人口统计分布。


<details>
  <summary>Details</summary>
Motivation: 当前手动检查机器可读旅行证件(MRTDs)中面部图像是否符合ISO/IEC和ICAO标准的方法在高需求环境中效率低下，需要开发自动化的合规性验证方法。

Method: 创建了DFIC数据集，包含约58,000张标注图像和2,706个视频，覆盖1,000多个受试者。基于该数据集，微调了一种主要依赖空间注意力机制的自动ICAO合规性验证方法。

Result: 与现有的ICAO合规性验证最先进方法相比，基于DFIC数据集的方法取得了改进的结果。数据集已公开提供，可用于训练和验证新模型。

Conclusion: DFIC数据集具有增强自动化ICAO合规性方法的潜力，同时也可用于改进面部识别系统的安全性、隐私性和公平性的其他应用。
...
TL;DR: 将SVD启发的注意力机制（SVDA）应用于Vision Transformer，提升注意力机制的可解释性、稀疏性和谱结构，在多个基准数据集上保持分类精度的同时获得更可解释的注意力模式。
...
Method: 将先前提出的SVD-Inspired Attention（SVDA）机制适配到ViT架构中，引入几何基础公式化方法，使用可解释性指标监控训练过程中的注意力动态并评估学习表示的结构特性。
...
TL;DR: 论文提出了一种SVD启发式注意力机制(SVDA)，用于改进密集预测Transformer中的注意力机制，使其具有光谱结构化和可解释性。
...
Method: 将SVD启发式注意力(SVDA)引入密集预测Transformer(DPT)，通过在学习查询-键交互中嵌入可学习的对角矩阵，将方向对齐与光谱调制解耦。

Result: 在KITTI和NYU-v2数据集上的实验表明，SVDA保持或略微提高了预测精度，同时只增加了少量计算开销。更重要的是，SVDA解锁了六个光谱指标来量化注意力特性。

Conclusion: SVDA将注意力的角色从黑盒机制转变为可量化的描述符，重新定义了单目深度估计中的可解释性，为透明密集预测模型开辟了新的途径。
...
<details>
  <summary>Details</summary>
Motivation: 手术室中手术器械的准确计数对患者安全至关重要，但在密集场景中（器械紧密堆积）现有方法难以准确计数。传统物体检测方法无序且难以处理密集场景，需要新的解决方案。

Method: 1. 提出Chain-of-Look视觉推理框架，模仿人类顺序计数过程，通过结构化视觉链引导模型沿着连贯的空间轨迹计数；2. 引入相邻损失函数，明确建模密集手术器械固有的空间约束；3. 创建SurgCount-HD数据集，包含1,464张高密度手术器械图像。
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督深度估计方法面临双重挑战：要么架构笨重影响实用性，要么轻量级模型牺牲结构精度，需要开发既轻量又结构精确的架构
...
Conclusion: PuriLight通过三个创新模块的有效协作，成功解决了自监督深度估计中轻量化和精度保持的双重挑战
...
TL;DR: 本文介绍了基于步态压力模式的生物识别技术，通过UNB StepUP-P150数据集和国际竞赛推动该领域发展，最佳团队使用GRM优化策略达到10.77%的EER，但鞋类变化仍是主要挑战。


<details>
  <summary>Details</summary>
Motivation: 步态生物识别技术面临缺乏大规模多样化数据集的问题，限制了模型对新用户和鞋类、步速等变化的泛化能力。UNB StepUP-P150数据集的发布为解决这些挑战提供了机会。

Method: 举办了首届国际StepUP竞赛，要求参赛者使用StepUP-P150数据集开发稳健的识别模型，并在专门设计的测试集上评估验证性能。最佳团队Saeid_UCC采用了生成式奖励机器（GRM）优化策略。

Result: 竞赛吸引了全球23个团队参与，最佳团队Saeid_UCC实现了10.77%的等错误率（EER）。竞赛展示了强大的解决方案，但对陌生鞋类的泛化能力仍是持续挑战。

Conclusion: 步态生物识别领域通过大规模数据集和竞赛取得了进展，但鞋类变化带来的泛化问题仍是未来研究的关键方向。竞赛为评估和推进该领域技术提供了重要平台。
...
<details>
  <summary>Details</summary>
Motivation: 现有评判模型主要针对通用视觉领域（如图像描述、问答）训练，而涉及感知、因果推理和规划的物理AI任务研究不足，需要专门优化的评判模型。

Method: 采用两阶段RLVR流程：1) 物理技能预热阶段增强物理导向的感知和推理；2) 自参考评判微调阶段，模型在评判候选回答前先生成自己的预测作为内部参考，提高评判稳定性和物理正确性。

Result: PhyCritic在物理和通用多模态评判基准上都取得了显著性能提升，优于开源基线模型。当作为策略模型应用时，还能进一步提升物理基础任务的感知和推理能力。

Conclusion: PhyCritic通过专门针对物理AI优化的两阶段训练流程，有效填补了现有评判模型在物理任务领域的空白，为物理AI的评估和对齐提供了可靠工具。
...
TL;DR: SurfPhase：一种从稀疏相机视角重建两相流3D界面动力学的新方法，结合动态高斯面元与符号距离函数，利用视频扩散模型合成新视角视频，在池沸腾数据集上验证了高质量视图合成和速度估计。


<details>
  <summary>Details</summary>
Motivation: 两相流中的界面动力学控制动量、热量和质量传递，但实验测量困难。传统技术在移动界面附近存在固有局限，现有神经渲染方法针对单相流且无法处理尖锐、可变形液-汽界面。

Method: 提出SurfPhase模型，集成动态高斯面元与符号距离函数以确保几何一致性，利用视频扩散模型合成新视角视频以优化稀疏观测的重建。

Result: 在高速池沸腾视频新数据集上评估，仅用两个相机视角即可实现高质量视图合成和速度估计。

Conclusion: SurfPhase能够有效重建两相流3D界面动力学，为界面动态测量提供了新方法。
...
TL;DR: 本文提出一个LLM驱动的框架，用于检测同行评审中的"懒惰思维"问题，通过将评审分解为论证段落，结合神经符号方法识别多种问题，并生成针对性的改进反馈。


<details>
  <summary>Details</summary>
Motivation: 同行评审是科学质量的核心，但依赖简单启发式（懒惰思维）降低了标准。现有研究将懒惰思维检测视为单标签任务，但评审段落可能同时存在多种问题，包括清晰度和具体性问题。目前缺乏基于指南的可操作反馈机制。
...
Conclusion: 该框架能有效检测同行评审中的多种问题，并提供可操作的改进建议，显著提升评审质量。发布的数据集为未来研究提供了资源。
...
TL;DR: LT-Tuning提出了一种新的潜在思维调整框架，通过上下文-预测-融合机制结合隐藏状态和词汇嵌入空间的语义指导，解决现有潜在推理方法中的特征崩溃和不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 显式思维链（CoT）虽然赋予大语言模型强大的推理能力，但需要模型用文本标记表达每个中间步骤，将模型思维限制在离散词汇空间中。最近，连续潜在空间中的推理成为一种有前景的替代方案，但当前潜在范式存在特征崩溃和不稳定性问题，主要源于递归使用隐藏状态作为输入嵌入时的分布不匹配，或依赖辅助模型时的对齐问题。

Method: 提出Latent Thoughts Tuning（LT-Tuning）框架，引入上下文-预测-融合机制，联合利用上下文隐藏状态和词汇嵌入空间的预测语义指导，而不是仅依赖原始隐藏状态。结合渐进式三阶段课程学习流程，使模型能够在潜在思维和显式思维模式之间动态切换。

Result: 实验表明，该方法优于现有的潜在推理基线，有效缓解了特征崩溃问题，实现了稳健的推理准确性。

Conclusion: LT-Tuning通过重新定义潜在思维的构建和部署方式，解决了现有潜在推理方法的局限性，为连续潜在空间中的稳健推理提供了有效解决方案。
...
TL;DR: 该研究探讨语言模型是否共享计算机制来处理心理理论和语言语用推理，以验证其是否发展出可跨任务重用的"社会世界模型"（功能整合假说）。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索语言模型是否能够发展出"社会世界模型"——即能够跨任务重用的心理状态表征，这有助于回答语言模型是否具备类似人类的社会认知能力这一根本问题。
...
Conclusion: 该研究贡献了新的心理理论定位数据、功能定位技术的改进方法，以及对人工系统中社会认知能力涌现的实证见解，为理解语言模型的社会认知能力提供了重要基础。
...
TL;DR: 论文研究了大型语言模型在推理时间扩展下是否会出现资源理性行为，发现模型会根据任务复杂度自适应调整推理策略，即使没有明确的计算成本奖励。


<details>
  <summary>Details</summary>
Motivation: 人类推理受资源理性（在约束下优化性能）影响。虽然指令调优模型和大型推理模型通过推理时间扩展提升性能，但尚不清楚在没有明确计算成本奖励的情况下，资源理性是否能从这种扩展中自然涌现。

Method: 引入变量归因任务，让模型根据候选变量、输入输出试验和预定义逻辑函数推断哪些变量决定结果。通过改变候选变量数量和试验次数来系统操控任务复杂度，比较指令调优模型和大型推理模型的表现。

Result: 随着复杂度增加，两种模型都表现出从暴力搜索到分析策略的转变。指令调优模型在XOR和XNOR函数上性能下降，而大型推理模型保持稳健。模型能根据任务复杂度调整推理行为。

Conclusion: 研究发现模型即使没有明确的成本奖励，也能根据任务复杂度自适应调整推理策略，这表明资源理性是推理时间扩展本身涌现的特性。
...
TL;DR: 论文利用洛杉矶警察局随身摄像机视频，构建了首个大规模交通拦截数据集，包含来自不同社区视角的尊重度评分和解释，开发了视角感知模型框架来预测个性化尊重评分并生成解释。


<details>
  <summary>Details</summary>
Motivation: 交通拦截是最常见的警民互动之一，尊重是这些互动的核心维度，影响公众信任和合法性感知。然而，对尊重的理解具有主观性，受生活经历影响，因此社区特定视角至关重要。目前缺乏大规模、多视角的交通拦截尊重度标注数据。

Method: 1) 基于程序正义理论、LAPD培训材料和实地调查开发领域特定评估标准；2) 引入标准驱动的偏好数据构建框架，实现视角一致性对齐；3) 提出视角感知建模框架，从交通拦截记录中预测个性化尊重评分，并为警官和驾驶员生成标注者特定的解释。

Result: 在所有三个标注者群体（警察关联、司法系统受影响、无关联的洛杉矶居民）中，该方法提高了评分预测性能和解释对齐度。视角感知框架使执法部门能更好地理解不同社区的期望。

Conclusion: 该研究为执法部门提供了理解不同社区期望的重要工具，有助于建立公众信任和程序合法性。视角感知框架能够预测个性化尊重评分并生成特定解释，为改善警民互动提供了数据驱动的洞察。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究表明多语言语音模型的中间层通常比最终输出层编码更准确的音素表示，本研究旨在探索音素级预测在编码器各层如何演变，特别关注低资源语言Campidanese Sardinian。

Method: 应用层解码策略到预训练的Wav2Vec2模型，通过截断上层transformer层来研究音素错误率变化，进行细粒度对齐分析，并引入回归错误概念来评估模型行为。

Result: 截断上层transformer层能改善音素错误率，最佳性能出现在最终层之前两层；中间层预测能更好地保持音段身份、避免过度生成并减少某些类型的音系错误；发现了回归错误现象。

Conclusion: 研究支持使用早期层探测作为ASR模型的诊断工具，特别是在低资源设置中，标准评估指标可能无法捕捉到语言学上有意义的行为，深层可能过度泛化或抽象化声学细节。
...
TL;DR: 训练轻量级适配器来解读冻结语言模型的内部状态，仅需少量参数就能实现可靠的自解释，性能随模型规模提升


<details>
  <summary>Details</summary>
Motivation: 现有的自解释方法存在超参数敏感性问题，不够可靠。需要一种更稳定、可扩展的方法来解读语言模型的内部状态，而不需要修改模型本身

Method: 在冻结的语言模型上训练轻量级适配器（仅需d_model+1个参数的标量仿射适配器），使用可解释性数据作为训练标签。适配器学习从模型内部状态生成特征标签

Result: 训练后的适配器性能显著优于训练标签本身（71% vs 63%），在主题识别任务中达到94%召回率（基线仅1%），能解码多跳推理中的隐式实体。仅偏置向量就贡献了85%的性能提升

Conclusion: 自解释能力随模型规模提升而增强，无需修改被解释的模型。轻量级适配器方法为语言模型的可解释性提供了可靠、可扩展的解决方案
...
### [67] [Physically Interpretable AlphaEarth Foundation Model Embeddings Enable LLM-Based Land Surface Intelligence](https://arxiv.org/abs/2602.10354)
*Mashrekur Rahman*

Main category: cs.CL

TL;DR: 卫星基础模型生成密集嵌入向量，但其物理可解释性尚不明确。本研究通过1210万样本分析AlphaEarth的64维嵌入与26个环境变量的关系，发现嵌入维度与特定地表属性对应，并基于此开发了能处理自然语言环境查询的陆地表面智能系统。


<details>
  <summary>Details</summary>
Motivation: 卫星基础模型产生的嵌入向量虽然密集，但其物理可解释性较差，这限制了它们融入环境决策系统的能力。需要理解这些嵌入如何对应实际的环境变量，以便更好地应用于环境智能系统。

Method: 1. 使用1210万个美国大陆样本(2017-2023年)分析Google AlphaEarth的64维嵌入与26个环境变量(气候、植被、水文、温度、地形)的关系；2. 结合线性、非线性和基于注意力的方法进行可解释性分析；3. 开发基于FAISS索引嵌入数据库的陆地表面智能系统，实现检索增强生成；4. 使用LLM-as-Judge方法在360个查询-响应周期中评估系统性能。

Result: 1. 单个嵌入维度映射到特定的地表属性，完整嵌入空间能高保真重建大多数环境变量(12/26变量R²>0.90，温度和海拔R²=0.97)；2. 最强的维度-变量关系在所有三种分析方法中一致，空间块交叉验证稳健(平均ΔR²=0.017)，时间稳定性高(7年间平均相关性r=0.963)；3. 陆地表面智能系统在LLM评估中获得加权得分μ=3.74±0.77(1-5分)，其中基础性(μ=3.93)和连贯性(μ=4.25)表现最强。

Conclusion: 卫星基础模型嵌入是具有物理结构的表示，可以操作化用于环境和地理空间智能。这些嵌入不仅具有物理可解释性，还能有效支持自然语言环境查询系统，为环境决策提供了新的技术途径。
...
### [68] [The Alignment Bottleneck in Decomposition-Based Claim Verification](https://arxiv.org/abs/2602.10380)
*Mahmud Elahi Akhter,Federico Ruggeri,Iman Munire Bilal,Rob Procter,Maria Liakata*

Main category: cs.CL

TL;DR: 论文研究了结构化声明分解在验证复杂多面声明时的效果不一致问题，发现证据对齐和子声明错误模式是关键瓶颈，提出新的数据集和评估框架。
...
Method: 引入包含时间限定证据和人工标注子声明证据范围的真实世界复杂声明数据集，在两种证据对齐设置下评估分解效果：子声明对齐证据（SAE）和重复声明级证据（SRE）。

Result: 分解仅在证据粒度细且严格对齐时带来显著性能提升；而依赖重复声明级证据的标准设置（SRE）不仅无法改进，反而经常降低性能（在PHEMEPlus、MMM-Fact、COVID-Fact等数据集上）。在存在噪声子声明标签时，保守的"弃权"策略相比激进但错误的预测能显著减少错误传播。
...
### [69] [Triggers Hijack Language Circuits: A Mechanistic Analysis of Backdoor Behaviors in Large Language Models](https://arxiv.org/abs/2602.10382)
...
Conclusion: 后门检测方法应监控已知功能组件而非搜索隐藏电路，缓解策略可利用注入行为与自然行为之间的纠缠关系，这对后门防御具有重要意义。
...
### [70] [When Tables Go Crazy: Evaluating Multimodal Models on French Financial Documents](https://arxiv.org/abs/2602.10384)
*Virginie Mouilleron,Théo Lasnier,Djamé Seddah*

Main category: cs.CL

TL;DR: 该研究提出了首个法语金融文档理解的多模态基准测试Multimodal Finance Eval，评估了6个开源视觉语言模型在金融文档理解任务上的表现，发现模型在文本和表格任务上表现良好（85-90%准确率），但在图表解读（34-62%）和多轮对话推理（约50%）方面存在显著缺陷。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在通用文档理解任务上表现良好，但在专业非英语领域（特别是金融领域）的可靠性尚未充分探索。金融文档包含密集的监管文本、数值表格和可视化图表，提取错误可能产生现实后果，因此需要专门的评估基准。

Method: 1. 构建Multimodal Finance Eval基准数据集：包含1,204个专家验证的问题，涵盖文本提取、表格理解、图表解读和多轮对话推理，数据来自真实的投资说明书、KIDs和PRIIPs文档
2. 评估6个开源视觉语言模型（参数量8B-124B）
3. 使用LLM-as-judge协议进行自动评估

Result: 1. 模型在文本和表格任务上表现良好（85-90%准确率）
2. 模型在图表解读任务上表现较差（34-62%准确率）
3. 多轮对话推理揭示了一个严重问题：早期错误会在对话轮次间传播，导致准确率降至约50%，且与模型大小无关
4. 当前VLMs在明确定义的提取任务上有效，但在交互式多步骤金融分析中仍然脆弱

Conclusion: 当前视觉语言模型在金融文档理解方面存在局限性，特别是在图表解读和多轮交互推理方面。Multimodal Finance Eval基准为这一高风险领域提供了具有挑战性的评估工具，有助于推动相关技术进步。
...
### [71] [Less is Enough: Synthesizing Diverse Data in Feature Space of LLMs](https://arxiv.org/abs/2602.10388)
*Zhongzhi Li,Xuansheng Wu,Yijiang Li,Lijie Hu,Ninghao Liu*

Main category: cs.CL

TL;DR: 提出特征激活覆盖度(FAC)来衡量数据多样性，并基于此开发FAC Synthesis框架，通过稀疏自编码器识别缺失特征并生成相应样本来提升LLM下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用基于文本的指标来衡量后训练数据的多样性，但这些指标只能捕捉语言变化，对决定下游性能的任务相关特征提供弱信号，需要更有效的多样性度量方法。

Method: 引入特征激活覆盖度(FAC)在可解释特征空间中衡量数据多样性；提出FAC Synthesis框架：1)使用稀疏自编码器从种子数据集中识别缺失特征，2)生成明确反映这些特征的合成样本。

Result: 实验表明该方法在各种任务上一致提升了数据多样性和下游性能，包括指令跟随、毒性检测、奖励建模和行为引导；发现跨模型家族(LLaMA、Mistral、Qwen)共享可解释特征空间，支持跨模型知识迁移。

Conclusion: 该工作为探索以数据为中心的LLM优化提供了坚实实用的方法学，通过特征空间的多样性度量显著提升了模型性能。
...
### [72] [When are We Worried? Temporal Trends of Anxiety and What They Reveal about Us](https://arxiv.org/abs/2602.10400)
*Saif M. Mohammad*

Main category: cs.CL

TL;DR: 使用焦虑词汇词典分析美加社交媒体数据，发现焦虑水平在一天中呈现规律变化（早上8点最高，中午最低），周末最低，周中最高；过去时态焦虑最高，未来时态最低；第三人称代词焦虑高于第一、二人称，主语代词焦虑高于宾语代词。


<details>
  <summary>Details</summary>
Motivation: 探索社交媒体上焦虑表达的时间模式和语言特征，了解焦虑何时发生以及不同表达方式（时态、人称代词）如何反映焦虑水平，为理解人类焦虑模式提供数据驱动的洞察。

Method: 使用新创建的词汇-焦虑关联词典，分析大量美国和加拿大的社交媒体数据（推文），通过计算焦虑词汇的出现频率来量化焦虑水平，并分析时间模式（一天内、一周内）、时态（过去、现在、未来）以及人称代词使用与焦虑的关系。

Result: 1. 焦虑水平在一天中呈现规律变化：早上8点最高（与皮质醇水平高峰一致），中午最低；2. 周末焦虑最低，周中最高；3. 过去时态句子中焦虑最高，未来时态最低；4. 第三人称代词（他、他们）相关的帖子焦虑高于第一、二人称；5. 主语代词（我、他、她、他们）相关的帖子焦虑高于宾语代词（我、他、她、他们）。

Conclusion: 社交媒体上的焦虑表达呈现系统性模式，不仅揭示了焦虑发生的时间规律，还显示了不同关注类型（未来、过去、自我、外向等）与焦虑的关联，为理解人类焦虑提供了有价值的见解。
...
### [73] [EVOKE: Emotion Vocabulary Of Korean and English](https://arxiv.org/abs/2602.10414)
*Yoonwon Jung,Hagyeong Shin,Benjamin K. Bergen*

Main category: cs.CL

TL;DR: EVOKE是一个包含英语和韩语情感词汇的平行数据集，提供两种语言间的情感词汇全面覆盖、多对多翻译以及语言特定情感词汇识别。
...
Method: 构建包含1,427个韩语词汇和1,399个英语词汇的平行数据集，系统标注819个韩语和924个英语形容词和动词，包括多义词义项标注、情感相关隐喻识别以及语言间多对多翻译关系。

Result: 创建了迄今为止最全面、最系统、理论中立的英语和韩语情感词汇数据集，包含语言特定情感词汇识别、多义词分析和情感隐喻标注，数据集已公开可用。

Conclusion: EVOKE数据集为跨语言情感研究提供了实用工具，支持不同理论视角的研究，有助于推动情感科学、语言学和自然语言处理领域的发展。
...
### [74] [Neuro-Symbolic Synergy for Interactive World Modeling](https://arxiv.org/abs/2602.10480)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型作为世界模型时经常产生幻觉，特别是在需要严格遵守确定性转换规则的角落情况下；而符号世界模型虽然逻辑一致但缺乏语义表达性。需要一种既能保持表达性又具备鲁棒性的方法。

Method: 提出神经符号协同框架，整合LLM的概率语义先验与可执行符号规则，通过交替训练两种模型，符号世界模型直接约束LLM的输出概率分布，神经世界模型仅在符号规则未覆盖的轨迹上进行微调。

Result: 在ScienceWorld、Webshop和Plancraft三个交互环境中，NeSyS在预测准确性和数据效率方面均优于基线方法，训练数据减少50%而不损失准确性。

Conclusion: NeSyS框架成功整合了神经和符号方法的优势，实现了表达性与鲁棒性的平衡，为构建更可靠的世界模型提供了有效途径。
...
### [75] [Canvas-of-Thought: Grounding Reasoning via Mutable Structured States](https://arxiv.org/abs/2602.10494)
...
TL;DR: Canvas-of-Thought (Canvas-CoT) 使用HTML Canvas作为外部推理基底，通过DOM操作实现原子级状态更新，结合渲染批判循环，显著提升多模态大语言模型在复杂任务中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 传统Chain-of-Thought提示在多模态推理中存在局限性：线性文本序列无法有效处理复杂任务，视觉元素被当作静态快照，错误修正需要冗长的下游修正或完全重新生成，增加了token消耗和认知负担。在高维领域（如几何和SVG设计）中，纯文本表达缺乏视觉指导，限制了推理精度。

Method: 提出Canvas-of-Thought框架，利用HTML Canvas作为外部推理基底，支持基于DOM的原子级CRUD操作（创建、读取、更新、删除），实现原地状态修订而不破坏上下文。集成渲染批判循环作为硬约束验证器，提供明确的视觉反馈来解决难以用文本表达的复杂任务。

Result: 在VCode、RBench-V和MathVista等基准测试上的广泛实验表明，Canvas-CoT显著优于现有基线方法，为上下文高效的多模态推理建立了新范式。

Conclusion: Canvas-of-Thought通过引入外部可视化推理基底和渲染批判机制，有效解决了传统CoT在多模态推理中的局限性，为复杂高维任务提供了更精确、更高效的推理框架。
...
### [76] [On the Robustness of Knowledge Editing for Detoxification](https://arxiv.org/abs/2602.10504)
*Ming Dong,Shiyi Tang,Ziyan Peng,Guanyi Chen,Tingting He*

Main category: cs.CL

TL;DR: 本文提出了一个面向鲁棒性的知识编辑去毒评估框架，发现现有基于毒性分类器的评估存在伪去毒问题，去毒效果在组合编辑、跨语言场景下存在局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于知识编辑的去毒方法评估主要依赖自动毒性分类器，隐含假设降低毒性分数就代表真正抑制了有害行为。作者认为这种评估不够全面，需要从鲁棒性角度更深入地评估去毒方法的可靠性。

Method: 提出了一个面向鲁棒性的评估框架，从三个维度评估知识编辑去毒方法：优化鲁棒性（检查是否存在伪去毒）、组合鲁棒性（多个有害行为联合编辑时的效果）、跨语言鲁棒性（不同语言间的效果迁移）。

Result: 发现了伪去毒这一常见失败模式，即表面毒性降低源于退化的生成行为而非真正抑制不安全内容；去毒效果在多个不安全行为联合编辑时会下降；单语和跨语言去毒仅在特定模型-方法组合下有效。

Conclusion: 基于知识编辑的去毒方法仅在特定模型、有限数量的去毒目标和部分语言中具有鲁棒性，需要更全面的评估框架来确保去毒效果的真实可靠性。
...
### [77] [When to Memorize and When to Stop: Gated Recurrent Memory for Long-Context Reasoning](https://arxiv.org/abs/2602.10560)
*Leheng Sheng,Yongtao Zhang,Wenchang Ma,Yaorui Shi,Ting Huang,Xiang Wang,An Zhang,Ke Shen,Tat-Seng Chua*

Main category: cs.CL

TL;DR: GRU-Mem通过引入文本控制的门控机制改进长上下文推理，解决了MemAgent中内存爆炸和无退出机制的问题，实现了更稳定高效的推理性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理长上下文时存在性能下降问题。现有MemAgent方法采用RNN式循环处理上下文块并更新文本内存，但存在两个关键缺陷：1) 内存可能因无差别更新而爆炸式增长；2) 循环缺乏退出机制，即使收集到足够证据后仍进行不必要计算。

Method: 提出GRU-Mem方法，引入两个文本控制的门控机制：更新门控制内存更新时机，退出门控制循环退出时机。通过端到端强化学习训练，使用两个奖励信号r_update和r_exit分别奖励正确的更新和退出行为。
...
### [78] [Step 3.5 Flash: Open Frontier-Level Intelligence with 11B Active Parameters](https://arxiv.org/abs/2602.10604)
...
TL;DR: Step 3.5 Flash是一个稀疏专家混合模型，通过196B参数基础和11B活跃参数实现高效推理，在数学、代码和工具使用任务上达到前沿模型性能水平。


<details>
  <summary>Details</summary>
Motivation: 构建智能体时需要关注推理能力和执行效率，现有模型在计算效率和智能体性能之间存在平衡问题。本文旨在开发一个既能达到前沿智能水平又具有计算效率的智能体模型。

Method: 采用稀疏专家混合架构，结合3:1滑动窗口/全注意力交替机制和多令牌预测来降低延迟。设计了可扩展的强化学习框架，结合可验证信号和偏好反馈，支持大规模离策略训练下的稳定自改进。
...
Conclusion: Step 3.5 Flash通过重新定义效率边界，为在真实工业环境中部署复杂智能体提供了高密度基础，实现了前沿智能与计算效率的良好平衡。
...
### [79] [Online Causal Kalman Filtering for Stable and Effective Policy Optimization](https://arxiv.org/abs/2602.10609)
*Shuo He,Lang Feng,Xin Cheng,Lei Feng,Bo An*

Main category: cs.CL

TL;DR: KPO使用在线因果卡尔曼滤波来稳定大语言模型的强化学习，通过建模IS比率为跨token演化的潜在状态，解决了token级重要性采样比率的高方差问题


<details>
  <summary>Details</summary>
Motivation: 大语言模型的强化学习面临token级重要性采样比率高方差问题，现有方法要么使用固定的序列级IS比率，要么单独调整每个token的IS比率，忽略了序列中token间的时序离策略推导，导致训练不稳定

Method: 提出在线因果卡尔曼滤波策略优化(KPO)，将期望的IS比率建模为跨token演化的潜在状态，应用卡尔曼滤波器基于过去token的状态在线自回归地更新该状态，保留token级局部结构感知变化的同时平滑噪声尖峰

Result: 在具有挑战性的数学推理数据集上，KPO相比最先进的对比方法取得了优越的结果

Conclusion: KPO通过在线因果卡尔曼滤波有效解决了大语言模型强化学习中token级重要性采样比率的高方差问题，实现了更稳定和有效的策略更新
...
### [80] [UMEM: Unified Memory Extraction and Management Framework for Generalizable Memory](https://arxiv.org/abs/2602.10652)
*Yongshi Ye,Hui Jiang,Feihu Jiang,Tian Lan,Yichao Du,Biao Fu,Xiaodong Shi,Qianghuai Jia,Longyue Wang,Weihua Luo*

Main category: cs.CL

TL;DR: UMEM框架联合优化LLM智能体的记忆提取与管理，通过语义邻域建模和GRPO奖励机制提升记忆泛化能力，在多轮交互任务中显著优于基线方法
...
TL;DR: 研究发现基准测试性能主要受预训练数据与评估数据间词汇重叠度影响，而非模型规模或数据量本身


<details>
  <summary>Details</summary>
Motivation: 探究高质量预训练数据的构成要素，验证基准测试性能是否主要由预训练语料与评估数据集之间的统计模式重叠度驱动

Method: 使用词级单字交叉熵和词频统计测量重叠度，在10个零样本基准、4个预训练数据集（8.5B-60B tokens）和不同模型规模（400M-3B参数）上进行控制实验

Result: 词级单字交叉熵与基准性能呈稳健的负相关关系，表明常用基准测试受训练与评估数据间词汇重叠的强烈影响；具有相似词级单字交叉熵的更大预训练子集能改善下游结果

Conclusion: 许多标准基准测试相对于预训练语料库的分布偏移较弱，简单的词汇重叠统计就能预测基准性能
...
TL;DR: 该研究评估了Transformer语言模型在格鲁吉亚语分裂作格对齐上的表现，发现模型在作格标记上表现最差，与三种格形式在训练数据中的频率分布相关（主格 > 与格 > 作格）。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估Transformer语言模型在格鲁吉亚语这种罕见的分裂作格对齐系统上的表现，特别是主语和宾语的格标记问题。由于格鲁吉亚语是低资源语言，缺乏相关基准测试，需要开发系统性的评估方法。

Method: 采用基于树库的方法，使用Grew查询语言生成最小对比对。创建了包含370个句法测试的数据集，分为7个任务，每个任务50-70个样本。评估了5个编码器模型和2个解码器模型，使用词级和/或句子级准确率指标。

Result: 无论具体句法结构如何，模型在正确分配作格方面表现最差，在正确分配主格方面表现最好。性能与三种格形式（主格 > 与格 > 作格）的整体频率分布相关。作格的特定语法角色和训练数据缺乏可能是导致其表现不佳的原因。

Conclusion: 该研究为低资源语言的句法评估提供了有趣的方法论途径，公开了数据集。结果表明，语言模型在罕见语法现象上的表现受到训练数据频率分布的影响，特别是在作格这种特定语法角色的标记上。
...
TL;DR: LoCoMo-Plus是一个评估LLM在长对话中认知记忆能力的基准，关注隐含约束的保留与应用，而非表面事实回忆。


<details>
  <summary>Details</summary>
Motivation: 现有对话记忆基准主要关注表面事实回忆，但在真实对话中，恰当回应往往依赖于未明确查询的隐含约束（如用户状态、目标、价值观）。需要评估模型在语义断开线索下保留和应用潜在约束的能力。

Method: 提出LoCoMo-Plus基准，评估认知记忆在"线索-触发语义断开"场景下的表现；设计基于约束一致性的统一评估框架，替代传统的字符串匹配指标和显式任务类型提示。

Result: 实验涵盖多种骨干模型、基于检索的方法和记忆系统，结果显示认知记忆仍具挑战性，现有基准未能捕捉到的失败模式被揭示。

Conclusion: 认知记忆在长对话中仍面临挑战，需要超越表面事实回忆的评估方法；LoCoMo-Plus基准和约束一致性框架为这一方向提供了有效工具。
...
TL;DR: Macaron是一个多语言基准测试，通过100个语言无关模板分离推理类型和文化方面，包含11,862个实例，涵盖20个国家/文化背景、10种文字和20种语言，用于评估LLM在文化基础推理上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有多语言基准测试很少测试基于文化前提的推理：翻译的数据集保持英语中心场景，而文化优先的数据集往往缺乏对所需推理的控制。

Method: 使用100个语言无关模板覆盖7种推理类型和22个文化方面，由母语标注者创建场景对齐的英语和本地语言多选题，并系统推导出真/假问题。

Result: 在21个多语言LLM的零样本评估中，推理模式模型表现最强，英语和本地语言接近对等，而开源模型在本地语言中表现大幅下降，真/假任务常接近随机水平。文化基础的数学和计数模板始终最难。

Conclusion: Macaron基准测试填补了多语言文化基础推理评估的空白，揭示了当前LLM在跨文化推理方面的局限性，特别是开源模型在非英语语言和文化特定场景中的表现不足。
...
TL;DR: 本文提出RCPA方法，通过课程感知的渐进调制机制，在保持通用能力的同时增强视觉语言模型在专业领域的适应性。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在通用领域表现出色，但在医学影像、几何问题解决等专业领域表现不足。监督微调会导致灾难性遗忘，而持续预训练对视觉语言模型计算成本过高。现有强化学习方法在领域适应场景中容易优化崩溃，需要一种既能获取领域知识又能保持通用多模态能力的后训练适应方法。

Method: 提出强化课程预对齐（RCPA）方法，采用课程感知的渐进调制机制。早期阶段应用部分输出约束，安全地让模型接触新领域概念；随着模型对领域熟悉度增加，训练逐渐过渡到完全生成优化，精炼响应并与领域特定偏好对齐。

Result: 在多个专业领域和通用基准测试上的广泛实验验证了RCPA的有效性，为构建高性能和领域自适应的视觉语言模型提供了实用途径。

Conclusion: RCPA通过分阶段的适应过程，平衡了领域知识获取与通用多模态能力保持，解决了视觉语言模型在专业领域适应中的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型存在内容生成失真（幻觉）问题，核心原因是LLM缺乏对自身存储内部知识的认知，无法像人类一样表达超出知识边界的问题的知识状态。现有研究主要集中在白盒LLM，而适合仅提供API访问的黑盒LLM的方法尚未充分探索。

Method: 基于知识蒸馏框架设计深度学习模型，以黑盒LLM的输入问题、输出答案和token概率作为输入，构建输入与模型内部知识状态之间的映射关系，从而量化表达黑盒LLM的知识边界。针对不支持token概率访问的黑盒LLM，提出了自适应替代方法。

Result: 在多个公共数据集和主流黑盒LLM上的实验表明，LSCL能有效帮助黑盒LLM准确表达知识边界，在准确率和召回率等指标上显著优于现有基线模型。自适应替代方法的性能接近LSCL，也优于基线模型。

Conclusion: LSCL方法成功解决了黑盒LLM的知识边界表达问题，为减少LLM幻觉提供了有效解决方案，特别是在仅能通过API访问的黑盒LLM场景下具有重要应用价值。
...
TL;DR: 本文提出了一种新的度量标准Token Constraint Bound (δ_TCB)，用于量化大语言模型在保持主导下一个token预测不变的情况下所能承受的最大内部状态扰动，解决了传统指标无法评估LLM局部预测鲁棒性的问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然表现出令人印象深刻的能力，但对输入上下文的微小变化非常敏感，这影响了其可靠性。传统的准确率和困惑度等指标无法评估局部预测的鲁棒性，因为归一化的输出概率可能会掩盖LLM内部状态对扰动的潜在弹性。

Method: 提出了Token Constraint Bound (δ_TCB)这一新指标，它量化了LLM在主导下一个token预测发生显著变化之前所能承受的最大内部状态扰动。该指标与输出嵌入空间的几何结构内在相关，能够洞察模型内部预测承诺的稳定性。

Result: 实验表明，δ_TCB与有效的提示工程相关，并且在上下文学习和文本生成过程中，能够发现困惑度指标所遗漏的关键预测不稳定性。该指标为分析LLM预测的上下文稳定性提供了新的视角。

Conclusion: δ_TCB提供了一个原则性的、补充性的方法来分析和潜在改进LLM预测的上下文稳定性，为解决LLM对输入扰动敏感的问题提供了新的评估工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动提示优化方法常受到噪声和冲突更新信号的困扰，导致优化过程不稳定，需要一种更稳健的优化框架来提升大型语言模型的提示性能。

Method: 提出C-MOP框架，包含两个核心技术：1) 边界感知对比采样(BACS)，利用批次信息挖掘硬负样本、锚点和边界对来精确表征正负提示样本的典型表示和决策边界；2) 动量引导语义聚类(MGSC)，引入具有时间衰减的文本动量机制，从迭代中波动的梯度中提取持久共识。

Result: 在广泛实验中，C-MOP持续优于PromptWizard和ProTeGi等SOTA基线方法，平均提升分别为1.58%和3.35%。值得注意的是，C-MOP使仅激活3B参数的通用LLM能够超越70B参数的领域特定密集LLM。

Conclusion: C-MOP通过边界感知对比采样和动量引导语义聚类有效解决了提示优化中的噪声和冲突信号问题，实现了精确的提示演化，显著提升了LLM性能，证明了其在驱动精确提示进化方面的有效性。
...
TL;DR: LLMs在系统综述和元分析中的证据提取表现不佳，特别是在需要变量、角色、统计方法和效应量之间稳定绑定的复杂结构任务上，导致元分析关联元组提取可靠性接近零。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLMs）发展迅速，但尚不清楚它们是否能满足系统综述和元分析的结构要求，这些要求依赖于在文档间保持角色、方法和效应量归因，而不仅仅是识别孤立实体。

Method: 提出了一个结构化的诊断框架，将LLM证据提取评估为一系列模式约束查询的进展，具有递增的关系和数值复杂性。使用手动整理的跨五个科学领域的语料库，以及统一的查询套件和评估协议，评估了两种最先进的LLM在单文档和长上下文多文档输入机制下的表现。
...
Method: 通过建立三个互相关联的任务：金融考试问答、多语言金融问答(PolyFiQA)和金融决策制定，构建一个全面的评估套件，衡量模型在不同语言和模态下的推理、泛化和行动能力。

Result: 该实验室提供了首个多语言多模态金融LLM评估框架，数据集和评估资源将公开发布以支持可重复研究。

Conclusion: FinMMEval 2026旨在促进稳健、透明和全球包容的金融AI系统发展，通过全面的多语言多模态评估推动金融自然语言处理领域的进步。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理大规模自然语言语料库时搜索速度较慢，特别是在处理语义变化的查询时面临组合爆炸问题。需要一种能够快速搜索万亿级语料库同时处理语义变化的方法。

Method: 基于后缀数组的字符串匹配方法，采用两个关键算法思想：1）通过磁盘感知设计实现快速精确查找；2）动态语料感知剪枝。利用自然语言的统计特性来抑制搜索空间相对于查询长度的指数增长。

Result: 在FineWeb-Edu（1.4T tokens）实验中，该方法比现有方法（infini-gram、infini-gram mini、SoftMatcha）显著降低搜索延迟。作为实际应用，成功识别了现有方法未发现的训练语料库中的基准污染。

Conclusion: 提出了一种超快速、可扩展的搜索算法，能够高效处理万亿级自然语言语料库的语义搜索，在速度和准确性方面优于现有方法，并提供了七种语言的在线演示。
...
TL;DR: 该研究通过整合现象学访谈和计算方法，发现自闭症个体在时间体验上的核心挑战主要是生活不可预测性，而非叙事建构问题。


<details>
  <summary>Details</summary>
Motivation: 自闭症的时间性障碍（如与社会环境不同步及其不可预测性）被认为是自闭症的核心特征，对人际关系有深远影响。但现有研究存在三个局限：1）以缺陷为基础的医学模型主导；2）质性研究样本量小；3）计算研究缺乏现象学基础。本研究旨在弥合现象学与计算方法之间的差距，并克服样本量限制。

Method: 整合三种方法：研究A：使用跨诊断时间体验评估对自闭症个体进行结构化现象学访谈；研究B：对为此目的构建的自闭症自传叙事语料库进行计算分析；研究C：使用叙事流测量复制计算研究，评估自闭症自传的现象学真实性感知。

Result: 访谈显示，自闭症组与对照组最显著的差异在于体验的不可预测性。计算结果与此一致：自闭症叙事中的时间词汇显著更负面，特别是"即时性与突然性"类别。离群值分析发现与感知不连续性相关的术语（不可预测地、突然地、急剧地）高度负面。叙事流计算分析发现，语料库中的自闭症叙事在数量上更类似于自传故事而非虚构故事。

Conclusion: 自闭症个体经历的时间挑战主要涉及生活不可预测性，源于生活体验的内容，而非自闭症叙事建构本身。
...
Method: 提出LoRA-Squeeze方法：1）使用较高源秩进行微调；2）重建或高效近似完整权重更新矩阵；3）使用随机奇异值分解(RSVD)创建压缩后的低目标秩LoRA模块。包括后处理压缩和训练中动态秩退火两种变体

Result: 在13个文本和10个视觉语言任务上的实验表明：后处理压缩通常能产生优于直接训练目标秩的适配器，特别是在目标秩上进行少量微调步骤时。训练中秩退火变体始终实现最佳的LoRA大小-性能权衡

Conclusion: LoRA-Squeeze通过先学习高秩表达再压缩的方法，有效解决了标准LoRA的秩选择挑战，提供了更好的性能-效率权衡，简化了参数高效微调过程
...
TL;DR: 该研究使用三种语言表征分析痴呆症患者的自发语言，发现句法和语法特征在缺乏词汇内容时仍能有效区分认知衰退，支持基于语言特征的透明认知筛查方法。
...
Method: 使用DementiaBank Pitt Corpus的自发语言转录，采用三种语言表征：原始清理文本、结合词汇和语法信息的词性增强表征、以及纯词性句法表征。使用逻辑回归和随机森林模型，在两种协议下评估：转录级训练-测试分割和受试者级五折交叉验证。通过全局特征重要性和Mann-Whitney U检验与Cliff's delta效应量进行可解释性分析和统计验证。

Result: 所有表征下模型表现稳定，句法和语法特征在缺乏词汇内容时仍保持强区分能力。受试者级评估结果更保守但一致，特别是词性增强和纯词性表征。统计分析显示功能词使用、词汇多样性、句子结构和话语连贯性存在显著组间差异，与机器学习特征重要性结果高度一致。
...
TL;DR: 提出一种基于梯度优化的方法，通过将语言模型视为词元分布序列的函数，实现从目标输出反推最优输入提示


<details>
  <summary>Details</summary>
Motivation: 尽管语言模型研究日益增多，但很少有方法分析语言模型的可逆性。给定语言模型和期望的目标输出序列，确定能够产生该目标输出的输入提示仍然是一个开放问题

Method: 将问题表述为经典的基于梯度的优化问题。首先提出一个简单算法实现给定（冻结）语言模型的端到端可微性，然后通过梯度下降找到优化的提示。核心洞察是将语言模型视为在词元分布序列上操作的函数，而不是传统的词元序列函数

Result: 实验和消融研究表明，DLM驱动的反转方法能够可靠且高效地优化长度为10和80的提示，针对长度为20的目标，适用于多个白盒语言模型（开箱即用）

Conclusion: 该方法成功解决了语言模型可逆性问题，通过梯度优化实现了从目标输出反推输入提示的有效解决方案
...
TL;DR: 将嵌入反演重新定义为条件掩码扩散过程，通过迭代去噪并行恢复所有token，而非顺序自回归生成


<details>
  <summary>Details</summary>
Motivation: 传统嵌入反演方法通常采用顺序自回归生成，效率较低。本文旨在开发一种更高效的并行方法，通过条件掩码扩散模型直接从目标嵌入恢复原始文本

Method: 提出条件掩码扩散语言模型，通过自适应层归一化将目标嵌入作为条件，仅需8次前向传播通过7800万参数模型，无需访问目标编码器

Result: 在三个嵌入模型的32-token序列上，该方法达到81.3%的token准确率和0.87的余弦相似度

Conclusion: 条件掩码扩散为嵌入反演提供了一种高效的并行方法，显著优于传统顺序生成方法，在保持高质量的同时大幅减少计算开销
...
TL;DR: 论文提出了一种基于多级感知和图思维（GoT）的框架，用于建模全双工会话中的意图-行为路径，通过分层标注方案预测交流意图和言语行为，并在合成和真实对话数据上验证了其有效性。
...
Method: 1. 提出多级感知框架，将对话过程建模为分层标注方案，预测高层交流意图和低层言语行为；2. 开发高质量语料库，将可控、事件丰富的对话数据与人工标注标签配对；3. 采用图思维（GoT）框架，将流式预测结构化为演化图，使Transformer能够预测下一个言语行为、生成决策理由并动态优化推理。

Result: 在合成和真实全双工会话上的实验表明：1. 框架实现了稳健的行为检测；2. 产生了可解释的推理链；3. 为全双工口语对话系统中的会话推理基准测试奠定了基础。

Conclusion: 该框架成功建模了对话中的意图-行为路径，通过分层感知和图思维结构实现了可解释的会话推理，为构建更自然的全双工交互系统提供了理论基础和实践工具。
...
TL;DR: Hibiki-Zero是一种无需词级对齐的端到端语音翻译系统，通过强化学习优化延迟，在多种语言上实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 传统语音翻译方法依赖词级对齐数据，这种数据难以大规模收集，且需要语言特定的启发式对齐方法，限制了系统的可扩展性和性能

Method: 首先在句子级对齐数据上训练高延迟语音翻译，然后使用GRPO强化学习策略优化延迟同时保持翻译质量，完全消除词级对齐需求

Result: 在五个X到英语任务中实现翻译准确性、延迟、语音传递和自然度的SOTA性能，且仅需少于1000小时语音数据即可适应新输入语言
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在需要严格形式规则、精确术语和法律约束结构的领域表现不佳，税法领域特别具有挑战性，需要精确的法条引用、结构化法律论证和数值准确性。

Method: 1) 算法生成SteuerEx基准测试：包含115个专家验证的德国大学税法考试问题，涵盖六个核心税法领域和多个学术级别，采用语句级部分评分框架；2) 开发SteuerLLM：基于大规模合成数据集训练，使用受控检索增强管道从真实考试材料生成。

Result: SteuerLLM（280亿参数）在SteuerEx基准测试中持续优于同等规模的通用指令调优模型，在某些情况下甚至优于参数规模大得多的系统，表明领域特定数据和架构适应比参数规模对现实法律推理任务性能更具决定性。

Conclusion: 领域特定数据和架构适应对于法律推理任务比模型参数规模更重要，研究提供了完整的开放资源（基准数据、训练数据集、模型权重和评估代码）以支持领域特定法律人工智能的可重复研究。
...
TL;DR: 本文介绍了MisAlign-Profile基准测试，用于系统评估大语言模型在安全、价值和文化的多维度错位权衡问题，构建了包含112个规范领域的MISALIGNTRADE数据集。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试（如SAFETUNEBED、VALUEBENCH、WORLDVIEW-BENCH）主要孤立评估安全、价值和文化维度，无法系统分析这些维度间的交互和权衡关系，导致对实际场景中多维度同时出现时的模型行为理解有限。

Method: 1. 构建MISALIGNTRADE数据集：包含112个规范领域（14个安全、56个价值、42个文化），每个提示按三种正交语义类型分类（对象、属性、关系错位）；2. 使用Gemma-2-9B-it和Qwen3-30B-A3B-Instruct-2507进行扩展和去重；3. 通过两阶段拒绝采样为每个提示配对错位和对齐响应；4. 在通用、微调和开源权重LLMs上进行基准测试。

Result: 基准测试显示，不同LLMs在跨维度错位权衡方面存在12%-34%的错位率，揭示了模型在同时满足安全、价值和文化维度时的系统性挑战。

Conclusion: MisAlign-Profile基准测试填补了现有评估方法的空白，能够系统量化LLMs在多维度规范约束下的错位权衡，为理解和改进模型对齐提供了重要工具。
...
TL;DR: SafeThink是一种轻量级推理时防御方法，通过监控推理轨迹并在安全阈值被违反时注入优化前缀来恢复多模态大模型的安全性，同时保持推理性能。


<details>
  <summary>Details</summary>
Motivation: 基于强化学习的后训练（如GRPO）虽然提升了多模态大模型的推理能力，但会同时降低安全对齐性并增加越狱成功率，需要一种能在推理时恢复安全性的方法。

Method: 提出SafeThink方法：1）使用安全奖励模型监控推理轨迹；2）当安全阈值被违反时，注入优化的短前缀（"Wait, think safely"）；3）将安全恢复视为满足约束而非最大化目标。

Result: 在6个开源MLRM和4个越狱基准测试中，SafeThink将攻击成功率降低30-60%（如LlamaV-o1从63.33%降至5.74%），同时保持推理性能（MathVista准确率从65.20%降至65.00%）。

Conclusion: SafeThink是一种有效的推理时安全防御方法，实验发现安全恢复通常只需在前1-3个推理步骤中进行干预即可将生成重定向到安全完成。
...
TL;DR: 提出TEG方法，通过图结构表示文本并结合外部知识库来提升虚假信息检测性能，并进一步扩展为TEGRA框架整合领域特定知识。


<details>
  <summary>Details</summary>
Motivation: 虚假信息检测任务可以像人工事实核查一样从外部知识整合中显著受益，但现有方法在有效结合文本和结构化知识方面存在不足。

Method: 提出TEG方法：从文本中提取图结构信息，同时对文本和图进行编码用于分类。进一步扩展为TEGRA框架，整合领域特定知识。

Result: 实验表明，这种混合表示相比单独使用语言模型能提升虚假信息检测性能，TEGRA框架在大多数情况下能进一步提高分类准确率。

Conclusion: 结合文本和图结构的混合表示方法能有效提升虚假信息检测性能，整合领域特定知识能进一步增强分类效果。
...
TL;DR: 研究发现，在固定更新预算下，对思维链数据进行监督微调时，使用更少样本进行多轮训练（重复训练）比使用更多样本进行单轮训练效果更好，这挑战了传统机器学习中"更多训练样本带来更好泛化"的直觉。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习直觉认为，使用更多独特的训练样本会带来更好的泛化能力。然而，作者发现对于推理语言模型的思维链监督微调，这种直觉可能不成立，因此探索了重复训练对模型性能的影响。

Method: 在固定更新预算下，比较了两种训练策略：1）使用大量样本进行单轮训练；2）使用少量样本进行多轮重复训练。使用Olmo3-7B模型，在AIME'24/25和GPQA基准上测试，通过训练token准确率作为停止标准来监测训练饱和点。

Result: 在400个样本上训练128轮的表现优于在51200个样本上训练1轮，性能提升12-26个百分点，且没有额外的灾难性遗忘。研究发现，当token准确率达到完全记忆时，额外训练轮次的改进会达到平台期。

Conclusion: 重复训练优势为推理SFT提供了实用方法：通过扩展训练轮次并使用token准确率作为停止标准，可以替代昂贵的无方向数据扩展。完全记忆与改进泛化同时出现的现象，为理解大语言模型训练动态提出了新的开放性问题。
