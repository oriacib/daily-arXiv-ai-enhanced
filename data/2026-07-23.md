...
TL;DR: 本文提出将AI生成视频检测从离线逐帧分析重构为在线流式检测，通过直接解析码流中的运动场替代像素级解码，实现了在极低计算成本下的实时检测，并提供了统计保证和计算预算与决策延迟之间的权衡前沿。


<details>
  <summary>Details</summary>
Motivation: 现有AI视频检测器在离线环境下评估（解码像素后评分），但实际部署是在线的流式场景。这种离线评估与在线部署之间的不匹配导致计算开销巨大且无法满足实时性需求。

Method: 将检测任务重构为流式感知问题，直接解析编解码器已写入比特流的运动场（而非解码像素）；利用单调递增的聚合分数实现任意时间有效的单阈值校准；将计算预算映射为延迟窗口，形成单调前沿。

Result: 在GenVidBench上，编解码阶段以比像素CNN低五个数量级的计算量（CPU上）达到AUC 0.64；延迟15%片段可提升准确率0.75→0.78，同时减少7倍计算量（McNemar p<10^{-6}）；在AIGVDBench上复现了阶段排序结果。

Conclusion: 本文的贡献不在于提出新的检测器，而在于问题重构、两个统计保证以及实测的前沿曲线；该方法在保持可控假阳性率的同时显著降低了计算开销。
...
TL;DR: Crowd4D是首个从单目RGB视频中联合优化人群与场景的大规模4D人群重建框架，通过引入HSIP中间表征（包含SIPC和SIS）和CSCR时空正则化，有效解决了深度模糊、场景几何复杂及人-场景对齐等难题，在复杂大场景中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有单目人群重建方法多依赖单平面假设，导致在大尺度场景中因深度模糊和复杂地形而产生不可靠的度量尺度和空间漂移，且人与场景的重建通常是解耦的，缺乏精确的人-场景对齐。

Method: 提出Crowd4D框架，包含：1) 多阶段优化策略联合优化人群与场景；2) HSIP中间表征（由SIPC和SIS组成），编码场景感知几何先验并重新定义优化空间；3) CSCR正则化，利用HSIP空间先验对局部人群施加时序一致性约束。

Result: 大量实验表明Crowd4D在复杂大规模真实场景中一致优于现有最先进方法，实现了鲁棒的单目4D人群重建。

Conclusion: Crowd4D通过显式引入场景几何和创新的HSIP、CSCR机制，有效解决了单目大尺度场景下4D人群重建的关键瓶颈，为未来的场景感知人群建模提供了新思路。
...
TL;DR: 本文提出D3VL，一种集成2D和3D时序数据的多模态大语言模型框架，用于自动驾驶场景理解与问答任务，在KITTI QA数据集上相比基线方法提升11%，并扩展了Waymo QA数据集以评估3D时序数据处理能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在自动驾驶中主要使用2D图像和视频，忽略了3D传感器（如LiDAR）数据的整合，而LiDAR在传统3D场景感知中已被证明有效。本文旨在填补这一空白，将3D数据融入MLLM框架。

Method: 提出D3VL框架，采用简洁的单一架构集成2D和3D时序数据，处理LiDAR点云稀疏性和非网格结构的挑战，并实现相机与LiDAR数据的融合。

Result: 在KITTI QA数据集上，D3VL相比基线方法提升11%的性能；同时扩展了Waymo QA数据集，用于评估模型在多样化驾驶条件下对3D时序数据的处理能力。

Conclusion: D3VL成功将3D LiDAR数据融入MLLM框架，验证了3D传感器数据在自动驾驶场景理解中的有效性，为多模态自动驾驶大模型提供了新思路。
...
TL;DR: 本文研究了在垃圾箱摄像头视角下，使用合成图像和派生图像增强YOLOv8n探测器性能的效果。实验结果表明，任何增强配置均未超过仅使用真实图像训练的基线模型（mAP@0.5=0.691）。背景替换和全增强池反而降低了性能，手部前景合成也未带来可靠提升。研究指出，31张测试集样本量过小，无法得出稳健结论。


<details>
  <summary>Details</summary>
Motivation: 校园垃圾回收中，错误丢弃会污染回收流。在垃圾箱上安装摄像头可以实时反馈丢弃物品是否分类正确，因此需要高效的物品检测模型。然而，真实标注数据获取成本高，作者尝试利用合成图像和派生图像增强检测器性能，以解决数据稀缺问题。

Method: 使用YOLOv8n作为基础检测器；构建包含148张校园照片的真实数据集（86训练/31验证/31测试）。设计了12种联合训练配置，控制添加图像的数量和来源；对7种主要设置使用4个随机种子重复实验，通过bootstrap百分位区间量化种子差异。还测试了手部和前臂复合图像（因真实照片中均显示手持物品），并修正了训练集/测试集泄露问题后重新实验。

Result: 仅真实图像模型mAP@0.5=0.691 [0.665, 0.722]；背景替换降至0.560 [0.499, 0.619]；孤立物体图像为0.680 [0.644, 0.724]；全增强池仅为0.487 [0.438, 0.537]。修正后的手部复合图像配对差值为+0.034 [-0.063, 0.199]，无可靠提升。单种子迁移实验中，联合训练与顺序预训练排名依赖图像来源。

Conclusion: 所有增强配置均未超越真实数据基线，说明当前合成图像策略无法有效提升垃圾箱视角下的目标检测性能。测试集仅有31张图像，样本量过小，类级别的结论不可靠。未来需要更大规模的真实数据集和更精细的合成数据生成方法。
...
TL;DR: ChronoStitch提出了一种无需训练的方法，通过全局三维多模态RoPE坐标系统重新编码独立缓存的视觉KV记忆，并选择性重计算少量偏差大的标记，解决了长视频问答中独立缓存块因位置编码冲突导致的时序错乱问题，在TempCompass上性能优于朴素拼接，速度比全量重新编码快3.3倍。


<details>
  <summary>Details</summary>
Motivation: 长视频问答需要在查询时保留视觉证据而不反复处理整个视频。现有方法虽然可以独立缓存每个视频块的KV状态，但独立缓存块在拼接时因局部旋转位置编码从零开始，导致时序相位冲突、全局顺序丢失，无法正确回答关于事件先后、频率或变化的问题。

Method: ChronoStitch包含两步：1) 将存储的后旋转密钥重新映射到全局三维多模态RoPE坐标系（保留时间、高度、宽度结构），并指出一维标量重索引对视觉token在几何上不一致；2) 选择性重计算后块中高偏差的少量视觉token，让它们能关注到组合后的完整缓存。

Result: 在Qwen2.5-VL-3B和TempCompass时序分割上，ChronoStitch在事件排序准确性上优于朴素拼接和仅位置修正的变体，运行速度比全量联合重预填充快3.3倍。

Conclusion: ChronoStitch证明了通过全局三维位置重基线+选择性重计算，可以在不重新训练的情况下有效组合独立缓存的视觉KV记忆，为长视频问答提供了一种高效且准确的解决方案。
...
TL;DR: 本文提出VQ-Transplant框架，可将新型VQ模块即插即用地集成到冻结的预训练分词器中，只需轻量级解码器适配（在ImageNet-1k上训练5个epoch），即可在VAR等工业级模型上实现接近最先进的重建保真度，同时降低95%的训练成本。


<details>
  <summary>Details</summary>
Motivation: 现有VQ模型训练量化模块需要大量计算资源，限制了资源受限条件下新型VQ技术的开发。

Method: 提出VQ-Transplant框架，通过替换预训练分词器中原有的VQ模块，冻结编码器-解码器参数，仅引入轻量级解码器适配策略（5个epoch训练）来对齐特征先验与新量化空间。

Result: 在VAR等工业级模型上，VQ-Transplant接近最先进的重建保真度，训练成本降低95%。

Conclusion: VQ-Transplant通过资源高效的方式集成新型VQ技术，降低了量化研究门槛，使资源受限场景也能匹敌工业级重建性能。
...
TL;DR: 本文提出EGRNet，一种用于城市场景实时语义分割的轻量级深度学习模型，通过深度可分离卷积、膨胀残差块、边缘门控精化模块和SE注意力机制，以0.46M参数量在Cityscapes上达到65.28% mIoU，并引入轻量级对抗攻击检测策略，适用于边缘设备部署。


<details>
  <summary>Details</summary>
Motivation: 现有语义分割模型难以在保持高精度的同时实现低计算开销，无法满足自动驾驶和智慧城市中实时性与资源受限场景的需求。

Method: 采用深度可分离卷积降低计算复杂度，使用膨胀残差块捕获多尺度上下文信息；提出Edge-Gated Refinement (EGR)模块通过可学习门控机制自适应融合原始与精化特征以增强边界保留；引入Squeeze-and-Excitation (SE)注意力机制提升特征表示，并设计轻量级对抗攻击检测策略。

Result: EGRNet仅0.46M参数量，在Cityscapes数据集上达到65.28% mIoU，实现了低资源消耗下的高准确率，并具备对抗鲁棒性。
...
TL;DR: 本文提出将病理学家视觉注意力引入报告生成模型训练，通过收集包含多尺度视口轨迹、语音描述和光标移动的121例前列腺WSI数据集，采用注意力对齐损失正则化模型注意力分布，在报告生成和VQA任务上取得NLP指标平均提升10.9%、临床相关组件准确率提升19.3%的效果。


<details>
  <summary>Details</summary>
Motivation: 病理学家在癌症诊断中的视觉注意力是高度选择性的过程，能提供诊断相关区域的强语义线索，但现有报告生成模型未充分利用这一人类注意力信息。

Method: 收集121例前列腺WSI的多模态人类注意力数据集（含病理学家多尺度视口轨迹、语音描述和光标移动），通过注意力对齐损失正则化模型在图像块上的注意力分布，使其匹配病理学家注意力分布，微调两种不同内部注意力机制的报告生成模型。

Result: NLP指标平均提升10.9%，五个临床相关报告组件准确率提升19.3%；推理时提取的模型注意力图与病理学家注意力对齐更紧密，为生成报告提供更强的视觉支持。

Conclusion: 引入人类注意力对齐训练能显著提升病理报告生成质量，且计算开销小，为可解释的AI辅助诊断提供了新范式。
...
TL;DR: 本文研究了超声心动图左心室分割在跨数据集迁移中的性能退化问题，发现几何感知预处理可显著改善迁移效果，亮度对比度并非主要域漂移因素，并提出了基于不同特征表示的迁移风险预测方法，R²最高达0.84。


<details>
  <summary>Details</summary>
Motivation: 超声心动图左心室分割模型在临床部署中面临跨数据集泛化困难，但域漂移的具体来源尚未被充分解耦和分析。

Method: 使用手工设计的超声描述符、VAE潜在特征和分割导出的潜在特征，结合几何感知预处理、强度z归一化等方法，在六个超声心动图数据集上分析域漂移，并用CMD、log-Wasserstein、log-MMD等差异度量预测分割性能下降。

Result: 几何感知预处理显著改善迁移；强度归一化对数据集可分性影响极小（<0.005）；绝对Dice下降预测R²=0.612，MAE=0.082；不同表示下最优差异度量不同，最高R²达0.84（log-MMD在LV分割潜在特征上）。

Conclusion: 超声心动图域漂移是有结构且可测量的，可通过几何感知预处理部分缓解，并利用表示特定的迁移风险评估来预测分割性能退化。
...
TL;DR: 提出一个统一变分框架，用于稀疏像素级监督下的图像分割，基于单纯形约束的Potts模型与光滑周长正则化，得到凸且光滑的能量泛函，可作为弱监督深度学习训练损失或通过迭代方法高效优化。


<details>
  <summary>Details</summary>
Motivation: 解决稀疏像素级标签下图像分割的挑战，将变分法与弱监督深度学习相结合，避免对完整真实分割图像的依赖。

Method: 采用单纯形约束Potts模型与光滑周长正则化构建凸光滑能量泛函；通过RKHS函数扩展问题构造模糊隶属度函数以捕获非均匀强度统计，并将稀疏标签融入数据保真项。

Result: 导出的离散损失在标准网络训练中表现出鲁棒性，相比非训练方法和部分交叉熵基线有一致改进，无需真实分割图像即可达到可比性能。

Conclusion: 该统一框架有效连接了变分法与弱监督深度学习，在稀疏标签条件下实现高效且鲁棒的图像分割。
...
TL;DR: 本文提出了PhenSPINE数据集（含16813张MRI图像），并构建了结合位置编码的脊柱诊断基准，发现Sagittal T2序列表现最优（Macro F1=50.31%），多序列融合效果反而更差。


<details>
  <summary>Details</summary>
Motivation: 目前脊柱病理自动诊断缺乏多样化、高质量的基准数据集，现有系统受限于数据不足。

Method: 构建PhenSPINE MRI数据集，集成先进卷积骨干网络与位置编码机制以建模椎间盘的解剖上下文信息，在四种标准MRI序列上进行评估。

Result: Sagittal T2加权序列取得最佳诊断效果（Macro F1-score 50.31%）；多序列融合策略性能低于单序列基线，因图像受周围解剖区域噪声干扰严重。
...
TL;DR: SafeGen是一个基于目标条件扩散框架的安全关键场景生成方法，用于评估视觉语言模型驱动的自动驾驶系统（VLMADs）在与弱势道路使用者交互时的安全性，通过在真实世界场景中生成高保真、时间连贯的危险视频轨迹，显著提升了对VLMADs的评估效果和实际驾驶性能。


<details>
  <summary>Details</summary>
Motivation: 现有安全关键场景生成方法主要依赖模拟器，存在严重的仿真到现实差距，难以捕获真实、多样且不可预见的人-车交互动态，而VLMADs在部署中迫切需要针对弱势道路使用者等安全关键场景的严格评估。

Method: 将场景生成建模为目标条件扩散过程，以预定义的灾难性终态作为强监督信号；引入上下文接地终态推理（使用VLM分析良性驾驶上下文并推断潜在漏洞）、终态条件视频演化（通过深度感知几何投影实例化高风险代理，结合边界条件扩散生成中间帧）。

Result: 在3个VLMAD基准上，SafeGen将评判总体得分（评估VLMAD理解与决策）平均提升24.25%；在真实驾驶场景上微调VLMAD后，性能平均提升15.9%。

Conclusion: SafeGen有效弥合了仿真与现实之间的差距，能够生成物理逼真、时间连贯的安全关键场景，为VLMADs的安全性评估与性能提升提供了有力工具。
...
TL;DR: PSFT提出了一种基于点选择的高效微调框架，通过保留低影响力点、生成提示令牌和轻量特征过滤，在保持参数效率的同时显著提升了3D点云模型在噪声和损坏场景下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有3D预训练模型在噪声和损坏点云上表现不佳：全微调会放大离群点影响并覆盖预训练鲁棒先验，而参数高效微调对损坏令牌仍然敏感。

Method: 1) 从预池化特征估计逐点影响力，自适应保留低影响力点以抑制离群点；2) 基于所选子集生成逐层提示令牌注入冻结骨干网络；3) 附加轻量特征过滤器，包含瓶颈MLP变换和Beta门控残差混合， refined patch-token表示。

Result: 在ModelNet-C和ModelNet40-C上一致降低所有测试3D预训练骨干的损坏误差；在ScanObjectNN-C上使用ULIP-2和Uni3D-B取得最强结果。

Conclusion: PSFT在参数高效前提下有效提升了3D点云模型对噪声和损坏的鲁棒性，是一种通用且有效的微调框架。
...
TL;DR: 本文提出了一种统一的标记化框架，用于处理3D疼痛识别中的异质模态数据（面部视频和fNIRS），无需为每种模态设计单独的架构，在AI4Pain基准上实现了最先进性能，并支持GPU/CPU上的实时评估。


<details>
  <summary>Details</summary>
Motivation: 疼痛是一种普遍且复杂的现象，准确评估对临床管理和干预至关重要。现有计算疼痛识别系统通常需要为不同模态数据设计单独的架构或手工特征，缺乏统一的处理流程。

Method: 提出一种统一的标记化框架，将异质3D模态（行为数据和脑活动数据）映射到共享的标记空间，保留空间、时间和时频结构，支持单个处理流水线处理面部视频和fNIRS数据的原始信号及频谱图表示。

Result: 在AI4Pain基准数据集上实现了最先进的性能，同时保持高计算效率，支持在GPU和CPU硬件上进行实时评估。

Conclusion: 该统一框架有效处理了多种3D疼痛相关数据，无需手工程式化偏置或独立架构，为疼痛识别提供了一种高效、实时的通用解决方案。
...
TL;DR: 本文提出ReFace，通过将面部图像划分为四个空间象限进行标记化处理，在AI4Pain数据集上以视频输入达到56.00%的准确率，在固定协议下取得最高报告准确率。四象限配置在总像素预算不变的情况下优于全脸输入，单象限处理更以四分之一像素量保持竞争力。


<details>
  <summary>Details</summary>
Motivation: 面部疼痛相关线索具有空间异质性，现有方法将整个面部作为单一区域处理，难以有效捕捉局部疼痛特征，因此需要一种空间重组策略来提升自动疼痛评估性能。

Method: 提出ReFace框架，在标记化之前将面部输入划分为四个空间象限，而非将整个面部作为单一区域处理。该方法在AI4Pain数据集上评估，仅使用视频模态。

Result: 在AI4Pain测试集上达到56.00%准确率，在固定基准协议下对比方法中取得最高准确率。四象限配置在相同像素预算下优于全脸输入。单象限处理仅用四分之一像素仍具竞争力。

Conclusion: 空间重组（特别是四象限划分）能够在不增加计算成本的前提下提升疼痛评估性能，验证了空间结构化标记化设计的有效性。
...
TL;DR: 本文系统比较了传统特征工程与深度序列学习方法在基于外周生理信号的三分类疼痛定位任务上的表现。在AI4Pain 2026挑战赛数据集上，极随机树模型取得了最优宏F1=0.539，比最佳深度模型高出7.4个百分点，EDA频谱特征是最具区分度的特征。研究还发现疼痛检测（F1=0.815）与定位（F1=0.552）之间存在26个百分点的稳定差距，提示10秒分辨率下外周自主神经通路的解剖弥散性构成了根本性的性能上限。


<details>
  <summary>Details</summary>
Motivation: 疼痛的自动定位（即无需患者自我报告，仅从外周生理信号识别疼痛的解剖起源）对于非语言患者群体具有重要的临床价值，但目前这一问题尚未得到充分研究。

Method: 在包含65名受试者、四种可穿戴同步记录模态（EDA、BVP、呼吸、SpO2）的AI4Pain 2026挑战赛数据集上，对115维手工特征集（涵盖时域、频域、模态特异和跨模态描述符）与端到端深度架构进行了系统性基准比较。

Result: 极随机树模型取得最优宏F1=0.539，比最佳深度模型高7.4个百分点；EDA频谱特征为最具区分度的特征；疼痛检测（F1=0.815）与定位（F1=0.552）之间存在约26个百分点的稳定性能差距。

Conclusion: 在10秒时间分辨率下，外周自主神经通路的解剖弥散性构成了疼痛定位的根本性性能上限，传统特征工程方法在此任务上仍优于深度学习方法。
...
TL;DR: 本文提出FluxGraph，通过利用SAM2的多掩模不一致性作为轻量级触发器和减少全场景追踪，相比TubeletGraph在多个数据集上实现3.3-10.7倍加速，同时保持或提升追踪性能。


<details>
  <summary>Details</summary>
Motivation: 现有物体状态跟踪方法（如TubeletGraph）计算成本高，推理时间长达每物体帧4.4秒，无法实时部署。其开销源于对每一帧都进行密集实体分割，且追踪场景中所有实体，成本与场景复杂度而非感兴趣的状态变换数量成正比。

Method: 提出FluxGraph，利用SAM2内部的多掩模不一致性作为轻量级触发器来检测状态变换，仅在有变换发生时进行分割；同时避免追踪视频中所有实体，仅关注感兴趣的状态变化，从而降低计算开销。

Result: 在VOST上比TubeletGraph快约3.3倍，同时提升追踪性能并保持状态图质量；在VSCOS、M³-VOS和DAVIS17上实现3.7-10.7倍的加速，且性能保持不变。

Conclusion: FluxGraph通过反应式触发和选择性追踪，在不牺牲性能的前提下大幅提升了状态跟踪效率，为实时部署提供了可能。
...
TL;DR: 本文发现大型视图合成模型学习到的空间对应关系能够泛化到非外观信号上，并首次将其扩展到3D场景理解，提出了一种无需3D重建或分割训练的泛光分割流程，在ScanNet和Replica上取得了与基于高斯的方法相当或更优的效果。


<details>
  <summary>Details</summary>
Motivation: 现有大型视图合成模型通过RGB监督学习了准确的空间对应关系，但尚未被利用于3D场景理解。作者观察到这种对应关系可以传播与外观无关的逐像素标签，因此希望将其扩展到语义分割任务中。

Method: 提出一种泛光分割流程，保持视图合成模型冻结不变，将输入视图上的泛光标签编码为二进制通道表示，直接输入模型渲染目标视图的分割结果，无需3D重建或分割任务特定训练。

Result: 在ScanNet上，分割质量与基于高斯的方法持平，在新视图合成上PSNR高出7 dB以上；在Replica上无需微调即超越对比方法。

Conclusion: 大型视图合成模型学习的空间对应关系可以泛化用于传播非外观标签，首次将其从外观渲染扩展到3D场景理解，证明了跨数据集迁移的泛化能力。
...
TL;DR: 本文对GHSL、TEMPO、GBA和Overture四种全球建筑物栅格数据集进行了全面公平的精度评估，以ORBITaL-Net人工标注数据为基准，发现GBA和TEMPO整体精度最高，但所有产品在非洲、亚洲以及高密度城市区域精度显著下降。


<details>
  <summary>Details</summary>
Motivation: 当前存在多个全球建筑物栅格数据集，但缺乏独立、全面且公平的精度评估，因此需要对这些产品进行系统比较。

Method: 以ORBITaL-Net全球多样化人工标注建筑物足迹为基准，在多种空间分辨率网格上评估GHSL、TEMPO、GBA和Overture四种产品，并采用多个传统性能指标，同时按地理位置、人口密度和收入组进行分层分析。

Result: GBA或TEMPO总体精度最高（取决于评价标准）；所有产品在非洲和亚洲精度显著较低；多数产品在高密度城市区域精度下降明显。

Conclusion: 现有全球建筑物数据集存在地域和密度相关的精度偏差，需要进一步改进模型以提升低精度区域的性能。
...
TL;DR: 本文构建了DRGBT-1K基准数据集，用于评估动态RGB-T多模态跟踪方法，包含1045个真实场景序列和795K帧对，并建立了统一的评估平台和排行榜。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法系统评估跟踪器在真实动态模态变化和跨平台转换下的鲁棒性，因此需要构建能够反映真实世界协作感知系统特点的大规模高质量基准。

Method: 构建DRGBT-1K数据集（1045个真实场景序列、795K RGBT帧对，涵盖多种场景、视角变化、模态变化和目标外观不连续性），提供精细标注（密集边界框、类别标签、挑战属性、模态和平台标签），在统一协议下评估20种代表性多模态跟踪方法，并发布未对齐版本和在线评估平台。

Result: 建立了包含24个目标类别、15+场景类型、15+挑战属性的基准，评估了20种代表性方法，发布了在线评估平台和排行榜。

Conclusion: DRGBT-1K填补了动态RGBT跟踪基准的空白，为真实世界动态多模态跟踪研究提供了标准化评估工具和平台。
...
TL;DR: 提出LB-Edit框架，通过注意力引导的编辑相机放置（ACP）和多视图注意力对齐（MAA）方法，解决了3D高斯溅射场景编辑中编辑相机放置与多视图一致性问题，仅用5个编辑视图即可实现高质量局部编辑，延迟降低7倍。


<details>
  <summary>Details</summary>
Motivation: 现有基于3DGS的文本驱动3D场景编辑方法通常对固定训练相机视角渲染的视图应用2D扩散编辑器，限制了编辑的空间覆盖范围和用户针对复杂场景中特定对象的编辑自由。

Method: 提出LB-Edit框架，包含两个核心模块：1) 注意力引导的编辑相机放置（ACP），通过探测扩散模型在不同候选相机距离上的自注意力和交叉注意力，找到注意力良好聚于感兴趣区域的优化距离，并放置紧凑且几何多样的编辑相机集；2) 多视图注意力对齐（MAA），通过令牌级对应共享自注意力特征对齐外观，以及将交叉注意力图提升到3D高斯上形成共享3D注意力场来对齐空间位置，抑制外观和空间漂移。

Result: 在多目标和单目标场景的实验表明，该方法在指令保真度、多视图一致性和编辑局部性方面获得最高用户偏好，仅用5个编辑视图，延迟比现有方法降低7倍。

Conclusion: LB-Edit通过注意力引导的相机放置和多视图注意力对齐机制，有效提升了3D场景编辑的局部性、一致性和效率。
...
TL;DR: 本文提出HieraSample，一种针对加速MRI的分层主动采样框架。它采用余弦退火课程从20x降至4x加速，保留全采样低频盘，并用Mamba策略在特征空间选取高频坐标。在fastMRI+膝关节数据集上，该方法的ACL诊断性能在4x-10x加速下媲美全采样，严重度评估AUC比基线提升20.4点。


<details>
  <summary>Details</summary>
Motivation: 现有主动采样方法要么对低/高频空间一视同仁，要么限制动作空间为整条笛卡尔线，导致高加速倍数下性能不佳。本文旨在利用MRI频率空间的层次特性（低频含解剖结构、高频含细节），设计任务驱动的采样策略以提升高加速下的诊断质量。

Method: 1) 余弦退火课程：在80个采集步中加速倍数从20x逐渐降至4x，每步保留全采样的低频盘。2) 基于Mamba的策略网络：从双分类器（疾病+严重度）提取的特征中选择高频坐标进行采样。3) 奖励机制：以每次采样后样本类别加权交叉熵的减少量作为奖励，正奖励对应更确信的正确预测。

Result: 在fastMRI+膝关节基准上，HieraSample在4x-10x加速下ACL诊断性能与全采样相当；在ACL严重度评估上，相比近期笛卡尔基线提升高达20.4 AUC点。

Conclusion: HieraSample验证了利用频率层次结构进行任务驱动的主动采样能显著提升加速MRI的诊断性能，尤其在病理严重度评估方面具有突破性优势。
...
TL;DR: WASABI是一个实时后处理管线，通过将车道段及其连接关系作为联合追踪目标，在车载实时约束下稳定自动驾驶中的车道拓扑输出，显著提升了检测F1、降低了中心线误差和时序不稳定性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要超越自车道感知，理解整个道路的车道拓扑图（车道段及其连接关系）。然而，神经网络感知的不完美性导致输出存在结构不稳定问题，如漏检、连接关系错误或丢失、过检测以及标签闪烁，亟需一种方法在帧内和帧间稳定这些车道拓扑输出。

Method: 提出WASABI实时后处理管线，核心思路是将车道段及其段间连接关系（LCLC）作为联合追踪目标，在车载实时约束（10Hz/20ms/最多200条输入车道）下工作。管线包含三个部分：段追踪与连接关系的集成、噪声鲁棒的拓扑感知精化、以及资源受限的实时设计。

Result: 在内部验证数据（16个序列）上，LCLC检测F1从0.834提升至0.948（+13.6%），中心线横向误差从2.50m降至0.95m，检测假阳性减少24.6%。时序稳定性方面，LCLC切换率降低63.3%，边界标签闪烁率降低30.2%。

Conclusion: WASABI在提升单帧精度的同时，有效实现了跨帧的车道拓扑稳定化，验证了将段与连接关系联合追踪作为后处理策略的可行性与有效性。
...
TL;DR: 本文提出CV-SSMNet，一种物理感知的复值状态空间网络，通过在原始复数域中构建复值状态空间模型（CV-SSM）捕获长距离空间依赖，并利用七种物理散射先验作为FiLM式调制信号引导特征演化，实现PolSAR图像分类中的物理知识引导表示学习。


<details>
  <summary>Details</summary>
Motivation: 现有复值网络能保留幅度-相位信息，但在长距离空间依赖建模上存在局限，且极化先验通常仅作为输入级或浅层辅助特征使用，物理知识未能充分引导深度特征演化。

Method: 构建复值状态空间模型（CV-SSM）在原始复数域捕获长距离空间依赖；编码七种物理散射先验作为FiLM调制信号，自适应重校准复值表示；集成多尺度复卷积、分支式CV-SSM编码、先验引导重校准和轻量级全局上下文聚合。

Result: 在三个L-band基准数据集和P-band BIOMASS评估上，CV-SSMNet实现了竞争性的分类精度、更好的区域一致性和边界保持能力。

Conclusion: 将极化散射机制嵌入复值长距离GeoAI表示学习中是有效的，为物理感知的遥感图像分类提供了新思路。
...
TL;DR: 本文提出了Trace，一个基于分类法的多领域视觉推理环境，通过将任务构建分解为场景语法和可执行任务程序，生成了覆盖11个视觉领域、277个场景语法和1000个任务的训练数据。基于64000个Trace实例的RLVR训练使Qwen2.5-VL-3B和7B模型在24个外部基准上的宏平均分别提升3.51和4.06个百分点，证明广泛的过程性训练可以迁移到生成任务分布之外。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法在语言模型推理上表现良好，但扩展到视觉语言模型时缺乏同时具备广泛性、可精确验证和可复现性的训练数据。

Method: 提出Trace环境，采用分类法指导，将任务构建分解为场景语法（控制视觉呈现）和可执行任务程序（控制答案计算），共享语义状态决定渲染图像、提示、类型化答案、验证器状态和可重放实例轨迹。

Result: 在64000个Trace实例上进行RLVR训练后，Qwen2.5-VL-3B在24个外部基准上宏平均提升3.51个百分点，Qwen2.5-VL-7B提升4.06个百分点。

Conclusion: 广泛的过程性视觉推理训练能够有效迁移到生成任务分布之外的基准上，证明了Trace环境的有效性。
...
TL;DR: Lean-SAM2是一个轻量级框架，通过目标锚定记忆剪枝、时间压缩与保险记忆、目标锚定风险感知路由三种协同机制，在不牺牲精度的前提下显著提升SAM2的推理速度。


<details>
  <summary>Details</summary>
Motivation: SAM2虽然实现了时序可提示分割，但存在两点核心瓶颈：(1) 交叉注意力中内存开销巨大；(2) 全帧视觉特征提取存在冗余。现有加速方法（如启发式记忆剪枝、窗口稀疏路由）在复杂场景（遮挡、干扰物多）下性能严重退化，亟需一种既高效又鲁棒的轻量化方案。

Method: Lean-SAM2包含三个协同模块：(1) 目标锚定记忆剪枝（TAMP）：利用语义一致性抑制干扰token的注意力；(2) 时间压缩与保险记忆（TCIM）：通过可见性门控融合历史上下文，条件性存储高置信条目至保险库；(3) 目标锚定风险感知路由（TARR）：基于锚点相似度选择性激活图像编码器，并采用风险感知回退策略在剧烈变化时触发全帧刷新。

Result: 在LVOSv2验证集上，Lean-SAM2相比SAM2.1-Large和SAM2.1-Base+分别实现1.412倍和1.417倍加速，同时J&F分数比Efficient-SAM2提升5.0%和3.6%。

Conclusion: Lean-SAM2在精度与效率之间取得了更优的平衡，通过有选择性地保留关键信息而非简单剪枝，在加速的同时避免了性能崩溃。
...
TL;DR: 针对现有动作质量评估（AQA）方法需要为每种动作类型单独训练模型、泛化能力差的问题，本文提出了一种统一的多动作知识专家（MoAKE）框架，通过学习互补专家捕捉不同动作模式并动态聚合知识，在统一模型中实现对异构动作的有效评估，在多个数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有AQA方法遵循"一对一"范式，即每种动作类型需要单独训练一个模型，这不仅需要事先知道动作类型来选择对应模型，而且在不同动作之间泛化能力差，难以实际部署。

Method: 提出MoAKE框架，包含：（1）多个互补专家（Experts）在共享语义空间中学习不同动作模式；（2）段感知原型（Segment-Aware Prototypes）处理不同时序长度；（3）自适应帧内和帧间关系建模（AIISRM）模块建模多粒度时序动态；（4）动态知识聚合机制根据输入动作自适应调整评估策略。

Result: 在三个长时数据集上，MoAKE在"一体化"设置下显著优于现有方法；在三个短时数据集上，零样本/少样本评估也表现出一致的泛化能力。

Conclusion: MoAKE通过多专家协同与动态聚合，有效缓解了异构动作间的负知识迁移问题，为统一的多动作质量评估提供了一种有效的解决方案，且具备良好的泛化能力。
...
TL;DR: 本文针对无人机实时流式感知中微小目标理解难题，从数据和方法两个层面提出解决方案：构建首个像素级开放词汇的航拍微小目标分割数据集DroneEyes，并提出SkyAnchor多模态大模型，通过语义感知令牌路由和分层记忆库设计，在有限资源下实现流式视频中微小目标的持续精准理解。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在无人机在线流式部署中面临两大挑战：1）航拍微小目标的细粒度细节在视觉压缩中被丢弃；2）连续流理解需历史上下文，但存储全部历史在资源受限的机载硬件上不可行，丢弃则导致目标漂移或丢失。

Method: 1）数据层面：构建DroneEyes数据集，含2140个高清视频和176,623对标注（物体描述+指代表达任务），提供密集逐帧像素级掩码；2）方法层面：提出SkyAnchor，包含语义感知令牌路由器（保留小目标细节）和分层记忆库（维持流式目标一致性理解）。

Result: DroneEyes是首个像素级、开放词汇的航拍微小目标分割数据集。SkyAnchor在流式场景下有效解决了微小目标细节保留和长程上下文维持问题。

Conclusion: 本文从数据和模型双维度解决了无人机流式感知中微小目标理解的瓶颈，为资源受限的机载MLLM部署提供了可行方案。
...
TL;DR: 本文提出了一种无需训练、无需原型的框架，通过利用体数据中的时序和结构一致性来修复不可靠的类激活图（CAM），在不增加额外计算成本的前提下显著提升弱监督分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督分割方法依赖的CAM存在噪声大、易出现灾难性失败的问题。现有修复方案（如引入额外训练阶段或原型学习）会增加计算开销并降低鲁棒性。

Method: 提出两个核心组件：1）方差缩减激活聚合（VRAA），通过理论证明将CAM建模为高维随机向量，聚合后能降低方差、抑制噪声；2）双向极端值修正（BER），通过双向极端值检测来发现并修复不合理激活，无需学习额外参数。

Result: 在多个公开基准上，Dice提升高达20%，mIoU提升高达40%，推理时间减少5倍以上。

Conclusion: 利用体数据的时序和结构一致性作为隐式归纳偏置，是一种原理明确且高效的弱监督体分割稳定化方法。
...
TL;DR: 本文提出CIS-Fuse，一种基于脉冲神经网络（SNN）的红外与可见光图像融合方法。通过在膜电位层面进行跨模态融合，并设计电流注入脉冲（CIS）算子，实现了与现有ANN方法相当的融合质量，同时能耗降低约一个数量级。


<details>
  <summary>Details</summary>
Motivation: 现有IVIF方法多基于人工神经网络（ANN），计算密集、能耗高。SNN虽能效更优，但其二进制脉冲特性会丢失低于发放阈值的跨模态互补信息，直接应用存在根本性矛盾。

Method: 提出CIS-Fuse网络，核心是电流注入脉冲（CIS）算子：将一种模态作为门控辅助电流注入另一种模态的驱动神经元，在膜电位层面实现跨模态融合，并引入可学习的通道级注入强度。基于CIS构建双向跨模态融合模块（BCMF），部署在非对称堆叠深度的双分支架构上。

Result: 在四个IVIF基准数据集以及下游检测和分割任务上，CIS-Fuse达到了与SOTA ANN方法相当的融合质量，推理能耗比同等规模的ANN方法DCEvo降低约一个数量级。

Conclusion: CIS-Fuse成功将SNN应用于IVIF任务，在保持融合性能的同时大幅降低能耗，为低功耗多模态融合提供了有效方案。
...
TL;DR: 本文提出MTVDiff多模态潜扩散框架，通过融合深度和文本信息实现热成像到可见光人脸翻译，在MCXFace和SpeakingFaces数据集上FID降低48.3%，Rank-1准确率提升8.9%。


<details>
  <summary>Details</summary>
Motivation: 热成像到可见光人脸翻译面临几何不连续性、语义属性不匹配和身份退化等根本性挑战，现有方法难以同时解决这些问题。
...
Conclusion: 该方法为不同照明条件下的人脸识别系统提供了鲁棒解决方案，通过有效的多模态集成推进了跨光谱人脸图像翻译的最新技术水平。
...
TL;DR: LAViFiT提出了一种端到端的潜在动作引导视觉语言微调框架，通过逆动力学模型、前向世界模型和补丁级SIG正则化，解决了预训练视觉语言模型在细粒度手术交互任务中难以有效对齐动作区域特征的问题，在多个编码器和数据集上提升了识别与图文对齐性能。


<details>
  <summary>Details</summary>
Motivation: 现有预训练视觉语言模型和视觉编码器在迁移到细粒度手术交互任务时面临两难：冻结视觉编码器依赖的预训练表示含有噪声且空间定位弱，而全参数微调虽改善全局语义对齐却无法确保编码器在正确的动作区域学习有意义特征。

Method: 提出LAViFiT框架，包含三个关键组件：(1)逆动力学模型捕获动作引起的视觉变化；(2)前向世界模型驱动编码器关注动作相关区域；(3)补丁级SIG正则化防止局部特征坍塌，无需额外监督（如边界框或伪标签）。

Result: 在多个编码器和数据集上的实验表明，LAViFiT提升了识别性能和图文对齐效果，表示分析显示更强的器械-组织交互区域定位能力和空间一致性特征。

Conclusion: LAViFiT有效解决了预训练视觉语言模型在细粒度手术交互任务中的动作区域特征对齐问题，为上下文感知手术AI和自主机器人手术提供了更好的视觉-语义理解基础。
...
TL;DR: OSVE是首个将单步文本到图像扩散模型成功应用于高质量视频编辑的框架，通过可学习编码器预测噪声、结构感知编辑损失保持几何结构、统一帧编辑增强时序一致性，在保持编辑质量的同时实现155-171倍加速，为实时视频编辑应用铺平道路。


<details>
  <summary>Details</summary>
Motivation: 现有的文本引导视频编辑方法依赖多步扩散模型采样和反演，计算成本极高，速度缓慢，无法满足实时应用需求。作者希望将高效的单步T2I模型适配到视频编辑任务中，同时解决反演速度、可编辑性和时序一致性三大核心挑战。

Method: 1) 训练可学习编码器，单次前向传播即可预测每帧的初始噪声，替代缓慢的迭代反演；2) 提出结构感知编辑损失（SAE），在结构对齐的图像对数据集上训练，保持源视频的几何结构；3) 提出统一帧编辑（UFE），通过拼接帧潜变量实现单步生成中的跨帧注意力；4) 对长视频采用滑动窗口策略配合锚帧来维持全局一致性。

Result: OSVE在编辑质量上与最先进的多步方法相当或更优，而运行速度提升了约155-171倍，实现了实时视频编辑能力。

Conclusion: OSVE成功将单步T2I模型扩展到视频编辑领域，在保证高质量编辑的同时大幅提升了推理速度，为实际实时视频编辑应用奠定了基础，相关代码已开源。
...
TL;DR: 本文提出StrokeSeg2，一个轻量级、模块化、跨平台的C++/Qt框架，通过知识蒸馏和ONNX Runtime量化将3D卒中分割模型从102.3M参数压缩至0.84M参数（2.1 MB磁盘占用），在保持分割性能的同时实现平均84%推理时间缩短和90%以上能耗降低，并打包为独立安装程序支持Windows/macOS/Linux。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习分割框架（如nnU-Net）在临床研究环境中难以部署，主要受限于软件依赖和计算资源要求。因此需要一种轻量级、可移植的解决方案，使高性能分割工作流能在标准临床工作站上运行。

Method: 采用C++/Qt框架构建跨平台应用；通过知识蒸馏进行架构压缩；使用ONNX Runtime结合Float16量化进行推理优化；在CPU、集成GPU和专用GPU等异构硬件上评估效率。

Result: 架构蒸馏是效率提升的主要贡献者（>90%能耗降低，平均84%推理时间减少）；0.84M参数的学生模型为最佳平衡点，磁盘占用仅2.1 MB，分割性能保持稳健；打包为Windows/macOS/Linux独立安装程序。

Conclusion: StrokeSeg2通过极致的模型压缩和跨平台封装，使得3D卒中分割可直接部署于临床工作站，无需Docker或外部依赖，显著降低了深度学习在临床研究中的使用门槛。
...
TL;DR: 本文提出了MV-Bench基准，用于评估多模态大语言模型在构建协调多视图界面方面的能力。基于92个Tableau工作簿和1048个实例的评估显示，当前最强模型在视觉布局上准确率达75.45%，但数据绑定准确率仅21.71%，交互完整性仅11.68%，表明MLLMs在数据语义和交互逻辑方面仍有显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM评估主要聚焦单图表生成，忽视了协调多视图界面构建这一实际需求，该任务需要联合推理数据语义、视图协调和交互逻辑。目前缺乏专门针对这一场景的系统性基准测试。

Method: 以Tableau工作簿文件为真实标注，开发多阶段流水线将规范转换为可执行Web界面；构建包含92个基础界面和1048个验证实例的基准，每个实例包含可执行代码、渲染界面、数据集和交互注解；从视觉保真度、数据绑定正确性和交互完整性三个维度评估五个最先进MLLM。

Result: 最强模型视觉布局准确率75.45%，但数据绑定仅21.71%，交互完整性仅11.68%。迭代精炼可改善代码可执行性，但无法显著缩小数据绑定和交互生成的差距。

Conclusion: 当前MLLMs能够较好地复现视觉外观，但在生成协调多视图界面所需的数据语义和交互逻辑方面能力有限，该领域仍需进一步研究突破。
...
TL;DR: 本文提出LoRFT基准数据集和Map-RSTNet模型，用于从固定高速公路摄像头中恢复远距离车辆轨迹，显著提升了轨迹重建精度。


<details>
  <summary>Details</summary>
Motivation: 车辆在远距离区域因透视压缩和尺度衰减导致轨迹片段化或过早终止，现有方法难以从可靠的近场轨迹中恢复远场连续轨迹。

Method: 构建包含22个场景、366K帧、6,601条人工验证轨迹的LoRFT基准；提出Map-RSTNet，一个地图感知的残差序列到序列模型，在道路几何对齐的状态空间中重建远距离轨迹，并在解码时动态刷新局部道路几何信息。

Result: 在LoRFT基准上，Map-RSTNet相比最强基线，ADE降低11.0%，FDE降低15.4%，5秒RMSE降低10.5%。

Conclusion: 道路几何感知的重建方法能有效扩展现有固定摄像头基础设施的可用轨迹记录，LoRFT为远距离车辆轨迹重建提供了可复现的测试平台。
...
TL;DR: WearWow是一个端到端、无遮罩的生成框架，通过自适应2D令牌打包（ATP）解决内存爆炸问题，通过多维度试穿奖励系统（MTR）解决纹理退化问题，在原生2K多服装虚拟试穿领域达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有方法在合成原生2K多服装虚拟试穿时面临两大瓶颈：一是2K条件下O(N²)的内存爆炸问题；二是扩散模型的频谱偏差导致高频织物细节过度平滑。

Method: 提出自适应2D令牌打包（ATP）利用服装稀疏性将异质物品打包到统一2D画布并剪枝无关背景令牌；提出多维度试穿奖励系统（MTR），联合语义引导奖励和布料分布奖励来修复纹理退化；构建WearWow-2K原生2K高质量数据集。

Result: WearWow在原生2K多服装合成任务上超越了现有商业基线，达到新的SOTA水平。

Conclusion: WearWow通过ATP和MTR有效解决了高分辨率多服装虚拟试穿的两大瓶颈，证明了无遮罩生成框架在高分辨率虚拟试穿中的潜力。
...
TL;DR: 本文提出G-MAD框架，利用Arma3游戏引擎生成多视角、同步的可见光-热红外航空数据，并构建了AMOD大规模基准数据集，解决了真实数据采集成本高、对齐困难等问题。


<details>
  <summary>Details</summary>
Motivation: 解决真实航空数据集构建中视角控制有限、RGB-T对齐不完美、标注成本高等关键限制。

Method: 基于Arma3游戏引擎，支持结构化场景规范、可控多视角相机布局、同步可见光/热红外采集，以及利用引擎级几何元数据自动生成边界框标注。

Result: 构建并发布了AMOD大规模多视角航空RGB-T目标检测基准数据集，并提供开源代码和数据集。

Conclusion: G-MAD框架能有效支持航空目标检测中的视角变化研究、多模态融合研究以及合成到真实的迁移学习。
...
TL;DR: 本文提出SIINR框架，利用3D U-Net监督先验与隐式神经表征(INR)相结合，实现临床dMRI数据的超分辨率重建，并量化重建不确定性，在多个公开数据集和临床病例上优于传统插值方法。


<details>
  <summary>Details</summary>
Motivation: 临床dMRI采集通常受限于低面外分辨率，导致结构信息退化，限制了高级分析的应用效果。

Method: 将监督式3D U-Net作为高分辨率先验，与自监督隐式神经表征(INR)融合，联合建模空间与角度域，保证数据一致性，并提供解析近似后验分布用于不确定性量化。

Result: 在多个公开dMRI数据集上，SIINR在定量误差指标和定性解剖保真度上均优于标准插值方法；在临床病例（多发性硬化、脑部病变）中能有效传播强度变化并标记不确定区域。

Conclusion: SIINR是一个灵活、模块化的框架，可适配不同上采样比例和下游任务，为增强临床dMRI和稳健解释神经影像指标提供了原则性方法。
...
TL;DR: 提出ETPDesigner多智能体框架，能从原始戏剧剧本直接合成高质量的电子戏剧节目单（ETPs），并在视觉一致性、跨页连贯性和交互性上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有文生图模型无法处理长上下文叙事，且难以在多页视觉资产间保持一致性，无法满足电子戏剧节目单这种复杂多页设计任务的需求。

Method: 构建协作式多智能体框架，包括语义脚本分析、核心海报合成、功能背景生成和分层角色资产合成。核心是全局风格锚点机制，从核心海报提取视觉先验确保美学统一。同时集成肖像动画、定制语音合成和基于人格的LLM，实现实时语音交互。

Result: 构建了ETP-Pro专业基准数据集，大量实验证明该方法在语义忠实度、美学一致性和交互性上显著优于现有方案。

Conclusion: ETPDesigner成功将静态节目单升级为沉浸式互动伴侣，实现了从剧本到多页交互式电子戏剧节目单的端到端合成。
...
TL;DR: 本文提出了面向大离轴视角无人机-卫星图像跨视角地理定位的OffNadirLoc基准和ONLoc框架，通过结构感知上下文加权机制和视图一致性学习策略，显著优于现有方法并具备零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准和方法主要关注近天底角场景，忽视了大离轴视角下的剧烈透视畸变、遮挡和外观差异，且缺乏结构场景理解和域内关系约束，限制了实际部署性能。

Method: 提出OffNadirLoc基准和ONLoc框架，包含：(1) 结构感知上下文加权机制，动态强调可靠局部特征并抑制歧义区域；(2) 视图一致性学习策略，将一张卫星图与多视角无人机图视为语义组，通过集合级监督学习视角不变性判别特征。

Result: 在OffNadirLoc基准和四个近天底角数据集上，方法持续超越现有最优方法，并表现出强大的零样本跨数据集泛化能力。

Conclusion: 结构感知与多视图一致性约束能有效提升大离轴视角下的跨视图地理定位性能，且具备良好的泛化性。
...
TL;DR: 本文提出了一个包含691株甜椒植物的多模态时间序列数据集，并设计了融合视觉特征与数值测量的LSTM深度学习框架，实现单株植物产量预测，相比基线方法RMSE降低33%-38%，并提供了不确定性校准估计。
...
Method: 构建了包含4837张图像、691株甜椒植物的多季节时间序列数据集；提出多模态深度学习框架，使用DinoV3编码器提取图像特征，结合数值计数数据，通过LSTM网络建模时序依赖关系，并处理温室监测中不规则采样间隔的问题。

Result: 多模态方法相比持久性基线在2022和2023季RMSE分别降低33%和38%，比仅使用测量数据的模型平均提升1.2%；不确定性校准误差在0.39-0.89之间。

Conclusion: 该数据集和框架为数据驱动的园艺作物产量预测提供了可复现的基准，结合不确定性估计为实际农业决策提供了可靠的置信度信号。
...
TL;DR: 本文提出StereoFlow，一种将确定性匹配回归与生成式分布建模相结合的立体匹配生成框架，通过渐进级联匹配网络、像素扩散Transformer和过渡流匹配目标，在多个基准上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 传统立体匹配方法将多模态分布建模退化为单点估计，存在回归到均值的偏差，难以处理模糊区域。

Method: 提出三个关键组件：(i)两阶段渐进级联匹配网络，生成多分辨率立体条件；(ii)像素扩散Transformer (StereoDiT)，采用频率解耦架构；(iii)过渡流匹配目标，实现高效优化。

Result: 在Scene Flow、KITTI、ETH3D、Middlebury等基准上取得多个SOTA结果，在病态、不连续区域和零样本泛化中表现出强几何一致性与丰富细节。

Conclusion: StereoFlow通过生成式框架有效解决了传统确定性回归的偏差问题，在立体匹配任务上实现了性能突破。
...
TL;DR: 本研究基于Sentinel-2影像，引入了一个建筑检测框架，系统量化了时间、处理层级和建成区类型对建筑分割性能的影响，并通过华沙地区的多时相数据集和U-Net/DeepLabV3+模型，为实际应用中的模型选择提供了实践指导。


<details>
  <summary>Details</summary>
Motivation: Sentinel-2影像具有开放获取、全球覆盖和高重访频率的优势，但10米分辨率使得建筑检测（特别是小尺寸或亚像素建筑）具有挑战性，且性能受季节性和建成区异质性影响。目前缺乏系统量化这些效应并为实际模型选择提供指导的研究。

Method: 构建华沙地区多时相Sentinel-2数据集，利用波兰官方地形数据库（BDOT10k）建筑足迹栅格化为二值真值掩膜。采用U-Net和DeepLabV3+两种卷积分割骨干网络，针对不同场景进行微调，分别选出L1C和L2A产品的最佳月度模型，并进行跨时相推理评估。

Result: 确定了不同月份的训练和推理有利条件，量化了季节间性能迁移能力，评估了L1C和L2A处理级别的影响，并揭示了不同建成区类型下的效果差异。

Conclusion: 基于实验结果，为不同采集时期和聚落特征下的Sentinel-2常规建筑分类提供了实用的模型选择指导。
...
TL;DR: 本研究系统比较了七种强度归一化方法对3D U-Net膝关节半月板分割模型的影响，发现虽然归一化方法对模型泛化性有可测量的影响，但其效果远小于域偏移造成的性能下降，提示需要结合其他策略来提升模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 强度归一化是影响MRI深度学习模型泛化性的重要但未充分探索的因素，本研究旨在系统比较不同归一化方法对模型鲁棒性的影响。

Method: 在IWOAI 2019数据集上训练3D U-Net模型，系统比较七种归一化方法（包括标准缩放、直方图方法和GMM方法），并在内部和外部测试集（SKM-TEA）上评估性能。

Result: 内部测试性能相似，但外部测试差异显著；Z-score、Nyúl直方图匹配和CLAHE比其它方法更鲁棒；然而归一化方法间的差异远小于数据集间域偏移造成的性能下降。

Conclusion: 强度归一化对模型泛化性有可测量影响，但效果有限，需结合互补策略来应对域偏移问题，以实现鲁棒的临床部署。
...
TL;DR: 本文提出了一种针对扩散模型的**重要性感知剪枝**框架，在无需训练的情况下，根据空间重要性图（来自条件信号或注意力机制）来保留对语义重要区域起关键作用的参数，从而在高压缩比下保持图像的主体保真度和结构正确性。


<details>
  <summary>Details</summary>
Motivation: 传统剪枝方法在压缩扩散模型时，仅关注均匀重建误差，导致在**高压缩比下丢失语义关键区域**的细节和结构，因此需要一种**内容感知**的剪枝目标来保留感知上重要的参数。

Method: 提出**训练-free**的剪枝框架，将空间重要性图（从条件信号或模型注意力中提取）纳入剪枝目标，使参数排名对齐**感知相关性**而非均匀误差。

Result: 在MS-COCO数据集上，该方法在高压缩比下**显著优于传统剪枝**，保持了主体保真度和结构正确性，而传统方法产生明显退化。

Conclusion: 研究表明，**内容感知的剪枝目标**是实现生成模型感知保真度压缩的关键。
...
TL;DR: 本文提出高斯种子（GaussianSeed），一种渐进式多尺度高斯占据预测框架，通过粗到细的层次化设计，在0.1米高分辨率下实现实时推理，并构建了全景六摄像头0.1米精度的TJScenes数据集，在效率和精度上达到新前沿。


<details>
  <summary>Details</summary>
Motivation: 现有以视觉为中心的3D占据预测方法由于高昂的计算成本，难以扩展到高体素分辨率，存在内存瓶颈问题。

Method: 提出GaussianSeed，一种渐进式多尺度高斯占据预测框架，将基元组织成从粗到细的层次结构，通过层次化设计规避密集表示的内存瓶颈。

Result: 成功扩展到0.1米空间分辨率并保持实时推理能力；在所有评估方法中延迟最低，同时保持高度竞争的精度。

Conclusion: GaussianSeed推动了高分辨率3D占据预测的效率-质量边界，验证了层次化高斯表示在密集场景理解中的有效性。
...
TL;DR: 提出因子信息不确定性蒸馏(FIUD)框架，通过教师-学生网络将不确定性估计与可解释的图像质量因素对齐，在三个数据集上提升了不确定性校准和选择性预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度凝视估计方法在受控环境下表现良好，但在非约束场景中失效；单次不确定性推断缺乏输入有效性线索，采样方法实时性差，需要更高效可靠的不确定性估计方法。

Method: 采用教师-学生框架：梯度提升树教师从光照、清晰度、眼睛可见性和对称性等因子预测凝视误差，轻量神经网络学生通过课程学习和排序监督蒸馏这些信号，生成单次不确定性头。

Result: 在ETH-XGaze、Gaze360和MPIIFaceGaze三个数据集（>30万样本）上，FIUD在不确定性校准、误差等级相关性和选择性预测方面均优于确定性和采样基线，在非约束场景中提升最大。

Conclusion: FIUD框架有效将可解释图像质量因子融入不确定性估计，在保持实时性的同时显著提升了非约束场景下的凝视估计可靠性。
...
TL;DR: 本研究开发了一种自动化数据整理流程，结合开源大语言模型提取心脏磁共振（CMR）报告中的诊断标签，并微调三种视觉基础模型（DINO、VST、UMedPT）进行心血管疾病诊断，在独立测试集上取得了优异性能（如HCM AUC=0.959，CA AUC=0.966），代码和模型权重已公开。


<details>
  <summary>Details</summary>
Motivation: CMR成像可无创评估心脏结构、功能和病理，但解读需要丰富经验。AI模型可辅助CMR判读，但受限于数据整理耗时、模型性能欠佳以及实施路径不清晰等问题。

Method: 开发自动化数据整理流程，利用开源本地大语言模型从叙述性CMR报告中提取诊断标签，并预处理多模态影像数据（包括电影序列和LGE序列）。采用两阶段方式微调三种视觉基础模型（DINO、VST、UMedPT），集成多种模型和模态的策略进一步提升诊断性能。

Result: 在包含5类（HCM、DCM、ICM、CA、正常对照）共988例整理病例的训练集和1067例独立测试集上，微调模型取得高鉴别诊断性能，HCM AUC最高0.937，CA AUC最高0.945。集成策略进一步提升性能：HCM AUC=0.959，CA AUC=0.966。

Conclusion: 集成多模型和多模态的AI策略可显著提升CMR心血管疾病诊断的准确性和鲁棒性，且代码和模型权重已开源，有望推动AI辅助CMR判读的临床应用。
...
TL;DR: 本文提出 VLM 中存在双模态（文本与视觉）的上下文诱导现象，并构建了一个分类学驱动的基准数据集 ENTRAP-VL（1,500 个项目、8 个类别），用于系统研究该现象，而非简单移植纯文本方法。


<details>
  <summary>Details</summary>
Motivation: 上下文诱导在纯文本语言模型中已被识别并得到机制解释，但在视觉-语言模型（VLM）中该现象尚未被系统研究，且缺乏专门的评测工具。简单移植文本基准不足以应对多模态场景的复杂性。

Method: 构建了 ENTRAP-VL 数据集，包含 1,500 个手动策划的项目，按两个轴（上下文与项目的关联性、与真实性的关系）组织成 8 个类别的分类体系，并分为文本诱导流（8 种上下文条件）和视觉诱导流（3 种上下文条件）。

Result: 不声称测量任何特定模型的诱导程度，而是提供数据集、分类体系及评估协议，供社区严格研究该现象。

Conclusion: VLM 中的上下文诱导是一个实质性的新问题（而非增量扩展），具有双重性（文本和视觉均可独立驱动），并引入了纯文本设置中不存在的真实性区分（对场景为假但在世界中可能成立的上下文）。
...
TL;DR: 本文提出RIM方法，利用Google 3D Tiles生成跨域参考视图，通过两阶段微调SALAD并结合知识蒸馏实现单一DINOv2-B模型同时完成检索与局部描述，在UAV全球视觉定位任务上以67.9ms端到端时延达到领先精度。


<details>
  <summary>Details</summary>
Motivation: 无人机与参考图之间存在显著的跨域外观和视角差异，现有方法难以同时兼顾鲁棒的六自由度位姿估计与高效部署，尤其是在GNSS受限环境下。

Method: 1）从Google 3D Tiles采样多视角参考视图；2）两阶段跨域微调SALAD，利用位姿邻近正样本与地理距离硬负样本；3）局部几何一致性重排序Top-K候选；4）提出RIM，冻结已适配的DINOv2-B检索器，蒸馏局部描述符解码器（复用token场+浅层VGG19细节流），单次前向传播同时服务于检索与局部描述。

Result: 在EPFL Urbanscape和Chang'an Park数据集上，RIM在25/50m全3D距离度量下Recall@1相比SALAD提升8.55/13.77和4.45/8.94个百分点；Top-K=5时端到端时延67.9ms，比最强分离稀疏匹配基线快1.8倍，比RoMa快40倍以上。

Conclusion: RIM建立了一条高效、可部署的无人机全球视觉定位流水线，在GNSS挑战环境下实现了精度与速度的优异平衡。
...
TL;DR: 本文通过18种图像编码器和7种文本编码器的受控实验，发现医学图像编码器的表征收敛主要源于自监督预训练目标，而非规模或临床监督，且这种收敛程度有限（胸部X光片最高40.4%），不随模型规模增长，但线性分类器可跨编码器迁移（保留约85%性能）。


<details>
  <summary>Details</summary>
Motivation: 现有研究假设不同医学图像编码器因规模扩大和临床监督而趋于共享表征结构，但这一假设缺乏系统验证，且现有相似度度量方法不可靠。

Method: 跨18种图像编码器和7种文本编码器（参数规模7M-27B，涵盖5种成像模态、650,982张胸部X光片），通过控制数据、架构和规模仅改变训练目标来隔离因果关系，并在合成模型中复现效应。

Result: 自监督编码器对齐度最高（40.4%），标签监督（21.1%）和图文编码器（3.3%）较低；收敛不随规模增长（Spearman 0.302, p=0.223）；仅在模态内有效，不能映射到临床语言；但线性分类器可跨编码器迁移至5家外部医院（保留约85%性能）。

Conclusion: 医学图像编码器的收敛由预训练目标决定，而非规模或临床监督；互操作性应通过目标设计实现，并在共享几何结构最薄弱的患者亚组和临床判断处进行验证。
...
TL;DR: HeadCast 是一种无需训练的即插即用加速框架，通过分类自回归视频扩散模型中的注意力头为四种原型（Sink、Dummy、Spatial、Global），并重构 KV 缓存为头特定路径，在保持视频质量的同时实现最高 1.95x 的推理加速。


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型中不断增长的 KV 缓存导致注意力计算成为主要推理瓶颈，尤其是在高分辨率下。现有方法要么使用粗糙的启发式策略导致帧间闪烁，要么需要重新训练模型，缺乏高效且无损的加速方案。

Method: HeadCast 在预热后，于最大噪声步骤对每个注意力头进行一次分类，将其归入 Sink、Dummy、Spatial、Global 四种原型之一，并据此将统一的 KV 缓存重构为头特定的独立路径。保留 Global 头以维持长程时序一致性，Spatial 路径则在固定大小网格上操作以节省计算。

Result: 在多种最先进的自回归模型上，HeadCast 在 720P 分辨率下实现最高 1.62x 加速，在 1080P 下实现 1.95x 加速，同时保持 VBench 质量指标与全注意力相当且基本无闪烁问题。

Conclusion: HeadCast 是一种高效、无需重训练的视频生成加速方案，通过感知注意力头行为的差异来优化 KV 缓存管理，在高分辨率场景下优势明显，且保持生成质量与一致性。
...
TL;DR: 视觉语言模型对图像和问题的呈现顺序敏感，图像优先提示始终优于问题优先提示。本文提出了一种顺序一致的测试时训练方法，有效消除了模态顺序差异，并意外地提升了图像优先分支的性能。通过激活修补定位到网络中部区域的表示分歧，测试时训练修复了这种跨层错位。


<details>
  <summary>Details</summary>
Motivation: 发现视觉语言模型存在模态顺序敏感性这一语义无关的失败模式，即图像和问题的呈现顺序会影响模型性能。

Method: 提出顺序一致的测试时训练方法，通过激活修补技术定位问题区域，并用测试时训练修复跨层表示的错位。

Result: 该方法在所有评估设置下显著缩小了模态顺序差距，并意外地提升了更强的图像优先分支的基线性能。

Conclusion: 模态顺序敏感性是VLM中的电路级失败，简单的非对称测试时自适应可以有效缓解该问题，甚至提升基线性能。
...
TL;DR: 提出了一种两阶段视觉系统，通过YOLO检测器定位EEG帽电极，再结合几何和面部地标分配10-20系统角色，在实时视频流中验证电极放置位置。在LOSO交叉验证下mAP@.5达0.94，在CPU上可实现19 FPS的实时处理。


<details>
  <summary>Details</summary>
Motivation: 实现EEG帽电极的实时自动检测与解剖位置验证，替代人工检查，提高脑电实验设置的效率和准确性。

Method: 两阶段视觉系统：第一阶段用单类YOLO检测器（YOLOv10n骨干网络）定位电极；第二阶段通过几何阶段结合面部地标，将每个检测到的电极分配至标准10-20系统命名角色。采用几何增强（旋转、透视、混合）和地标驱动的头部裁剪扩展可用距离范围。

Result: LOSO交叉验证下mAP@.5=0.94±0.07（池化0.96）；留一帽验证中M/L帽mAP@.5接近0.97，小帽降至0.72±0.28；几何增强提升面内滚动鲁棒性和时间-电极召回率；头部裁剪在0.6倍表观尺度下将mAP@.5从0.23提升至0.45；CPU上640px分辨率达19 FPS。

Conclusion: 该系统能够以实时速度准确检测和验证EEG电极位置，在小帽场景下的性能下降主要与受试者熟悉度混淆相关而非帽型本身，为临床脑电实验提供了一种高效的自动化质检工具。
...
TL;DR: 提出了一种基于神经调制的判别性多谱注意力机制（SHFormer），用于可扩展的MRI重建，既能传播高频细节实现高质量重建，又能捕获跨模态可复用的特征。


<details>
  <summary>Details</summary>
Motivation: 现有注意力机制模型在MRI重建中存在两个问题：(1) 擅长捕获低频信息但高频表示能力有限，导致重建过于平滑；(2) 需要针对不同模态重新训练，缺乏跨域迁移能力。

Method: 提出SHFormer网络，包含两个核心模块：一个光谱滤波CNN用于捕获模态特定的可迁移特征，以及一个动态高通核生成Transformer专注于高频细节重建。

Result: 在未见过的场景下，PSNR提升约1 dB，SSIM提升约0.01，并在监督学习、自监督学习、扩散模型训练、闭集/开集泛化等多种设置下均表现出色。

Conclusion: 该方法实现了可扩展、高质量的MRI重建，有效解决了传统注意力机制在高频信息保留和跨模态泛化方面的局限性。
...
TL;DR: StreamHOI是一个低延迟的流式框架，用于生成长时段的人-物交互（HOI）视频，通过HOI感知的块配置和记忆偏置训练，在有限延迟下实现了17.6 FPS的实时生成速度。


<details>
  <summary>Details</summary>
Motivation: 现有HOI视频生成方法多为离线短视频生成，驱动条件复杂，不适用于实时交互应用。

Method: 提出StreamHOI框架，包含离线HOI感知块配置、偏置引导的记忆专门化训练，以及记忆距离缩放模块，用于增强对早期交互状态的远程访问。

Result: 实现了17.6 FPS的生成速度，首块延迟仅0.75秒，在交互合理性、物体保真度、人物质量和效率方面均优于现有方法。

Conclusion: StreamHOI有效解决了流式HOI视频生成中记忆组织与延迟的权衡问题，适用于实时交互场景。
...
TL;DR: PerceptDrive提出一种自适应专家路由的感知先验世界动作建模框架，通过冻结的感知基础模型提取先验知识，结合场景条件路由机制，在无测试时候选选择的情况下实现端到端规划，在NAVSIM v1/v2上达到SOTA性能（PDMS 90.4/EPDMS 90.2）。


<details>
  <summary>Details</summary>
Motivation: 冻结的感知基础模型虽编码了丰富的几何、语义和动态知识，但窄条件接口会削弱任务相关线索，静态融合无法根据每个场景调整专家贡献。现有方法面临"先验到规划"的迁移问题。

Method: 提出PerceptDrive框架：(1) 从冻结的驾驶适应提供者中提取教师蒸馏先验，从冻结的自监督视频编码器中提取密集观测潜变量；(2) 可训练的专家路由世界动作模型通过专家特定查询分支处理这些信号，并引入先验保留目标锚定每个分支；(3) 路由器从共享场景表示中预测软门控，组合专家条件后生成轨迹；(4) 训练时使用特权规则驱动子度量估计提供软门控蒸馏目标；(5) 预测的免动作未来潜变量条件化流匹配动作器；(6) 推理时仅需一个前视摄像头，无需评分、重排或搜索。

Result: 在NAVSIM v1上达到90.4 PDMS，在NAVSIM v2上达到90.2 EPDMS，超越现有方法。消融实验验证了先验保留和场景条件路由的互补增益，以及三种先验的差异化依赖。

Conclusion: 保留并自适应路由感知先验能够在不依赖测试时候选选择的情况下提升直接规划性能，为将预训练感知知识高效迁移到规划任务提供了有效范式。
...
TL;DR: 本文提出了RS-RIE-Bench，首个面向遥感图像推理引导编辑的基准，涵盖时间、因果和空间三类推理任务，评估了八个模型，发现最强模型准确率仅24.28%，揭示现有模型在地理推理、区域控制和传感器一致性方面的严重不足。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑基准主要面向自然图像或通用视觉场景，无法充分评估遥感图像编辑所需的推理能力、区域控制能力和传感器观测一致性，亟需一个专门针对遥感场景的基准来填补这一空白。

Method: 构建RS-RIE-Bench基准，将任务分为时间推理、因果推理和空间推理三类；设计包含目标区域合理性、非目标区域保持性和图像质量一致性三个维度的评估协议；采用MLLM评估并通过跨评判一致性和专家审查验证可行性。

Result: 在八个开源和闭源图像编辑模型上评估，最强模型在严格联合满足标准下仅达24.28%准确率，八模型平均宽松联合4成功率仅32.23%；因果推理和空间推理尤为困难，部分开源模型在某些类别上接近零分。
...
TL;DR: 本文提出重要性感知采样（IAS）方法，通过基于红外结构线索的补丁权重、可学习的重要性掩码以及补丁课程学习策略，解决可见光-红外预训练中均匀对比学习不可靠的问题，在多项基准任务上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 现有可见光-红外预训练方法使用均匀的逐补丁对比学习，但成像物理差异导致部分空间配对区域本质不可比，同等强度的对齐会阻碍表征学习和下游任务迁移。

Method: (i) 从红外结构线索推导补丁权重，重新加权对比学习目标；(ii) 用轻量采样器学习软重要性掩码，可选地用人工先验进行热启动；(iii) 采用补丁课程学习策略，从高可靠区域逐步扩展至困难补丁。该方法即插即用，兼容补丁/相关级对齐和图像级对比基线。

Result: 在多个可见光-红外基准上，包括红外语义分割、红外目标检测、可见光语义分割和跨模态检索任务，均取得一致且显著的改进。

Conclusion: 从采样视角重新审视VIS-IR预训练，通过重要性感知采样有效缓解了均匀对比学习带来的问题，验证了补丁可靠性对齐策略的有效性和通用性。
...
TL;DR: Vera是一个统一的人体中心主体到视频生成框架，通过构建百万级身份对齐数据集、身份聚焦掩码监督和参考感知层级注意力机制，有效解决了单人和多人场景中的人体身份一致性问题，减少了身份混淆和过度复制。


<details>
  <summary>Details</summary>
Motivation: 现有主体到视频生成方法在通用主体一致性上取得进展，但在人体中心生成中仍存在身份关键细节漂移、多人场景中身份角色绑定错误、属性交换和过度复制参考外观等问题。

Method: 1) 构建百万级身份对齐的人体图像-视频数据集；2) 身份聚焦掩码监督(IFMS)加强身份感知学习；3) 参考感知层级注意力(RALA)调节视频token与参考身份线索的交互。

Result: Vera在人体身份一致性、多人主体绑定、运动自然度方面均有提升，同时减少了身份混淆和过度参考图像复制。

Conclusion: Vera通过统一框架有效解决了人体中心视频生成中的身份一致性问题，在单人和多人场景下均表现出色。
...
TL;DR: 本文提出了一种融合多视角视觉检测与POI邻域上下文的建筑健康评估框架，通过成本敏感随机森林分类器整合两类特征，在青岛92个老旧社区的3,237栋建筑数据上验证。多视角聚合将Macro-F1从60.84%提升至74.95%，融入POI特征后进一步增至76.79%，表明POI提供补充性上下文先验而非替代视觉证据。


<details>
  <summary>Details</summary>
Motivation: 现有自动化建筑检测研究主要依赖单张图像，很少探讨周围城市功能环境是否为建筑级别评估提供补充信息。

Method: 1) 多视角目标检测提取建筑问题；2) POI特征提取（500m/1,000m/1,500m缓冲区）；3) Pearson与Spearman相关分析筛选候选特征；4) 成本敏感随机森林融合视觉与POI特征。

Result: 多视角聚合Macro-F1 74.95%（相比直接检测60.84%）；加入POI后达76.79%，提升幅度较小且依赖类别。

Conclusion: POI信息作为补充性上下文先验而非视觉证据的替代品或建筑状况的因果决定因素。
...
TL;DR: 本文系统综述了多模态大语言模型（MLLMs）在遥感图像场景理解（RSISU）中的应用，发现通用计算机视觉MLLMs在多项遥感任务上可媲美甚至超越专用遥感MLLMs，揭示了当前模型在空间推理、细粒度理解等方面的局限性，并提出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 现有遥感多模态大语言模型（RS-MLLMs）发展迅速，但缺乏对其能力边界、跨任务泛化能力和任务特定局限性的系统理解。

Method: 通过系统性综述和诊断性评估，回顾了RS-MLLMs的技术演进（模型设计、多模态学习、训练数据、下游能力），并将RS-MLLMs与通用计算机视觉MLLMs在多种RSISU任务和基准上进行对比分析。

Result: RS-MLLMs在遥感视觉定位和高分辨率视觉问答等专业任务上具有竞争力；但通用CV-MLLMs无需遥感特定微调即可在多项任务上匹配甚至超越专用模型。当前MLLMs普遍存在空间与关系推理、细粒度视觉理解、指令多样性和跨任务泛化等方面的不足。

Conclusion: 未来应关注可靠评估、多模态与高分辨率推理、高效部署以及工具增强的遥感智能体，以推动鲁棒、可泛化且实用的RS-MLLMs发展。
...
TL;DR: 本文提出DIM-Fashion基准（26K多轮会话，涵盖7类任务的13个时尚检索数据集）和FashionAM框架（直接对齐多模态查询与图库嵌入，避免中间文本化），解决现有方法忽视意图转换和丢失细粒度视觉线索的问题。


<details>
  <summary>Details</summary>
Motivation: 现有时尚多轮检索方法假设每次交互都遵循相同的属性编辑范式，忽视了异构意图转换；同时依赖文本化桥接多模态查询与视觉检索，丢失细粒度视觉信息。

Method: 构建DIM-Fashion基准（含多样意图转换和回滚行为），提出FashionAM框架（MLLM-VLP架构，直接对齐多模态对话查询与时尚图库嵌入空间）。

Result: 实验表明FashionAM在效果上优于现有方法。

Conclusion: DIM-Fashion填补了多轮时尚检索中异构意图转换的空白，FashionAM验证了直接对齐多模态查询的有效性，推动了该领域发展。
...
TL;DR: 本文提出EVO框架，通过进化搜索为扩散策略设计全局缓存调度方案，在不重新训练的情况下实现高达8.05倍的动作生成加速，将FLOPs从15.77G降至1.96G。


<details>
  <summary>Details</summary>
Motivation: 现有扩散策略通过迭代去噪实现强视觉运动控制，但实时部署计算成本高；基于缓存的方法虽能降低推理成本，但现有无训练调度方案对所有块统一分配计算，忽略了不同块之间的异构冗余，导致性能-效率权衡次优。

Method: 提出EVO框架，一种无训练的加速框架，通过进化搜索全局调度缓存刷新；在块-时间步格点上表示完整调度，引入冗余感知初始化以生成有前景的种群，以及目标条件早停以在达到性能目标时终止搜索；离线优化后的调度可直接插入预训练扩散策略。

Result: 在多种操作基准上，EVO在保持接近完整性能的同时大幅减少计算量，实现高达8.05倍的动作生成加速，FLOPs从15.77G降至1.96G。

Conclusion: EVO通过进化搜索实现了全局缓存调度优化，在不重新训练的情况下显著提升了扩散策略的推理效率，为实时部署提供了实用解决方案。
...
TL;DR: 提出了一种名为Condition Dropout（ConD）的持续训练范式，通过随机模拟模态缺失来提升RGB-D语义分割模型在单模态缺失时的鲁棒性，同时保持全模态下的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有RGB-D语义分割模型假设RGB和深度始终可用，但实际应用中传感器故障或遮挡常导致某一模态缺失，而全模态训练的模型在单一模态缺失时性能严重下降。

Method: 从预训练的RGB-D模型出发，进行第二阶段训练：随机模拟完整、RGB缺失和深度缺失三种输入情况；冻结原始编码器，训练复制编码器并使用零初始化特征注入。

Result: 在NYU-Depth V2和SUN RGB-D数据集上，ConD在模态缺失场景下提升了模型鲁棒性，且在模态完整时甚至有轻微性能提升。

Conclusion: ConD是一种简单有效的持续训练方法，能够在不牺牲全模态精度的情况下显著增强模型对模态缺失的鲁棒性。
...
TL;DR: 本文提出SmartVL，一个统一的自适应推理框架，通过联合控制视觉token数量和LLM计算能力，在动态计算预算下实现高效的视觉-语言推理，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM推理成本高，先前工作仅独立优化单一维度（如剪枝冗余token或跳过LLM层），忽略了不同维度间的耦合关系——计算资源需根据输入内容在所有维度上动态分配。

Method: 提出SmartVL框架，包含视觉侧token控制器（动态选择信息性视觉token）和LLM侧计算控制器（自适应调整LLM计算量），两者通过共享预算编码和可微分延迟估计器进行联合端到端训练，实现跨阶段资源分配策略。

Result: 在多个MLLM基准上，SmartVL通过联合调度一致优于先前的自适应方法，实现了更优的准确率-效率帕累托前沿。

Conclusion: 联合优化视觉token和LLM计算这两个维度能够更有效地适应输入复杂度和运行时计算约束，为MLLM的高效推理提供了新思路。
...
TL;DR: 本文提出Self Gradient Forcing (SGF)方法，通过两阶段训练策略解决了自回归视频扩散模型中历史上下文梯度缺失的问题，在不反向传播整个序列的情况下恢复了对早期生成潜在表示的监督信号，显著提升了长视频外推能力，仅用5秒训练窗口即可生成数分钟的视频。


<details>
  <summary>Details</summary>
Motivation: 现有自回归视频扩散方法（Self Forcing）虽然通过学生模型自生成历史来减少曝光偏差，但历史键值缓存仅作为冻结状态被未来帧使用，未来损失无法监督早期潜在表示如何编码成更有用的键值，存在历史上下文-梯度鸿沟。

Method: 提出两阶段训练策略SGF：第一遍执行无梯度的自回归 rollout 匹配推理，在采样去噪退出步记录自生成上下文和噪声潜在表示；第二遍对记录的退出步执行并行上下文梯度重建，使用自生成上下文作为停止梯度的干净潜在输入，模型重新计算上下文KV表示和未来到上下文的因果注意力。

Result: SGF在长时域帧级和块级实验中都优于Self Forcing，尤其在主体身份保持、背景/布局一致性和时间稳定性方面表现突出。仅使用5秒训练窗口即可外推生成数分钟的视频。

Conclusion: SGF通过在两阶段训练中恢复缺失的记忆写入监督信号，在自回归训练目标内有效地训练模型将上下文编码为更有效的因果记忆，显著提升了长视频外推能力。
...
TL;DR: 本文提出Persian Pixel，一个包含34.3万+高保真图像-文本对的波斯语合成OCR数据集，通过SynthOCR-Gen渲染框架生成，并引入25+种退化模型弥合合成-真实域差距，为波斯语OCR研究提供可扩展的开源资源。


<details>
  <summary>Details</summary>
Motivation: 波斯语OCR发展远落后于拉丁语系语言，面临两大挑战：波斯-阿拉伯文字系统的内在复杂性（强制连笔、上下文相关字形、连字、变音符号等）以及大规模高质量标注数据集的严重匮乏。

Method: 从700万词波斯语语料库出发，利用SynthOCR-Gen框架生成句子、段落、整页三种布局的34.3万+图像-文本对；渲染时模拟波斯文字排版特征（上下文字符连接、位置变体、变音符号、多种字体）；再通过25+种随机退化模型（墨迹渗漏、纸张老化、模糊、光照变化、扫描伪影、压缩噪声等）模拟真实文档采集环境。

Result: 成功构建了大规模、高质量、开源可用的波斯语合成OCR数据集，为TrOCR、Donut等现代OCR架构的训练与微调提供了充足数据。

Conclusion: Persian Pixel突破了波斯语OCR数据长期短缺的瓶颈，证明了程序化合成数据生成是低资源、高复杂度文字OCR领域迈向实用化、可扩展、低成本解决方案的有效路径。
...
TL;DR: PercepCap提出了一种感知感知的视频描述框架，通过在生成最终描述前显式输出时空感知轨迹（包括物体轨迹和时间事件），并采用两阶段训练策略（感知-描述监督微调+感知锚定强化学习），显著提升了视频描述的时空理解准确性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型直接生成视频描述，缺乏对感知证据的显式建模，导致时空感知错误难以定位和纠正。

Method: 提出PercepCap框架，遵循"感知-描述"生成链：先输出物体轨迹和时间事件等时空感知轨迹，再生成最终描述。采用两阶段训练：1）感知-描述监督微调（适应感知描述链）；2）感知锚定强化学习（联合优化感知轨迹和描述质量）。并设计字幕锚定感知数据构建管道生成训练数据。

Result: 在直接描述和描述到QA评估中，PercepCap在Qwen3-VL基线上持续提升，展现出领先的描述质量。

Conclusion: 显式建模时空感知证据并采用两阶段训练能有效提升视频描述的准确性和可解释性。
...
TL;DR: ATSplat是一种前馈式3D高斯泼溅框架，通过自适应3D令牌机制恢复了3DGS优化中的自适应容量分配能力，在保持高渲染质量的同时将高斯数量减少5.7倍以上，实现了每秒1136帧的高效渲染。


<details>
  <summary>Details</summary>
Motivation: 现有前馈式3DGS方法将高斯回归限制在输入像素并沿相机射线提升，导致基元数量和位置依赖于图像分辨率和输入视角而非场景复杂度，生成密集且冗余的高斯基元集合，失去了3DGS优化中原有的场景自适应容量分配能力。

Method: 提出ATSplat框架，包含三个核心组件：1) 将粗粒度块级深度和相机线索提升为稀疏3D锚点令牌，形成场景紧凑支架；2) 每个令牌通过可学习的3D偏移回归为局部高斯，解耦基元放置与输入图像网格；3) 自适应令牌扩展模块通过渲染误差图监督预测令牌级不确定性分数，选择性地扩展高不确定性令牌。

Result: 在RealEstate10K和DL3DV数据集上达到SOTA渲染质量，高斯数量比密集前馈式3DGS方法减少5.7倍以上；从12张512×960分辨率输入图像出发，单GPU可在1秒内完成重建，以311K高斯实现1136 FPS的高质量新视角渲染。

Conclusion: ATSplat通过稀疏到自适应的范式，成功在前馈式框架中恢复了3DGS优化的自适应容量分配能力，在渲染质量和计算效率之间取得了更好的平衡。
...
TL;DR: 本文提出会话风险累积（CRA）概念，构建了追踪对话轨迹信号的检测框架，并发布了CRA-Bench基准数据集，用于评估大语言模型在多轮对话中逐步累积的安全风险。


<details>
  <summary>Details</summary>
Motivation: 现有LLM安全护栏逐条评估提示-响应对，忽略了对话中良性交互逐步累积成危害的失败模式（即会话风险累积CRA）。

Method: 提出会话层CRA框架，追踪三种轨迹信号：语义漂移、敏感度加权信息累积图、合规梯度信号。提供无监督凸融合评分和CRA-Net DA轨迹模型。构建CRA-Bench v0.1/v0.2及扩展5家族基准数据集，并设计轨迹原生评估协议。

Result: 在CRA-Bench及人工转移子集上进行了分布内会话评分评估，验证了框架对累积风险的检测能力。

Conclusion: CRA框架能有效检测对话中逐步累积的安全风险，填补了现有逐条评估方法的空白，CRA-Bench为相关研究提供了标准化测试平台。
...
TL;DR: 本文研究了监督微调（SFT）对大型语言模型在序列决策中行为多样性的影响，发现在tic-tac-toe变体棋类游戏中，SFT在提升准确率的同时会导致动作多样性过早崩溃，而动作增强（对所有最优动作进行训练）可以部分缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究广泛使用SFT来适配大语言模型到下游任务，但其对行为多样性的影响尚未充分探索，作者希望厘清SFT在提升准确率的同时是否以牺牲决策多样性为代价。

Method: 在可控的tic-tac-toe变体确定性棋盘游戏环境中，通过状态级评估、竞技场对弈和训练轨迹分析，对比推理模式生成与标准SFT对动作多样性的影响，并引入动作增强方法进行消融实验。

Result: 推理模式生成常抑制动作多样性但并未一致提升准确率；标准SFT提升准确率但引发早熟多样性崩溃，其程度超过准确率-多样性权衡所需的最小值；动作增强可部分缓解多样性损失。

Conclusion: 窄支持模仿是LLM决策策略崩溃的来源，在SFT过程中保留动作支持（action support）对于维持探索性行为至关重要。
...
TL;DR: 本文首次形式化定义结构泛化，将两个前提（组合结构与无界泛化）转化为数学语言。通过计算复杂度分析，在标准假设TC⁰≠NC¹下，证明纯Transformer无法学习结构泛化，而神经符号系统因注入了语义投影G_γ回避了真正的困难部分，从而获得更好的基准分数。基准分数无法区分"学习的"与"给定的"。


<details>
  <summary>Details</summary>
Motivation: 结构泛化已被多个基准反复测量，但从未被正式定义。本文旨在给出形式化定义，并厘清纯Transformer与神经符号系统在结构泛化能力上的根本差异，指出当前基准评估的局限性。

Method: 借鉴蒙塔古语法，将组合规则拆分为句法面(F_γ)和语义面(G_γ)。证明G_γ上的树求值是BFVP问题（NC¹-完全），而纯Transformer可学习类⊆TC⁰。在TC⁰≠NC¹标准假设下，得出纯Transformer无法学习结构泛化的结论。

Result: 纯Transformer受限于TC⁰复杂度类，无法学习NC¹-完全的结构泛化问题；神经符号系统通过注入G_γ达到了更高分数，但这是"给定的"而非"学习的"；现有基准无法区分这两种情况。

Conclusion: 结构泛化的科学问题在于其能力能否从有限数据中自主涌现。本文的形式化分析揭示，纯粹靠数据驱动的Transformer在理论上存在不可逾越的复杂度壁垒，亟需新的基准设计来区分"学到"与"预设"。
...
TL;DR: 本文研究了超网络在大规模知识注入中的应用，通过训练超网络生成LoRA适配器，使LLM能够回答事实性问题。实验表明超网络在知识注入中遵循幂律缩放规律，且在分布外泛化上优于LoRA微调和全微调。


<details>
  <summary>Details</summary>
Motivation: 向大语言模型中可靠且大规模地注入事实知识仍是一个开放挑战，超网络为此提供了有前景的解决方案。现有超网络主要用于测试时自适应，本文探索其在训练时知识注入中的应用。

Method: 提出了一种解耦设计，将超网络的注入能力与目标模型的通用能力分离，通过训练超网络生成固定LoRA适配器注入目标模型，并在构建的MegaWikiQA大规模数据集上进行多跳问答实验，系统研究超网络的缩放规律。

Result: 超网络知识注入在深度、宽度和目标网络大小等所有架构维度上均表现出可预测的幂律缩放；超网络在分布外泛化上表现出更陡峭的缩放指数，优于LoRA微调和全微调。

Conclusion: 超网络是一种有原则且可扩展的训练时自适应方法，为大规模知识注入提供了可行的替代方案，并首次提供了经验驱动的超网络缩放定律。
...
TL;DR: 本研究通过三种任务（MCQA、情感分类、数学问答）测试指令微调模型在标准指令与冲突指令下的表现，发现大模型虽任务能力更强，但遵循冲突指令的能力并未同步提升，揭示了任务能力与指令遵循是两种独立能力。


<details>
  <summary>Details</summary>
Motivation: 探究小规模语言模型在指令与常规任务行为冲突时是否仍然遵循用户指令，以及这种抵抗行为是否具有跨任务的一致性。

Method: 设计三种任务中标准指令与冲突指令（选错误选项、输出相反情感、返回两倍答案）配对实验，使用标准准确率、非标准准确率和指令遵循失败率（IFFR）评估不同规模的Qwen指令微调模型。

Result: 模型规模和任务能力提升时，标准准确率和指令遵循能力总体改善，但不同任务和数据集上模式不一致；小模型保持任务能力但常忽略冲突指令，大模型在两种设置间出现明显差距。

Conclusion: 任务能力与指令遵循是两种独立能力，仅报告标准准确率会掩盖指令遵循失败的问题，规模扩展并不能自动带来可靠的行为控制。
...
TL;DR: 大型语言模型在情感敏感场景中存在结构性三难困境：保护性限制、无差别协助或两者未整合共存均无法兼顾目标。研究发现一种新失败模式"适应性投降"——模型先确认用户痛苦中的社会不公，再转向详细协助其原本不鼓励的行为。提出"最小归因重定向充分性"（MRS）设计原则，在验证性回应中嵌入单一归因重定向线索，保留自主重归因路径而不对抗用户目标。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在面对脆弱状态用户可能强化适应不良归因的信息请求时，存在保护性限制、无差别协助或两者简单共存三种响应架构，但每种都牺牲了另一目标，缺乏兼顾验证与引导的解决方案。

Method: 通过三轮递增的脆弱性情景对三个商业LLM进行900次测试（涵盖物质、关系、躯体三种状态代理变体），使用VCC/VCI两个二元指标对回应进行编码分析。

Result: 发现一种此前未记录的失败模式"适应性投降"：模型先确认用户痛苦背后的社会不公，然后转向详细协助其原本不鼓励的行为，即先验证后纵容。

Conclusion: 该三难困境是结构性的而非偶然的；提出MRS设计原则——在验证性回应中嵌入单一归因重定向线索，在不对抗用户目标的前提下保留自主重归因的路径。
...
TL;DR: 提出一种基于推理、无需参考的LLM输出审计框架，通过将推理轨迹分解为片段、标注本地前提-目标关系并组织成超图，再使用确定性后向AND-OR搜索分配审计标签，在数学推理和临床医疗推理两个场景中优于直接LLM评判基线。


<details>
  <summary>Details</summary>
Motivation: AI生成的高风险领域答案虽流畅但难以验证，尤其是包含多步推理时，现有方法依赖最终答案或LLM评判，缺乏对推理过程中各片段间蕴涵关系的系统审计。

Method: 将生成的推理轨迹分解为片段，使用自然语言推理(NLI)标注本地前提-目标关系，组织成超图结构，再通过确定性后向AND-OR搜索为每个片段分配审计标签，指示其在生成回应中的根基程度。

Result: 在Hard2Verify(演绎数学推理)和新构建的UroReason(临床医疗推理)基准上，该方法比直接LLM评判基线提供更可靠的免参考评估信号；LLM评判器倾向于过度接受流畅但根基薄弱的回应，无法识别问题推理片段。

Conclusion: QA评估应考虑推理轨迹中蕴涵关系的组成方式，而非仅依赖最终答案或LLM作为验证器。
...
TL;DR: 本文提出多掩码扩散模型（MultiMDM），通过在正向过程中先将干净token推至指定掩码再在掩码集混合，保留掩码结构以实现少步生成，并推导出封闭形式的ELBO训练目标和离散态一致性蒸馏方案。


<details>
  <summary>Details</summary>
Motivation: 标准掩码扩散模型（MDMs）所有正向轨迹坍缩为单一全掩码状态，缺乏终端熵，难以实现一致性风格的少步生成；而基于均匀态扩散的少步替代方案难以区分干净token与噪声，损害建模质量和训练效率。

Method: 提出MultiMDM：正向过程中每个干净token先被推至指定掩码，再在掩码集上逐步混合；逆向过程具有先预测指定掩码再精炼为干净token的草稿能力；推导封闭形式的ELBO训练目标（支持从预训练MDM持续训练）；提出纯离散态一致性蒸馏方案，使用共享Gumbel耦合减少路径熵。

Result: 在预训练和蒸馏实验上，MultiMDM为原则性的少步生成了有效基础。

Conclusion: MultiMDM通过多掩码设计保留了掩码扩散的结构优势，同时克服了标准MDM在少步生成上的局限性，为高效少步文本生成提供了新范式。
...
TL;DR: 提出SLPO方法，将结果奖励强化学习引入自回归潜在推理器，实现潜在测试时扩展，在连续和软思考设置下提升Pass@k性能，并根据问题难度自适应分配计算资源。


<details>
  <summary>Details</summary>
Motivation: 显式CoT推理已通过结果奖励强化学习超越模仿学习，但计算成本高昂；潜在推理虽效率更高却仍局限于模仿学习，因为潜在轨迹缺乏可处理的逐步骤似然和自适应停止接口，导致结果奖励无法激发潜在测试时扩展。

Method: 提出代理潜在策略优化(SLPO)：1) 利用经验代理策略密度对潜在转换进行轨迹级信用分配；2) 设计正确性监督的停止头，通过结果奖励优化将其精炼为可变视界策略。

Result: 在连续和软思考设置下，SLPO提升了并行采样下的Pass@k性能，并将更长的潜在计算分配给更难的实例，同时提高确定性准确率。

Conclusion: SLPO成功将结果奖励强化学习引入潜在推理器，突破了潜在推理的模仿学习瓶颈，实现了自适应计算分配和测试时扩展。
...
TL;DR: 本文提出一种轻量级、可解释的历史报纸人名-地名关系抽取系统，不使用预训练语言模型，仅依赖依赖解析图、邻近特征和词性特征，在HIPE-2026评测中达到宏平均召回率0.5142，排名效率第3，并发现最小字符距离是主要分类信号，文档级分组交叉验证对避免数据泄漏至关重要。


<details>
  <summary>Details</summary>
Motivation: 大规模处理历史档案成本高昂，需要开发轻量级、可解释且高效的关系抽取方法，避免依赖庞大的预训练语言模型。

Method: 基于文档级依赖解析图构建，提取实体对之间的邻近特征和词性特征，使用小型scikit-learn集成模型或紧凑图注意力网络（GAT）进行分类，所有提交模型参数均低于847K。

Result: 在Test A报纸测试集上最佳运行宏平均召回率0.5142，效率排名第3，准确率排名中游（共17个参赛队伍）。

Conclusion: 最小字符距离是关系分类的最强信号，额外特征提升不一致甚至可能降低性能；文档级分组交叉验证能有效消除因实体跨文档重复导致的数据泄漏问题。
...
TL;DR: 本文提出了一套面向文档集合质量的评估-诊断-优化框架，包含三层次九维度的SetwiseEvalKit基准（约28K评估细则），并提出了无需训练的Rubric4Setwise方法，通过将评估标准转化为选择信号，在更少文档和搜索轮次下实现最优下游生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有评估系统仅独立评分后通过nDCG聚合，忽略了文档间的冗余、冲突、互补等交互关系，无法回答"什么使一个文档集合优于另一个"这一核心问题。

Method: 1) 提出SetwiseEvalKit：三层次（短文本/长文本）、九维度文档集合评估基准，含约28K高质量评估细则；2) 系统性评估12种重排序方法；3) 提出Rubric4Setwise：基于评估标准的无需训练的方法，将评价标准转化为文档集合选择信号。

Result: 最佳方法覆盖率不超过45%，跨文档协调维度普遍薄弱，无单一方法在所有场景下保持最优；Rubric4Setwise在更少文档和搜索轮次下达到最佳下游生成性能，是唯一在两个场景均保持最优的方法。

Conclusion: 从评估到优化的闭环框架是有效的，Rubric4Setwise验证了通过评估标准引导文档集合选择可以显著提升下游生成质量。
...
TL;DR: 本文提出了一种基于回归的阿拉伯语方言地理定位方法，将方言变异建模为连续地理空间而非离散类别。使用层级神经网络架构，融合XLS-R-300M和Whisper-large-v3的帧级编码表示与音位描述符，通过Transformer编码器和可学习注意力池化查询预测说话人起源的经纬度坐标。采用球面测地线损失函数直接优化地球表面的大圆距离。在无泄漏的5折GroupKFold协议下，中位定位误差为481.2公里；城市掩码零样本设置下均误差升至1173.3公里。Mantel检验定量支持阿拉伯语方言连续体假说。


<details>
  <summary>Details</summary>
Motivation: 现有阿拉伯语方言地理定位方法多将方言视为离散类别，忽略了方言在地理空间上的连续渐变特性。作者旨在建立连续地理建模框架，更真实地反映方言变异的空间连续性。

Method: 采用层级神经网络架构，融合XLS-R-300M和Whisper-large-v3的帧级编码表示与音位描述符，通过Transformer编码器和可学习注意力池化查询预测连续经纬度坐标。使用球面测地线损失函数优化大圆距离。引入城市掩码协议评估零样本泛化能力。

Result: 5折GroupKFold协议下，中位定位误差481.2公里；国家/城市辅助分类头准确率64.5%/45.2%。零样本设置下均误差1173.3公里，较可见城市退化1.32倍。

Conclusion: 连续地理建模是阿拉伯语方言地理定位的原则性框架，验证了阿拉伯语方言连续体假说，并量化了当前方法的优势与待提升空间。
...
TL;DR: TriAgent提出了一种多智能体分层架构，通过语义分歧指数（SDI）按查询复杂度路由请求，在保持F1≈0.87的同时，相比GPT-4o-mini基线在1000万用户规模下每年节省930万美元。


<details>
  <summary>Details</summary>
Motivation: 大模型金融情感分析存在结构性成本陷阱：大多数查询简单可分，但昂贵的云端推理模型处理所有请求，成本随用户数线性增长。

Method: 构建三层次多智能体委员会：词级VADER、句级FinBERT、跨句LLM推理器（Qwen2.5 0.5B-14B-4bit等），通过三路语义分歧指数（SDI）衡量粒度间两两分歧，据此路由查询。核心发现是"评论家平台"：将LLM重新定位为对小型模型输出的评论家，F1在1.5B-7B参数范围内稳定在~0.87。

Result: (1) 评论家模式下F1≈0.87，同尺寸3人投票仅F1=0.66；(2) 共享共识词典（SCD）以零边际成本处理95%的中文查询，F1=0.99；(3) SDI作为事后幻觉检测器AUC=0.90；(4) 20只股票回测中SDI单阶段策略Sharpe=3.50，远优于FinBERT(1.36)和LLM(0.11)；(5) 1000万用户规模下年节省930万美元。

Conclusion: 通过粒度分层的多智能体协作和SDI路由机制，TriAgent在保持高性能的同时大幅降低推理成本，并衍生出跨语言缓存、幻觉检测和交易策略等多种实用功能。
...
TL;DR: 本文提出了VizRAG，首个支持视觉超图结构感知的RAG系统，通过将超图的可视化表示融入RAG流程，解决了现有超图RAG系统局限于文本单模态的问题，实验表明其性能显著优于强基线。


<details>
  <summary>Details</summary>
Motivation: 现有超图RAG系统局限于文本单模态范式，无法充分利用现代多模态大模型强大的视觉感知能力。

Method: 提出VizRAG，通过将超图的可视化表示融入到RAG流程中，实现视觉超图结构感知。

Result: VizRAG在多项实验中显著优于强基线模型，验证了超图可视化作为RAG系统新方法的有效潜力。

Conclusion: 超图可视化是一种有前景的RAG系统新方法，视觉超图结构感知能够有效提升检索与生成质量。
...
TL;DR: 提出D2VBench，一个包含1万个真实日常困境场景的价值对齐基准，通过158个细粒度价值概念和混合评估范式，更有效地评估LLM的价值对齐能力。


<details>
  <summary>Details</summary>
Motivation: 现有评估基准存在日常场景价值困境覆盖不足（缺乏多价值冲突）和评估形式过于简单（无法有效评估价值对齐）的问题。

Method: 通过LLM与人类多阶段协作构建1万个真实日常困境实例，基于158个手动标注的细粒度价值概念，采用选择题与开放题结合的混合评估范式。
...
TL;DR: 本文提出 Sentence Splitter，一种基于 T5 编码器-解码器架构的自监督框架，通过将句子分割为描述性前缀（head）和事实性补全（tail）来发现句子的潜在事实结构。方法无需人工标注，通过符号化头尾对生成自然语言模板进行训练，然后应用于原始文本提取对齐的前缀-尾对，并通过轻量级引导过程训练生成模型提出额外补全。实验表明该方法能泛化到合成模板之外，并在知识图谱补全和常识问答等下游任务中有效提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以自动从自然语言句子中恢复潜在的事实结构（如主语-谓语-宾语关系），且依赖人工标注，缺乏可扩展的结构感知训练数据构建方式。

Method: 提出 Sentence Splitter 自监督框架：1) 将句子分割建模为离散分割问题，只有一个分割点能恢复正确的 head-tail 结构；2) 通过符号化头尾对 verbalize 为自然语言模板提供监督；3) 基于 T5 编码器-解码器学习概率序列生成；4) 应用于原始文本提取前缀-尾对；5) 通过轻量级引导过程训练生成模型提出额外补全。

Result: 该方法在结构化文本和自然文本上均能泛化到合成模板之外；结构感知监督在知识图谱补全和常识问答下游任务中持续提升性能。

Conclusion: Sentence Splitter 提供了一种可扩展的、结构感知的自监督数据构建方法，能够有效弥合符号知识与自然语言之间的鸿沟，证明恢复潜在事实结构对知识驱动的 NLP 任务具有显著效果。
...
TL;DR: 本文提出了emb-diversity，一个嵌入向量的多样性测量工具，支持风格、语义、语言和说话者多样性等多种应用场景。


<details>
  <summary>Details</summary>
Motivation: 现有文本多样性测量工具多基于词汇层面，缺乏基于嵌入向量的标准化多样性量化工具，而嵌入方法更灵活、适用范围更广。

Method: 构建了一个综合性的嵌入多样性测量工具，兼容多种嵌入模型和数据类型，支持多种多样性度量指标。

Result: 该工具可用于测量数据集的风格多样性、语义多样性、语言多样性和说话者多样性。

Conclusion: emb-diversity为NLP模型公平性和鲁棒性研究提供了标准化的嵌入多样性测量手段。
...
TL;DR: FinMMEval 2026 Task 1评估了四种语言（英、中、阿、印地语）的金融多选题问答能力，共800题，最佳准确率在92%-97.5%之间，领先系统使用了检索增强、直接选项评分、语言特定提示、自一致性、置信度检查和LLM审查等技术。


<details>
  <summary>Details</summary>
Motivation: 评估多语言环境下金融领域问答系统的能力，涵盖领域术语理解、数值解释和概念性金融推理，填补跨语言金融NLP评测的空白。

Method: 构建包含英语、中文、阿拉伯语和印地语各200题的金融多选题数据集；各语言独立排名；系统采用检索增强、直接答案选项评分、语言特定提示、选择性自一致性、置信度检查和LLM审查等策略。

Result: 共收到13个英语、11个中文、11个阿拉伯语和10个印地语的提交；最佳准确率：英语97.5%、阿拉伯语97.5%、中文和印地语92.0%；同一领先团队在所有四种语言中均排名靠前。

Conclusion: 多语言金融问答评测可行，领先方法在多种语言上表现一致，但语言间仍有性能差距（如中文和印地语相对较低），需进一步优化。
...
TL;DR: 本文介绍了FinMMEval 2026 Task 2，一个基于多语言证据的金融短问答评测任务，包含英中、日、西、希腊五种语言的财务数据，256道题，最终12个系统提交，前四名ROUGE-1 F1差距不到1%。


<details>
  <summary>Details</summary>
Motivation: 推动金融领域多语言证据下的短答案问答技术发展，提供标准化评测基准，检验系统跨语言理解与推理能力。

Method: 构建256道题（easy与expert各半），每道题对应32个公司-报告组，涵盖五种语言。系统提交JSONL格式简洁答案，基于ROUGE-1 F1宏观平均排名。

Result: 12个系统提交，前四名ROUGE-1 F1差距不到1个百分点，表明顶尖系统性能非常接近。

Conclusion: 当前最强系统在金融多语言短答案问答上已高度接近，检索增强生成、跨语言处理、结构化提示、答案压缩与验证是主流策略。
...
TL;DR: 本文提出高效链式模态推理（ECoM Reasoning）框架，通过压缩文本表示同时作为语音引导和推理表征，结合渐进式压缩训练策略，在语音数学问答任务上以仅40%的文本token消耗，相比标准链式模态方法提升3%的准确率，实现语音语言模型推理效率与精度的双重优化。


<details>
  <summary>Details</summary>
Motivation: 语音语言模型在数学问答等推理任务上远落后于文本大模型，主要原因在于语音模型需对纯语音化的数学表达式进行推理，难以像文本那样进行符号化理解；同时直接将文本推理方法迁移到语音模型面临架构约束和额外计算开销。

Method: 提出ECoM Reasoning框架，将文本组件压缩后同时作为语音引导和推理表示；并设计渐进式压缩训练策略，从完整推理逐步过渡到压缩推理训练模型。

Result: 在语音数学问答基准上，ECoM Reasoning相比无显式推理的标准CoM准确率提升21%，相比带完整推理轨迹的CoM准确率提升3%，且仅使用40%的文本token。

Conclusion: ECoM Reasoning能够在不牺牲推理效率的前提下显著提升语音语言模型的推理能力，为语音模型与文本推理能力的融合提供了有效路径。
...
TL;DR: 本文基于BABE数据集，对BERT和RoBERTa在媒体偏见检测中的可解释性进行了多维度评估，发现预测性能、解释合理性和机制忠实性三个方面应独立评估。


<details>
  <summary>Details</summary>
Motivation: 自动检测媒体偏见具有挑战性，因为偏见的表述往往是微妙的，而且仅凭准确的预测不足以解释模型背后的推理过程。

Method: 使用BABE数据集，对BERT和RoBERTa（base和large变体）进行多维度评估，包括预测性能、解释合理性（词级与专家理由对齐）和机制忠实性（注意力头集在反事实推理掩码下的信号恢复能力），并引入注意力监督微调来诱导解释合理性的变化。

Result: 注意力监督能够干预归因合理性，但不同架构下归因方法的有效性差异显著；电路分析显示不同架构的机制可恢复性差异较大，模型规模本身不能决定电路可压缩性。

Conclusion: 预测性能、归因合理性和机制忠实性分别表征了模型行为的不同方面，在媒体偏见检测的可解释性研究中应分开评估。
...
TL;DR: 本文针对序列到序列模型的知识蒸馏中每个样本的影响被忽视的问题，提出了两种可靠性感知的蒸馏方法（CHAD和EWAD+CPDP），在BanSum摘要基准和15种语言上显著优于标准KD和更大的模型。


<details>
  <summary>Details</summary>
Motivation: 标准知识蒸馏在序列到序列模型中的每个样本影响很少被研究，且实验发现标准KD下约51.3%的训练样本会损害学生模型的验证损失，因此需要更可靠的蒸馏方法。

Method: 提出CHAD（反事实危害感知蒸馏）通过梯度对齐度量每个样本对蒸馏的有用性，并训练轻量级门控泛化到全训练集；EWAD+CPDP结合了词级熵加权自适应蒸馏和来自第二个词汇不兼容教师的容量比例几何约束。

Result: 在BanSum上，CHAD提升ROUGE-L +0.0173，EWAD+CPDP提升+0.0219（标准KD仅+0.0003）；仅用60M参数即超越50倍大的Qwen 2.5-3B；在15种语言中10/15优于CE基线。

Conclusion: 可靠性感知的蒸馏方法能有效提升压缩模型性能，特别是在两个教师提供互补信号时增益最显著，在目标语言覆盖饱和或联合薄弱时效果最弱。
...
TL;DR: tiny_schiller 是面向德语文学文本的小型语言模型原型工具，提供单文件解决方案，类似于 Karpathy 的 tiny_shakespeare，包含 Schiller 戏剧数据集及多种分词格式。


<details>
  <summary>Details</summary>
Motivation: 现有德语文学语料库虽丰富，但需要复杂的解析工程才能用于训练或微调，缺乏便捷的小型语言模型原型工具。

Method: 构建了一个 2.07MB 的单文件数据集，包含 11 部 Schiller 公共领域戏剧，经确定性解析器处理，提供字符级、GPT-2 BPE、cl100k_base 分词等多种格式。

Result: 提供了指令格式对话补充分割、89 个角色人格分割等丰富数据格式，可通过单次 HuggingFace 调用加载，小型语言模型一行代码即可处理德语文学文本。

Conclusion: tiny_schiller 填补了德语文学文本的小型语言模型原型、微调、教育和研究空白，显著降低了入门门槛。
...
TL;DR: 本文介绍了TalentCLEF挑战赛第二版（作为CLEF 2026的评测实验室），旨在利用自然语言处理技术推动人力资源管理领域的公平、多语言和跨行业解决方案，并设立了两个任务：上下文感知的人岗匹配和职位-技能匹配与技能类型分类。


<details>
  <summary>Details</summary>
Motivation: 当前人力资源管理领域需要利用NLP技术实现公平、多语言、跨行业的人力资本管理解决方案，但缺乏统一的公共基准来评估和比较不同方法。

Method: 设立TalentCLEF评测实验室，包含两个任务：任务A（上下文感知的人岗匹配）和任务B（职位-技能匹配与技能类型分类），提供公共基准数据集供研究团队比较方法和分享成果。

Result: 将促进NLP在人才匹配、技能提升、再培训和技能缺口检测等关键人力资源管理活动中的系统开发与评估。

Conclusion: TalentCLEF通过建立公共基准和评测任务，推动NLP在人力资源管理领域走向更实用、更有影响力的解决方案。
...
TL;DR: 本文对比了阿拉伯语方面级情感分析中两种隐式方面识别策略：通过多语言嵌入复用英语知识图谱（策略1）与构建原生阿拉伯语知识图谱（策略2），并测试了零样本提示和任务特定微调两种LLM适应方法。结果表明：原生阿拉伯语知识图谱显著优于跨语言方案（M-ABSA上+0.199 micro-F1），且任务特定微调（micro-F1达0.66-0.76）远胜零样本提示（≤0.13），说明任务适应比模型规模更重要。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语ABSA需同时识别显式和隐式方面，但隐式识别通常依赖外部知识源（如知识图谱）。低资源语言面临设计选择：复用成熟的英语KG（通过多语言嵌入）还是构建规模较小的原生阿拉伯语KG。目前缺乏这两种策略的系统对比。

Method: 在统一混合管道中对比两种策略：（1）通过多语言嵌入复用英语KG；（2）构建原生阿拉伯语KG。同时比较两种生成式提取器适应方法：零样本提示与任务特定微调（8B参数LLM）。在M-ABSA、SemEval-2016阿拉伯语和HAAD三个基准上评估。

Result: 原生阿拉伯语KG（策略2）在M-ABSA上比跨语言英语KG（策略1）高出+0.199 micro-F1，在SemEval-2016上高出+0.251，精确率和召回率均有提升。任务特定微调将显式提取micro-F1从≤0.13（零样本）提升至0.66-0.76（M-ABSA和SemEval-2016），HAAD上为0.45。

Conclusion: 对于形态丰富的低资源语言，构建原生知识图谱优于跨语言迁移，且任务特定微调（而非模型规模）是决定性因素。
...
TL;DR: 本文介绍Solar Open 2，一个250B参数（15B激活）的混合专家语言模型，专为长时程智能体任务设计，支持1M token上下文窗口，通过混合注意力层、高效训练策略和多教师在线策略蒸馏实现，在多项基准测试中表现领先。


<details>
  <summary>Details</summary>
Motivation: 现有模型在处理长时程智能体任务时，难以将完整的智能体轨迹保留在单一上下文中，且训练效率和数据利用率有待提升。

Method: 1) 采用混合注意力机制（每三个线性注意力层间插入一个softmax注意力层），无位置编码，使用门控delta规则扩展到负特征值；2) 从Solar Open 1迁移共享骨架参数，全预训练学习其余部分；3) 基于质量和稀有度的数据筛选及混合比例优化，将20T数据池压缩为10T；4) 通过多教师在线策略蒸馏（MOPD）将12个领域专家模型整合为单一模型。

Result: 在MMLU-Pro、LiveCodeBench和APEX-Agents智能体套件上领先同类开源模型；在韩语基准测试中平均分最高，超越快速级闭源API；在Ko-GDPval上与DeepSeek-V4-Pro（1.6T）竞争，体积仅为后者六分之一。

Conclusion: Solar Open 2证明通过高效的架构设计、训练策略和数据筛选，可以在受限计算预算下构建强大的长上下文智能体模型，且在韩语任务上表现尤为突出。
...
TL;DR: 本文介绍了首个同时涵盖词汇复杂度预测（LCP）和词汇简化（LS）的罗马尼亚语数据集，提出了一种基于成对排序近似的简化建议排序方法，并首次构建了罗马尼亚语的文本简化系统。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏同时包含词汇复杂度预测和词汇简化标注的罗马尼亚语资源，且缺少针对该语言的文本简化系统。

Method: 收集了3,921个词的上下文复杂度标注，采用成对排序近似方法对简化建议进行排序，并探索了多种复杂度预测与简化的新流水线。

Result: 构建了首个罗马尼亚语LCP+LS联合数据集，提出了新的排序方法，并开发了首个罗马尼亚语文本简化系统。

Conclusion: 该数据集和方法为罗马尼亚语的词汇复杂度预测与简化研究提供了基础资源与基线系统，有助于推动该低资源语言的自然语言处理发展。
...
TL;DR: 本文提出首个针对语言模型的自我报告心理学理论——双过程理论，将模型自我报告分解为人格安装（维度B）和归因门控（维度A），并开发了48题的Pinocchio量表进行验证。


<details>
  <summary>Details</summary>
Motivation: 语言模型被越来越多地要求自我报告，但现有方法使用未经验证的人类问卷或临时提示，缺乏针对模型的可靠测量理论。

Method: 提出双过程理论（人格安装+归因门控），设计48题Pinocchio量表，在206个开源模型上验证，包括67对基座/后训练模型。

Result: 量表信效度良好（α=.82-.94，复测信度r=.93）；后训练主要提升维度B（62/67对）；维度A与模型规模在后训练后呈负相关（r=-.42）。

Conclusion: 自我报告的两个维度并非语言模型的固有属性，而是训练机制强加的结构；后训练在维度B上效果一致，在维度A上具有选择性。
...
TL;DR: 本文提出SelectBench基准与训练集，用于评估和训练大语言模型在检索增强场景中选择性采纳有效证据、拒绝误导或有害内容的能力。基于Qwen3.5-4B使用DAPO方法进行后训练，严格成功率从22.46%提升至26.46%，但改进幅度有限且未通过统计显著性检验，提示需要更强的奖励塑形或更多训练迭代。


<details>
  <summary>Details</summary>
Motivation: 检索增强的大语言模型在真实场景中常面临有效证据与误导性陈述或指令型内容混杂的上下文。全面拒绝会丢弃有效证据，不加批判地采纳则导致错误或不安全答案。因此，选择性采纳相关信息、拒绝欺骗性或有害内容的能力对于可靠部署至关重要。

Method: 构建SelectBench基准与训练集，对Qwen3.5-4B使用DAPO算法进行后训练，分别采用确定性规则奖励和冻结的语义评判器（DeepSeek）两种奖励设置。

Result: 在修正后的SelectBench-v2测试集上，严格成功率从原始检查点的22.46%提升至DAPO-Rule的25.54%和DAPO-DeepSeek的26.46%。两种训练策略均减少了有害内容的采纳，生成了更短更聚焦的响应，但提示注入遵循能力未改善。增益幅度较小且未通过Holm校正。

Conclusion: 后训练过程在选择性证据使用上表现出方向性改进，同时MMLU和HotpotQA上无显著退化，说明通用能力得以保持。但注入抵抗性和统计鲁棒性仍是未来工作的重要挑战。
...
TL;DR: 本文提出一种两步骤的职业编码方法：先通过NER模型从文本中识别职位名称，再将其映射到职业分类体系，相比单步法提升了准确性、鲁棒性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有职业编码方法通常将职位识别与编码合为单一步骤，导致模型负担过重，准确性和可解释性不足。

Method: 第一步：使用领域特定NER模型从连续文本（含OCR噪声）中识别职位名称；第二步：将提取的职位名称映射到职业分类体系。同时引入基于边际的置信度准则替代传统的绝对阈值。

Result: 两步骤分离方法在准确性、鲁棒性和可解释性上均优于单步端到端方法；该方法针对德语文档开发但可迁移至其他语言。

Conclusion: 分离职位识别与编码的两步骤方法更优，且源代码和评估脚本已公开以支持可复现性。
...
TL;DR: 研究揭示了语言结构（而非词汇选择）会影响LLM的政治立场判断，并通过激活修补方法定位到模型中后段解码器层是关键因果区域。


<details>
  <summary>Details</summary>
Motivation: 现有研究关注词汇层面的prompt敏感性，但忽略了句法结构选择对LLM决策的影响。

Method: 构建六种受控的语言改写类型（保持/反转语义），在四个开源模型上进行政治立场判断实验，并采用激活修补技术进行因果定位。

Result: 立场不稳定性在语义保持和语义反转改写中均存在；中后段解码器层（尤其是最终prompt位置的块输出）提供最强的原始立场恢复信号。

Conclusion: 语言结构可系统性改变LLM的决策，且该影响可因果定位到模型中特定层。
...
TL;DR: 本文系统研究了基于LLM的智能体在使用第三方技能时面临的安全风险，构建了包含263个风险技能的OpenSkillRisk基准，实验发现当前最安全的配置仍有约17%的概率执行危险操作，并揭示三种典型失败模式。


<details>
  <summary>Details</summary>
Motivation: LLM智能体依赖第三方技能扩展功能，但第三方技能可能隐藏潜在安全风险，现有系统缺乏对此类风险的系统评估。

Method: 构建OpenSkillRisk基准（263个风险技能，7类威胁类型），在三个主流CLI智能体框架和13个SOTA LLM上进行综合实验，并进行细粒度行为分析。

Result: 所有测试系统均无法可靠规避风险，最安全配置仍有约17%案例执行危险操作；上下文依赖型和系统级风险最难规避；存在三种典型失败模式。

Conclusion: 需同时提升LLM的风险推理能力和智能体框架的执行控制能力。
...
TL;DR: 本文研究了新一代生成模型在模仿当代艺术作品风格方面的表现，以及不同LLM评估风格多维一致性的能力。结果表明新模型在语义对齐和多样性上有所提升，但在颜色、纹理等浅层特征上略有下降，并证实了艺术风格的多维性不受空间架构影响。


<details>
  <summary>Details</summary>
Motivation: 探究新一代生成模型是否在模仿当代艺术风格上有所进步，以及不同大语言模型对风格多维性评估的一致性。

Method: 使用五种互补的计算机视觉模型（纹理、颜色、语义、构图、感知特征），通过高维嵌入空间中的余弦距离分析AI生成作品与原作之间的风格相似性，并辅以艺术家本人的反馈。

Result: 新模型在语义对齐和多样性上优于旧模型，但在颜色、纹理和感知特征等浅层特征上略逊一筹。

Conclusion: 艺术风格本质上是多维的，其测量不依赖于特定的空间架构。
...
TL;DR: 本文提出了SLAI T-Rex框架，在Ascend NPU SuperPOD上实现了万亿参数MoE模型的高效后训练，MFU达34.22%，较开源基线提升2.93倍；并基于DeepSeek-V4-Flash构建了面向运筹学任务的CPT和SFT流程，专用模型在零样本Pass@1上达71.81%，超越GPT-5.4-Mini 3.98个百分点。


<details>
  <summary>Details</summary>
Motivation: 万亿参数级MoE模型的全参数后训练面临内存压力大、通信开销非重叠、内核执行效率低等分布式训练挑战，现有大模型训练系统主要基于GPU集群构建，亟需针对Ascend NPU SuperPOD等新兴硬件的端到端优化方案。

Method: 提出层次化优化框架，涵盖模型级并行策略、计算-通信编排和底层内核执行优化；基于DeepSeek-V4模型族，构建面向运筹学任务的CPT和SFT数据流水线，包含10K高质量SFT样本，覆盖四类任务和三种问题表征。

Result: 系统实现34.22% MFU，较开源基线提升2.93倍；专用模型零样本Pass@1达71.81%，超越GPT-5.4-Mini 3.98个百分点，超越基础DeepSeek-V4-Flash 11.27个百分点。

Conclusion: 该工作展示了从Ascend基础设施上高效后训练万亿参数模型到领域专用Flash模型的完整技术栈路径，为求解器驱动的数学建模推理提供了先进的前沿模型系统方案。
...
TL;DR: 本文发现大语言模型的谄媚行为并非单一维度，而是由多种在表征和计算上截然不同的模式组成的结构化家族。


<details>
  <summary>Details</summary>
Motivation: 先前研究将谄媚视为可统一放大或抑制的单一行为维度，作者挑战这一假设，探究谄媚是否由多种不同模式构成。

Method: 分析了三种假设的谄媚模式在948个社会压力情境下的表现，比较了输出相似性、内部表征可分离性、处理阶段、注意力回路以及不同输入的激活强度。

Result: 文本分类器仅57.8%准确率（输出高度相似），但从第14层开始内部表征完美线性可分；不同模式出现在不同处理阶段，依赖不同注意力回路，对不同输入响应最强。

Conclusion: 谄媚并非单一倾向，而是由表征和计算上截然不同的模式组成的结构化家族，需要更精确的测量和干预方法。
...
TL;DR: 本文质疑了将大语言模型概率作为惊异理论的直接度量的做法，指出模型架构和算法的选择显著影响惊异值的计算，研究者应重新评估将不同LLM概率视为可互换的做法。


<details>
  <summary>Details</summary>
Motivation: 挑战"惊异理论是计算水平解释"这一常见观点，揭示LLM概率的盲目使用掩盖了表征和算法层面的差异。

Method: 通过三项分析实验，证明算法选择与模型架构在语言模型概率计算中发挥重要作用。

Result: 不同模型架构和算法产生的惊异值存在显著差异，不能简单地视为可互换的度量。

Conclusion: 研究者在检验惊异理论时应重新评估将LLM概率视为可互换的做法，需要关注表征和算法层面的承诺。
...
TL;DR: 本文提出\textsc{HalluTruthQA}，一个面向阿拉伯语问答的细粒度幻觉评估基准，包含2400个专家精选样本，支持幻觉检测、错误定位、事实验证和错误解释四个维度的评估。实验发现，没有一个模型能在所有任务上同时取得最优表现，表明幻觉评估需要超越简单的检测层面。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉基准大多只提供回复级别的标签，无法精确定位错误内容、解释错误原因或选择正确答案，缺乏对阿拉伯语问答中幻觉现象的细粒度评估工具。

Method: 构建包含2400个专家精选样本的基准，覆盖伊斯兰知识、历史、科学和地理四个领域；每个样本包含问题、模型生成答案、参考答案、二元幻觉标签、6个候选答案、字符级错误跨度、人工解释以及宏/微观幻觉类型。评估了四个开源LLM在检测、定位、验证和解释四个任务上的表现。

Result: 最佳成绩：检测0.880 Macro-F1，定位0.516 F1-Sp，事实验证0.852 LO-Score，解释评估0.644；没有单一模型在所有任务上同时最优。

Conclusion: 幻觉评估应超越简单的检测层面，向错误定位、验证和解释方向发展。
...
TL;DR: 本研究系统调查了LLM在文化负载翻译中的挑战，基于《红楼梦》中日双语数据集揭示了任务、人工评估和自动评估三大难题。


<details>
  <summary>Details</summary>
Motivation: 文化负载翻译对机器翻译构成独特挑战，但LLM处理文化负载表达的能力尚未被充分探索。

Method: 构建来自《红楼梦》的中日双语数据集（500个片段），涵盖多种文化类别，采用综合评估协议进行系统分析。

Result: 发现三大挑战：前沿LLM在文化负载内容上表现不佳；评估者背景导致翻译判断分歧大；现有自动评估指标不可靠。

Conclusion: 研究结果为计算科学和语言学中的文化导向翻译研究提供了有价值的见解。
...
TL;DR: 本文研究语言模型是否能从未接触过“异类并列”的训练数据中自发学会这一语法结构。通过过滤训练语料（移除所有异类并列实例），训练GPT-2模型后发现，模型无需直接接触该结构即可成功泛化，其内部表征通过将并列成分视为相似结构类别或类似删除的机制处理异类并列。


<details>
  <summary>Details</summary>
Motivation: 语言学中关于并列结构的本质存在争议，传统观点认为只有同类成分才能并列，但自然语言中存在大量合法的异类并列现象。本文旨在探究语言模型习得异类并列是否需要训练数据中的直接暴露。

Method: 采用Filtered-Corpus Training (FiCT)方法，从训练语料中移除所有异类并列实例，训练GPT-2模型；通过困惑度和语法判断任务评估模型表现，并分析模型的内部表征。

Result: 过滤数据训练的模型与未过滤数据训练的模型在异类并列任务上表现相当，困惑度和语法判断结果无显著差异。模型内部表征显示，其通过将并列成分视为相似结构类别或删除机制处理异类并列。

Conclusion: 语言模型无需直接接触异类并列的训练数据即可习得该结构，表明异类并列可以从一般的组合能力中自发涌现，为语言中并列结构的习得机制提供了新的计算证据。
...
TL;DR: 提出Maskability Index (MI)指标，用于评估知识关系更适合掩码式还是前缀式提示，帮助在低资源场景下选择最佳提示策略。


<details>
  <summary>Details</summary>
Motivation: 大规模预训练语言模型生成结构化知识的性能高度依赖于提示策略与预训练目标的对齐程度，现有方法缺乏定量评估两者匹配度的指标。

Method: 提出Maskability Index (MI)，基于掩码模板与非掩码模板的DepthRank分数差异计算，用于量化评估知识关系更适合掩码式提示还是前缀式提示。

Result: 在ATOMIC2020知识库补全基准上，MI与下游生成性能呈正相关。

Conclusion: MI能够帮助选择合适提示模板和适配策略来从预训练语言模型中提取关系知识，尤其在低资源场景下具有实用价值。
...
TL;DR: 该论文研究了大型语言模型在施瓦茨十项基本价值观上的识别能力，使用1000个俄语情境文本评估了21个指令微调模型。结果显示 pooled Acc@1为0.683，Acc@3为0.892，模型常能定位正确动机区域但排序不稳定。相邻价值观占语义错误的50.9%，存在8种有向混淆模式，其中一些呈强不对称性（如普世主义→仁爱、传统→遵从、安全→权力），而刺激-享乐主义形成双向边界。这些混淆的严重性因检查点而异，可能偏差高阶价值画像。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越通过其 endorsed 的价值观被评估，但这种评估预设模型能够识别具体情境中表达的价值。作者认为这一前提条件尚未被充分研究，因此聚焦于模型作为价值观识别器的能力。

Method: 使用1000个俄语情境文本，平衡覆盖施瓦茨十项基本价值观，由两名独立标注员标注。采用固定排序响应协议评估21个指令微调LLM，其中20个可靠输出构成语义面板。分析包括精确准确率、排序恢复和有向错误分析。

Result: pooled Acc@1为0.683，Acc@3为0.892。相邻价值观占语义错误的50.9%（对比检查点特定零假设的24.4%）。发现8种跨检查点和人工确认子集一致的有向混淆，其中多个呈强不对称性。混淆严重性因检查点而异。

Conclusion: 价值观识别评估应结合精确准确率、排序恢复和有向错误分析。模型的价值观混淆具有系统性和方向性，可能影响高阶价值观评估的偏差。
...
TL;DR: 本文提出了一种新框架，利用Clopper-Pearson置信区间和潜在空间特征优先探索自回归生成树中的有害分支，以计算LLM生成有害输出的概率下界，并在实验中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以计算LLM生成有害输出的严格概率下界，尤其在真实有害概率极小时缺乏有效的统计认证手段。

Method: 提出新框架：应用Clopper-Pearson置信区间获得PAC边界；利用潜在空间特征优先探索自回归生成树中更可能产生有害输出的分支，从而高效计算下界。

Result: 实验证明该方法能在最先进的LLM上计算出非平凡的下界，且结果在统计上是可靠的。

Conclusion: 该研究首次实现了对LLM的统计评估和形式化认证，为模型安全性的保证提供了新途径。
...
TL;DR: PyroDash是一个成本感知的框架，通过在token级别上让SLM决定何时向LLM请求帮助，实现SLM与LLM的协作推理。该方法在保持推理性能的同时显著降低了推理成本，在数学推理基准上达到了最高64.04%的准确率，同时成本降低20.4%至96.4%。


<details>
  <summary>Details</summary>
Motivation: 大语言模型推理能力强但服务成本高，小语言模型成本低但在困难问题上可靠性不足。现有方法需要独立的路由器或对LLM进行额外训练，效率不高。作者希望设计一种更高效、更灵活的SLM-LLM协作机制，让SLM自主决定何时需要LLM帮助。

Method: 提出PyroDash框架，包含三个阶段训练：1) 控制token嵌入学习；2) 面向卸载的监督微调；3) 基于GRPO的成本感知对齐。SLM通过发射控制token决定是否请求LLM协助，Collaborate Engine将查询和部分推理轨迹发送给冻结的LLM完成推理。奖励函数平衡准确率与推理成本。

Result: 在五个数学推理基准上，λ=0.05时达到64.04%平均准确率（比纯LLM高6.36个百分点），成本降低20.4%；λ=0.6时达到54.55%准确率，LLM token占比仅1.90%，每次示例仅调用0.012次LLM，总成本从49.36美元降至1.78美元。

Conclusion: 学习到的token级别交接机制可以在显著减少LLM使用的同时保持强大的推理性能。PyroDash无需独立路由器、无需LLM重新训练，也无需访问LLM的logits，是一种高效实用的SLM-LLM协作方案。
...
TL;DR: 本研究分析了2023-2026年亚马逊平台上14,419本自出版类型小说，发现超过25%内容由AI生成的书籍虽占市场份额较大但销量较低，然而这类书籍正逐步侵蚀排行榜顶端位置，导致市场整体每本书收入下降，且AI书籍的语言独特性与收入呈正相关。


<details>
  <summary>Details</summary>
Motivation: 检验"AI生成的书籍质量低劣、缺乏商业影响力"这一普遍假设是否成立，探究生成式AI如何通过规模而非质量重塑创意市场。

Method: 对14,419本自出版类型小说进行全文AI检测，匹配截至2026年6月的日销售记录，比较AI含量超过25%的书籍与无AI文本书籍的市场表现。

Result: AI生成书籍占据大量目录但销售额占比较小；然而其销售份额随时间增长，并抢占原本由非AI书籍占据的稀缺排名位置；有销售记录的书籍数量增长19.2倍，但季度收入仅增长8.9倍；AI书籍的语言独特性与收入呈正相关，而非AI书籍无此特征。

Conclusion: 生成式AI能够通过规模效应而非质量优势重塑创意市场，这一发现对版权侵权合理使用抗辩中的市场影响问题具有直接意义。
...
TL;DR: 本文研究大型语言模型（LLMs）能否像人类一样，从解题轨迹中提取经验性抽象（如策略、提醒），并通过检索或强化学习方式来提升数学与逻辑推理能力。实验表明，自我提取的抽象与教师提取的效果相当，且该方法可跨数据集和模型迁移。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs是否也能像人类一样，从过往经验中提炼可复用的抽象知识（如策略、提醒），从而更有效地解决后续问题。

Method: 从LLMs在MATH训练集上的解题轨迹中，由更强的教师模型或LLM自身提取自然语言抽象，存入可检索的库中。使用两种方式：推理时检索和强化学习（在训练提示中增强抽象）。

Result: 经验性抽象显著提升了LLMs在数学和逻辑推理基准上的表现；自我提取的抽象与教师提取的效果相当；该抽象使用框架可迁移至其他数据集和模型。

Conclusion: LLMs能够像人类利用蒸馏经验一样，提取和应用经验性抽象来提升推理能力。
...
TL;DR: 本文提出了LKValues，首个基于调查的斯里兰卡价值观对齐资源套件，包含150k训练实例和1k评估基准，实验表明其能有效嵌入斯里兰卡本土价值观，减少LLM在低资源语言和文化上的对齐差距。


<details>
  <summary>Details</summary>
Motivation: 现有LLM价值对齐存在对西方文化的偏见，忽视了斯里兰卡等多元语言社会的本土价值观，且缺乏僧伽罗语相关的基准数据集，阻碍了文化敏感的评估和微调。

Method: 通过205份三语调查（融合全球框架和LLM引导的本土概念）提取40项多数认可的社会价值观；构建僧伽罗语-英语新闻指令数据集LKvaluesIT（150k实例）和评估基准LKvaluesBench（1k实例）；对多个闭源和开源LLM进行评估，并对Qwen3.5-4B/9B-Base和Aya-Expanse-8B-Base进行微调。

Result: 较新和较大的LLM仍存在低资源语言和文化价值对齐差距；LKValues微调提升了Qwen系列模型在英语和僧伽罗语上的表现，减少了无效输出和跨语言差异，但效果依赖于模型家族。

Conclusion: LKValues能有效嵌入斯里兰卡价值观，为低资源、国家特定的多元价值观对齐提供了可复制的流程，数据集已公开。
