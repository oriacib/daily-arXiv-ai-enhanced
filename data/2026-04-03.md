...
TL;DR: DOne框架通过解耦结构理解和元素渲染来解决VLM在Design-to-Code中的"整体瓶颈"问题，在复杂布局数据集上显著优于现有方法。
...
Conclusion: DOne通过解耦结构理解和元素渲染的方法有效解决了VLM在Design-to-Code中的瓶颈问题，在复杂布局场景下实现了更好的代码生成质量和效率。
...
TL;DR: Look Twice (LoT)是一个无需训练、在推理时改进多模态大语言模型使用视觉和文本证据的框架，通过注意力模式识别相关证据并用提示标记突出显示，提升知识密集型视觉问答性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在回答知识密集型视觉问题时，难以识别最相关的视觉和文本证据，需要整合视觉线索与可能嘈杂或不完全相关的检索文本证据，同时定位图像中的细粒度视觉信息。

Method: 利用模型注意力模式估计与查询相关的视觉区域和检索文本元素，然后基于这些突出显示的证据生成答案。通过轻量级的提示级标记突出选定的线索，鼓励模型在生成过程中重新关注相关证据。

Result: 在多个基于知识的视觉问答基准测试中，相比零样本多模态大语言模型取得了持续改进。在纯视觉中心和幻觉导向基准上的额外评估进一步表明，仅视觉证据突出显示也能在没有文本上下文的情况下提升模型性能。

Conclusion: Look Twice框架无需额外训练或架构修改，通过推理时重新关注相关多模态证据，有效提升了预训练多模态大语言模型在知识密集型视觉问答任务中的表现。
...
TL;DR: MedQwen是一个参数高效的医学视觉语言模型，通过频谱路由的专家混合架构和理论支持的缩放规则，在保持基础架构不变的情况下，实现了接近全参数微调的性能，同时大幅减少可训练参数并缓解灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在通用基准上表现出色，但在医学影像领域缺乏鲁棒性，存在跨数据集干扰、对数据机制敏感等问题。在临床工作流中，数据和任务按顺序到达，简单的持续训练会导致灾难性遗忘。

Method: 提出MedQwen模型，结合频谱路由的专家混合架构和理论支持的缩放规则。从预训练权重的非重叠奇异值分解段初始化每个专家，引入残差补偿和缩放方案，实现稳定的专家专业化和分布偏移下的一致路由。

Result: 在23个医学数据集上（包括视觉问答、报告生成、放射学分类和幻觉缓解），MedQwen实现了强大可靠的性能：在零样本分类上接近全参数微调，可训练参数减少339倍；将顺序遗忘减少到约5%，而强基线模型性能下降20-50%。

Conclusion: MedQwen通过参数高效的架构设计，成功解决了医学视觉语言模型中的跨数据集干扰、数据机制敏感性和灾难性遗忘问题，为临床工作流中的持续学习提供了有效解决方案。
...
TL;DR: 基于视觉Transformer的视频分析方法，通过处理类别不平衡问题，在扩展的美式足球训练视频数据集中检测危险擒抱动作，相比基线方法提升了8%以上的危险召回率。
...
Method: 使用基于视觉Transformer的模型，采用不平衡感知训练方法。构建了包含733个单人擒抱假人视频片段的数据集，每个片段围绕首次接触点进行时间定位，并使用标准擒抱技术评估(SATT-3)的撞击区域组件进行标注。

Result: 在交叉验证下获得危险召回率0.67和危险F1分数0.59。相比先前在较小子集上的基线方法（危险召回率0.58，危险F1分数0.56），危险召回率提升了超过8个百分点。

Conclusion: 视觉Transformer视频分析结合仔细处理类别不平衡问题，能够可靠地检测罕见但安全关键的擒抱模式，为教练中心的伤害预防工具提供了实用途径。
...
TL;DR: 使用合成蹦床姿态数据集微调ViTPose模型，显著提升极端姿态下的2D和3D姿态估计精度


<details>
  <summary>Details</summary>
Motivation: 蹦床体操涉及极端人体姿态和不常见视角，现有姿态估计模型在这些情况下表现不佳，需要专门的数据集来提升性能

Method: 从蹦床动作捕捉数据生成合成蹦床姿态数据集(STP)，将噪声动作捕捉数据拟合到参数化人体模型，生成多视角真实图像，用此数据微调ViTPose模型

Result: 在2D姿态估计上获得最先进结果，缩小了常见姿态与极端姿态之间的性能差距；在3D姿态估计上，MPJPE减少了12.5毫米，相比预训练ViTPose模型提升了19.6%

Conclusion: 通过合成数据微调可以有效提升姿态估计模型在极端姿态和不常见视角下的性能，为特定运动领域的姿态分析提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: ViT的注意力分数通常非零且分散，导致注意力图噪声大、解释性差。需要量化注意力分数的不确定性，获得正则化的注意力分数以提升解释质量。

Method: 提出注意力正则化方法：将ViT注意力分数置于统计框架下，通过自助法重采样输入特征生成注意力分数的基线分布，据此估计注意力分数的显著性和后验概率，去除噪声引起的虚假注意力。

Result: 在自然图像和医学图像上，该方法能有效去除噪声产生的虚假注意力，显著提高注意力图的收缩性和稀疏性。通过模拟和真实数据集进行了定量评估。

Conclusion: 自助法可作为实用的正则化工具，在使用注意力分数作为ViT解释时提高其可靠性和解释性。
...
TL;DR: 研究发现CNN模型作为视觉系统模型的质量与其纹理感知能力之间没有关联，表明纹理感知机制不同于基于物体识别的CNN建模方法


<details>
  <summary>Details</summary>
Motivation: 探索CNN模型作为视觉系统模型的质量是否与其纹理感知能力相关，特别是那些被认为更好的视觉系统模型是否具有更接近人类的纹理表示

Method: 比较不同CNN模型特征相关性捕获的感知内容，并将其与Brain-Score测量的模型与哺乳动物视觉系统的感知对齐进行比较

Result: 令人惊讶地发现，CNN作为视觉系统模型的常规质量测量与其与人类纹理感知的对齐之间没有联系

Conclusion: 纹理感知涉及与基于物体识别训练的CNN建模方法不同的机制，可能依赖于上下文信息的整合
...
<details>
  <summary>Details</summary>
Motivation: 当前手术自动化方法在临床部署中存在局限性：缺乏对器械在组织表面交互位置的预测能力，缺少强制执行工具-动作特定安全交互区域的显式条件输入。需要解决这一差距以实现更安全的手术自动化。

Method: 结合了三个核心组件：1) 捕捉多视角下工具运动和组织动态的时间视觉编码器；2) 实现跨不同器械-动作对泛化的语言条件化模块；3) 用于密集可操作性预测的DiT风格解码器。建立了首个组织可操作性基准数据集，包含103个胆囊切除术的15,638个视频片段。

Result: 实验显示相比视觉语言模型基线有显著改进（20.6像素ASSD vs. 60.2像素的Molmo-VLM），表明任务特定架构在大规模基础模型上表现更优，能有效预测工具-动作特定的组织可操作性区域。

Conclusion: AffordTissue通过预测工具-动作特定的组织可操作性区域，为安全手术自动化提供了显式空间推理，可能解锁对适当组织区域的显式策略指导，并在器械偏离预测安全区域时实现早期安全停止。
...
TL;DR: GRAZE是一个无需训练的流程，用于在美式足球训练视频中定位"首次接触点"，无需接触标注，通过目标检测、运动感知时间推理和像素级接触验证实现鲁棒定位。
...
Method: GRAZE采用无训练流程：1) 使用Grounding DINO发现候选球员-假人交互；2) 通过运动感知时间推理进行细化；3) 使用SAM2作为显式像素级接触验证器，不依赖检测置信度。候选发现和接触确认的分离使方法对杂乱场景和冲击附近不稳定的定位具有鲁棒性。

Result: 在738个擒抱训练视频上，GRAZE在97.4%的视频中产生有效输出，在77.5%的视频中在±10帧内定位首次接触点，在82.7%的视频中在±20帧内定位首次接触点。

Conclusion: 结果表明，在真实世界训练视频中实现帧级准确的接触开始定位是可行的，无需任务特定训练。候选发现和接触验证的分离方法对杂乱场景具有鲁棒性。
...
TL;DR: 提出基于稀疏体素栅格化的新框架，解决3D高斯泼溅在开放词汇3D场景理解中的空间模糊和多级语义模糊问题，实现确定性特征注册并提升细粒度查询性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的开放词汇3D场景理解方法存在两个关键限制：1) 非结构化、重叠的高斯分布导致空间模糊，需要概率特征注册；2) 基于对象级掩码的特征池化导致多级语义模糊，稀释细粒度细节。

Method: 使用稀疏体素栅格化作为结构化、不相交的几何表示，通过单目深度和法线先验进行正则化，建立稳定的几何基础。实现确定性、置信度感知的特征注册过程，并利用基础模型AM-RADIO的密集对齐特性解决多级语义模糊，避免分层训练的计算开销。

Result: 在开放词汇3D对象检索和点云理解基准测试中达到最先进性能，特别是在细粒度查询方面表现出色，而传统注册方法在这些任务上通常失败。

Conclusion: 稀疏体素栅格化提供了一种有效的结构化几何表示，能够解决3D高斯泼溅中的空间和多级语义模糊问题，通过确定性特征注册和利用基础模型的密集对齐特性，显著提升了开放词汇3D场景理解的性能。
...
Conclusion: 该通用分析流程能够从复杂姿态时间序列中提取理论上有意义的见解，支持跨不同实验环境的人类运动行为分析
...
TL;DR: 该论文提出了一种结构化奖励机制来解决多模态大语言模型在视频理解中的幻觉问题，通过分解视频描述为事实和时间单元进行细粒度监督，显著提升了模型的视觉和时间基础能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频理解中虽然能生成看似合理的输出，但经常存在视觉和时间基础不准确的问题，如虚构对象存在、分配错误属性或压缩重复事件。标准的句子级监督无法有效检测这些细粒度的基础错误，需要更精细的评估和训练方法。

Method: 提出了一种结构化奖励机制，包含三个互补组件：1）实例感知的场景图奖励，用于监督事实对象、属性和关系；2）时间奖励，用于监督事件排序和重复；3）视频基础的可视问答奖励，用于分层自验证。该方法通过强化学习将句子级奖励替换为结构化奖励。

Result: 在时间理解、通用视频理解和幻觉导向的基准测试中，该结构化奖励方法在开源骨干模型上取得了持续的性能提升，表明结构化奖励塑造是提升视频理解忠实性的实用途径。

Conclusion: 结构化奖励机制能有效解决多模态大语言模型在视频理解中的幻觉问题，通过细粒度的监督提升模型的视觉和时间基础能力，为更忠实的视频理解提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D建模面临重建保真度与生成合理性之间的基本矛盾：前馈重建方法效率高且与输入对齐，但缺乏全局先验导致结构不完整；而基于扩散的生成方法能提供丰富的几何细节，但难以保持多视图一致性。

Method: 提出UniRecGen统一框架，将重建和生成两种范式整合到单一协作系统中。通过将两种模型在共享规范空间中对齐，解决坐标空间、3D表示和训练目标的内在冲突。采用解耦协作学习，保持训练稳定性并在推理时实现无缝协作：重建模块提供规范几何锚点，扩散生成器利用潜在增强条件来细化和补全几何结构。

Result: 实验结果表明，UniRecGen在从稀疏观测创建完整且一致的3D模型方面，实现了卓越的保真度和鲁棒性，优于现有方法。

Conclusion: UniRecGen通过统一重建和生成范式，在共享规范空间中实现协作学习，有效解决了稀疏视图3D建模中的基本矛盾，能够创建更完整、更一致的3D模型。
...
TL;DR: TAG框架通过非参数纹理恢复解决热成像中的鬼影效应，首次实现夜间人脸的清晰表情恢复，超越了现有HADAR技术的局限。


<details>
  <summary>Details</summary>
Motivation: 热成像在夜视中至关重要，但受到鬼影效应（纹理细节丢失）的困扰。现有HADAR技术仅适用于材质均匀的场景，而现实世界中材质不均匀是普遍存在的，因此需要一种通用的抗鬼影成像方法。

Method: 提出TAG（热抗鬼影）通用计算热成像框架，利用高光谱光子流进行非参数纹理恢复，解决材质不均匀问题，实现高保真夜视。

Result: TAG在多种场景中普遍优于HADAR，首次实现了鬼影人脸的清晰表情恢复，展示了热3D拓扑对齐和情绪检测能力，揭示了材质不均匀对HADAR效果边界的影响。
...
<details>
  <summary>Details</summary>
Motivation: 无人机低空图像语义分割面临极端尺度变化、复杂边界和边缘设备计算资源有限等挑战。现有transformer方法计算开销大，轻量级方法难以捕捉高分辨率航拍场景的细粒度细节
...
Conclusion: PBSeg为无人机应用提供了一种高效的原型分割框架，在计算复杂度和分割质量之间取得了良好平衡，适用于资源受限的边缘设备部署
...
Method: ReFlow包含三个核心模块：1）完整规范空间构建模块，增强静态和动态区域的初始化；2）基于分离的动态场景建模模块，解耦静态和动态组件；3）自校正流匹配机制，包括全流匹配和对齐3D场景流与时间变化的2D观测，以及相机流匹配确保静态对象的多视角一致性。
...
TL;DR: VideoZeroBench：一个用于长视频问答的分层基准测试，通过严格验证时空证据来评估视频多模态大语言模型的真实推理能力，发现现有模型在需要精确时空定位时表现极差。


<details>
  <summary>Details</summary>
Motivation: 当前视频多模态大语言模型的评估存在两个关键局限：(1) 夸大的分数掩盖了细粒度视觉理解和推理的缺陷；(2) 答案正确性的衡量通常不验证模型是否识别了支持其预测的精确时空证据。需要一个新的基准测试来评估模型的真实证据推理能力。

Method: 提出VideoZeroBench，一个用于挑战性长视频问答的分层基准测试。包含500个手动标注的问题，涵盖13个领域，每个问题都配有作为证据的时间区间和空间边界框。引入五级评估协议，逐步收紧证据要求，以区分答案生成、时间定位和空间定位能力。

Result: 实验显示，即使在标准端到端QA设置下（Level-3），Gemini-3-Pro正确回答的问题也不到17%。当施加定位约束时，性能急剧下降：在需要正确答案和准确时空定位的情况下（Level-5），没有模型超过1%的准确率，大多数模型无法实现任何正确的定位预测。这些结果揭示了表面答案正确性与真实证据推理之间的显著差距。

Conclusion: 基于证据的视频理解仍然是长视频问答的瓶颈。VideoZeroBench暴露了当前模型在时空定位方面的严重不足，为未来基于证据的视频推理研究提供了重要见解。基准测试和代码将公开提供。
...
Result: 在广泛使用的数据集上，与各种最先进的表格-图像融合基准和测试时表格缺失基准相比，该方法显示出优越性
...
TL;DR: 提出卫星图像无关训练框架SFT，通过无人机多视角序列重建3D场景生成伪正射影像，实现无卫星数据训练的跨视图地理定位
...
Method: 提出三阶段卫星无关训练框架：1) 使用3D高斯溅射从多视角无人机图像重建密集3D场景；2) 通过PCA引导的正射投影生成几何归一化的伪正射影像；3) 提取DINOv3特征并学习Fisher向量聚合模型，仅使用无人机数据进行训练

Result: 在University-1652和SUES-200数据集上的实验表明，SFT框架显著优于卫星无关泛化基线，缩小了与使用卫星图像训练方法的性能差距

Conclusion: 提出的卫星无关训练框架能够有效解决实际部署中卫星数据不可用的问题，通过无人机端3D重建和几何归一化表示，实现了不依赖卫星数据的跨视图地理定位
...
<details>
  <summary>Details</summary>
Motivation: 现有评估指标（如mAP）将HOI类别视为离散分类标签，无法正确评估语义有效但词汇不同的预测（如"靠在沙发上"vs"坐在沙发上"），限制了开放词汇预测的评估能力。

Method: SHOE将每个HOI预测分解为动词和物体组件，使用多个大语言模型（LLMs）的平均值估计它们的语义相似度，然后组合成相似度分数，实现超越精确字符串匹配的评估。

Result: SHOE分数比现有指标（包括基于LLM和嵌入的基线）更接近人类判断，与平均人类评分的吻合度达到85.73%。
...
Result: 在CIFAR-10-C、CIFAR-100-C、Tiny-ImageNet-C和ImageNet-C上实现了强大的OOD检测能力，同时保持高ID分类性能；在Cityscapes和HAC数据集上也验证了方法的有效性

Conclusion: ROSETTA方法通过新的损失函数设计，有效解决了开放集测试时适应中ID分类和OOD检测的权衡问题，在多个数据集上表现出色
...
TL;DR: F3DGS是一个联邦3D高斯溅射框架，用于去中心化的多智能体3D重建，通过共享几何骨架和可见性感知聚合解决分布式机器人场景中的几何一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法需要集中访问所有观测数据，这在分布式机器人系统中不适用，因为智能体独立运行且集中式数据聚合可能受限。直接将集中式训练扩展到多智能体系统会带来通信开销和几何不一致问题。

Method: 首先通过注册多个客户端的局部合并LiDAR点云构建共享几何骨架，初始化全局3DGS模型。联邦优化期间固定高斯位置保持几何对齐，每个客户端只更新外观相关属性（协方差、不透明度、球谐系数）。服务器使用可见性感知聚合更新，根据每个客户端观察每个高斯的频率加权贡献。

Result: F3DGS实现了与集中式训练相当的重建质量，同时支持跨智能体的分布式优化。收集了包含同步LiDAR、RGB和IMU测量的多序列室内数据集进行评估。

Conclusion: F3DGS框架成功解决了多智能体3D重建中的分布式优化挑战，通过固定几何位置和可见性感知聚合保持了重建质量，适用于分布式机器人应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA模型对物理可实现对抗攻击的鲁棒性研究不足。虽然已有研究通过语言扰动和2D视觉攻击揭示脆弱性，但这些攻击要么代表性不足，要么物理真实性有限。相比之下，对抗性3D纹理更具物理可行性和破坏性，但标准3D模拟器缺乏从VLA目标函数到物体外观的可微分优化路径。

Method: 提出Tex3D框架：1) 前景-背景解耦(FBD)：通过双渲染器对齐实现可微分纹理优化，同时保留原始模拟环境；2) 轨迹感知对抗优化(TAAO)：优先处理行为关键帧，并使用基于顶点的参数化稳定优化。

Result: 在模拟和真实机器人实验中，Tex3D显著降低了VLA在多个操作任务中的性能，任务失败率高达96.7%。

Conclusion: Tex3D暴露了VLA系统对物理3D对抗攻击的关键脆弱性，强调了鲁棒性感知训练的必要性。这是首个在VLA模拟环境中端到端优化3D对抗纹理的框架。
...
TL;DR: 该论文提出了一种利用稀疏自编码器和视觉语言提示来自动化提取生物形态特征的方法，构建了包含8万条特征标注的Bioscan-Traits数据集，为大规模生态研究提供了可扩展的解决方案。


<details>
  <summary>Details</summary>
Motivation: 形态特征是生物与环境相互作用的重要指标，但传统提取方法依赖专家标注，成本高、速度慢，限制了在大规模生态研究中的应用。主要瓶颈在于缺乏连接生物图像与特征级别标注的高质量数据集。

Method: 1. 在基础模型特征上训练稀疏自编码器，获得单语义、空间定位的神经元，这些神经元能持续激活有意义的形态部位；2. 引入特征标注流程：首先定位显著区域，然后使用视觉语言提示生成可解释的特征描述；3. 构建Bioscan-Traits数据集，包含19K昆虫图像的80K特征标注。

Result: 1. 构建了Bioscan-Traits数据集，涵盖BIOSCAN-5M中的19K昆虫图像和80K特征标注；2. 人类评估确认了生成的形态描述具有生物学合理性；3. 通过全面的消融研究评估设计敏感性，系统变化关键设计选择并测量其对特征描述质量的影响。
...
Method: 1. 渐进构建全局一致的运动场；2. 引入几何感知对齐模块解决视图间的方向和尺度模糊性；3. 使用紧凑的基于哈希的运动场表示，支持高效查询和动态稳定传播；4. 支持渲染中的双向运动传播，无需昂贵的视频细化。

Result: 在单个RTX 5090 GPU上，每个新场景扩展步骤需要9秒生成，随后3秒进行运动对齐和运动场更新，实现了具有全局一致环境动态的交互式4D世界生成。

Conclusion: LivingWorld框架成功解决了从单张图像生成具有环境动态的4D世界的挑战，通过渐进式运动场构建和紧凑表示实现了高效、交互式的动态场景生成。
...
<details>
  <summary>Details</summary>
Motivation: 单目3D目标检测在密集标注数据集上表现良好，但3D标注成本高昂，现实场景中通常只有部分目标被标注。现有方法在稀疏标注设置下表现不佳，需要解决稀疏监督下的检测问题。

Method: 提出包含两个关键模块的框架：1) 道路感知补丁增强(RAPA)，将分割的目标补丁增强到道路区域，同时保持3D几何一致性；2) 原型过滤(PBF)，通过原型相似性和深度不确定性过滤预测结果，生成高质量伪标签。

Result: 通过几何保持增强和原型引导的伪标注相结合的训练策略，在稀疏监督下实现了鲁棒检测。大量实验证明了所提方法的有效性。

Conclusion: 该框架有效解决了稀疏标注单目3D目标检测问题，通过创新的增强和过滤机制在有限标注下实现了良好性能，代码已开源。
...
TL;DR: 该论文提出了一种基于物理的认证方法，利用莫尔效应区分真实摄像机拍摄的视频和AI生成的视频。


<details>
  <summary>Details</summary>
Motivation: 随着视频生成技术的进步，AI合成内容越来越难以与真实视频区分，需要可靠的物理基础认证方法来识别AI生成的视频。

Method: 利用莫尔效应：当摄像机拍摄紧凑双层光栅结构时产生的干涉条纹。推导出莫尔运动不变性，证明条纹相位和光栅图像位移通过光学几何线性耦合，与观察距离和光栅结构无关。验证器从视频中提取这两个信号并测试它们的相关性。

Result: 在真实拍摄和多个最先进AI生成器的视频上进行验证，发现真实视频和AI生成视频产生显著不同的相关性特征，表明这是一种鲁棒的区分方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型在处理高度动态运动或需要精细运动控制时表现不佳，主要原因是训练数据集中缺乏此类示例。真实数据中难以获取多样化的运动模式和精确的控制信号。

Method: 提出DynaVid框架，利用计算机图形学管道渲染的合成运动数据（表示为光流）。采用两阶段生成框架：运动生成器先合成运动，然后运动引导的视频生成器基于该运动生成视频帧。这种解耦设计使模型能从合成数据学习动态运动模式，同时从真实视频保持视觉真实感。

Result: 在剧烈人体运动生成和极端相机运动控制这两个具有挑战性的场景中，DynaVid显著提升了动态运动生成和相机运动控制的真实性和可控性。

Conclusion: 通过利用合成运动数据，DynaVid有效解决了视频扩散模型在动态运动生成方面的局限性，实现了更好的运动控制能力，同时避免了合成视频的不自然外观问题。
...
TL;DR: 提出了一种无需域ID和样本的增量学习框架，用于具身多媒体系统，通过解耦表示机制和权重融合策略，在动态环境分布漂移中实现鲁棒的连续环境适应。


<details>
  <summary>Details</summary>
Motivation: 具身感知系统在开放物理空间持续交互时面临动态环境分布漂移的严峻挑战。现有域增量感知方法通常依赖测试阶段预先获取的域ID，限制了在未知交互场景中的实用性。同时，模型容易过拟合到特定上下文感知噪声，导致泛化能力不足和灾难性遗忘。

Method: 提出域ID和样本自由的增量学习框架：1）设计解耦表示机制，去除非必要的环境风格干扰，引导模型专注于提取跨场景共享的语义内在特征；2）采用权重融合策略，在参数空间中动态整合新旧环境知识，使模型适应新分布而无需存储历史数据，并最大限度保留旧环境的判别能力。

Result: 在多个标准基准数据集上的大量实验表明，所提方法在完全无样本和无域ID的设置下显著减少了灾难性遗忘，其准确率优于现有的最先进方法。

Conclusion: 该方法为具身多媒体系统提供了一种实用的增量学习解决方案，能够在未知交互场景中实现鲁棒的连续环境适应，解决了现有方法对域ID依赖和过拟合感知噪声的问题。
...
TL;DR: 该论文提出频率域适应（FDA）和谐波约束最优传输（HOT）方法，用于提升远程光电容积描记（rPPG）模型在跨域场景下的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 远程光电容积描记（rPPG）技术虽然能从面部视频实现非接触式生理测量，但在实际部署中常因域偏移导致性能显著下降。现有深度学习方法虽然在单个数据集上表现良好，但容易过度拟合与外观相关的因素（如光照、相机特性、颜色响应），这些因素在不同域间差异很大。

Method: 1. 提出频率域适应（FDA）策略，通过转移编码域依赖外观特征的低频频谱成分，鼓励rPPG模型学习对外观变化的不变性，同时保留心脏诱导信号。2. 提出谐波约束最优传输（HOT），利用心脏信号的谐波特性来指导原始表示和FDA转移表示之间的对齐，确保生理一致性。

Result: 广泛的跨数据集实验表明，提出的FDA和HOT框架有效增强了rPPG模型在不同数据集间的鲁棒性和泛化能力。

Conclusion: 频率域适应和谐波约束最优传输为rPPG模型提供了一种原则性策略，能够有效处理外观变化带来的域偏移问题，提高了模型在实际部署中的实用性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于高斯的方法主要关注外观渲染，但缺乏实例级结构感知，限制了在高度动态场景中的稳定跟踪和语义推理能力。需要一种能够同时处理高质量渲染和语义理解的统一表示方法。

Method: 1) 利用时间对齐的实例掩码和多模态大语言模型的句子嵌入，通过两个MLP解码器监督每个高斯的可学习语义特征；2) 将2D光流与4D高斯桥接，微调其运动以增强时间稳定性；3) 引入几何感知的SDF约束和正则化项，增强动态前景建模的时间一致性。

Result: 实验表明Director能够实现时间一致的4D重建，同时支持实例分割和开放词汇查询，在动态场景理解和语义推理方面表现出色。

Conclusion: 嵌入实例一致性语义自然地补充了4D建模，实现了更准确的场景分解，同时支持鲁棒的动态场景理解，为体积视频的语义感知重建提供了有效解决方案。
...
TL;DR: 该论文提出了一种新的视频对象移除方法，通过外部知识蒸馏和内部上下文交叉注意力机制，解决了现有方法难以处理对象诱导副作用（如阴影、反射、光照变化）的问题，实现了更清晰、更一致的对象移除效果。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的视频对象移除方法存在局限性，难以有效消除目标对象引起的副作用（如阴影、反射、光照变化），同时保持时空一致性。这种限制源于对目标对象及其与场景交互的物理和语义理解不足。

Method: 提出从两个互补视角引入理解：1）外部：通过知识蒸馏方案，将视觉基础模型中对象与其诱导效果之间的关系迁移到视频扩散模型中；2）内部：提出帧级上下文交叉注意力机制，使每个去噪块都基于目标区域周围信息丰富的未掩码上下文进行推理。外部和内部指导共同使模型能够理解目标对象、其诱导效果和全局背景上下文。

Result: 大量实验证明了该方法的最先进性能，并建立了首个真实世界的视频对象移除基准，以促进未来研究和社区进展。代码、数据和模型均已开源。

Conclusion: 通过结合外部知识蒸馏和内部上下文注意力机制，该方法能够更好地理解目标对象及其与场景的交互，实现了更清晰、更一致的对象移除效果，为视频编辑任务提供了新的解决方案。
...
Method: 采用两步处理：1）基于群体推理中估计的个体注视注意力热图和群体成员标量生成共享注意力热图；2）根据初始共享注意力热图细化初始群体成员关系，并最终预测共享注意力热图。

Result: 实验表明该方法在群体检测和共享注意力估计方面优于其他方法。额外分析验证了所提出组件的有效性。

Conclusion: 提出的端到端方法通过同时进行群体检测和共享注意力估计，解决了现有方法的局限性，在性能上超越了其他方法，为共享注意力检测的实际应用提供了更好的解决方案。
...
TL;DR: SteerFlow是一个模型无关的图像编辑框架，通过隐式拉直前向轨迹和自适应混合编辑/重建速度，在保持源图像保真度的同时实现文本引导的图像编辑。
...
Result: 在FLUX.1-dev和Stable Diffusion 3.5 Medium上的广泛实验表明，SteerFlow比现有方法始终获得更好的编辑质量，并能自然扩展到复杂多轮编辑范式而不累积漂移。
...
TL;DR: 提出了一种几何感知的无人机地理定位框架，通过3D场景重建和鸟瞰图渲染统一粗粒度位置识别和细粒度姿态估计，在GNSS拒止环境中实现米级定位精度。


<details>
  <summary>Details</summary>
Motivation: 无人机在GNSS拒止环境中的跨视角地理定位面临挑战，主要因为倾斜的无人机图像与正交卫星地图之间存在严重几何差异。现有方法通常采用位置检索和姿态估计的解耦流程，将透视畸变视为外观噪声而非显式几何变换。

Method: 提出几何感知的无人机地理定位框架：1）使用视觉几何基础Transformer从多视角无人机图像序列重建局部3D场景；2）渲染虚拟鸟瞰图表示，将无人机视角正交校正以对齐卫星图像；3）引入卫星注意力块，隔离每个卫星候选与重建无人机场景的交互，防止候选间干扰；4）发布重新校准的University-1652数据集，包含精确坐标标注和空间重叠分析。

Result: 在重新校准的University-1652基准和SUES-200数据集上的广泛实验表明，该方法显著优于最先进的基线方法，实现了鲁棒的米级定位精度，并在复杂城市环境中表现出更好的泛化能力。
...
TL;DR: 该研究构建了US-365K超声图像-文本数据集，提出超声诊断分类学（UDT）包含两个层次知识框架，并开发了Ultrasound-CLIP框架，通过语义软标签和语义损失改进样本区分，在分类和检索任务上达到SOTA性能。
...
Method: 1. 构建US-365K大规模超声图像-文本数据集（36.5万配对样本，52个解剖类别）；2. 建立超声诊断分类学（UDT），包含超声层次解剖分类学和超声诊断属性框架；3. 提出Ultrasound-CLIP框架，引入语义软标签和语义损失进行语义感知对比学习；4. 构建基于UDAF文本表示的异构图模态，支持对病灶-属性关系的结构化推理。

Result: 在患者级数据划分的实验中，该方法在分类和检索基准测试中达到最先进的性能，同时在零样本学习、线性探测和微调任务上表现出强大的泛化能力。

Conclusion: 通过构建大规模超声数据集和专门的诊断分类学，结合语义感知对比学习框架，成功解决了超声图像-文本预训练的挑战，为超声诊断AI提供了有效的预训练解决方案。
...
TL;DR: 本文提出了一种轻量级架构和训练策略，利用自监督学习特征作为预训练视频扩散模型的通用条件信号，实现视频领域迁移和3D到视频生成，通过解耦外观特征提升可控性。


<details>
  <summary>Details</summary>
Motivation: 视频模型在内容生成、新视角合成和世界模拟方面取得了成功，但通常通过感知、几何或简单语义信号进行条件控制。虽然自监督学习获得的高维特征被用作视觉模型的通用接口，但尚未被广泛用作预训练视频扩散模型的通用条件信号。自监督特征（如DINO）包含大量关于场景风格、光照和语义的纠缠信息，这使其在重建任务中表现出色，但限制了生成能力。

Method: 提出轻量级架构和训练策略，解耦外观特征与其他需要保留的特征（如几何、语义），使模型能够稳健控制外观变化（如风格化和重光照）。同时发现低空间分辨率可以通过更高特征维度来补偿，从而改善从显式空间表示进行生成渲染的可控性。

Result: 该方法能够有效利用自监督学习特征作为条件信号，成功应用于视频领域迁移和从3D生成视频等任务。通过解耦外观特征，实现了对外观变化（如风格化、重光照）的稳健控制，同时保持了其他场景特征的完整性。

Conclusion: 自监督学习特征可以作为预训练视频扩散模型的通用条件信号，通过适当的架构设计和训练策略解耦外观特征，能够显著提升生成模型的可控性和应用范围，特别是在视频领域迁移和3D到视频生成等任务中表现出色。
...
TL;DR: 该论文提出了RebusBench基准测试，用于评估大视觉语言模型在解决需要神经符号推理的谜语谜题方面的能力，发现当前模型表现严重不足。


<details>
  <summary>Details</summary>
Motivation: 当前大视觉语言模型在显式视觉识别方面表现出色，但在需要复杂多步推理的问题上存在认知差距，特别是当视觉输入仅作为线索而非答案时，模型难以进行神经符号推理。

Method: 研究者构建了RebusBench基准测试，包含1,164个谜语谜题，用于评估模型整合感知和知识的能力。评估了包括Qwen、InternVL和LLaVA在内的最先进模型，测试了模型缩放和上下文学习的影响。

Result: 评估结果显示模型表现严重不足：精确匹配率低于10%，语义准确率低于20%。模型缩放和上下文学习均未带来显著改进，表明模型虽然具备必要的视觉和语言组件，但缺乏连接这些组件的认知推理能力。

Conclusion: 大视觉语言模型在需要神经符号推理的任务上存在严重缺陷，即使具备必要的感知和语言能力，也缺乏将这些能力整合起来的认知推理"胶水"。这揭示了当前模型在复杂推理任务上的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有世界-动作模型主要关注2D外观或潜在表示建模，缺乏几何基础，而几何信息对于在物理世界中运行的具身系统至关重要。需要开发一个能统一深度生成、未来预测和运动规划的几何感知模型。

Method: 使用大语言模型处理语言指令、多视角图像和动作，然后通过三个轻量级生成器分别生成深度、未来视频和动作。学习几何感知的世界表示，并用其指导未来预测和规划，保持模块化和可控延迟。

Result: 在Navsim v1和v2基准测试中表现出色，分别达到89.2 PDMS和88.7 EPDMS，优于现有基于世界模型的方法，同时产生更高质量的深度预测和未来视频。消融研究显示显式深度学习为视频想象提供互补优势并提高规划鲁棒性。

Conclusion: DriveDreamer-Policy通过集成几何感知的世界表示，实现了更连贯的未来想象和更明智的驾驶动作，为具身系统提供了统一的驾驶世界-动作模型框架，在闭环规划和世界生成任务中均取得优异表现。
...
TL;DR: 提出FSKD知识蒸馏框架，使用多模态教师融合LiDAR与RGBI数据，训练仅用RGBI的学生模型，实现零样本高精度森林结构预测


<details>
  <summary>Details</summary>
Motivation: 机载LiDAR成本高且不频繁，但却是森林结构监测的黄金标准。需要开发仅使用RGBI图像就能准确预测森林结构指标的方法，以实现可扩展的监测

Method: FSKD知识蒸馏框架：多模态教师融合RGBI图像与LiDAR数据（平面指标和垂直剖面），通过交叉注意力机制；RGBI-only SegFormer学生模型学习复制教师输出；在384平方公里森林数据上训练

Result: 学生模型在零样本CHM预测上达到SOTA性能（MedAE 4.17m，R²=0.51，IoU 0.87），比基线方法提升29-46% MAE；多模态融合比仅用RGBI训练提升10-26%性能；能同时预测CHM、PAI和FHD

Conclusion: FSKD框架通过知识蒸馏实现了仅用RGBI图像就能准确预测森林结构指标，突破了严格同步采集的限制，支持时间不匹配数据，为大规模森林监测提供了可行方案
...
TL;DR: GardenDesigner是一个基于智能体链和程序化建模的江南园林自动生成框架，能够通过文本输入在1分钟内创建美观的江南园林数字资产。
...
Method: 提出GardenDesigner框架，编码江南园林美学原则，基于程序化建模构建智能体链：地形分布智能体应用水中心地形规则，道路生成智能体应用探索性路径规则，资产选择和布局优化智能体遵循美学文化约束，并引入GardenVerse知识库增强资产布置。

Result: 开发了Unity交互界面和工具，非专业用户可通过文本输入在1分钟内构建江南园林。实验和人工评估表明，GardenDesigner能够生成多样且美观的江南园林。

Conclusion: GardenDesigner成功实现了江南园林的自动化生成，为数字资产创作提供了高效解决方案，在数字娱乐和旅游领域具有应用价值。
...
TL;DR: 提出了一种利用轮式里程计实现时间一致性单目深度估计的框架，通过光流三角化估计相机位姿和稀疏深度，使用递归贝叶斯估计更新度量尺度，然后重新缩放预训练深度估计基础模型的相对深度预测。


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计方法在连续帧间难以保持时间一致性，导致深度估计抖动，且在深度范围突变时容易失败。这对于自动驾驶和移动机器人的感知系统是一个重要问题。

Method: 提出一致性感知的单目深度估计框架，利用移动机器人的轮式里程计信息。通过连续帧间的光流进行三角化，估计相机位姿和稀疏深度。使用稀疏深度估计通过递归贝叶斯估计更新度量尺度，然后将该尺度应用于重新缩放预训练深度估计基础模型预测的相对深度。

Result: 在KITTI、TartanAir、MS2和自有数据集上进行评估，证明了该方法能够实现鲁棒且准确的深度估计性能。
...
TL;DR: STRIVE是一个用于视频问答的强化学习框架，通过构建视频的多个时空变体并进行联合归一化，解决了传统方法中奖励方差低的问题，提高了策略更新的稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于群体的策略优化方法在处理视频问答时，当回答具有相似正确性时会出现奖励方差低的问题，导致优势估计弱或不稳定，限制了强化学习在视频推理中的应用效果。

Method: STRIVE框架包含三个核心组件：1) 为每个输入视频构建多个时空变体；2) 在文本生成和视觉变体上进行联合归一化；3) 引入重要性感知采样机制，优先选择与问题最相关的帧，同时保持时间覆盖。这种方法将群体比较从语言多样性扩展到结构化视觉扰动。

Result: 在六个具有挑战性的视频推理基准测试（VideoMME、TempCompass、VideoMMMU、MMVU、VSI-Bench、PerceptionTest）上的实验表明，STRIVE在多个大型多模态模型上相比现有强化学习基线方法取得了持续改进。

Conclusion: 结构化时空探索作为一种原则性机制，能够稳定多模态强化学习并提高视频推理性能。STRIVE通过丰富奖励信号和促进更稳定的策略更新，展示了在互补视觉视角上进行鲁棒推理的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 基于整流流变换器的T2I模型（如SD3、FLUX）虽然生成质量高，但对不安全语义（特别是多令牌交互触发）仍然脆弱。现有缓解方法主要依赖微调或注意力调制，计算成本高且针对U-Net设计，难以直接适配变换器扩散模型（如MMDiT）。

Method: 1. 分析MMDiT注意力机制，发现不安全语义集中在头级别的可解释低维子空间；2. 构建头级别不安全子空间，通过投影计算潜在风险分数；3. 引入头级别RoPE扰动，对查询和键向量嵌入进行风险特定的头级别旋转，精确抑制不安全输出。

Result: 实验表明SafeRoPE在平衡有害内容缓解和效用保持方面达到最先进性能，有效抑制不安全语义同时保持生成保真度。

Conclusion: SafeRoPE为MMDiT提供了一个轻量级、细粒度的安全生成框架，通过扰动RoPE有效抑制不安全内容，同时保持生成质量，解决了现有方法计算成本高且不适用于变换器扩散模型的问题。
...
TL;DR: 研究表明，通过添加桥接训练阶段，语言预训练模型可以有效地适应视觉任务，无需手动标注，且部分桥接训练效果更佳。


<details>
  <summary>Details</summary>
Motivation: 语言预训练模型和视觉预训练模型的异常参数比例差异显著，使得跨模态（语言和视觉）适应比跨域适应更具挑战性。先前研究多集中于跨域迁移，认为语言预训练模型不适合下游视觉任务，因为参数空间差异太大。

Method: 提出随机标签桥接训练方法，作为模态适应学习器，无需手动标注，帮助大语言模型参数适应视觉基础任务。同时发现部分桥接训练通常更有优势，因为LLM中的某些层展现出强大的基础特性，即使未经视觉任务微调也保持有益。

Result: 研究表明桥接训练阶段可以有效对齐大语言模型参数与视觉任务，部分桥接训练效果更佳，某些LLM层的基础特性在视觉任务中仍然有益。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分析中存在域偏移问题，现有半监督域适应方法在处理具有自然顺序标签的严重程度分类时效果不佳，因为类别边界不清晰
...
Conclusion: 该方法通过利用类别顺序信息和秩分数对齐，有效解决了医学图像严重程度分类中的半监督域适应问题
...
TL;DR: 本文提出了一种纹理感知的Transformer模型，直接从网格面的原始像素学习，结合新的分层学习方案进行多尺度特征聚合，在语义网格分割任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 纹理3D网格同时包含几何、拓扑和外观信息，但其不规则结构给基于深度学习的语义分割带来挑战。现有方法通常直接处理网格但忽略了丰富的纹理信息。

Method: 提出纹理感知Transformer：1）纹理分支将面级像素汇总为可学习token；2）与几何描述符融合；3）通过两阶段Transformer块（TSTB）处理，支持局部和全局信息流；4）采用新的分层学习方案进行多尺度特征聚合。

Result: 在Semantic Urban Meshes（SUM）基准测试中达到81.9% mF1和94.3% OA，在新构建的文化遗产数据集（带损坏类型标注的纹理屋顶瓦片）上达到49.7% mF1和72.8% OA，显著优于现有方法。

Conclusion: 该方法有效利用纹理3D网格中的纹理信息，通过纹理感知Transformer和分层学习方案，在语义网格分割任务上取得了显著性能提升，特别是在文化遗产保护等应用场景中表现出色。
...
TL;DR: PI-VQ提出了一种位置无关的离散表示方法，通过消除潜在代码中的位置信息来捕获全局语义特征，并使用匹配量化提高信息容量，实现无需学习先验的直接图像插值。


<details>
  <summary>Details</summary>
Motivation: 传统向量量化方法（如VQ-VAE、VQ-GAN）学习的离散表示具有位置依赖性：代码在空间上排列且上下文纠缠，需要自回归或扩散先验来建模依赖关系。本研究探讨位置信息对于空间对齐数据的离散表示是否必要。

Method: 提出位置无关向量量化自编码器（PI-VQ），约束潜在代码不携带位置信息。引入匹配量化算法，基于最优二分匹配进行向量量化，相比朴素最近邻量化提高3.5倍的有效瓶颈容量。利用学习代码的组合结构实现基于插值的采样。

Result: 在CelebA、CelebA-HQ和FFHQ数据集上评估，获得了具有竞争力的精度、密度和覆盖度指标。位置无关表示鼓励代码捕获全局语义特征，无需学习先验即可实现图像间的直接插值。

Conclusion: 位置无关表示在可分离性和潜在代码可解释性方面存在权衡，为未来工作指出了多个方向。PI-VQ展示了无需位置信息的离散表示在捕获语义特征和实现高效图像合成方面的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 尽管高斯泼溅在CT重建中表现良好，但其优势不足以促使从成熟算法转向。现有GS方法仍存在显著限制，需要更快速、灵活的重建框架。

Method: 提出FaCT-GS框架，通过深度优化体素化和光栅化流程，实现快速CT重建。改进的体素化能快速将高斯拟合到现有体积，可作为先验或压缩表示。

Result: 在标准512x512投影上比现有GS CT重建快4倍以上，在2k投影上快13倍以上。能良好扩展投影和输出体积大小，并支持快速拟合现有体积作为先验。

Conclusion: FaCT-GS解决了GS在CT重建中的主要限制，提供了显著更快、更灵活的重建方法，可作为现有算法的替代方案或压缩表示工具。
...
TL;DR: 研究发现最先进的视觉语言模型在基本几何变换下存在系统性脆弱性，缺乏空间不变性和等变性，导致在简单旋转、缩放和恒等变换下识别物体时性能显著下降。
...
Method: 通过系统评估方法，在符号草图、自然照片和抽象艺术等不同视觉领域中测试模型对旋转、缩放和恒等变换的鲁棒性，涵盖不同架构、模型容量和提示策略。

Result: 当语义内容变得稀疏时，模型性能急剧下降；这种脆弱性在不同架构、模型容量和提示策略中普遍存在，表明当前视觉语言模型缺乏基本的空间不变性和等变性。
...
TL;DR: HieraVid是一个分层剪枝框架，通过分段级、帧级和层级三个层次逐步动态减少视频冗余，在仅保留30%令牌的情况下实现SOTA性能，保持LLaVA-Video-7B和LLaVA-OneVision-7B 98%以上性能。


<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在视频理解方面表现出色，但大量输入视频令牌带来了显著的计算负担。现有方法主要在输入级别剪枝视频令牌，忽略了视频和大语言模型内部固有的信息结构。

Method: 基于视频具有分段-帧结构和大语言模型内部单向传播多模态信息的观察，提出三级分层剪枝：1) 分段级：视频令牌先进行时间分段和空间合并；2) 帧级：同一分段内相似帧联合剪枝以保持多样性；3) 层级：随着LLM层数增加，冗余逐渐减少而不影响性能。

Result: 在四个广泛使用的视频理解基准测试上进行实验，仅保留30%令牌的情况下，HieraVid实现了新的最先进性能，同时保持了LLaVA-Video-7B 98%以上和LLaVA-OneVision-7B 99%以上的性能。

Conclusion: HieraVid通过分层剪枝框架有效减少了视频大语言模型的计算负担，在保持高性能的同时显著降低了令牌数量，为视频理解模型的部署提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前时序动作分割方法依赖复杂架构，不利于实际部署。需要简单有效的方法来提升细粒度分割质量。

Method: 提出双损失训练框架：1)边界回归损失通过单通道边界预测提升时间定位精度；2)CDF-based段级正则化损失通过匹配预测段和真实段的累积分布来增强段内结构一致性。
...
<details>
  <summary>Details</summary>
Motivation: 自回归模型在文本到图像生成中面临两大挑战：1）生成的图像质量可能不符合人类期望标准；2）难以处理可能有多种有效解释的模糊提示。

Method: 提出MAR-MAER框架，包含两个主要组件：1）度量感知嵌入正则化方法，使用轻量级投影头和自适应核回归损失函数，将模型内部表示与人类偏好的质量度量（如CLIPScore和HPSv2）对齐；2）条件变分模块，在分层标记生成过程中引入受控随机性，处理模糊语义。

Result: 在COCO和新开发的模糊提示基准测试中，MAR-MAER在度量一致性和语义灵活性方面表现优异，相比基线Hi-MAR模型，CLIPScore提升+1.6，HPSv2提升+5.3，对于模糊输入能产生更广泛的输出多样性。

Conclusion: MAR-MAER通过结合度量感知正则化和概率潜在建模，有效提升了文本到图像生成的质量和多样性，特别是在处理模糊提示时表现出色，这一结果通过人类评估和自动度量得到了验证。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅(3DGS)在实时渲染方面表现出色，但大量高斯点导致高内存成本。现有剪枝方法虽然节省内存，但会破坏空间一致性并产生渲染伪影。

Method: 提出基于证据下界(ELBO)的自适应致密化策略控制高斯点生成；提出透明度感知渐进剪枝策略动态移除低透明度点；设计图基特征编码模块通过特征引导点位移优化空间分布

Result: GS^2仅需约12.5%的高斯点就能达到比原始3DGS更高的PSNR，在渲染质量和内存效率方面均优于所有基线方法

Conclusion: GS^2通过优化高斯点的空间分布实现了紧凑高效的3D高斯表示，在保持高质量渲染的同时显著降低内存消耗
...
TL;DR: 研究发现现代文本到图像生成模型容易受到低成本的提示词越狱攻击，仅通过自然语言提示就能绕过安全过滤器，攻击成功率高达74.47%。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成模型广泛应用于创意工具和在线平台，虽然依赖安全过滤器和审核管道来阻止有害内容，但现有系统仍然存在安全漏洞。本研究旨在揭示这些模型对简单提示词攻击的脆弱性。

Method: 提出了系统化的提示词越狱攻击研究，包括艺术重构、材质替换、伪教育框架、生活美学伪装和模糊动作替换等视觉越狱技术分类。这些策略利用提示词审核和视觉安全过滤的弱点，将不安全意图隐藏在良性语义上下文中。

Result: 在多个最先进的文本到图像系统上评估这些攻击，发现简单的语言修改可以可靠地规避现有安全措施并生成受限图像。在所有测试模型和攻击类别中，攻击成功率最高达到74.47%。

Conclusion: 研究结果凸显了表面级提示词过滤与检测生成媒体系统中对抗性意图所需的语义理解之间存在关键差距，需要更强大的安全机制来应对这类攻击。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感视觉定位方法主要依赖句子级的视觉-语言对齐，难以充分利用细粒度语言线索（如空间关系和对象属性），而这些线索对于区分相似特征的对象至关重要。不同定位阶段这些线索应发挥不同作用，需要更明确的指导。

Method: 1. 将语言表达解耦为全局上下文、空间关系和对象属性三个部分；2. 采用渐进式跨模态调制器，通过"调查-定位-验证"方案动态调制视觉注意力，实现从粗到细的对齐；3. 引入跨尺度融合模块处理遥感图像的大尺度变化；4. 使用语言引导的校准解码器在预测时细化跨模态对齐；5. 统一的多任务头支持指称表达理解和分割任务。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在合成任意拓扑形状时难以平衡细节学习、计算效率和模型简洁性，需要一种能够同时优化这些方面的新方法。
...
Conclusion: SHARC框架通过球谐函数表示和优化参考点放置，提供了一种高效、准确且简洁的任意拓扑形状合成方法。
...
TL;DR: FTPFusion是一种基于频率感知的红外与可见光视频融合方法，通过时间扰动和稀疏跨模态交互来解决时空稳定性和细节保留的挑战。


<details>
  <summary>Details</summary>
Motivation: 红外与可见光视频融合在智能监控和低光监测中至关重要，但现有方法在保持时间稳定性的同时保留空间细节方面存在挑战。现有方法要么侧重于帧级增强而时间建模有限，要么依赖于牺牲高频细节的沉重时空聚合。

Method: FTPFusion将特征表示分解为高频和低频分量进行协同建模。高频分支执行稀疏跨模态时空交互以捕捉运动相关上下文和互补细节；低频分支引入时间扰动策略以增强对复杂视频变化（如闪烁、抖动和局部错位）的鲁棒性；还设计了偏移感知时间一致性约束来显式稳定时间扰动下的跨帧表示。

Result: 在多个公共基准测试上的广泛实验表明，FTPFusion在空间保真度和时间一致性方面均优于现有最先进方法。

Conclusion: FTPFusion通过频率分解、稀疏跨模态交互和时间扰动策略，有效解决了红外与可见光视频融合中的时空稳定性和细节保留问题，在多个指标上表现出优越性能。
...
Method: 1. 将ResNet中的卷积替换为KAN卷积，实现自适应特征提取；2. 使用适合SAR数据的Gram多项式作为激活函数，捕捉复杂非线性关系；3. 采用参数共享策略：每个核在通道间共享参数，在保留独特特征的同时减少参数量和FLOPs。

Result: 在MSTAR、FUSAR-Ship和SAR-ACD数据集上分别达到99.09%、93.01%和97.26%的准确率。在1024×1024的MSTAR数据集上，相比VGG16，FLOPs减少82.90倍，参数量减少163.78倍。

Conclusion: Light-ResKAN模型在SAR图像识别任务中实现了精度与效率的良好平衡，为边缘设备上的SAR图像识别提供了高效的解决方案。
...
TL;DR: 提出了一种基于几何驱动的多光斑检测与匹配管道，将角膜反射光斑视为结构化星座而非独立斑点，提高眼动追踪系统的可复现性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 角膜反射光斑检测在P-CR眼动追踪中很重要，但现有方法通常作为启发式方法嵌入大系统中，导致在不同硬件设置下难以复现。

Method: 提出2D几何驱动的星座式管道，采用相似性-布局对齐(SLA)程序，结合受控过检测、自适应候选回退、外观感知评分和可选的语义布局先验，保持检测与对应关系分离。

Result: 在公开多LED数据集上评估，系统在噪声条件下提供稳定的身份保持对应关系。

Conclusion: 该方法提高了多LED眼动追踪的可复现性和鲁棒性，通过代码、预设和评估脚本的发布支持透明复现、比较和数据集标注。
...
TL;DR: KnowMVG通过知识增强提示和全局-局部注意力机制，提升医学视觉定位的空间精度，在多个基准测试中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在医学视觉定位任务中空间定位精度不足，主要原因是缺乏显式的定位先验知识，仅依赖潜在嵌入无法提供足够的空间感知能力。

Method: 提出KnowMVG框架：1) 知识增强提示策略，将医学知识编码为紧凑嵌入；2) 全局-局部注意力机制，联合利用粗粒度全局信息和细粒度局部线索来指导精确区域定位。

Result: 在四个MVG基准测试中，KnowMVG一致优于现有方法，在AP50指标上提升3.0%，在mIoU指标上提升2.6%，超越先前的最先进方法。

Conclusion: KnowMVG通过显式增强解码过程中的空间感知能力，有效连接了高级语义理解和细粒度视觉感知，无需引入额外的文本推理开销，显著提升了医学视觉定位的精度。
...
TL;DR: 该研究探讨了能否直接从波束成形前的天线级距离-多普勒测量中学习有意义的空间结构，而不是先进行波束成形构建角度域表示。使用6发8收汽车雷达，通过双啁啾共享权重编码器进行端到端数据驱动训练，以鸟瞰图占用率作为几何探针评估空间可恢复性。
...
Method: 使用6发8收（48个虚拟天线）商用汽车雷达，采用A/B啁啾序列调频连续波发射方案。使用双啁啾共享权重编码器对波束成形前的天线级距离-多普勒张量进行端到端数据驱动训练。监督信号来自激光雷达，采用可见性感知的跨模态监督，明确建模雷达视场和基于射线的可见性分析。

Result: 通过啁啾消融实验（仅A、仅B、A+B）、距离带分析和物理对齐基线评估，结果表明可以直接从波束成形前的天线级距离-多普勒张量中学习空间结构，无需显式的角度域构建或手工信号处理阶段。不同发射配置会影响几何可恢复性。

Conclusion: 空间结构可以直接从波束成形前的天线级距离-多普勒测量中学习，无需先进行角度域构建。这为汽车雷达感知提供了新的表示学习方法，避免了传统信号处理流程的限制。
...
TL;DR: 论文提出了ECAC基准数据集和RSRS混合训练框架，用于提升早期儿童教育场景的图像描述质量，解决了领域特定数据缺乏和传统训练方法在专业物体描述上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有早期儿童教育图像描述方法面临两大挑战：1）缺乏大规模领域特定数据集，导致模型无法捕捉ECE场景的细粒度语义概念，产生通用且不精确的描述；2）传统训练范式在提升专业物体描述能力方面存在局限，监督学习偏向高频表达，而强化学习在困难样本上优化不稳定。

Method: 1）构建ECAC大规模基准数据集，包含256,121张真实世界图像，配有专家级描述和细粒度标签；2）提出领域导向评估协议TTS（教学玩具识别分数）来显式衡量专业物体命名准确性；3）提出RSRS混合训练框架，动态切换强化学习和监督微调，将零奖励的困难样本重新路由到监督微调，缓解优势崩溃并实现细粒度识别的稳定优化。

Result: 基于ECAC和RSRS开发了KinderMM-Cap-3B领域自适应多模态大语言模型。实验表明该模型TTS达到51.06，显著优于最先进的基线方法，同时保持优越的描述质量。

Conclusion: 提出的ECAC基准和RSRS训练框架有效解决了ECE图像描述的关键挑战，KinderMM-Cap-3B模型在专业物体识别和描述质量方面表现出色，展示了其在专门教育应用中的潜力。
...
TL;DR: 本文提出AMIMV方法解决医学图像分类中的数据稀缺和类别不平衡问题，在11个MedMNIST数据集上评估了8种SSL方法，在多个数据集上取得了显著性能提升。
...
Conclusion: AMIMV方法有效解决了医学图像分类中的数据稀缺和类别不平衡问题，在多个医学图像数据集上表现出优于现有SSL方法的性能，为处理医学图像中的长尾分布提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有红外与可见光视频融合方法大多针对静态图像设计，无法有效处理视频中的帧间运动。当前视频融合方法虽然通过引入跨帧交互改善了时间一致性，但计算成本高。
...
Conclusion: MAVFusion通过解耦动态和静态区域的处理，在保持高质量视频融合的同时显著加速推理，为红外与可见光视频融合提供了高效解决方案。
...
TL;DR: 使用nnU-Net v2框架开发的多模态深度学习模型，用于mpMRI中前列腺腺体的自动分割，在PI-CAI数据集上训练，在外部验证中表现出良好的泛化能力。
...
Method: 基于nnU-Net v2框架的深度学习模型，利用多模态mpMRI数据（T2加权成像、DWI、ADC图）提取互补组织信息，在PI-CAI数据集的981个病例上进行训练。

Result: 交叉验证平均Dice分数0.96±0.00，外部测试集Dice分数0.82；相比之下，通用方法TotalSegmentator仅得0.15分，主要原因是腺体欠分割。

Conclusion: 任务特定的多模态分割策略对于前列腺分割至关重要，所提方法具有良好的临床研究应用潜力，模型已容器化并可作为即用推理工具使用。
...
TL;DR: 该论文提出了首个针对多模态大语言模型在自我中心视频中个性化问答能力的系统分析，并创建了MyEgo数据集来评估模型理解、记忆和推理相机佩戴者的能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在需要"自我接地"（理解相机佩戴者）的个性化问答任务中存在不足，缺乏系统评估此类能力的基准数据集。

Method: 创建MyEgo数据集，包含541个长视频和5K个个性化问题，涵盖"我的物品"、"我的活动"和"我的过去"三类。对多种MLLMs进行基准测试，包括开源vs专有、思考vs非思考、小vs大规模模型。

Result: 所有测试模型在MyEgo上表现不佳：顶级闭源和开源模型准确率分别仅为~46%和36%，远低于人类表现（差距约40%和50%）。显式推理和模型缩放未能带来一致改进，提供相关证据时模型有所改善但随时间增益下降。

Conclusion: 研究强调了自我接地和长程记忆在自我中心视频个性化问答中的关键作用，MyEgo数据集和分析将推动该领域发展，实现更好的自我中心个性化辅助。
...
TL;DR: SDesc3D：基于短文本描述的3D室内场景生成框架，通过多视角结构先验和区域功能感知提升物理合理性和细节丰富度
...
Method: 1. 多视角场景先验增强：通过聚合多视角结构知识丰富欠指定的文本输入；2. 功能感知布局定位：利用区域功能定位作为隐式空间锚点，进行分层布局推理；3. 迭代反思-修正方案：通过自修正实现渐进式结构合理性优化

Result: 在短文本条件下的3D室内场景生成任务上，该方法优于现有方法，表现出更好的物理合理性和语义一致性

Conclusion: SDesc3D通过整合多视角结构先验和区域功能暗示，有效解决了短文本条件下3D场景生成的物理合理性和细节丰富度问题，为交互式3D环境构建提供了更实用的解决方案
...
TL;DR: 该论文提出了交互式跟踪新范式，构建了首个大规模交互式跟踪基准InteractTrack，并提出动态记忆增强跟踪基线IMAT，以解决传统跟踪器无法适应人类实时指导的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉跟踪器主要以"发射后不管"的非交互方式工作，无法适应需要人机协同的现实场景。传统跟踪器缺乏接收人类语言指令并实时调整的能力，限制了其在真实应用中的实用性。

Method: 1) 创建InteractTrack基准：包含150个视频，密集边界框标注和时间戳语言指令；2) 设计全面评估协议，评估25个代表性跟踪器；3) 提出IMAT基线方法，采用动态记忆机制从用户反馈中学习并更新跟踪行为。

Result: 评估显示最先进的跟踪方法在交互场景中表现不佳，传统基准上的强性能无法迁移到交互场景。IMAT作为新基线，能够有效学习用户反馈并调整跟踪行为，为交互式跟踪系统奠定基础。

Conclusion: 该研究建立了交互式跟踪的基础框架，填补了自动感知与人类指导之间的鸿沟，为开发更智能、自适应和协作的跟踪系统提供了基准、协议和基线方法。
...
TL;DR: Curia-2是一个用于医学影像分析的基础模型，通过改进预训练策略和表示质量，支持扩展到十亿参数规模的视觉Transformer，并在CT和MRI多模态分析中表现出色。


<details>
  <summary>Details</summary>
Motivation: 医学影像的快速增长给放射科医生带来了不可持续的工作负担，现有基础模型在处理复杂放射学数据方面仍有优化空间，需要更好的方法来捕捉放射学数据的特性。

Method: 基于Curia框架，改进预训练策略和表示质量，支持扩展到十亿参数规模的视觉Transformer，并扩展CuriaBench评估框架为2D切片和3D体积两个评估轨道。

Result: Curia-2在视觉任务上优于所有基础模型，在临床复杂任务（如病灶检测）上与视觉语言模型竞争性相当，是首个支持CT和MRI多模态分析的十亿参数规模模型。

Conclusion: Curia-2通过优化的预训练策略和可扩展架构，显著提升了医学影像基础模型的性能，为放射学数据分析提供了更强大的工具，并将公开权重促进进一步研究。
...
TL;DR: 多模态大语言模型中的视觉注意力存在惯性问题，导致在早期解码步骤后注意力基本保持静态，无法支持认知推理所需的组合理解。本文提出无需训练的惯性感知视觉激发方法，通过打破注意力惯性模式来缓解认知幻觉。


<details>
  <summary>Details</summary>
Motivation: 研究发现多模态大语言模型的视觉注意力存在明显的惯性现象，一旦在早期解码步骤中稳定下来，就会保持基本静态，无法支持需要对象间关系推理的认知理解。现有的幻觉缓解方法主要针对对象存在或属性的感知幻觉，对需要关系推理的认知幻觉效果有限。

Method: 提出无需训练的惯性感知视觉激发方法：1）将认知推理建模为视觉注意力的动态响应；2）选择相对于历史注意力趋势动态涌现的视觉标记，同时区分表现出惯性行为的标记；3）引入惯性感知惩罚机制，防止注意力过度集中并限制在局部区域的持续性。

Result: 大量实验表明，IVE方法在各种基础MLLM和多个幻觉基准测试中都有效，特别是在缓解认知幻觉方面表现突出。

Conclusion: 视觉注意力惯性是多模态大语言模型中认知幻觉的关键因素，提出的IVE方法通过打破注意力惯性模式，有效提升了模型对组合推理和关系推断的能力，为缓解认知幻觉提供了有效的训练免费解决方案。
...
Conclusion: Resonance4D通过创新的双域运动监督和全参数物理恢复方法，解决了现有4D动态模拟中的计算成本和物理真实性矛盾，为消费级硬件上的高质量物理驱动4D模拟提供了可行方案
...
TL;DR: ProDiG：一种渐进式扩散引导框架，通过中间高度视图合成和几何感知优化，从纯航拍图像生成逼真的地面级视图和一致的3D场景模型


<details>
  <summary>Details</summary>
Motivation: 现有方法在从纯航拍图像生成地面级视图和3D场景模型时面临挑战：极端视角变化、中间观测缺失和大尺度变化导致几何不一致；现有方法要么后处理渲染效果差，要么依赖多高度真实数据（通常不可得）；高斯泼溅和扩散方法在小变化下有效，但在大航拍-地面视角差距下失效

Method: ProDiG（渐进式高度高斯泼溅）框架：1）渐进式将航拍3D表示向地面级保真度转换；2）合成中间高度视图；3）使用几何感知因果注意力模块在每个阶段细化高斯表示，将极线结构注入参考视图扩散；4）距离自适应高斯模块根据相机距离动态调整高斯尺度和不透明度，确保大视角差距下的稳定重建

Result: 在合成和真实世界数据集上的大量实验表明，ProDiG能生成视觉逼真的地面级渲染和一致的3D几何，在视觉质量、几何一致性和对极端视角变化的鲁棒性方面显著优于现有方法

Conclusion: ProDiG通过渐进式扩散引导框架，无需额外地面真实视角，实现了从纯航拍图像生成高质量地面级视图和几何一致3D场景模型，解决了现有方法在大视角差距下的局限性
...
TL;DR: DR-Seg提出了一种解耦-校正框架，用于遥感图像的开集语义分割，通过分离CLIP特征为语义主导和结构主导子空间，在保持语言对齐语义的同时增强结构细节。


<details>
  <summary>Details</summary>
Motivation: 遥感领域的开集语义分割需要语言对齐识别和细粒度空间划分。虽然CLIP具有强大的语义泛化能力，但其全局对齐的视觉表示难以捕捉结构细节。现有方法引入遥感预训练的DINO特征进行补偿，但将CLIP表示视为单一语义空间，无法定位需要结构增强的区域，既不能有效划分边界，又可能破坏CLIP的语义完整性。

Method: DR-Seg基于CLIP特征通道具有功能异质性的关键观察，将CLIP特征解耦为语义主导和结构主导两个子空间。通过解耦，可以在不扭曲语言对齐语义的前提下，使用DINO进行有针对性的结构增强。随后，先验驱动的图校正模块在DINO指导下注入高保真结构先验形成精炼分支，不确定性引导的自适应融合模块动态集成精炼分支和原始CLIP分支进行最终预测。

Result: 在八个基准测试上的综合实验表明，DR-Seg建立了新的最先进性能。

Conclusion: DR-Seg通过解耦CLIP特征为语义和结构子空间，实现了在保持语言对齐语义完整性的同时进行有针对性的结构增强，有效解决了遥感开集语义分割中语言对齐识别和细粒度空间划分的双重需求。
...
TL;DR: 论文提出了LinkS²Bench，首个评估视觉语言模型在无人机与卫星协同空间智能方面的基准，包含17.9k高质量问答对，覆盖12个细粒度任务，发现现有模型在跨视图动态对齐方面存在显著差距。
...
Method: 构建LinkS²Bench基准，将1,022分钟动态无人机视频与覆盖200+平方公里高分辨率卫星图像关联；通过LMM辅助流程和人工标注创建17.9k高质量问答对，涵盖感知、定位、关系、推理四个维度的12个任务；提出跨视图对齐适配器来改善模型性能。

Result: 评估18个代表性视觉语言模型显示，与人类基线相比存在显著差距，准确跨视图动态对齐是关键瓶颈；跨视图对齐适配器显著提升模型性能；微调实验证明LinkS²Bench在推进复杂空间推理方面的潜力。

Conclusion: LinkS²Bench填补了评估视觉语言模型在无人机-卫星协同空间智能方面的空白，揭示了现有模型的局限性，提出的跨视图对齐适配器有效改善了性能，为推进复杂空间推理提供了重要基准和方向。
...
TL;DR: 该论文提出了一种解决自编码器中空间不平衡问题的新方法，通过自熵损失和样本传播机制来提升对罕见空间位置的重建质量。


<details>
  <summary>Details</summary>
Motivation: 自编码器在处理空间非均匀采样的图像时面临挑战，特别是在医学影像、生物学和物理学领域，信息丰富的模式在特定坐标上出现频率低，而背景占据主导地位，导致模型偏向于多数模式，丢失细节并产生模糊重建。
...
Conclusion: 该方法有效解决了自编码器中的空间不平衡问题，强调了批次中数据表示的重要性，并突出了无监督图像重建中罕见样本的价值。
...
TL;DR: IndoorCrowd是一个多场景室内人群数据集，包含31个视频、9913帧，用于人体检测、实例分割和多目标跟踪，在四个校园地点采集，并提供了基准测试结果。
...
Method: 在四个校园位置收集31个视频（9913帧，5fps），提供人工验证的实例分割掩码。使用620帧控制子集评估SAM3、GroundingSAM和EfficientGroundingSAM三种基础模型自动标注器，使用Cohen's κ、AP、精确率、召回率和掩码IoU指标。另外2552帧子集支持多目标跟踪，采用MOTChallenge格式。使用YOLOv8n、YOLOv26n和RT-DETR-L结合ByteTrack、BoT-SORT和OC-SORT建立基准。

Result: 场景分析显示挑战性差异显著：ACS-EC场景（79.3%密集帧，平均实例尺度60.8像素）是最具挑战性的场景。建立了检测、分割和跟踪的基准性能。

Conclusion: IndoorCrowd数据集填补了大规模真实室内人群数据集的空白，为室内人类行为理解提供了重要资源，并展示了不同场景下算法性能的显著差异。
...
TL;DR: WISE提出了一种高效推理范式，通过"思考两次"原则训练模型生成结构化序列：简洁推理→最终答案→详细解释，在推理时省略详细解释，实现5倍推理长度压缩的同时保持SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有思维链推理方法在语言引导分割中虽然性能显著，但生成冗长推理文本导致计算成本过高，限制了实际应用。需要一种既能保持高性能又能大幅减少推理长度的解决方案。

Method: WISE采用"思考两次"原则：1)训练阶段：模型生成结构化序列（简洁推理→答案→详细解释），通过自蒸馏目标强化语义保真度和简洁性；2)推理阶段：使用WISE-S策略，通过简洁指令提示激活学习到的简洁推理策略，省略详细解释。

Result: 在ReasonSeg基准测试中，WISE-S实现了58.3 cIoU的零样本SOTA性能，同时将平均推理长度从112个token减少到23个token，压缩了近5倍。

Conclusion: WISE通过结构化训练和简洁推理策略，在保持高性能的同时大幅降低了计算成本，为大型多模态模型的高效推理提供了新范式，平衡了准确性和效率。
...
Result: 使用Jagle训练的2.2B模型在日语任务上表现优异，在十个日语评估任务中平均得分超过InternVL3.5-2B，接近Qwen3-VL-2B-Instruct（差距在5分以内）。结合Jagle和FineVision训练不仅不会降低英语性能，反而比单独使用FineVision训练效果更好。
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数虚拟人创建流程依赖未经色彩校准的摄影输入，导致肤色再现存在不一致性和偏差，影响真实感、身份保持和公平性。需要系统评估肤色保真度的方法。

Method: 提出完整工作流程：1) 肤色和光照提取；2) 纹理重着色；3) 实时渲染；4) 定量色彩分析。使用芝加哥人脸数据库，比较基于脸颊区域采样和多维掩码的肤色提取策略，结合TRUST框架进行光照隔离。在CIELAB色彩空间中使用ΔE度量和个体类型学角度(ITA)客观评估肤色一致性。

Result: 分析了约19,848个渲染实例，结果显示提取策略表现出表型依赖性行为，深色肤色的色度误差始终更高。

Conclusion: 提出了一种无需人工干预、计算成本低、可大规模评估的虚拟人肤色保真度评估框架，揭示了肤色提取策略的表型依赖性和深色肤色再现的更大挑战。
...
TL;DR: 论文提出Multi-Object Affordance Grounding under Intent-Driven Instructions新任务，要求在多物体点云中根据隐含自然语言意图预测正确物体的逐点功能掩码，并构建首个专注于混淆多物体场景中隐含意图的基准CompassAD和相应框架CompassNet。


<details>
  <summary>Details</summary>
Motivation: 现有3D功能预测方法主要评估孤立单个物体，且通常查询中提供明确类别名称，忽略了真实场景中多个物体可能具有相同功能但只有一个是任务上下文下合适的这一挑战（混淆对问题）。

Method: 提出CompassNet框架，包含两个专门模块：1) Instance-bounded Cross Injection (ICI) - 将语言-几何对齐约束在物体边界内，防止跨物体语义泄漏；2) Bi-level Contrastive Refinement (BCR) - 在几何组和点级别强制执行区分，增强目标与混淆表面之间的差异。

Result: 在CompassAD基准上进行了广泛实验，在已见和未见查询上都取得了最先进的结果，并在机器人机械臂上部署，确认了在混淆多物体场景中有效转移到真实世界抓取任务。

Conclusion: 该工作形式化了意图驱动的多物体功能定位新任务，构建了首个专注于混淆对场景的基准，提出的CompassNet框架通过ICI和BCR模块有效解决了跨物体语义泄漏和混淆表面区分问题，展示了从虚拟到真实世界的有效转移。
...
<details>
  <summary>Details</summary>
Motivation: 现有的通用多模态嵌入方法通过生成显式思维链来改善查询意图理解，但这带来了显著的推理开销，并且将丰富的多模态证据压缩到狭窄的文本瓶颈中

Method: PLUME采用潜在推理框架，用连续潜在状态的短自回归展开替代显式思维链；引入语义锚点引导的转换适配器来引导不同推理轨迹；采用渐进式显式到潜在课程学习来稳定训练

Result: 在78个任务的MMEB-v2基准测试中，PLUME优于强显式思维链基线，同时将推理从数百个生成标记减少到少于10个潜在步骤，推理速度提升超过30倍
...
TL;DR: FlowSlider是一种无需训练的连续图像编辑方法，通过分解编辑更新为保真项和转向项，实现平滑的滑块式控制
...
Method: 在Rectified Flow中，将FlowEdit的更新分解为：(1)保真项：作为源条件稳定器，保持身份和结构；(2)转向项：驱动语义向目标编辑转变。几何分析和实证测量显示这两项近似正交，通过仅缩放转向项实现稳定的强度控制

Result: FlowSlider无需后训练即可提供平滑可靠的控制，在各种任务中提高了连续编辑质量

Conclusion: FlowSlider是一种无需训练的连续编辑方法，通过正交分解实现稳定的滑块式控制，避免了现有方法对训练分布的依赖问题
...
TL;DR: GroundVTS：一种用于视频大语言模型的视觉令牌采样方法，通过查询引导的细粒度机制筛选关键视觉令牌，解决均匀帧采样导致的关键帧稀疏问题，提升视频时序定位性能。
...
Method: 提出Grounded Visual Token Sampling（GroundVTS）架构，采用细粒度的查询引导机制在输入LLM前筛选视觉令牌，保留关键时空信息；引入渐进优化策略使LLM适应非均匀视觉特征分布。

Result: 在三个标准VTG基准测试中超越现有方法，在时刻检索任务上mIoU提升7.7点，在高光检测任务上mAP提升12.0点。

Conclusion: GroundVTS通过聚焦信息最丰富的时序片段，有效解决了Vid-LLMs中的关键帧稀疏问题，显著提升了视频时序定位性能，为视频大语言模型在更广泛应用中的扩展提供了关键技术。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一模型需要像素解码作为视觉理解与生成之间的桥梁，这既低效又低效。不同模态的分离表示限制了跨模态推理的灵活性和效率。

Method: 引入LatentUM，将所有模态表示在共享的语义潜在空间中，无需像素空间中介。这种设计自然支持灵活的跨模态推理和生成。

Result: LatentUM在视觉空间规划基准测试中达到最先进性能，通过自反思提升视觉生成质量，支持在共享语义潜在空间中预测未来视觉状态的世界建模。

Conclusion: 共享语义潜在空间表示显著提高了计算效率，减轻了编解码器偏差，增强了跨模态对齐，为跨模态推理和生成提供了更有效的统一模型框架。
...
TL;DR: 本文提出CoRegOVCD，一种无需训练的开集词汇变化检测框架，通过竞争性后验校准和语义后验差异将原始概念响应转化为可比较的语义变化证据，结合几何令牌一致性门和区域共识差异提升空间一致性。


<details>
  <summary>Details</summary>
Motivation: 传统遥感变化检测方法假设固定的标签空间，无法响应用户定义的任意查询。开集词汇变化检测(OVCD)旨在检测查询概念的变化掩码，但在完全无需训练的情况下，直接比较不同时间点的密集概念响应存在困难：外观变化、弱跨概念竞争以及许多土地覆盖类别的空间连续性会导致噪声、碎片化和语义不可靠的变化证据。

Method: 提出一致性正则化开集词汇变化检测(CoRegOVCD)框架，包含：1)竞争性后验校准(CPC)和语义后验差异(SPD)将原始概念响应转化为竞争感知的查询概念后验并量化跨时间差异；2)几何令牌一致性门(GeoGate)和区域共识差异(RCD)通过几何感知结构验证和区域共识抑制不支持响应并提升空间一致性。

Result: 在四个基准测试中（涵盖建筑导向和多类别设置），CoRegOVCD相比之前最强的无需训练基线持续提升了2.24到4.98个F1_C点，在SECOND数据集上达到六类平均47.50%的F1_C分数。

Conclusion: CoRegOVCD通过后验校准和一致性正则化机制，有效解决了开集词汇变化检测中跨时间概念响应可比性问题，显著提升了无需训练设置下的变化检测性能。
...
TL;DR: 本文提出了一种反射生成方法，通过向基础扩散模型注入反射位置和外观的先验信息，并采用类型感知的模型设计，生成了物理一致且视觉逼真的反射效果。
...
Conclusion: 本文提出的反射生成方法通过注入先验信息和类型感知设计，成功生成了物理一致且视觉逼真的反射效果，填补了反射生成研究领域的空白。
...
Method: 开发了ViT-Explainer系统，提供Vision Transformer推理的集成可视化，包括动画演示、分块级注意力覆盖和视觉适配的Logit Lens，支持引导和自由探索两种模式。

Result: 对6名参与者的用户研究表明，ViT-Explainer易于学习和使用，能够帮助用户解释和理解Vision Transformer的行为。

Conclusion: ViT-Explainer填补了Vision Transformer可解释性工具的空白，通过集成可视化方法提供了对完整推理流程的端到端理解，有助于促进Transformer模型在视觉领域的透明度和可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在自动驾驶任务中面临空间感知与语义推理的困境：直接采用2D VLM空间感知有限，而增强3D表示又会损害VLM的原生推理能力。这种困境源于空间感知和语义推理在共享模型参数中的耦合优化。

Method: 提出基于Mixture-of-Transformers的UniDriveVLA模型，包含驾驶理解、场景感知和动作规划三个专家模块，通过掩码联合注意力协调。结合稀疏感知范式和三阶段渐进训练策略，在保持语义推理能力的同时提升空间感知。

Result: 在nuScenes的开环评估和Bench2Drive的闭环评估中达到最先进性能。在3D检测、在线建图、运动预测和驾驶导向VQA等多种感知、预测和理解任务上表现出色，展示了作为自动驾驶统一模型的广泛适用性。

Conclusion: UniDriveVLA通过专家解耦成功解决了自动驾驶VLA模型中空间感知与语义推理的冲突，实现了高性能的统一模型，为自动驾驶系统提供了更强的认知能力。
...
TL;DR: SCALE是一个轻量级确定性框架，通过语义和置信度感知的列表能量排序解决零样本骨架动作识别问题，避免了脆弱的显式骨架-文本对齐。


<details>
  <summary>Details</summary>
Motivation: 现有零样本骨架动作识别方法依赖显式的骨架-文本对齐，当动作名称无法充分描述细粒度动态特征或未见类别语义混淆时，这种方法很脆弱。需要更鲁棒的框架来处理这些挑战。

Method: 提出SCALE框架，将ZSAR建模为类条件能量排序。使用文本条件变分自编码器，冻结文本表示参数化潜在先验和解码器。引入语义和置信度感知的列表能量损失，强调语义相似的困难负样本，并融入后验不确定性来调整决策边界和重新加权模糊训练实例。还使用潜在原型对比目标来对齐后验均值与文本派生的潜在原型。

Result: 在NTU-60和NTU-120数据集上的实验表明，SCALE相比之前的VAE和对齐基线方法有持续改进，同时与基于扩散的方法保持竞争力。

Conclusion: SCALE通过能量排序框架和语义感知的损失函数，为ZSAR提供了一种更鲁棒的解决方案，避免了脆弱的显式对齐，同时保持了轻量级和确定性特性。
...
<details>
  <summary>Details</summary>
Motivation: 基础视觉Transformer在需要细粒度空间理解的任务中存在局限性，特别是在密集预测场景中，现有滑动窗口方法计算成本高昂

Method: 通过特征回归损失将细步长滑动窗口教师模型的空间推理能力蒸馏到单次前向传播的学生模型中，无需架构更改或像素级监督

Result: SPAR将单次前向传播基线提升高达10.5 mIoU，甚至超过了教师模型，在高效高分辨率推理中表现出色

Conclusion: SPAR为视觉Transformer提供了一种高效的高分辨率推理解决方案，在保持计算效率的同时显著提升了细粒度空间理解能力
...
TL;DR: 提出了一种基于推理时引导的文本到图像生成安全控制框架，利用冻结的预训练基础模型作为语义能量估计器，无需修改生成器即可实现模块化、免训练的安全控制。
...
Method: 提出推理时引导框架，利用冻结的视觉语言基础模型作为现成的监督信号，通过在每个采样步骤注入梯度反馈，将安全引导构建为基于能量的采样问题。

Result: 在NSFW红队测试基准上达到最先进的鲁棒性，能有效进行多目标引导，同时在良性非目标提示上保持高生成质量。框架兼容扩散模型和流匹配模型。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在统一文本和图像理解与生成方面表现出色，但扩展到3D领域面临挑战，主要原因是高质量3D数据稀缺，导致3D合成约束不足。现有方法通常依赖间接流程，先在2D中编辑，然后通过优化将结果提升到3D，这会牺牲几何一致性。

Method: 提出Omni123模型，将文本、图像和3D表示为共享序列空间中的离散标记，利用丰富的2D数据作为几何先验。采用交错X-to-X训练范式，协调异构配对数据集上的多样化跨模态任务，无需完全对齐的文本-图像-3D三元组。通过在自回归序列中遍历语义-视觉-几何循环（如文本→图像→3D→图像），联合强制执行语义对齐、外观保真度和多视图几何一致性。

Result: 实验表明，Omni123显著改善了文本引导的3D生成和编辑，展示了向多模态3D世界模型的可扩展路径。

Conclusion: Omni123通过统一文本到2D和文本到3D生成，利用跨模态一致性作为隐式结构约束，为3D原生基础模型提供了创新解决方案，有效解决了3D数据稀缺问题，推动了多模态3D世界模型的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有表面配准方法在效率和鲁棒性之间存在权衡：局部点匹配方法计算高效但对噪声和初始化敏感；全局点集对齐方法计算成本高。需要一种既能保持高效计算又具有鲁棒性的方法。

Method: 将表面网格表示为概率度量，将表面配准转化为分布优化问题。使用具有对数线性计算复杂度的切片Wasserstein距离测量网格间差异。提出AdamFlow优化方法，将Adam优化从欧几里得空间推广到概率空间。
...
TL;DR: VOID是一个视频对象移除框架，专注于处理对象移除后需要改变下游物理交互的复杂场景，通过生成物理上合理的反事实结果。


<details>
  <summary>Details</summary>
Motivation: 现有视频对象移除方法擅长处理对象"背后"的内容修复和外观层面的伪影（如阴影、反射），但当移除的对象与其他对象有更显著的物理交互（如碰撞）时，现有模型无法正确修正这些交互，产生不真实的结果。

Method: 1. 使用Kubric和HUMOTO生成新的配对反事实对象移除数据集；2. 推理时使用视觉语言模型识别受移除对象影响的场景区域；3. 用这些区域指导视频扩散模型生成物理一致的反事实结果。

Result: 在合成和真实数据上的实验表明，与现有视频对象移除方法相比，VOID方法在对象移除后能更好地保持一致的场景动态。

Conclusion: 该框架展示了如何通过高级因果推理使视频编辑模型成为更好的世界模拟器，为处理复杂物理交互的对象移除提供了新思路。
...
### [97] [A Simple Baseline for Streaming Video Understanding](https://arxiv.org/abs/2604.02317)
*Yujiao Shen,Shulin Tian,Jingkang Yang,Ziwei Liu*

Main category: cs.CV

TL;DR: 研究发现简单的滑动窗口方法（仅使用最近N帧）在流式视频理解任务上已能达到或超越复杂记忆机制模型的性能，提出了SimpleStream基准并揭示了感知-记忆权衡关系。


<details>
  <summary>Details</summary>
Motivation: 挑战当前流式视频理解领域过度依赖复杂记忆机制的趋势，发现简单的滑动窗口方法已经足够有效，需要建立更清晰的评估基准来区分近期感知和长期记忆能力。

Method: 提出SimpleStream基准方法，仅使用最近N帧作为输入，采用现成的视觉语言模型（VLM），在OVO-Bench和StreamingBench上评估了13个离线/在线视频LLM基线模型。

Result: 仅使用4个最近帧，SimpleStream在OVO-Bench上达到67.7%平均准确率，在StreamingBench上达到80.59%。研究发现长上下文的价值取决于模型骨干而非模型规模，并揭示了感知-记忆权衡：更多历史上下文能改善回忆但会削弱实时感知。

Conclusion: 未来流式视频理解研究应区分近期场景感知和长期记忆能力评估，复杂的记忆、检索或压缩模块只有在明确超越SimpleStream基准时才应被视为真正的进展。
...
TL;DR: LCA提出了一种大规模编解码器化身模型，通过预训练/后训练范式解决3D化身建模中保真度与泛化性的权衡问题，能在保持高质量的同时泛化到大规模人群。


<details>
  <summary>Details</summary>
Motivation: 当前3D化身建模面临保真度与泛化性的关键权衡：多视角工作室数据能实现高保真但泛化性差；大规模野外数据训练模型泛化性好但质量低。需要一种既能保持高质量又能泛化到大规模人群的解决方案。

Method: 采用预训练/后训练范式：先在100万野外视频上预训练学习外观和几何的广泛先验，然后在高质量策划数据上进行后训练以增强表现力和保真度。模型能泛化到不同发型、服装和人口统计特征，同时提供精细的面部表情和手指级关节控制。

Result: LCA实现了高质量全身3D化身建模，能以前馈方式泛化到世界规模人群，支持高效推理。模型表现出对重新照明和宽松服装的涌现泛化能力，对风格化图像具有零样本鲁棒性，尽管没有直接监督。

Conclusion: LCA通过大规模预训练/后训练范式成功解决了3D化身建模中保真度与泛化性的权衡问题，为高质量、可泛化的3D化身建模提供了有效解决方案，展现了类似大语言模型的涌现能力。
...
TL;DR: 提出场景式视觉定位新任务，引入RSC基准测试，包含段落级描述和难度标签，并开发ScenGround课程学习方法


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位基准主要评估图像区域与字面指称表达的对齐，模型可通过匹配显式类别名称获得成功。需要探索更具挑战性的场景式视觉定位，要求从角色、意图和关系上下文中推理目标

Method: 引入Referring Scenario Comprehension (RSC)基准，包含段落长度文本描述、可解释难度标签；提出ScenGround课程推理方法，结合监督预热和难度感知强化学习

Result: RSC包含约31k训练样本、4k域内测试样本和3k分布外样本；实验显示场景式查询暴露了现有模型的系统性失败，课程训练提高了挑战性子集性能并迁移到标准基准

Conclusion: 场景式视觉定位比传统基准更具挑战性，RSC基准支持细粒度分析，ScenGround课程学习方法有效提升性能，为视觉推理研究提供新方向
...
TL;DR: 该论文提出了一种可操控的视觉表示方法，能够通过自然语言引导视觉特征关注图像中不太显著的概念，同时保持基础表示质量。


<details>
  <summary>Details</summary>
Motivation: 现有预训练视觉Transformer（如DINOv2和MAE）虽然能提供通用图像特征，但倾向于关注图像中最显著的部分，无法引导其关注不太突出的感兴趣概念。而多模态大语言模型虽然可以通过文本提示引导，但得到的表示往往是语言中心的，在通用视觉任务上效果不佳。

Method: 提出可操控视觉表示，通过轻量级交叉注意力机制将文本直接注入视觉编码器的各层（早期融合），而不是像大多数视觉语言模型（如CLIP）那样在编码后进行融合（后期融合）。

Result: 该方法能够引导视觉特征关注图像中任何期望的对象，同时保持底层表示质量。在异常检测和个性化对象识别任务上匹配或优于专用方法，并展现出对分布外任务的零样本泛化能力。

Conclusion: 提出的可操控视觉表示方法成功解决了现有视觉表示无法灵活引导关注特定概念的问题，实现了通过自然语言精确控制视觉特征关注点的能力，在多种视觉任务上表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法独立处理各个视角，缺乏对多视角和多模态数据的有效整合。工业场景中的3D异常检测需要同时考虑不同视角和模态（如深度信息）的互补信息。

Method: 1) 采用跨模态特征映射范式学习跨模态和跨视角的特征映射；2) 通过特征调制显式建模视角依赖关系；3) 引入跨视角训练策略利用所有可能的视角组合；4) 训练并公开针对工业数据集优化的深度编码器。

Result: 在SiM3D基准测试中，ModMap取得了最先进的性能，大幅超越先前方法，证明了多视角多模态框架的有效性。

Conclusion: ModMap通过原生多视角多模态设计、跨视角特征映射和显式视角关系建模，为3D异常检测与分割提供了有效的解决方案，并在基准测试中表现出色。
...
TL;DR: 该论文提出了一个从AAA游戏中提取的大规模动态数据集，包含400万帧同步RGB和G-buffer数据，用于提升生成式逆渲染和正向渲染的真实感和时间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有合成数据集在真实感和时间一致性方面存在局限，阻碍了生成式逆渲染和正向渲染在真实场景中的扩展。需要弥合这一领域差距。

Method: 采用新颖的双屏拼接捕获方法，从视觉复杂的AAA游戏中提取400万连续帧（720p/30 FPS）的同步RGB和五个G-buffer通道数据，涵盖多样化场景、视觉效果和环境条件。同时提出基于VLM的评估协议来测量语义、空间和时间一致性。

Result: 在该数据集上微调的逆渲染器实现了优越的跨数据集泛化能力和可控生成。VLM评估结果与人类判断高度相关。结合工具包，正向渲染器允许用户通过文本提示从G-buffer编辑AAA游戏的风格。

Conclusion: 该大规模动态数据集显著推进了双向渲染技术，既支持鲁棒的野外几何和材质分解，又促进了高保真G-buffer引导的视频生成，为真实世界渲染应用提供了重要资源。
...
TL;DR: ActionParty是一个用于生成式视频游戏的动作可控多主体世界模型，解决了现有视频扩散模型中动作绑定问题，能够同时控制多达7个玩家在46个不同环境中互动。
...
Result: 在Melting Pot基准测试中评估，展示了首个能够同时控制多达7个玩家在46个不同环境中互动的视频世界模型。结果显示在动作跟随准确性和身份一致性方面有显著改进，同时能够通过复杂交互实现鲁棒的自回归主体跟踪。

Conclusion: ActionParty成功解决了多主体视频生成中的动作绑定问题，为生成式视频游戏开发提供了有效的多主体世界建模框架。
...
TL;DR: 该研究探讨了在线交流中常见的重复延长形式（RLF）对情感分析的重要性，创建了首个多领域RLF数据集，并提出了可解释指令调优框架来提升LLM对RLF的理解能力。


<details>
  <summary>Details</summary>
Motivation: 在线交流中人们经常使用非正式表达方式（如表情包、表情符号），但一种独特且强调性的重复延长形式（RLF）多年来一直被忽视。研究旨在探索RLF对情感分析的重要性以及语言模型是否能理解RLF。

Method: 1）创建了首个多领域RLF数据集Lengthening（85万样本）；2）提出了可解释指令调优框架ExpInstruct，采用两阶段指令调优方法；3）设计了统一方法来量化语言模型对非正式表达的理解能力。

Result: RLF句子是富有表现力的表达方式，可作为文档级情感的特征标记；微调的预训练语言模型在性能上能超越零样本GPT-4，但在解释性方面不如GPT-4；ExpInstruct框架能在有限样本下让开源LLM在RLF任务上达到零样本GPT-4的性能和解释性水平。

Conclusion: RLF对在线内容分析具有潜在价值，ExpInstruct框架能有效提升语言模型对重复延长形式的理解和解释能力，为情感分析中的非正式表达处理提供了新方法。
...
TL;DR: 通过训练时强化学习和测试时并行思考两种互补方法，扩展竞争性编程的推理令牌预算，实现高效推理


<details>
  <summary>Details</summary>
Motivation: 解决竞争性编程中单次生成推理在强化学习训练下计算成本高昂的问题，需要更高效的推理扩展方法

Method: 1) 训练时强化学习：使用验证RL预热和随机剪枝优化训练轨迹；2) 测试时多轮并行思考：将令牌预算分配到多个线程和轮次中，进行生成、验证和精炼

Result: 基于Seed-OSS-36B模型，16线程16轮次的完整系统在平均每个问题使用760万令牌的情况下，达到基础RL模型oracle pass@16的pass@1性能，并在AetherCode的456个难题上超越GPT-5-high

Conclusion: 结合训练时强化学习和测试时并行思考的互补方法，能够有效扩展竞争性编程的推理能力，实现高效的大规模推理
...
TL;DR: M2-Verify是一个大规模多模态数据集，用于评估科学声明与证据的一致性，包含46.9万+实例，覆盖16个领域，现有模型在复杂场景下表现不佳


<details>
  <summary>Details</summary>
Motivation: 现有基准测试缺乏规模、领域多样性和视觉复杂性，无法真实评估科学声明与多模态证据之间的一致性

Method: 从PubMed和arXiv收集数据，创建M2-Verify数据集，包含超过469K实例，覆盖16个领域，通过专家审核进行严格验证

Result: 最先进模型在低复杂度医学扰动上达到85.8% Micro-F1，但在解剖学变化等高复杂度挑战上性能降至61.6%，专家评估显示模型在生成科学解释时存在幻觉
...
TL;DR: 该研究提出了一种特征增强框架，通过添加可解释信号（如回复长度、拒绝指标、毒性分数等）来改进语言模型对人类偏好的学习，在Anthropic HHRLHF数据集上实现了从0.74到0.84 ROC AUC的性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型学习人类偏好面临根本性挑战，因为奖励建模依赖于微妙、主观的比较而非明确标签。现有方法在标准成对偏好设置下性能有限（ROC AUC低于0.74），难以捕捉人类判断的多维本质。

Method: 提出特征增强框架，在文本表示基础上添加可解释信号：回复长度、拒绝指标、毒性分数和提示-回复语义相似度。使用SHAP和LIME进行细粒度可解释性分析，在Anthropic HHRLHF数据集上评估10个不同的大型语言模型。

Result: 混合方法在所有模型上实现了一致的改进，最高达到0.84 ROC AUC和显著更高的成对准确率。DeBERTav3Large表现最佳。可解释性分析显示模型决策依赖于情境化的安全性和支持性框架而非孤立关键词。

Conclusion: 特征增强框架能有效提升语言模型对人类偏好的学习能力，通过可解释信号显式捕捉有用性、安全性和相关性等关键方面。研究还揭示了特征交互对偏好学习的影响，为改进奖励建模提供了新方向。
...
TL;DR: Reasoning Memory：一个检索增强生成框架，通过分解推理轨迹为子问题-子程序对，构建3200万条程序知识条目，在推理时检索并复用程序知识，显著提升语言模型在数学、科学和编程任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有测试时扩展方法通常孤立处理每个问题，未能系统性地复用先前推理轨迹中的知识，特别是程序性知识（如何重构问题、选择方法、验证或回溯）。

Method: 从逐步推理轨迹语料库中，将每个轨迹分解为自包含的子问题-子程序对，构建包含3200万条程序知识条目的数据存储。推理时，通过轻量级思维内提示让模型表达核心子问题，在其推理轨迹内检索相关子程序，并在多样检索到的子程序作为隐式程序先验下进行推理。

Result: 在六个数学、科学和编程基准测试中，Reasoning Memory始终优于基于文档、轨迹和模板知识的RAG方法，以及计算匹配的测试时扩展基线。在更高推理预算下，相比无检索提升达19.2%，相比最强计算匹配基线提升7.9%。

Conclusion: 性能提升来自两个关键因素：源轨迹的广泛程序覆盖性，以及分解和检索设计，共同实现了程序知识的有效提取和复用。
...
TL;DR: 研究发现语言模型通过稀疏的MLP神经元实现实体检索，这些神经元集中在早期层，能够通过因果干预影响实体相关的预测


<details>
  <summary>Details</summary>
Motivation: 语言模型能够回答实体相关的知识问题，但其内部机制尚不明确。本研究旨在探索语言模型在实体检索过程中的内部工作机制

Method: 使用模板化提示定位实体选择性MLP神经元，通过PopQA数据集验证，对200个实体进行因果干预实验，包括负向消融和受控注入

Result: 实体选择性神经元集中在早期层；负向消融导致实体特异性遗忘；单神经元激活即可恢复实体一致性预测；对别名、缩写、拼写错误和多语言形式具有鲁棒性；效果显著但不普遍，流行实体覆盖率高

Conclusion: 研究识别了稀疏、可因果操作的访问点，用于分析和调节实体条件化的事实行为，支持紧凑实体检索而非纯粹深度渐进富集的观点
...
<details>
  <summary>Details</summary>
Motivation: 翻译过程中的打字停顿通常被用来区分自动化翻译和需要认知处理的翻译过程，但如何确定区分这两种过程的停顿阈值一直存在争议。

Method: 比较了三种现有的计算停顿阈值的方法，并提出了一种新的生产单元中断计算方法进行评估。

Result: 通过比较不同方法，验证了新提出的生产单元中断计算方法的有效性。

Conclusion: 新提出的生产单元中断计算方法为更准确地识别翻译过程中的认知处理提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有数千个基准测试评估大语言模型性能，但研究表明模型性能可由少量潜在因素解释。需要更高效、有原则的基准测试方法，但缺乏比较不同方法质量的框架。研究基于预测效度，认为基准测试框架质量应基于其预测模型在未见任务上性能的效率。

Method: 收集WIDE-scale Item Level Dataset (WILD)，包含65个模型在163个任务（来自27个数据集）的109,564个独特项目-模型响应对。采用改进的多维项目反应理论(IRT)模型，结合基于最优实验设计的自适应项目选择，并引入成本感知折扣因子优化选择标准。

Result: 改进IRT模型与自适应项目选择能在仅观察16个项目后，以小于7%的平均绝对误差预测112个保留基准测试任务的性能。加入成本感知折扣因子后，达到7%MAE所需总token数从141,000降至22,000，评估成本降低85%。

Conclusion: 基于预测效度的基准测试框架能显著提高评估效率，结合改进IRT模型与自适应项目选择在有限预算下有效预测模型在未见任务上的性能，为更高效、有原则的基准测试提供了方法论基础。
...
TL;DR: 使用随机森林模型分析印地语同义词的词嵌入，发现即使语义无关，也能根据使用模式区分梵语和波斯-阿拉伯语起源，表明语境编码了词源信号，同义词反映了与起源相关的系统性差异。


<details>
  <summary>Details</summary>
Motivation: 研究同义词现象，特别是印地语中梵语和波斯-阿拉伯语借词形成的同义词对。虽然理论上绝对同义词不应存在，但缺乏定量证据证明同义词是否反映不同视角或文化关联。本研究旨在通过分布数据测试能否仅凭使用模式区分同义词的词源。

Method: 使用随机森林分类器训练印地语同义词的词嵌入数据。研究梵语和波斯-阿拉伯语起源的同义词对，分析其分布特征，即使这些词在语义上无关。通过机器学习方法探索词源信息是否编码在词语的使用模式中。

Result: 随机森林模型成功根据词嵌入区分了梵语和波斯-阿拉伯语起源的词语，准确率显著。即使语义无关的词语，其使用模式仍保留词源痕迹。这表明语境编码了词源信号，同义词之间存在与起源相关的系统性差异。

Conclusion: 研究提供了定量证据，表明语境编码词源信号，同义词可能反映与起源相关的微妙但系统性差异。支持同义词提供不同视角的观点，词源相关词语可能形成独特的概念子空间，创造由历史起源塑造的新型语义框架。结果突显了语境在捕捉超越传统语义相似性的细微差异方面的强大能力。
...
TL;DR: 研究发现：在AI生成内容中，引用粒度（句子级、段落级或文档级）对模型性能有显著影响。精细的句子级引用会降低16-276%的归因质量，而段落级引用表现最佳。模型规模越大，精细引用对其负面影响越明显。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索引用粒度（句子级、段落级或文档级）对AI模型归因质量的影响。虽然细粒度的引用通常被认为更便于人工验证，但其对模型性能的具体影响尚未得到充分研究。

Method: 研究方法包括分析四种不同规模的模型（8B-120B），比较不同引用粒度（句子级、段落级、文档级）下的归因质量。通过实验评估精细引用约束对模型性能的影响，并观察模型规模与最优引用粒度之间的关系。

Result: 实验结果显示：1）强制使用精细的句子级引用会使归因质量下降16-276%；2）归因质量在中间粒度（段落级）达到峰值；3）精细引用会破坏证据归因所需的语义依赖关系；4）过粗的引用会引入干扰噪声；5）模型规模越大，精细引用对其负面影响越明显；6）最优引用粒度不仅能提高归因质量，还能保持甚至改善答案正确性。

Conclusion: 结论是：单纯为了便于人工验证而采用精细引用会忽视模型约束，损害归因忠实性和生成可靠性。有效的归因需要将引用粒度与模型的自然语义范围对齐，段落级引用是最佳选择。
...
TL;DR: 该研究通过电路级机制分析揭示了LLM中过度自信表达的内部机制，发现特定MLP块和注意力头在中间到后期层中持续写入过度自信信号，并通过针对性干预改善了校准效果。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常表现出"过度自信"的问题：当产生事实错误的答案时，它们倾向于表达过高的置信度而非表示不确定性。这种过度自信的表达会误导用户并削弱置信分数作为可靠不确定性信号的价值，但其内部机制尚不清楚。

Method: 采用电路级机制分析方法，围绕三个轴展开：1) 将表达性置信度捕获为可微分的内部信号；2) 识别导致置信度膨胀的因果电路；3) 利用这些见解进行有针对性的推理时重新校准。在两个指令调优的LLM和三个数据集上进行实验。

Result: 研究发现，一组紧凑的MLP块和注意力头（主要集中在中间到后期层）持续在最终标记位置写入置信度膨胀信号。通过针对这些电路的推理时干预，显著改善了模型的校准效果。

Conclusion: LLM中的表达性过度自信是由可识别的内部电路驱动的，可以通过针对性干预来缓解。这为理解模型不确定性表达机制和改善校准提供了新的见解。
...
TL;DR: 该研究使用129,451首波斯诗歌构建多图层图，追踪诗歌中反复出现的象征形式家族，分析其随时间变化的内部权重和连接关系，发现波斯象征系统是长期演变的动态体系而非固定剧目。


<details>
  <summary>Details</summary>
Motivation: 传统计算方法将波斯诗歌材料简化为孤立词汇或宽泛文档语义，忽略了波斯诗学中相关形式以家族形式传播并通过重复关系获得力量的实践组织单元。

Method: 使用129,451首诗歌语料库，将反复出现的象征形式整合为可追踪的家族，将意象材料与神圣和宫廷参照分离，并在多图层图中映射它们的关系，按11个伊斯兰世纪时段进行分析。

Result: 象征核心相对稀疏，参照成分更密集，两者之间的连接区域具有选择性。某些家族（如夜、日、土）广泛分布，酒器、花园空间、火焰和抒情声音后期增强，而声望编码和英雄宫廷词汇早期权重更高。模块性增加，跨范围连接减少，宫廷桥梁减弱，神圣桥梁增强。

Conclusion: 波斯象征系统不是固定剧目，而是内部权重和连接随时间变化的长期系统，展现了动态演变的特性而非静态模式。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型研究中关于"多样性"的术语碎片化，缺乏对任务规范性目标的明确阐述，需要统一的框架来理解输出变化在不同任务语境下的意义和价值。

Method: 提出Magic, Madness, Heaven, Sin框架，将输出变化建模为同质-异质轴，组织任务到四个规范性语境：认知性（事实性）、交互性（用户效用）、社会性（代表性）、安全性（鲁棒性），并分析跨语境交互。

Result: 框架揭示了优化一个目标（如安全性）可能无意中损害人口代表性或创造性多样性，强调了输出变化是任务目标塑造的属性而非模型内在特性。

Conclusion: 需要基于语境感知的输出变化评估，将其重新定义为由任务目标塑造的属性，而不是模型的固有特征，以更好地理解和平衡不同规范性目标之间的权衡。
...
TL;DR: 扩散模型无法通过自然语言指令有效遗忘特定概念，指令级遗忘在图像生成模型中存在根本性限制


<details>
  <summary>Details</summary>
Motivation: 探索指令式遗忘范式是否适用于扩散模型等生成模型，验证自然语言指令能否有效控制扩散模型遗忘特定概念

Method: 通过多概念多提示变体的控制实验，分析CLIP文本编码器和去噪过程中的交叉注意力动态，研究指令式遗忘在扩散模型中的效果

Result: 扩散模型无法通过自然语言遗忘指令有效抑制目标概念，遗忘指令未能持续减少对目标概念标记的注意力，目标概念表示在整个生成过程中持续存在
...
TL;DR: 提出模型融合框架，通过合并临床基础模型和通用指令模型，解决医学领域微调中的灾难性遗忘问题，实现高效临床LLM适配。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医学临床文档应用中面临灾难性遗忘问题——当使用特定医学数据集微调时，会显著丧失指令跟随能力，这阻碍了通用LLM在临床应用中的采用。
...
Conclusion: 权重空间融合为开源LLM在临床应用中提供了一种高度可扩展的适配方案，有助于在资源受限的医疗环境中更广泛部署。
...
TL;DR: DeltaMem：一个基于强化学习的单智能体记忆管理系统，通过记忆编辑距离和RL框架优化人设记忆管理，在多个长期记忆基准测试中超越现有产品级基线


<details>
  <summary>Details</summary>
Motivation: 现有的人设中心记忆管理系统多为多智能体框架，存在信息丢失问题，在不同场景下表现脆弱，导致性能不佳。需要更鲁棒、高效的记忆管理方案。

Method: 1. 提出DeltaMem单智能体记忆管理系统，将人设记忆管理建模为端到端任务；2. 合成用户-助手对话数据集及操作级记忆更新标签；3. 引入基于记忆的Levenshtein距离形式化记忆更新奖励；4. 设计定制化强化学习框架提升管理能力

Result: DeltaMem（包括无训练和RL训练版本）在LoCoMo、HaluMem和PersonaMem等多个长期记忆基准测试中均优于所有产品级基线方法

Conclusion: DeltaMem通过单智能体架构、记忆编辑距离度量和强化学习框架，有效解决了现有复杂记忆管理系统中的信息丢失和场景脆弱性问题，在人设记忆管理任务上取得了显著性能提升
...
TL;DR: Swift-SVD是一个激活感知的闭式压缩框架，通过增量聚合输出激活的协方差并进行单次特征值分解，实现训练免费、快速且最优的层间低秩近似，在保证理论最优的同时提供3-70倍的端到端压缩速度提升。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的部署受到静态权重和动态键值缓存的内存和带宽需求的限制。虽然基于SVD的压缩提供了硬件友好的解决方案来降低这些成本，但现有方法存在两个关键限制：一些方法在重构误差上不是最优的，而另一些方法虽然理论上最优但实际效率低下。

Method: 提出Swift-SVD框架，通过增量聚合给定输入批次下输出激活的协方差，并在聚合后执行单次特征值分解。使用有效秩分析局部层间可压缩性，并设计动态秩分配策略，同时考虑局部重构损失和端到端层重要性。

Result: 在六个LLM和八个数据集上的广泛实验表明，Swift-SVD优于最先进的基线方法，在实现最优压缩精度的同时，端到端压缩时间提升了3-70倍。

Conclusion: Swift-SVD提供了一个同时保证理论最优、实际效率和数值稳定的激活感知闭式压缩框架，解决了现有SVD压缩方法的局限性，为LLM部署提供了高效的压缩解决方案。
...
TL;DR: 针对印度尼西亚K-12教师AI使用情况的全国性调查显示，AI在教学中应用增加但分布不均，主要用于减轻教学准备工作量，但仍面临通用输出、基础设施不足和情境适配等挑战。


<details>
  <summary>Details</summary>
Motivation: 尽管AI在印尼课堂中逐渐应用，但缺乏大规模、以教师为中心的实际使用证据和支持需求数据，这阻碍了开发适合当地情境的AI系统和政策制定。

Method: 对349名K-12教师（涵盖小学、初中和高中）进行全国性问卷调查，收集教师使用AI的实践情况和支持需求。

Result: 1. AI在教学中应用增加但分布不均：小学教师使用更一致，高中教师参与较少；职业生涯中期教师更重视AI，印尼东部地区教师认为AI价值更高。2. 教师主要使用AI减轻教学准备工作量（评估、课程规划、材料开发）。3. 主要障碍包括：AI输出过于通用、基础设施限制、情境适配有限。

Conclusion: 研究揭示了印尼教师AI使用的实际模式和挑战，为开发适合当地情境的AI教育系统和支持政策提供了实证基础，强调了需要解决基础设施、情境适配和教师支持等关键问题。
...
TL;DR: 研究发现现有声明验证基准主要测试证据提取能力，而多句综合和数值推理严重不足，高基准分数主要反映检索+蕴含能力而非真正推理能力


<details>
  <summary>Details</summary>
Motivation: 尽管声明验证领域进展迅速，但缺乏对现有基准实际测试推理能力的系统理解，需要分析这些基准真正测试的是什么推理类型

Method: 使用GPT-4o-mini为9个数据集的24K声明验证示例生成结构化推理轨迹，分析推理类型分布；使用1B参数推理验证器分析五种错误类型在不同领域的分布

Result: 直接证据提取占主导地位（占大部分案例），多句综合和数值推理严重不足；数据集存在明显偏见：有些几乎只测试词汇匹配，有些约一半案例需要信息综合；不同领域错误模式差异显著：通用领域主要受词汇重叠偏见影响，科学领域过度谨慎，数学领域算术推理失败

Conclusion: 高基准分数主要反映检索+蕴含能力而非真正推理能力，需要构建更具挑战性的评估套件来更好地测试验证系统所需的推理能力
...
<details>
  <summary>Details</summary>
Motivation: 现有情感支持对话方法在深度上下文理解方面存在不足，无法有效支持情感推理，需要更有效的方法来提升情感支持的质量。

Method: 提出PRCCF框架：1）人物引导检索机制，联合建模语义兼容性和人物对齐；2）因果感知认知过滤模块，优先考虑因果相关的外部知识，增强上下文认知理解。

Result: 在ESConv数据集上的广泛实验表明，PRCCF在自动指标和人工评估方面均优于最先进的基线方法。

Conclusion: PRCCF通过结合人物引导检索和因果感知认知过滤，有效提升了情感支持对话的深度上下文理解和情感推理能力。
...
TL;DR: 研究发现：在相同问题集上，使用不同模型生成的CoT轨迹进行SFT训练时，训练损失更低并不一定带来更好的泛化性能。DeepSeek-R1数据训练损失更低但泛化更差，原因是其推理模式发散且分支多，导致模型继承低效探索行为。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索不同来源的CoT轨迹如何影响大模型的泛化性能。虽然SFT在长CoT轨迹上已成为构建大推理模型的关键阶段，但不同来源的CoT轨迹对模型泛化能力的影响机制尚不明确。

Method: 研究方法包括：1）使用两个竞争模型（DeepSeek-R1-0528和gpt-oss-120b）在相同问题集上生成已验证的CoT轨迹；2）对比分析两种轨迹的SFT训练效果；3）通过多维度分析探究token级SFT损失和step级推理行为；4）提出基于轨迹分支频率的过滤方法改进SFT。

Result: 研究结果发现：1）DeepSeek-R1数据训练损失更低但泛化性能显著更差；2）gpt-oss-120b轨迹收敛性强、演绎性强，而DeepSeek-R1轨迹发散性强、分支多；3）过滤频繁分支轨迹后，DeepSeek-R1子集的训练使推理性能显著提升：AIME25提升5.1%，BeyondAIME提升5.5%，五个基准平均提升3.6%。

Conclusion: 研究结论：CoT轨迹的质量不仅取决于生成模型的性能，更取决于其推理模式。收敛性强的演绎式推理轨迹比发散性强的探索式轨迹更适合SFT训练。通过过滤频繁分支轨迹可以显著改善SFT的泛化性能。
...
TL;DR: EndoASR是一个专门为胃肠内窥镜领域设计的自动语音识别系统，通过两阶段适应策略显著提升了在复杂临床环境中的识别准确率和实时性能。


<details>
  <summary>Details</summary>
Motivation: 传统ASR系统在胃肠内窥镜临床环境中面临领域特定术语和复杂声学条件的挑战，限制了其在真实世界临床环境中的可靠性，需要专门针对该领域优化的解决方案。

Method: 开发了两阶段适应策略：基于合成内窥镜报告进行领域特定语言建模和噪声鲁棒性优化，构建了紧凑的220M参数模型，支持实时边缘部署。

Result: 在回顾性评估中，字符错误率从20.52%降至14.14%，医学术语准确率从54.30%提升至87.59%；前瞻性多中心研究中，字符错误率从16.20%降至14.97%，医学术语准确率从61.63%提升至84.16%，实时因子达到0.005，显著优于Whisper-large-v3。

Conclusion: EndoASR证明了领域适应ASR可以作为胃肠内窥镜中人机协作的可靠接口，其性能在多中心真实世界临床环境中得到一致验证，并能有效提升下游结构化信息提取和临床医生-AI交互质量。
...
TL;DR: 本文系统综述了LLM智能体中的记忆方法，提出了统一框架，在标准基准上全面比较现有方法，并设计出超越现有SOTA的新记忆方法。


<details>
  <summary>Details</summary>
Motivation: 记忆是LLM智能体处理长时程复杂任务（如多轮对话、游戏、科学发现）的核心模块，能实现知识积累、迭代推理和自我进化。现有记忆方法众多但缺乏系统比较，需要统一评估框架来深入理解不同方法的有效性。

Method: 1. 提出统一框架，从高层次视角整合现有所有智能体记忆方法；2. 在两个知名基准上广泛比较代表性记忆方法；3. 通过实验分析设计新的记忆方法，利用现有方法的模块组合；4. 基于发现提出未来研究方向。

Result: 1. 建立了统一的智能体记忆方法分析框架；2. 在标准基准上全面评估了现有方法的有效性；3. 设计的新记忆方法超越了现有最先进方法；4. 提供了对现有方法行为的深入理解。

Conclusion: 本文系统比较了LLM智能体记忆方法，建立了统一分析框架，通过实验验证设计出更优方法，为未来研究提供了有价值的见解和方向。对现有方法行为的深入理解能为未来研究提供重要参考。
...
<details>
  <summary>Details</summary>
Motivation: 越南语语音情感识别面临挑战：声学模式模糊、标注数据不足，尤其在真实场景中情感边界不清晰。传统数据驱动模型难以处理这些模糊情况。

Method: 1) 基于LLM的推理框架，结合声学特征模型提供置信度和特征级证据；2) 置信度路由机制区分简单和模糊样本，将不确定样本交给LLM进行深度推理；3) 基于人类标注行为的结构化规则指导LLM推理；4) 迭代优化策略通过错误分析和规则更新持续改进系统。

Result: 在包含2,764个样本的越南语语音数据集（三类情感：平静、愤怒、恐慌）上测试，标注者间一致性高（Fleiss Kappa = 0.8574）。方法达到86.59%准确率和Macro F1约0.85-0.86，有效处理模糊和难分类样本。

Conclusion: 该工作强调了数据驱动模型与人类推理结合的重要性，为低资源环境下的语音情感识别提供了鲁棒且模型无关的方法，特别适合处理模糊情感边界问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前在线毒性内容检测方案经常无意中屏蔽有价值信息，包括医学术语和少数群体相关文本，这促使研究者开发更细致的方法来识别保加利亚语中的毒性内容，同时保留重要信息的访问权限。

Method: 1) 提出保加利亚语潜在毒性词汇的本体模型；2) 构建包含4,384个手动标注句子的数据集，涵盖毒性语言、医学术语、非毒性语言和少数群体相关术语四个类别；3) 训练基于BERT的毒性语言分类模型。

Result: 训练的BERT模型在毒性语言分类任务上达到了0.89的宏观F1分数，该模型可直接应用于实际环境，并可作为毒性内容检测系统的组件进行集成。

Conclusion: 该研究提供了一种更细致的保加利亚语毒性内容检测方法，能够准确识别毒性内容，同时避免误判医学术语和少数群体相关内容，具有在各种在线平台和内容审核系统中应用的潜力。
...
TL;DR: LiveMathematicianBench是一个基于最新arXiv论文的动态数学推理基准测试，通过证明草图引导的干扰项设计和替换抵抗机制，评估LLMs在真实研究级数学问题上的能力。
...
Method: 1. 使用模型训练截止日期后发布的arXiv论文构建动态多选题基准测试；2. 引入13类逻辑分类法（蕴含、等价、存在性、唯一性等）；3. 采用证明草图引导的干扰项生成管道，基于高级证明策略构建看似合理但错误的答案选项；4. 引入替换抵抗机制区分答案识别与实质推理；5. 设计双模式协议评估证明草图访问对性能的影响。

Result: 基准测试远未饱和：最佳模型Gemini-3.1-pro-preview仅达到43.5%。在替换抵抗评估下，准确率大幅下降：GPT-5.4最高为30.6%，而Gemini-3.1-pro-preview降至17.6%（低于20%随机基线）。证明草图访问能带来一致的准确率提升，表明模型可以利用高级证明策略进行推理。

Conclusion: LiveMathematicianBench提供了一个可扩展、抗污染的研究级数学推理测试平台，能够更真实地评估LLMs的数学能力，揭示当前模型在实质性数学推理方面的局限性。
...
TL;DR: 论文提出基于指令微调的可控自动文本简化框架，使用离散控制标记控制可读性和压缩率，发现小模型也能表现良好，但可控性依赖于训练数据中目标属性的变化程度，现有评估指标不足以衡量控制效果。


<details>
  <summary>Details</summary>
Motivation: 当前可控自动文本简化(CATS)通常被视为解码问题，且使用不能真实反映控制效果的指标进行评估。研究发现CATS的可控性受到数据和评估方法的显著限制。

Method: 提出基于指令微调的领域无关CATS框架，使用离散控制标记引导开源模型达到目标可读性水平和压缩率。在三个模型家族(Llama、Mistral、Qwen，1-14B参数)和四个领域(医学、公共管理、新闻、百科文本)进行实验。

Result: 较小模型(1-3B)可以具有竞争力，但可靠的可控性强烈依赖于训练数据是否编码足够的目标属性变化。可读性控制(FKGL、ARI、Dale-Chall)学习一致，而压缩控制表现不佳，因为现有语料库中信号变化有限。标准简化和相似性指标不足以衡量控制效果。

Conclusion: 需要基于误差的度量来评估目标-输出对齐。朴素的训练-测试分割可能引入分布不匹配，破坏训练和评估效果。数据质量和评估方法对CATS可控性至关重要。
...
TL;DR: 本文提出DEFT框架，通过数据筛选和分布引导提升大语言模型对齐效率，减少训练时间同时保持泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有RLHF方法（如PPO）成本高、不稳定，而替代方法需要大量数据学习偏好且可能削弱LLMs的泛化能力，需要更高效的对齐方法

Method: 提出DEFT框架：1）基于语言模型输出分布和偏好数据差异分布计算差异分布奖励；2）用该奖励从原始数据筛选高质量子集；3）将筛选数据整合到现有对齐方法中引导模型输出分布
...
TL;DR: PLOT提出了一种基于最优传输的token级损失函数，用于增强LLM偏好学习，在保持模型稳定性的同时提升对齐性能
...
Method: PLOT通过最优传输问题形式化偏好学习，推导出token级损失函数，利用token嵌入捕捉语义关系，实现全局信息优化，同时保持LLM原始分布

Result: 在人类价值观和逻辑问题解决两大类、七个子偏好的实验中，PLOT一致提升了对齐性能，同时保持了流畅性和连贯性

Conclusion: 最优传输为LLM偏好学习提供了理论基础，建立了原则性方法论，为LLM偏好学习提供了新的理论见解
...
<details>
  <summary>Details</summary>
Motivation: 传统LLM代码补全采用硬补全范式，即使在上下文不足时也强制生成具体代码。分析300万真实交互发现，61%的生成建议被编辑或拒绝，尽管与用户后续代码相似度超过80%，表明模型在特定token位置经常做出错误预测。

Method: 提出自适应占位符补全（APC）协作框架，扩展HC范式，在高熵位置策略性地输出显式占位符，允许用户通过IDE导航直接填充。将代码补全形式化为不确定性下的成本最小化问题，基于填充占位符比纠正错误成本更低的观察，证明存在临界熵阈值，超过该阈值APC的期望成本严格低于HC。通过从过滤的真实编辑日志构建训练数据，设计基于成本的奖励函数进行强化学习。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态对话情感识别方法过于强调融合而忽略了噪声特征的不确定性和细粒度推理，需要提升对噪声的鲁棒性和上下文建模能力。
...
<details>
  <summary>Details</summary>
Motivation: 心力衰竭短期死亡率预测具有挑战性，特别是仅依赖结构化电子健康记录数据时。需要评估不同方法在临床预测中的有效性。

Method: 在法国心力衰竭队列上评估基于transformer的模型，比较纯文本、纯结构化、多模态和基于LLM的方法。使用实体级表示丰富临床文本，并采用监督多模态融合技术。

Result: 使用实体级表示比仅用CLS嵌入能改善预测效果；监督多模态融合文本和结构化变量获得最佳整体性能；大语言模型在不同模态和解码策略中表现不一致，纯文本提示优于结构化或多模态输入。

Conclusion: 实体感知多模态transformer为心力衰竭短期结果预测提供最可靠的解决方案，而当前LLM提示方法在临床决策支持方面仍有限制。
...
TL;DR: 本文提出了一种神经符号方法，结合fastText文本嵌入和符号概念特征（体裁、主题、说服技巧）来检测宣传性新闻，提高了分类的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 宣传性新闻特别阴险，因为它们将导向性信息与事实报道混合，看起来像可靠新闻。现有的基于BERT等语言模型的方法虽然很有前景，但由于数据收集中的偏见，往往过度拟合训练数据集。需要增强分类的鲁棒性并提高对新来源的泛化能力。

Method: 提出神经符号方法，结合非上下文文本嵌入（fastText）与符号概念特征，包括体裁、主题和说服技巧。这种混合方法旨在提高模型对宣传性新闻的检测能力。

Result: 结果显示该方法优于等效的纯文本方法。消融研究和可解释性分析证实了添加特征的好处，表明该方法提高了分类的鲁棒性和泛化能力。

Conclusion: 神经符号方法通过结合文本嵌入和符号概念特征，能够更有效地检测宣传性新闻，提高模型的鲁棒性和对新数据源的泛化能力，为信息失序检测提供了改进方案。
...
TL;DR: 该研究提出了一种测量词序变异相对于交换距离最小化最优程度的数学框架，发现跨语言手势至少77%最优，并引入二次分配问题作为语言研究中多种优化问题的统一框架。


<details>
  <summary>Details</summary>
Motivation: 研究动机是验证语言中词序是否遵循交换距离最小化假设，即给定源顺序时，在置换多面体中距离更近的词序应该成本更低、更可能被采用。研究者希望建立测量词序变异最优程度的数学方法。

Method: 研究者提出了一个新颖的数学框架来测量词序变异相对于交换距离最小化的最优程度。该方法基于置换多面体理论，其中顶点代表排列，相邻顶点通过交换相邻元素连接。研究引入二次分配问题作为语言研究中多种优化问题的统一框架。

Result: 研究发现跨语言手势至少达到77%的最优性，这种多次达到最优性的情况不太可能是偶然发生的。研究为基于交换距离最小化的词序或手势顺序最优性研究奠定了理论基础。

Conclusion: 研究建立了测量词序变异最优程度的数学框架，证实了跨语言手势的高度最优性，并提出了最优分配的一般原则，统一了包括交换距离最小化在内的各种语言学原则。二次分配问题为语言研究中的优化问题提供了统一的数学框架。
...
TL;DR: 该研究提出了一种三步自动化质量保证方法，用于评估机器翻译基准数据集的质量，发现COMET分数较低的数据集在span级别有更多准确性/误译错误，并发布了清理后的数据集版本。
...
Method: 采用三步自动化质量保证方法：1) 结构语料库审计与针对性修复；2) 使用神经度量COMET（无参考和基于参考）进行质量分析，并与DeepL/ChatGPT/Google翻译服务比较；3) 基于LLM的span级别翻译错误分析。

Result: 趋势一致：COMET分数较低的数据集在span级别表现出更高的准确性/误译错误比例（特别是HellaSwag；ARC相对干净）。基于参考的COMET在MMLU上针对人工编辑样本也指向相同方向。研究发布了清理/校正后的EU20数据集和可复现代码。
...
TL;DR: SAFE是一个动态基准测试框架，通过可验证的实体序列替代无根据的思维链，解决多跳QA中LLMs的虚假正确性问题，在训练时验证和推理时验证两个阶段确保推理的可验证性。


<details>
  <summary>Details</summary>
Motivation: 多跳QA基准测试经常奖励大型语言模型的虚假正确性，掩盖了无根据或有缺陷的推理步骤。现有方法缺乏严格的验证机制，导致模型可能通过错误的推理路径得到正确结果。

Method: 提出SAFE框架，包含两个阶段：1) 训练时验证：建立原子错误分类和基于知识图谱的验证流程，消除标准基准中的噪声监督；2) 推理时验证：基于验证数据集训练反馈模型，实时检测推理过程中的无根据步骤。

Result: 实验结果表明：1) SAFE在训练时能识别高达14%的实例为不可回答，暴露现有基准的关键缺陷；2) 在推理时显著优于标准基线，平均准确率提升8.4个百分点，同时保证推理轨迹的可验证性。

Conclusion: SAFE框架通过严格的验证机制解决了多跳QA中LLMs的虚假正确性问题，不仅揭示了现有基准的缺陷，还提供了可验证的推理轨迹，为更严谨的推理评估提供了有效方法。
...
TL;DR: 论文研究了高斯扩散模型在处理离散数据时遇到的采样困难，发现DDPM求解器在关键采样区间会进入低密度区域导致样本质量下降，提出了结合自条件和q采样的改进方法。


<details>
  <summary>Details</summary>
Motivation: 扩散模型已成为连续域生成建模的标准方法，但在离散数据上的应用仍然具有挑战性。本研究旨在探究为什么使用DDPM求解器的高斯扩散模型难以从离散分布中采样，这些离散分布在连续空间中表示为δ分布的混合。

Method: 使用玩具随机层次模型分析离散数据分布，识别出关键采样区间，在该区间内噪声化数据的密度变为多模态。提出结合自条件和q采样的混合方法，在关键区间内从DDPM切换到q采样以提高生成质量。

Result: 研究发现DDPM在关键采样区间会偶尔进入模态之间的低密度区域，产生超出分布的输入并降低样本质量。自条件和q采样有助于缓解这一问题，结合两者在关键区间内切换的方法在文本、编程代码和蛋白质等多个领域的条件和非条件任务中都提高了生成质量。

Conclusion: 高斯扩散模型处理离散数据时存在采样困难的根本原因是在关键区间内密度变为多模态，导致DDPM进入低密度区域。通过结合自条件和在关键区间切换到q采样的策略，可以有效改善离散数据的生成质量，这一发现在多个实际应用领域得到了验证。
...
TL;DR: 研究自监督语音模型训练过程中语言结构何时出现，通过分析Wav2Vec2和HuBERT模型在荷兰语上的训练过程，发现不同语言结构在不同层和训练阶段呈现不同的学习轨迹


<details>
  <summary>Details</summary>
Motivation: 自监督语音模型能学习有效的语音表示，这些表示反映了语言结构的各个方面，但研究者想知道这些语言结构在模型训练的哪个阶段出现

Method: 研究了六个在荷兰语上训练的Wav2Vec2和HuBERT模型，分析了不同语言结构在不同层和中间检查点中的编码情况，考察了语言结构的层级模式和学习轨迹

Result: 发现不同层级的语言结构在层间分布和学习轨迹上有显著差异，这些差异部分可由其与声学信号的抽象程度以及输入信息整合的时间尺度来解释；预训练目标定义的层级强烈影响语言结构的层间组织方式

Conclusion: 自监督语音模型中语言结构的出现时间和方式取决于结构的抽象程度和预训练目标的设计，高阶预测任务（如迭代细化的伪标签）能诱导更大的并行性
...
TL;DR: 将因果生成语言模型转化为双向编码器，提出BidirLM系列，在文本、视觉和音频表示基准上优于现有方法


<details>
  <summary>Details</summary>
Motivation: 当前将因果生成语言模型转化为双向编码器的方法存在局限性：缺乏最佳训练目标的共识、大规模时出现灾难性遗忘、难以灵活集成专业生成模型

Method: 通过系统消融实验确定成功适应的关键因素；引入结合线性权重合并和轻量级多领域数据混合的双重策略；通过合并专业因果模型增强编码器能力

Result: 开发出BidirLM系列五个编码器，在文本、视觉和音频表示基准上优于现有替代方案

Conclusion: 提出适用于任何因果解码器LLM的开源方案，成功将因果生成模型转化为高性能双向编码器，解决了现有方法的局限性
...
TL;DR: GOOSE是一种无需训练的推测解码框架，通过构建自适应脊柱树结构，利用不同来源token的接受率差异，实现比平衡树方法更高的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法在构建候选token树时，通常采用平衡树结构，没有区分不同来源token的质量差异。研究发现，从输入上下文复制的n-gram匹配token和从前向传递统计预测的token，在接受率上存在显著差异（中位数差距约6倍）。这种质量差异为优化树结构提供了机会。

Method: GOOSE提出自适应脊柱树结构：将高接受率的上下文匹配token组织成深链（脊柱），在每个节点上为低接受率的替代token创建宽分支。这种各向异性（不对称）结构突破了平衡树的深度限制，充分利用了不同来源token的质量差异。

Result: 在五个LLM模型（7B-33B）和五个基准测试上，GOOSE实现了1.9-4.3倍的无损加速，在相同预算下比平衡树基线方法性能提升12-33%。理论证明，GOOSE每个步骤接受的token数量至少与单独使用任一来源相当。

Conclusion: 通过识别和利用不同token来源的质量差异，构建各向异性的自适应脊柱树结构，可以显著提升推测解码的效率。GOOSE框架无需额外训练，在实际应用中实现了显著的推理加速效果。
...
Result: 在知识密集型基准测试中，RRPO显著优于包括强大的列表式重排序器RankZephyr在内的基线方法；框架具有良好泛化性，可适配不同LLM，与查询扩展模块正交集成，对噪声监督具有鲁棒性

Conclusion: RRPO通过直接对齐重排序与LLM生成质量，解决了传统重排序与下游生成过程脱节的问题，为检索增强生成提供了更有效的重排序方法
...
TL;DR: 本文提出了一种评估自监督语音模型对韵律对比敏感性的新方法——韵律ABX测试，并构建了包含英语、日语和汉语的韵律最小对数据集进行验证。


<details>
  <summary>Details</summary>
Motivation: 自监督语音模型（S3Ms）已知对音位对比敏感，但其对韵律对比的敏感性尚未得到直接测量。需要开发一种无需大量数据或显式标签的方法来评估模型对韵律特征的敏感性。

Method: 提出了韵律ABX测试框架，这是传统ABX音位对比测试的扩展，专门用于评估韵律对比。构建并发布了包含英语重音、日语音高重音和汉语声调的最小对数据集。使用这些数据集在不同实验条件下评估模型和层的表现。

Result: 研究发现模型和层的排名在不同实验条件下通常保持一致，这使得该方法在低资源环境下具有实用性。韵律ABX测试能够有效评估模型对英语重音、日语音高重音和汉语声调的敏感性。

Conclusion: 韵律ABX测试提供了一种简单有效的方法来评估自监督语音模型对韵律对比的敏感性，无需大量数据或显式标签。该方法在低资源环境下具有实用价值，有助于理解模型如何编码韵律信息。
...
TL;DR: 论文提出稳定性过滤方法，通过筛选行为稳定的边界来构建更有效的引导向量，解决现有方法中大多数检测到的行为边界不稳定且无法重现的问题，显著提升数学推理任务的准确率。


<details>
  <summary>Details</summary>
Motivation: 当前基于关键词匹配检测推理行为边界的方法存在严重缺陷：大多数检测到的边界是行为不稳定的，无法在相同前缀下重现目标行为，这导致构建的引导向量信号被稀释，效果不佳。

Method: 1) 提出概率模型将内在推理行为形式化为具有上下文依赖触发概率的随机事件；2) 开发稳定性过滤方法，仅保留模型能一致重现目标行为的边界；3) 结合内容子空间投影去除残差问题特定噪声。

Result: 在541个关键词检测的边界中，93.3%是行为不稳定的；稳定性过滤结合内容子空间投影的方法在MATH-500上达到0.784准确率（比最强基线提升5.0%）；引导向量可在同架构模型间迁移，提升Nemotron-Research-Reasoning-1.5B（+5.0）和DeepScaleR-1.5B-Preview（+6.0）。

Conclusion: 稳定性过滤能有效识别真正的行为信号，构建更纯净的引导向量，显著提升推理行为的控制效果，且具有跨模型迁移能力，为训练自由控制语言模型推理行为提供了更可靠的方法。
...
TL;DR: GaelEval是首个针对苏格兰盖尔语的多维度基准测试，包含形态句法MCQA、文化翻译和文化知识问答三个任务。评估19个LLM发现，Gemini 3 Pro Preview在语言任务上达到83.3%准确率，超过人类基线（78.1%）。专有模型优于开源模型，盖尔语提示有稳定优势。
...
Method: 开发GaelEval基准测试，包含三个维度：1）专家编写的形态句法多项选择题任务；2）文化背景翻译基准；3）大规模文化知识问答任务。评估19个大语言模型，并与30名流利说话者的人类基线对比，分析专有模型与开源模型差异，以及盖尔语提示的效果。

Result: Gemini 3 Pro Preview在语言任务上达到83.3%准确率，超过人类基线（78.1%）。专有模型持续优于开源系统，盖尔语提示带来稳定但小幅优势（+2.4%）。在文化任务上，领先模型超过90%准确率，但大多数系统在盖尔语提示下表现更差，且绝对分数相对于人工基准有所夸大。

Conclusion: 前沿模型在盖尔语语法的多个维度上达到超人类表现，盖尔语提示有积极效果，专有模型与开源模型存在持续性能差距。GaelEval为少数民族语言评估提供了首个全面基准，揭示了LLM在未受支持语言中的能力分布。
...
### [148] [AstroConcepts: A Large-Scale Multi-Label Classification Corpus for Astrophysics](https://arxiv.org/abs/2604.02156)
...
TL;DR: 该论文介绍了AstroConcepts语料库，包含21,702篇天体物理学论文摘要，标注了2,367个天文学概念，用于研究极端类别不平衡问题。研究发现词汇约束LLM在性能上可与领域适应模型竞争，领域适应对罕见术语改善更大，并提出频率分层评估方法。


<details>
  <summary>Details</summary>
Motivation: 科学多标签文本分类面临极端类别不平衡问题，专业术语呈现严重的幂律分布，现有科学语料库缺乏全面的受控词汇表，限制了极端不平衡问题的系统性研究。

Method: 构建AstroConcepts语料库（21,702篇天体物理学论文摘要，标注2,367个概念），建立传统、神经和词汇约束LLM方法的基线，提出频率分层评估方法，比较不同方法在极端不平衡条件下的表现。

Result: 1. 词汇约束LLM在性能上可与领域适应模型竞争，表明参数高效方法的潜力；2. 领域适应对罕见专业术语改善更大，但所有方法的绝对性能仍然有限；3. 频率分层评估揭示了被聚合分数隐藏的性能模式。

Conclusion: 该研究为科学NLP提供了可操作的见解，并为极端不平衡研究建立了基准，强调频率分层评估对科学多标签评估的重要性，词汇约束LLM可作为参数高效的科学分类方法。
...
TL;DR: 该论文在SOMD 2026跨文档软件提及共指消解任务中排名第二，比较了两种无需微调的方法：模糊匹配（基于字符串相似性）和上下文感知表示（结合提及级和文档级嵌入），两者均表现优异，后者在官方测试集上始终领先1分。


<details>
  <summary>Details</summary>
Motivation: 软件名称具有高度表面规律性，这降低了复杂语义推理的需求，但现有方法在噪声处理和扩展性方面存在差异，需要探索不同方法在不同场景下的适用性。

Method: 采用两种无需微调的方法：1) 模糊匹配（FM）- 基于词汇字符串相似性的方法；2) 上下文感知表示（CAR）- 结合提及级和文档级嵌入的方法。通过控制噪声注入研究分析两种方法的互补故障模式，并进行推理时间分析比较扩展性。

Result: 两种方法在所有子任务中都取得了竞争性表现（CoNLL F1为0.94-0.96），CAR在官方测试集上始终比FM高出1分。噪声注入研究表明：边界噪声增加时，CAR从干净到完全损坏输入的F1仅下降0.07，而FM下降0.20；在提及替换下，FM下降更平缓（0.52 vs 0.63）。推理时间分析显示FM随语料库大小呈超线性扩展，而CAR近似线性扩展。

Conclusion: 系统选择应同时考虑上游提及检测器的噪声特征和目标语料库的规模。CAR在大规模场景下是更高效的选择。作者发布了代码以支持这一未充分探索任务的未来研究。
...
TL;DR: Neuro-RIT是一种通过神经元级对齐增强检索增强语言模型鲁棒性的新框架，使用基于归因的神经元挖掘来区分处理相关与无关上下文的神经元，并通过两阶段指令调优实现噪声鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 检索增强语言模型在知识密集型任务中表现出色，但在面对无关或嘈杂的检索上下文时性能会下降。现有方法通常在层或模块级别进行粗粒度参数更新，忽略了大型语言模型固有的神经元级稀疏性。

Method: 提出Neuro-RIT框架，从密集适应转向精确驱动的神经元对齐。首先使用基于归因的神经元挖掘明确分离处理相关与无关上下文的神经元，然后引入两阶段指令调优策略：通过功能失活处理无关上下文专属神经元实现直接噪声抑制，同时优化目标层进行证据蒸馏。

Result: 在多样化的QA基准测试中进行广泛实验，Neuro-RIT持续优于强基线方法和鲁棒性增强方法。

Conclusion: Neuro-RIT通过神经元级对齐有效提升了检索增强语言模型在噪声环境下的鲁棒性，为增强语言模型对无关上下文的抵抗力提供了新范式。
...
TL;DR: L3TR是一个用于列表式人才推荐的LLM框架，通过块注意力机制和局部位置编码解决位置偏见和token偏见问题，相比点式方法能更好地捕捉候选人关系并减少token消耗。


<details>
  <summary>Details</summary>
Motivation: 现有的人才推荐系统大多采用点式范式，需要LLM重复处理文本，无法捕捉候选人列表中的关系，导致token消耗高且推荐效果不佳。此外，LLM在处理多项选择题和多个长文档时存在位置偏见和"中间迷失"问题。

Method: 提出L3TR框架，包含：1) 块注意力机制和局部位置编码方法，增强文档间处理并缓解位置偏见和并发token偏见；2) ID采样方法解决训练和推理阶段候选集大小不一致问题；3) 设计评估方法检测位置偏见和token偏见，以及训练无关的去偏见方法。

Result: 在两个真实世界数据集上的广泛实验验证了L3TR的有效性，相比现有基线方法显示出持续改进。

Conclusion: L3TR通过隐式利用LLM的潜在输出进行推荐任务，采用列表式方法解决了传统点式人才推荐系统的局限性，在减少token消耗的同时提高了推荐质量。
...
TL;DR: 该研究创建了首个阿拉伯语语音命名实体识别公开数据集CV-18 NER，并比较了级联系统和端到端方法，发现端到端模型在阿拉伯语语音NER任务上表现更优。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语语音命名实体识别研究不足，主要由于阿拉伯语的形态复杂性、缺少短元音以及标注资源有限。需要填补这一研究空白。

Method: 创建CV-18 NER数据集（基于Arabic Common Voice 18语料库，采用Wojood细粒度标注模式，21种实体类型），并对比级联系统（ASR+文本NER）和基于Whisper、AraBEST-RQ的端到端模型。
...
Conclusion: 端到端方法在阿拉伯语语音NER任务上优于级联系统，阿拉伯语特定预训练和多语言弱监督各有优势，大模型在低资源环境下可能更难适应。发布了首个阿拉伯语语音NER公开基准。
...
TL;DR: 论文研究如何从多个LLM中生成全面的答案集，提出多样性覆盖率指标评估模型生成多样化答案的能力，并开发了基于查询的路由器来选择最佳模型。


<details>
  <summary>Details</summary>
Motivation: 当面对允许多个有效答案的提示时，全面生成这些答案是满足多样化用户需求的第一步。现有LLM在生成全面答案集方面表现不一，需要系统评估和优化方法。

Method: 提出多样性覆盖率指标来评估答案集的全面性；评估18个LLM在多样化答案生成上的表现；开发基于查询的路由器来预测每个查询的最佳模型。

Result: 没有单一模型在所有提示上都表现最佳，但每个提示都存在显著优于其他模型的特定模型。训练的路由器在NB-Wildchat上优于单一最佳模型基线（26.3% vs 23.8%），并在跨域数据集和不同提示策略上展现出泛化能力。

Conclusion: 该研究为在拥有多个模型的情况下生成全面答案集奠定了基础，展示了基于查询的路由策略在优化多样化答案生成方面的有效性。
...
TL;DR: 本文分析了语言模型扩展新词汇时的初始化问题，发现传统的均值初始化会导致新token坍缩到退化子空间，提出基于语言学基础的token初始化方法GTI，在多个生成推荐基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型在扩展新可学习词汇token（如生成推荐中的语义ID token）时，通常采用将新token初始化为现有词汇嵌入的均值，然后通过监督微调学习其表示。作者通过谱分析和几何诊断发现，这种均值初始化方法会导致所有新token坍缩到一个退化子空间，抹去了token间的区别，后续微调难以完全恢复这些区别。

Method: 提出了"基于语言学基础的token初始化假设"：在微调之前，将新token基于语言学基础映射到预训练嵌入空间中的不同、有语义意义的位置，能更好地让模型利用其通用知识处理新token领域。作者将这一假设操作化为GTI（基于语言学基础的token初始化），这是一个轻量级的基础阶段，在微调之前仅使用配对的语言学监督将新token映射到预训练嵌入空间中的不同位置。

Result: GTI在大多数评估设置中优于均值初始化和现有的辅助任务适应方法，测试覆盖了多个生成推荐基准，包括工业级和公共数据集。进一步分析显示，基于基础的嵌入产生了更丰富的token间结构，这些结构在微调过程中得以保持。

Conclusion: token初始化是语言模型扩展新词汇时的关键瓶颈。基于语言学基础的初始化方法GTI通过将新token映射到预训练嵌入空间中的不同语义位置，能够更好地保留token间的区别，从而提高模型性能，验证了初始化质量是词汇扩展的关键瓶颈这一假设。
